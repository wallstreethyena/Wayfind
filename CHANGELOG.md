...
- **The controlling cap is still the Wayfind ledger** (`wayfind-ledger` — the `GOOGLE_PHOTOS_MONTH_CAP` ceiling when paid photos are on, the 950 free-tier fallback when they're off; `CAPS.photos` is untouched). Google's 32/day console override is a separate, tighter ceiling this release does not read live or change — raising it, if the owner wants to, is a console decision, not a code one. It shows up here only indirectly, through `today.quota` and `breaker.open`.
- **Not changed:** `lib/serverCache.js`'s cache clocks (20–27 day jitter, 25-day cron), `lib/stockPhoto.js`, the shuffle, `wf_photo_repair_queue`'s own logic, the Wikimedia lane, any `CAPS` value, and the `degraded !== false` guard. The new migration (`wf_spend_refund`, `wf_photo_outcome_daily`, `wf_photo_outcome_bump`) has not been applied to any environment by this change — the owner applies it.
