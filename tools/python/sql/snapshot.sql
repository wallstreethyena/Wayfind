...
jobs as (select id, job, ran_at, attempted, succeeded, failed, note
from public.wf_job_pulse where ran_at >= (select until - interval '7 days' from clock) and ran_at < (select until from clock)
order by ran_at, id)
select jsonb_build_object('schema_version',2,'source_kind','production','scope','all wf_inventory rows','consistency','single_statement','captured_at',(select until from clock),'since',(select until - interval '7 days' from clock),'until',(select until from clock),'expected_counts',jsonb_build_object('places',(select count(*) from places),'ledger',(select count(*) from ledger),'jobs',(select count(*) from jobs)),'datasets',jsonb_build_object('places',jsonb_build_object('columns','["place_id", "name", "lat", "lng", "metro", "category", "status", "excluded", "needs_review", "editorial_exists", "verified", "has_issues", "hook_chars", "why_chars", "sourced_facts"]'::jsonb,'rows',(select coalesce(jsonb_agg(jsonb_build_array(place_id,name,lat,lng,metro,category,status,excluded,needs_review,editorial_exists,verified,has_issues,hook_chars,why_chars,sourced_facts)), '[]'::jsonb) from places)),'ledger',jsonb_build_object('columns','["month", "sku", "used", "cap"]'::jsonb,'rows',(select coalesce(jsonb_agg(jsonb_build_array(month,sku,used,cap)), '[]'::jsonb) from ledger)),'jobs',jsonb_build_object('columns','["id", "job", "ran_at", "attempted", "succeeded", "failed", "note"]'::jsonb,'rows',(select coalesce(jsonb_agg(jsonb_build_array(id,job,ran_at,attempted,succeeded,failed,note)), '[]'::jsonb) from jobs)))) as snapshot;
