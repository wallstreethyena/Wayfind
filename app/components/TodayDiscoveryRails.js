...
export function TodayEntertainmentRail({ eventSurface }) {
  ...
  return <section aria-label="Concerts & Comedy" style={{ marginTop: 22 }}>
    <RailHeading title="Concerts & Comedy" description="The best live music and comedy happening near you.">
      {events.length
    ...
    {eventSurface?.pending && !events.length ? (
      <RailLoading label="Loading Concerts & Comedy" />
    ) : <>
...
export default function TodayDiscoveryRails({
  ...
  if (!payload && !failed) return <RailLoading label="Ranking today's best discoveries" />;
