...
export default function IntentPageClient({ intent }) {
  ...
  useEffect(() => {
    ...
    let dead = false;
    (async () => {
      // Date Night is inventory-only (owner, 2026-08-29): the same /api/rails
      // shortlist the homepage drop uses, hard-capped at 27 miles from THIS
      // page's center. No Places search, no Place Details, no photo backfill.
  ...
  useEffect(() => {
    ...
    // Date Night: ads off, no Book /go. Partner / Viator tours stay off.
