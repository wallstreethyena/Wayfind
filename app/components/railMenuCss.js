...
export const WF_RAIL_MENU_CSS =
  ...
  // Date Night locked poster is 3:4 (1086×1448). The TILE BOX stays on the
  // shared --wf8-ratio:.5625 (9:16) so this card matches Tonight / Worth the
  // Drive. object-fit:contain letterboxes the full frame — cover is what
  // clipped the left-aligned type (#1031). A shorter Date Night-only ratio
  // (#1032) made this tile shorter than its neighbors. Hover zoom is off
  // because scale plus overflow:hidden re-clips the same left edge.
  `.wf8-tile[data-id="datenight"] .wf8-tim{object-fit:contain;object-position:center center}.wf8-tile[data-id="datenight"]:hover .wf8-tim{transform:none}`;
