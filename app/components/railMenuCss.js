...
export const WF_RAIL_MENU_CSS =
  ...
  `.wf8-pcrail>.wf-place-card:nth-child(2){animation-delay:58ms}.wf8-pcrail>.wf-place-card:nth-child(3){animation-delay:116ms}.wf8-pcrail>.wf-place-card:nth-child(n+4):nth-child(-n+12){animation-delay:174ms}.wf8-pcrail>.wf-place-card:nth-child(n+13){animation:none}@keyframes wf8CardDrop{0%{opacity:0;transform:translateY(-30px) scale(.955)}62%{opacity:1;transform:translateY(6px) scale(1.008)}100%{opacity:1;transform:none}}` +
  ...
  // responsive: no dead space on either device. The mobile width fallback is
  // followed by an svh cap for modern browsers. svh uses Safari's smallest
  // visible viewport, including its expanded address and toolbar state, so a
  ...
  `@media(max-width:900px){.wf8-track{padding:10px var(--wf8-pad) 16px}.wf8-tile.is-sel,.wf8.is-open .wf8-tile.is-sel{transform:translateY(-6px) scale(1.03)}.wf8-tshare{width:36px;height:36px;top:9px;right:9px}.wf8-tsaid{height:36px;top:9px;right:53px}}@media(max-width:1400px){.wf8{--wf8-pcvis:2.4}}@media(max-width:1100px){.wf8{--wf8-pcvis:1.9}}@media(max-width:900px){.wf8{--wf8-tw:min(76vw,340px);--wf8-pcvis:1.35;--wf8-pcgap:11px;--wf8-pad:16px}.wf8-grail>li{flex:0 0 min(78vw,320px)}}@supports(height:100svh){@media(max-width:900px){.wf8{--wf8-tw:min(76vw,340px,calc(68svh * var(--wf8-ratio)))}}}@media(max-width:560px){.wf8{--wf8-pcvis:1.08;--wf8-pad:13px}}@media (prefers-reduced-motion:reduce){.wf8.is-open .wf8-menusec,.wf8-pcrail>.wf-place-card,.wf8 .wf-rail-exploding>.wf-rail-card,.wf8-gcard{animation:none}.wf8-tile,.wf8-gcard,.wf8-tim,.wf8-tshare{transition:none}.wf8-tile::before{display:none}.wf8-tile:hover,.wf8-tile:has(:focus-visible){transform:translateY(-4px)}.wf8-tile:hover .wf8-tim{transform:none}.wf8-tsaid{animation:none}}` +
  `@media(hover:none),(pointer:coarse){.wf8-track,.wf8-pcrail{scroll-snap-type:x proximity}.wf8-tile,.wf8-tile.is-sel,.wf8.is-open .wf8-tile,.wf8.is-open .wf8-tile.is-sel{animation:none;transition:none;transform:none;filter:none}.wf8-pcrail>.wf-place-card,.wf8 .wf-rail-exploding>.wf-rail-card{animation:none}.wf8-tim{transition:none}.wf8-tshare,.wf8-tsaid{backdrop-filter:none;-webkit-backdrop-filter:none}}`;
// v8.93 — THE DATE NIGHT EXCEPTION IS GONE, and its absence is the fix.
//
// The old poster was 3:4 (1086×1448) inside a 9:16 tile, so it was drawn with
// object-fit:contain to stop cover clipping its left-aligned type (#1031).
// contain fits the whole frame, which means it PADS — and the maroon band
// above the art in the owner's 2026-08-30 screenshot is that padding, not a
// header. Hover zoom was disabled for the same reason, so this one tile also
// sat still while every neighbour lifted.
//
// His new poster is 941×1672 = 0.5628, which is --wf8-ratio to three decimals.
// It cover-fits with nothing clipped and nothing padded, so the tile goes back
// to the shared rule and gets its hover back with it. Re-add an override only
// for another off-ratio poster — and read #1031/#1032 first, because a Date
// Night-only tile RATIO was the other attempt and it made this card shorter
// than its neighbours.
