...
/* v8.12 (owner, 2026-08-18: "the place cards has this empty space on it …
   i need everything at wayfind to be premium"). Cards in a drop row stretch
   to the tallest sibling; one with less copy left a dead void under its
   buttons. The content column now flexes and the ACTION ROW ANCHORS TO THE
   BOTTOM, so the breathing room sits between the copy and the actions — a
   deliberate spacing, not an abandoned gap. */
.wf8-pcrail .wf-place-card{height:100%;box-sizing:border-box;margin-bottom:0!important}
.wf8-pcrail .wf-place-card-layout{height:100%}
.wf8-pcrail .wf-place-card-content{display:flex;flex-direction:column;height:100%}
.wf8-pcrail .wf-place-card-actions{margin-top:auto;padding-top:10px}
.wf-place-card{
  position:relative;
