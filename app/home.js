...
// The hero card is 93% of the main column, and the column has three tiers (see
// WF_DESKTOP_BP / WF_WIDE_BP / WF_WIDE_COL in app/components/css.js): 480px on
// phones, 800px from 900px up, 1060px from 1180px up. Stated in CSS pixels at
// the SAME breakpoints so the browser picks the right srcSet candidate from the
// preload scanner, before layout — a `sizes` that disagrees with the CSS is how
// a responsive image quietly downloads the wrong file on every visit.
const HERO_IMG_SIZES = "(min-width: 1180px) 986px, (min-width: 900px) 744px, 93vw";
...
function DiscoveryHeroCard({ onOpen }) {
  return (
    ...
    >
      {/* v7.29 LCP: this is the largest element on the mobile first screen and
          it is preloaded at fetchPriority="high", so its bytes ARE the LCP.
          It used to ship the 1600x1066 / 473KB source JPEG into a card that is
          93% of a <=480px column — measured 2026-08-12 on production. The AVIF
          and WebP derivatives (scripts/build-brand-derivatives.mjs, committed
          under public/brand/opt/) are the same picture at the widths this card
          actually paints: 16.5KB at phone width instead of 473KB.
          The <img> keeps the WebP set rather than the JPEG so the preload React
          hoists for fetchPriority="high" can never be the 473KB original, and
          the original stays in the repo as the last-resort src. */}
      <picture>
        <source
          type="image/avif"
          srcSet="/brand/opt/wayfind-default-hero-480.avif 480w, /brand/opt/wayfind-default-hero-768.avif 768w, /brand/opt/wayfind-default-hero-1152.avif 1152w, /brand/opt/wayfind-default-hero-1600.avif 1600w"
          sizes={HERO_IMG_SIZES}
        />
        <img
          src="/brand/opt/wayfind-default-hero-480.webp"
          srcSet="/brand/opt/wayfind-default-hero-480.webp 480w, /brand/opt/wayfind-default-hero-768.webp 768w, /brand/opt/wayfind-default-hero-1152.webp 1152w, /brand/opt/wayfind-default-hero-1600.webp 1600w"
          sizes={HERO_IMG_SIZES}
          alt=""
          loading="eager"
          fetchPriority="high"
...
function PageInner({ initialEvents = null, localEditGuides = null }) {
  ...
  async function loadBlurbs(list) {
    ...
    const need = list.filter((p) => !seeded[p.id] && !blurbsInFlight.current.has(p.id)).slice(0, 3);
