...
export async function GET(request) {
  ...
  const mode = searchParams.get("mode") === "attractions" ? "attractions" : "detail";
  ...
  if (hasLat !== hasLng || ((hasLat || hasLng) && !coordinatesValid) || (mode === "detail" && !placeId) || (mode === "attractions" && !coordinatesValid)) {
    return Response.json({ error: mode === "attractions" ? "Valid center coordinates are required" : "A place ID and either two valid coordinates or neither are required", places: [] }, { status: 400, headers: noStore });
  }
  try {
