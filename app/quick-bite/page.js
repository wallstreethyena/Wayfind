// /quick-bite — on the intent-page template (v6.58). Dynamic + personal, so noindex.
...
export async function generateMetadata({ searchParams }) {
  ...
  const og = SITE_URL + "/api/og/intent?intent=quick-bite" + (city ? "&city=" + encodeURIComponent(city) : "") + (refOk ? "&img=" + encodeURIComponent(ref) : "");
  const def = INTENT_PAGES["quick-bite"];
