 ...
 *
 * NEVER SERVE A >1600px COMMONS ORIGINAL (2026-09-17). Before a place gets a
 * vault copy — or when the vault never manages to store one — the only
 * fallback used to be `row.image_url` itself, which for the Commons-direct
 * lane is the file's full ORIGINAL: production carried 13 active rows whose
 * originals ran 3,000-9,000px wide (Spaceship Earth, EPCOT at 6240px),
 * hotlinking a multi-megabyte image straight to a phone. `opts.renditionUrl`
 * (same injection shape as `opts.vaultPublicUrl`), when `row.width` is
 * unknown or already known to be over 1600px, is called as
 * `renditionUrl(row.image_url, 1280)` to serve the SAME file's 1280px
 * rendition instead — same licence, same attribution; `image_url` itself is
 * never rewritten, only what gets SERVED. When `row.width` is known to be
 * over 1600 and no usable rendition can be produced, the row is refused
 * (never falls back to the raw oversized original) — "unknown" width still
 * falls back to the original, matching every other "nothing regresses" path
 * in this module, since a small image with no recorded width is common and
 * legitimate.
 */
export function selectFreePhotoRow(row, opts = {}) {
  ...
  if (storagePath && typeof opts.vaultPublicUrl === "function") {
    let vaultUrl;
    try {
      vaultUrl = opts.width != null ? opts.vaultPublicUrl(storagePath, opts.supabaseUrl, { width: opts.width }) : opts.vaultPublicUrl(storagePath, opts.supabaseUrl);
    } catch {
      vaultUrl = "";
    }
  ...
  if (!url) {
    const originalUrl = validUrl(row.image_url, { httpsOnly: true });
    const width = Number.isFinite(row.width) ? Number(row.width) : null;
    const oversized = width != null && width > 1600;
    let renditionUrl = "";
    if (originalUrl && (width == null || oversized) && typeof opts.renditionUrl === "function") {
      let r;
      try {
        r = opts.renditionUrl(originalUrl, 1280);
      } catch {
        r = "";
      }
      renditionUrl = validUrl(r, { httpsOnly: true });
    }
    if (oversized) {
      // A known-oversized original is never servable as-is — only the
      // rendition (or nothing).
      url = renditionUrl;
    } else {
      url = renditionUrl || originalUrl;
    }
  }
...
// Same lazy-load seam as loadVaultPublicUrl above, for lib/photoVault.js's
// pure commonsRenditionUrl(url, width) helper (2026-09-17) — the >1600px
// Commons-original guard in selectFreePhotoRow needs it, and this module
// still never hard-depends on lib/photoVault.js existing.
let _cachedRenditionUrl;
async function loadRenditionUrl() {
  if (_cachedRenditionUrl !== undefined) return _cachedRenditionUrl;
  try {
    const mod = await import("./photoVault.js");
    _cachedRenditionUrl = typeof mod.commonsRenditionUrl === "function" ? mod.commonsRenditionUrl : null;
  } catch {
    _cachedRenditionUrl = null;
  }
  return _cachedRenditionUrl;
}
...
export async function findFreePhoto({ placeId, width } = {}, deps = {}) {
  ...
  u.searchParams.set("select", "image_url,license,attribution_text,attribution_url,source,storage_path,width");
  ...
  try {
    ...
    const renditionUrl = deps.renditionUrl !== undefined ? deps.renditionUrl : await loadRenditionUrl();
    return selectFreePhotoRow(row, { supabaseUrl: s.url, vaultPublicUrl, renditionUrl, width: width == null ? 640 : width });
  } catch {
    return null;
  } finally {
    clearTimeout(timer);
