 ...
 */
export function selectFreePhotoRow(row) {
  if (!row || typeof row !== "object") return null;
  const url = validUrl(row.image_url, { httpsOnly: true });
...
export async function findFreePhoto({ placeId, width } = {}, deps = {}) {
  ...
  u.searchParams.set("select", "image_url,license,attribution_text,attribution_url,source");
