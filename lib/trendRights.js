...
/**
 * Has a trend snapshot ever actually been imported, and is the cadence set in
 * every deploy environment?
 *
 * This is a COMMITTED DECLARATION, not a runtime probe, and that is the point.
 * On 2026-08-16 the homepage's first section — opened by default — rendered
 * "Trend recommendations are temporarily unavailable" to every visitor in
 * every metro, because EXPLODING_TOPICS_IMPORT_CADENCE was set in no
 * environment and wf_trend_snapshots / wf_trend_topics /
 * wf_trend_place_matches each held ZERO rows. Nothing in the code was wrong;
 * a surface had simply been mounted over a source nobody had ever filled.
 *
 * A guard cannot reach Vercel's env or the database from a clean checkout, so
 * the only honest hermetic check is a flag a human flips ON PURPOSE, in a
 * diff a reviewer reads. Flip it to true only after ALL THREE are true:
 *
 *   1. a snapshot is imported and wf_trend_snapshots has rows
 *   2. EXPLODING_TOPICS_IMPORT_CADENCE is set in production, preview AND
 *      development (all three — a preview-only value ships the error to prod)
 *   3. /api/trends/nearby returns status "ok" for at least one real metro
 *
 * scripts/check-trend-section-honesty.mjs refuses to let <ExplodingNearby>
 * mount while this is false.
 */
export const TREND_SNAPSHOT_IMPORTED = false;
