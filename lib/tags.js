...
export function resolveIdentity(types, isEvent) {
  ...
  // v4.15: tours and experiences are their own identity. A tiki cruise and a
  // city park both used to land in "attraction", which let parks surface as
  // "more like this" for tours. Checked before tourist_attraction because tour
  // operators usually carry both types.
  if (has("tour_agency", "travel_agency", "tour_operator", "boat_tour_agency")) return "tour";
  if (has("spa", "massage", "wellness_center", "sauna", "yoga_studio")) return "spa";
...
export const ALLOW = {
  // v8.24 — "datenight" joins dining only: the tag's evidence (restaurant
  // identity + price/room, IconicPlaceCard.experienceTags) already restricts
  // it to dining rooms; the identity gate here is the second lock.
  dining: ["gem", "value", "localfav", "bestof", "waterfront", "rooftop", "romantic", "datenight", "livemusic", "outdoor", "groups", "dog", "family", "instagram", "cocktails", "wine", "beer", "sports", "coffee", "breakfast", "pizza", "sushi", "steak", "seafood", "burgers", "mexican", "italian", "dessert", "entertainment"],
  ...
  tour: ["family", "entertainment", "instagram", "outdoor", "waterfront", "gem", "value", "localfav", "bestof"],
  spa: ["romantic", "gem", "value", "localfav", "bestof", "instagram"],
...
// Venue-name evidence only: a skyline is a view or brand, not a rooftop.
export function hasRooftopName(name) {
  return /\b(rooftop|roof top|roof deck|sky bar|skybar)\b/i.test(String(name || ""));
}
