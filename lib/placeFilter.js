...
export const NARROW_SUBS = new Set(["nightlife:clubs", "nightlife:karaoke", "nightlife:sports", "nightlife:speakeasy", "nightlife:bars", "shopping:malls", "shopping:boutiques", "shopping:markets"]);
...
export const SUB_ALLOW = {
  ...
  // v8.50 — Lunch is a midday MEAL, not "any food" and not Breakfast. The
  // chip used to be CATEGORY_WIDE so Keke's (breakfast_restaurant, 9.6/10)
  // led Food → Lunch near Parrish. isLunchPlace is the membership (meal, not
  // breakfast-only primary); this regex is the SUB_ALLOW key serveFromInventory
  // needs so the chip filters BEFORE the cap. NARROW: primary type only, so a
  // dessert shop wearing a secondary `restaurant` tag cannot ride in.
  "food:lunch": /_restaurant$|^restaurant$|^steak_house$|^sandwich_shop$|^diner$|^food_court$|^meal_takeaway$/i,
...
export function placeAllowed(categoryId, subId, p) {
  ...
  // v8.50 — Lunch membership is isLunchPlace (a meal that is not a
  // breakfast-only room). Refuse here, before SUB_ALLOW, so a
  // breakfast_restaurant cannot match the `_restaurant$` token and lead the
  // chip. Admission still walks through the allow test below so grocery /
  // lodging vetoes stay in front.
  if (categoryId === "food" && subId === "lunch" && !isLunchPlace(p)) return false;
  ...
  // v8.50 — Lunch already passed isLunchPlace above. Do not re-test the
  // `_restaurant$` token against a types string that ends in `food` — that
  // would drop every honest lunch room whose last type is generic.
  if (subKey === "food:lunch") return true;
