...
export const SUB_ALLOW = {
  ...
  "food:breakfast": /breakfast_restaurant|brunch_restaurant|breakfast|brunch|\bcafe\b|café|coffee_shop|\bcoffee\b|bakery|bagel|donut|doughnut|pancake|waffle|\bdiner\b|biscuit|creperie|crepe|omelet|\begg/i,
  ...
  // Dinner is a meal identity, not "any food". Bars stay (O'bricks is Dinner);
  // breakfast-primary / cafe / dessert do not. Do not copy Lunch's bar veto.
  "food:dinner": CAT_ALLOW.food,
  "food:quickbites": /\bfast_food_restaurant\b|\bmeal_takeaway\b|\bhamburger_restaurant\b|\bpizza_restaurant\b|\bsandwich_shop\b|\bdeli\b|\bhot_dog_stand\b|\bfood_truck\b/i,
  "food:dessert": /dessert|ice_cream|ice cream|bakery|donut|doughnut|confectionery|gelato|frozen_yogurt|frozen yogurt|chocolate|creamery|cupcake|patisserie|pastry|froyo|cheesecake/i,
  ...
  // Stays chips: same lodging union as Hotels·All; name-token vetoes below
  // keep Luxury / Budget / Beach / Boutique from becoming a second All.
  "hotels:luxury": CAT_ALLOW.hotels,
  "hotels:budget": CAT_ALLOW.hotels,
  "hotels:beach": CAT_ALLOW.hotels,
  "hotels:boutique": CAT_ALLOW.hotels,
...
export function placeAllowed(categoryId, subId, p) {
  ...
  if (categoryId === "family" && subId === "toddlers") {
    ...
    if (/\bmuseum\b/.test(j) && !/children|toddler|kiddie|sensory/.test(j)) return false;
    if (/\b(park|riverwalk|river walk|waterfront|sports_complex|stadium)\b/.test(j) && !/playground|splash|petting|carousel|kiddie|toddler|sensory|play ?cafe|indoor ?play|children/.test(j) && !/\b(zoo|aquarium|farm|garden|library|aviary)\b/.test(j)) return false;
  }
  if (categoryId === "family" && subId === "kids") {
    const j = name + " " + types;
    if (/escape/.test(j) && !/\b(kid|kids|child|children|family|junior|teen)\b/.test(name)) return false;
  }
  if (categoryId === "family" && subId === "rainy") {
    const j = name + " " + types;
    if (/ca['’]?\s*d['’]?\s*zan|ringling|tibbals|circus museum|\bmansion\b|\bvilla\b|historic (home|house|estate)/.test(j)) return false;
    if (/\bbeach\b/.test(j) && !/indoor|aquarium|museum|library/.test(j)) return false;
    if (/\b(park|trail|preserve|boardwalk|pier|marina)\b/.test(j) && !/indoor|museum|aquarium|library|arcade|bowling|planetarium|science/.test(j)) return false;
    if (!(/indoor|play ?cafe|indoor_playground|aquarium|planetarium|science_museum|childrens_museum|children.?s? ?museum|library|arcade|trampoline|bowling|climb|escape ?room|cinema|movie/.test(j) || (/\bmuseum\b/.test(j) && /science|children|kid|planetarium|natural.?histor/.test(j)))) return false;
  }
  if ((categoryId === "attractions" || categoryId === "beach") && subId === "beaches") {
    const j = name + " " + types;
    const primary = String(p.primaryType || p.primary_type || (Array.isArray(p.types) ? p.types[0] : "") || "").toLowerCase();
    if (primary === "museum" || /ca['’]?\s*d['’]?\s*zan|\bmansion\b|tennis|pickleball|parking|massage/.test(j)) return false;
  }
  if (categoryId === "hotels" && subId === "luxury" && !/luxury|ritz|four seasons|waldorf|st\.?\s*regis|palace|grand|\bresort\b/.test(name + " " + types)) return false;
  if (categoryId === "hotels" && subId === "budget" && !/motel|\binn\b|lodge|suites|extended stay|super 8|days inn|la quinta|red roof|econo/.test(name + " " + types)) return false;
  if (categoryId === "hotels" && subId === "beach" && !/beach|gulf|ocean|\bisland\b|\bkey\b/.test(name + " " + types)) return false;
  if (categoryId === "hotels" && subId === "boutique" && !/boutique|historic/.test(name + " " + types)) return false;
  ...
  if (subKey === "food:dinner" && /breakfast|brunch|^cafe$|coffee|dessert|ice_cream|yogurt|juice|bakery/.test(primaryToken)) return false;
