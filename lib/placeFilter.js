...
export function placeAllowed(categoryId, subId, p) {
  ...
  if (categoryId === "food" && (subId === "breakfast" || subId === "dessert" || subId === "cafes")) {
    ...
    if (subId === "breakfast" && /\b(chick[\s'-]*fil[\s'-]*a|mcdonald'?s?|burger king|wendy'?s|taco bell|subway|kfc|popeyes|sonic drive[\s-]*in|arby'?s|jack in the box|whataburger|white castle|checkers|rally'?s|culver'?s|five guys|raising cane'?s|zaxby'?s|bojangles)\b/i.test(name)) return false;
