...
//     A provider's boolean `openNow` snapshot counts as evidence ONLY while
//     demonstrably fresh (hoursAsOf within SNAPSHOT_TRUST_MS); an unstamped
//     or aged snapshot is an unknown, not a claim (v6.34 — the "Open now" +
//     "Hours not listed" contradiction).
...
// v6.34: how long a provider's boolean open_now snapshot stays trustworthy on
// rows with no structured hours. Fetch-time booleans survive the shared cache
// for weeks; beyond this window they degrade to "unknown", never an assertion.
export const SNAPSHOT_TRUST_MS = 90 * 60 * 1000; // 90 minutes
...
// ── OPERATIONAL gate ─────────────────────────────────────────────────────────
//
// THE ONE DEFINITION. This predicate already existed, correct, in two places:
// lib/inventoryServe.js:69 (serve) and scripts/seed-places.mjs:222 (write). It is
// lifted here verbatim in shape so every lane calls one definition instead of
// re-deriving it — the same reason businessStatus() itself exists.
//
// It is an ALLOWLIST on "OPERATIONAL", deliberately, not a denylist of
// CLOSED_PERMANENTLY / CLOSED_TEMPORARILY:
//   • null / undefined / "" means Google did not tell us — that is not evidence
//     of closure, so it passes (a place with no businessStatus is still a place).
//   • any value that is not OPERATIONAL is excluded, so an enum Google adds
//     later (CLOSED_SEASONALLY, whatever) is excluded on arrival rather than
//     silently recommended until someone notices.
//
// A denylist would have to be edited every time Google changes the enum. This
// never does.
//
// This is NOT an hours check. A permanently-closed business is not "closed right
// now", it is a place that no longer exists, and it must never reach any
// recommendation surface. Hours live in businessStatus() below.
export function isOperational(statusOrPlace) {
  const st = statusOrPlace && typeof statusOrPlace === "object"
    ? (statusOrPlace.status != null ? statusOrPlace.status : statusOrPlace.businessStatus)
    : statusOrPlace;
  return !st || st === "OPERATIONAL";
}

/** Filter helper — same predicate, list form. Never mutates the input. */
export function operationalOnly(rows) {
  return (Array.isArray(rows) ? rows : []).filter((r) => r && isOperational(r));
}
...
export function businessStatus(place, nowMs) {
  ...
  // gave an explicit boolean snapshot that is DEMONSTRABLY FRESH (stamped
  // hoursAsOf within SNAPSHOT_TRUST_MS). v6.34: an unstamped or aged snapshot
  // asserted "Open now" on venues whose hours panel said "Hours not listed"
  // (fsq open_now booleans surviving the shared cache) — an unverifiable
  // snapshot is an unknown, never a claim.
  if (!Array.isArray(periods) || !periods.length || off == null) {
    const snap = place && place.openNow;
    if ((snap === true || snap === false) && hoursAsOf != null && (now - hoursAsOf) <= SNAPSHOT_TRUST_MS) {
...
/**
 * v8.89 — TAKES THE MOMENT IT IS BEING ASKED ABOUT.
 *
 * This dropped `nowMs` on the floor and businessStatus therefore fell back to
 * Date.now(). That is invisible while the two agree, and wrong the instant they
 * do not — which is exactly what lib/daylight.js servableNow() does: it is
 * handed a `now` (a rail computed for a band, a guard walking a boundary, a
 * cached response evaluated for the minute it was built for) and asked whether
 * a place is open THEN.
 *
 * Found by test-rail-daylight going red on a clean tree between midnight and
 * 6am: the fixture asks "is a park publishing 06:00-23:59 open at 11pm?", the
 * hours layer answered for the real wall clock instead, and after midnight the
 * real answer is no. Every assertion about the other 18 hours of the day passed
 * every day, which is why it took a late run to surface.
 *
 * @param {number} [nowMs] the instant to answer for; defaults to now.
 */
export function openNowFromHours(regularOpeningHours, utcOffsetMinutes, nowMs) {
  const periods = regularOpeningHours && regularOpeningHours.periods;
  if (!periods || !periods.length || utcOffsetMinutes == null) return null;
  return businessStatus({ oh: { periods }, utcOffset: utcOffsetMinutes }, nowMs).open;
}
