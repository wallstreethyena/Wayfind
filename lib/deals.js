...
//   • Correction 2026-09-09: Cloudflare 403 is UNKNOWN. It proves neither
//     existence nor death. 404/410 are negative evidence; 5xx is transient.
//     Historic redirect evidence is not permission to generate audit clicks.
...
// TicketNetwork via CJ — SAME dlg raw-path form as Undercover Tourist above,
// on the same PID. Verified live 2026-08-11: a ticketnetwork.com destination in
// raw-path position returns 302 -> cj.dotomi.com (the real-attribution first
// hop), exactly like the UT form. Host-locked to ticketnetwork.com for the same
// reason rawPathDeepLink refuses every host but UT: the two registries must not
// be able to smuggle each other's destinations.
export function tnRawPathDeepLink(destUrl, sid = "venue") {
  if (!destUrl || !/^https:\/\//i.test(destUrl)) return null;
  try { if (!/^(?:www\.)?ticketnetwork\.com$/i.test(new URL(destUrl).hostname)) return null; }
  catch { return null; }
  const s = String(sid || "venue").replace(/[^\w.:-]/g, "").slice(0, 40) || "venue";
  return `https://www.anrdoezrs.net/links/${CJ_PID}/type/dlg/sid/${s}/${destUrl}`;
}

// Hand-verified Undercover Tourist wf_deals rows a PLACE card may hook
// (id -> the maps_to the row itself claims). Read live from wf_deals
// 2026-08-11: every id below was active=true AND link_ok=true. This pin is
// what lets check-intent-partner-picks assert a UT place hook points at a
// cron-health-checked row without a build-time DB read; the deals-health cron
// keeps the underlying affiliate_url attributed (hasCjPid) and alive.
export const UT_PLACE_DEAL_IDS = Object.freeze({
  "5": "walt disney world",
  "6": "universal orlando",
  "17": "kennedy space center",
});

// Hand-verified Undercover Tourist EVENT-TICKET rows (id -> the wf_events id
// whose OWN ticket that row sells). Read live from wf_deals 2026-09-03; every
// id below was inserted by scripts/seed-fall-ticket-deals-2026.mjs only after
// its CJ deep link 302'd to cj.dotomi.com (the attribution hop) and its UT page
// body was read (H1 quoted in that script). lib/eventTicketDeals.js may map a
// separately-ticketed event ONLY to an id pinned here, and only to the event
// named here — scripts/check-augtober-rail.mjs executes that pair, so a
// park-admission row can never be relabelled as an event ticket by mistake.
export const UT_EVENT_DEAL_IDS = Object.freeze({
  "8": "mnsshp-2026",
  "19": "hhn-orlando-2026",
  "20": "howl-o-scream-tampa-2026",
  "21": "howl-o-scream-seaworld-2026",
  // 2026-09-21 — current Undercover Tourist inventory exposes exact event
  // tickets for both Disney holiday parties. These are NOT park-admission
  // substitutes; each row sells the separately ticketed event itself.
  "38": "mvmcp-2026",
  "39": "jollywood-nights-2026",
});
...
//     treats 403 as unknown; a bot challenge cannot prove health or death;
...
// A Clipp destination we are willing to wrap: https, on clipp.com, and on one
// of the TWO path shapes that actually render. Anything else returns false so
// the caller can refuse rather than send a user to an error page carrying our
// affiliate tracking.
//
// Shape 2 (/all-offers/<slug>, added 2026-08-07): the per-merchant offer page.
// Same registry discipline as the market pages — every /all-offers/ slug we
// wrap lives in lib/clippOffers.js CLIPP_MERCHANT_OFFERS with the browser
// evidence that it rendered, because Clipp's Akamai 403 makes a slug
// unverifiable by anything that is not a browser. This widening is a dest
// ALLOWLIST-SHAPE change only; membership still comes from the registry, so a
// guessed slug has no path into a link (clippTrackedUrl resolves ids, never
// URLs, and unknown ids are null).
export function isClippDest(destUrl) {
  ...
  try {
    ...
    return /^\/states\/[a-z]{2}\/cities\/[a-z0-9-]+\/?$/i.test(u.pathname)
      || /^\/all-offers\/[a-z0-9-]+\/?$/i.test(u.pathname);
  } catch { return false; }
}
...
// Destination reachability only: 2xx true, 404/410 false, otherwise unknown.
// Neither a successful HTTP request nor a configured PID proves commission.
export function destIsAlive(status) {
  if (status >= 200 && status < 300) return true;
  if (status === 404 || status === 410) return false;
  return null; // blocked, unresolved redirect, transient error: unknown
}
...
export function judgeLink({ affFirstHop, affLocation, destStatus }) {
  ...
  return { pass: forwards && alive === true, forwards, alive, http_status: destStatus || 0 };
}
