...
import { listGuidePlaceIds } from "./guidePlaceIndex";
...
// generateStaticParams + sitemap). Silent Atlas-590 rows stay out. Also
// unioned with every GUIDES-linked placeId (lib/guidePlaceIndex.js) — a
// curated guide pick belongs in the sitemap even on the day it is first
// added, before any search has ever put it in wf_place_ids.
export async function listIndexedIds(limit = 500) {
  const indexed = (await listIndexedPlaces(limit)).map((x) => x.place_id);
  const withAtlas = unionIndexedAndAtlasIds(indexed, listPublishReadyAtlasIds());
  return unionIndexedAndAtlasIds(withAtlas, listGuidePlaceIds());
}
