...
// construction.
export const EDITORIAL_SERVABLE_SELECT = "place_id,why_here";
...
export async function listVerifiedEditorialIds() {
  ...
  try {
    const r = await fetchDeadline(
      `${s.url}/rest/v1/wf_editorial_servable?verified=is.true&select=${EDITORIAL_SERVABLE_SELECT}&order=place_id.asc&limit=1000`,
      { headers: { apikey: s.key, Authorization: `Bearer ${s.key}` }, cache: "no-store" },
      DB_DEADLINE_MS,
    );
    if (!r.ok) return [];
