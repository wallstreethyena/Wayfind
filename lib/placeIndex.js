...
export async function getVerifiedEditorial(id) {
  ...
  try {
    const r = await fetchDeadline(
      `${s.url}/rest/v1/wf_editorial_servable?place_id=eq.${encodeURIComponent(id)}&verified=is.true&select=place_id,name,why_here&limit=1`,
      { headers: { apikey: s.key, Authorization: `Bearer ${s.key}` }, cache: "no-store" },
      DB_DEADLINE_MS,
    );
    if (!r.ok) return null;
...
async function placeIdentityRows(ids) {
  const list = Array.isArray(ids) ? ids.filter(Boolean).slice(0, 500) : [];
  const s = sbEnv();
  if (!s || !list.length) return [];
  try {
    const r = await fetchDeadline(
      `${s.url}/rest/v1/wf_place_ids?select=place_id,name,lat,lng,category&place_id=in.(${list.map(encodeURIComponent).join(",")})`,
      { headers: { apikey: s.key, Authorization: `Bearer ${s.key}` }, cache: "no-store" },
      DB_DEADLINE_MS,
    );
    if (!r.ok) return [];
...
export async function listVerifiedEditorialIds() {
  ...
  try {
    const r = await fetchDeadline(
      `${s.url}/rest/v1/wf_editorial_servable?verified=is.true&select=place_id,why_here&order=place_id.asc&limit=1000`,
      { headers: { apikey: s.key, Authorization: `Bearer ${s.key}` }, cache: "no-store" },
      DB_DEADLINE_MS,
    );
    if (!r.ok) return [];
    ...
    const identity = await placeIdentityRows(withText.map((e) => e.place_id));
    const withIdentity = new Set(
      identity.filter((x) => x.name && (typeof x.lat === "number" || typeof x.lng === "number")).map((x) => x.place_id),
    );
...
export async function listIndexedIds(limit = 500) {
  ...
  // Curated (Atlas + GUIDES) ids are never truncated by `limit` — they are a
  // bounded, hand-vetted set (a few hundred ids) and dropping one because the
  // editorial set happened to be large would silently pull a real page out
  // of the sitemap. `limit` instead bounds how many editorial-only ids on
  // top of that can grow the set.
  const n = Math.min(Math.max(Number(limit) || 500, curated.length), 5000);
  return united.slice(0, n).sort();
}
...
export async function listEligiblePlaces(limit = 200) {
  ...
  if (unresolved.length) {
    const rows = await placeIdentityRows(unresolved);
    for (const r of rows) {
