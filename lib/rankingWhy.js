...
import { toHookLine, hostThemeOverlaps, isFillerWhyFits } from "./editorialHook.js";
...
export function placeCardHook(place, hostText) {
  if (!place) return "";
  const hook = sourcedRankingWhy({
  ...
  if (!hook) return "";
  const hosts = Array.isArray(hostText) ? hostText : (hostText != null ? [hostText] : []);
  for (const t of hosts) {
    if (hostThemeOverlaps(hook, t)) return "";
  }
  return hook;
}
...
export function rankingWhyLine(p) {
  ...
  if (why && !isFillerWhyFits(why)) bits.push(why);
  if (!bits.length) return "";
  return bits.join(" · ") + ".";
}

/**
 * Landing-list Why it fits. Same sourced fields as house cards (placeCardHook
 * / fleet why_here). Never invent. Never append miles-from-center. Generic
 * vibe + distance is empty, not a heading with filler.
 */
export function landingWhyFits(place, editorial) {
  if (!place) return "";
  const whyHere = (editorial && editorial.why_here) || place.why_here || "";
  if (whyHere && !isFillerWhyFits(whyHere)) {
    const compressed = toHookLine(whyHere, place.name);
    if (compressed) return String(whyHere).replace(/\s+/g, " ").trim();
  }
  const hook = placeCardHook({
    id: place.id || place.place_id,
    place_id: place.place_id || place.id,
    name: place.name,
    why_here: whyHere || undefined,
    whyGo: place.whyGo,
    hook: (editorial && editorial.hook) || place.hook,
  });
  if (!hook || isFillerWhyFits(hook)) return "";
  return hook;
}
