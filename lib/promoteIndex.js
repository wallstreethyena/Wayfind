...
export function validateInventoryRow(w, { metroKey, metros = PROMOTE_METROS } = {}) {
  ...
  if (!w.anchor) {
    const rec = classify({ types: w.google_types, primaryType: w.primary_type, name: w.name });
...
export function decidePromotion(place, metroKey, nowIso) {
  const built = buildInventoryRow(place, metroKey, { nowIso });
  if (!built.row) return { action: "reject", error: `unclassified: ${built.reason}` };
  const row = toWriteRow(built.row, nowIso);
  const v = validateInventoryRow(row, { metroKey });
  if (!v.ok) return { action: "reject", error: v.errors.join("; ").slice(0, 300) };
  return { action: "promote", row };
}
