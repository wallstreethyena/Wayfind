...
// Promotion metro bounding boxes — the OFFLINE FALLBACK, not the source of
// truth. public.wf_promote_metros (migration 20260813_wf_promote_metros.sql)
// is authoritative; the route (app/api/cron/promote-index) and the worker
// (scripts/promote-worker.mjs) fetch it live at run start and use THAT set,
// falling back to this constant only when the table can't be reached. This
// constant existing as an independent, separately-maintained list is exactly
// what let it go stale on 2026-08-23 (miami-dade/broward/palm-beach/keys/
// florida added to the table, never mirrored here) while nothing but a
// prebuild-only, file-vs-file parity guard was watching — see
// scripts/check-promote-metros-parity.mjs and scripts/check-promote-metros-live-drift.mjs.
// DELIBERATELY separate from seedPlaces METRO_BOUNDS: promotion buckets the
// EXISTING index geographically and must not change the seeder's grid metros.
// Boxes cover the metro core + near suburbs; they may overlap (Tampa/St.
// Pete) — bucketMetro breaks ties by nearest center.
export const PROMOTE_METROS = {
  ...
  // Read-only reconciliation on 2026-09-06: these five boxes are already
  // active in wf_promote_metros. Mirror them for database-read failures;
  // the live reader, enqueue rules, seed grid and spend caps do not change.
  "miami-dade":       { minLat: 25.50, maxLat: 25.98, minLng: -80.50, maxLng: -80.10 },
  "broward":          { minLat: 25.98, maxLat: 26.32, minLng: -80.35, maxLng: -80.08 },
  "palm-beach":       { minLat: 26.32, maxLat: 26.95, minLng: -80.25, maxLng: -80.03 },
  "keys":             { minLat: 24.50, maxLat: 25.30, minLng: -81.90, maxLng: -80.30 },
  "florida":          { minLat: 24.40, maxLat: 31.10, minLng: -87.70, maxLng: -79.90 },
  // "global" (WO7, 2026-09-02) — the whole-planet catch-all mirrored from
  // supabase/migrations/20260902_wf_promote_global_bucket_opt_in.sql, added so
  // a curated place outside every named metro still buckets to SOMETHING
  // instead of null. It is NOT a fifth served market: both automated enqueue
  // paths (wf_enqueue_promotion's trigger and wf_promotion_backfill's cron
  // sweep) explicitly skip a 'global' bucket unless a caller asks for it by
  // name — see that migration's comments. Present here only so this offline
  // fallback stays byte-for-byte in parity with the live table (enforced by
  // scripts/check-promote-metros-parity.mjs / check-promote-metros-live-drift.mjs).
...
// metrosFromRows — shape rows fetched from the LIVE public.wf_promote_metros
// table into the SAME {metro:{minLat,maxLat,minLng,maxLng}} shape as
// PROMOTE_METROS, so a DB-fetched set is a drop-in replacement anywhere a
// `metros` map is accepted (bucketMetro, computeMissing, validateInventoryRow,
// decidePromotion). PURE — takes plain rows, no network, no clock.
//
// This is the fix for the 2026-09-01 outage: PROMOTE_METROS was the ONLY
// metro set the JS side ever consulted, so when wf_promote_metros (the table
// migration 20260813_wf_promote_metros.sql calls authoritative) gained
// miami-dade/broward/palm-beach/keys/florida on 2026-08-23, every queued place
// in those metros was rejected by validateInventoryRow with "unknown metro:
// <name>" even though the enqueue trigger (wf_bucket_metro, reading the SAME
// table) had correctly queued them. The route/worker now fetch this table at
// run start and pass the result through as `metros`; PROMOTE_METROS is kept
// only as the offline fallback for when that fetch fails.
//
// A malformed or explicitly inactive row is dropped rather than corrupting the
// whole map. An EMPTY result is deliberately indistinguishable from "every row
// was bad" — the caller treats empty as "fetch failed" and falls back to
// PROMOTE_METROS, never as "zero metros are now valid" (that would silently
// reject every place in every metro).
export function metrosFromRows(rows) {
  const out = {};
  for (const r of Array.isArray(rows) ? rows : []) {
    if (!r || typeof r.metro !== "string" || !r.metro) continue;
    if (r.active === false) continue; // defensive — callers should already filter active=is.true
    const minLat = Number(r.min_lat), maxLat = Number(r.max_lat);
    const minLng = Number(r.min_lng), maxLng = Number(r.max_lng);
    if (![minLat, maxLat, minLng, maxLng].every(isFiniteNum)) continue;
    if (!(minLat < maxLat) || !(minLng < maxLng)) continue;
    out[r.metro] = { minLat, maxLat, minLng, maxLng };
  }
  return out;
}
...
export function validateInventoryRow(w, { metroKey, metros = PROMOTE_METROS, adjudicatedSection = null } = {}) {
  const errors = [];
  ...
  // Honest category: the stored list must be what classify() derives from
  // types / primaryType / name already on the row. A write that carries
  // category:"food" with no type or name signal is a guess — reject it.
  // Anchors are owner-asserted and may disagree with Google's types.
  if (!w.anchor) {
    const rec = classify({ types: w.google_types, primaryType: w.primary_type, name: w.name, adjudicatedSection });
    if (!rec.category) errors.push("category not evidenced by types/primaryType/name");
    else if (rec.category !== w.category) errors.push(`category "${w.category}" disagrees with signals (classify=${rec.category} via=${rec.via})`);
    // An adjudicated row must PROVE its provenance in the row it is writing.
    // Independently re-derived here rather than trusted from the builder, so a
    // future caller cannot launder a model verdict into a verified-looking row.
    if (rec.via === "adjudicated") {
      if (w.source !== "scout_adjudicated") errors.push('adjudicated category must carry source "scout_adjudicated"');
      if (!w.needs_review) errors.push("adjudicated category must land needs_review=true");
      if (w.last_verified_at != null) errors.push("adjudicated category may not claim last_verified_at");
    }
  }
...
// decidePromotion — the PER-PLACE VERDICT, extracted so the hand-run promoter,
// the /api/cron/promote-index worker and the test all share ONE decision. Given a
// raw Google Place Details resource, it returns exactly one of:
//
//   { action: "promote", row }     a validated wf_inventory write-row
//   { action: "reject", error }    a verdict about the DATA — unclassifiable, out
//                                  of bounds, or non-operational. Re-fetching will
//                                  return the same answer, so the queue must not
//                                  retry it and spend again.
//
// There is deliberately no "retry" outcome here: a transient fault (429, 5xx,
// socket) happens BEFORE this function is reached, because it is a property of
// the fetch, not of the place. Keeping the two apart is what stops a Google
// outage from permanently rejecting a thousand good places.
// `adjudicatedSection` and `metros` are the two params added after this
// function shipped, both optional, and omitting either (every pre-existing
// caller) reproduces the previous behaviour exactly:
//   - `adjudicatedSection` is consumed only by classify()'s last net — see
//     lib/scoutAdjudicate.js.
//   - `metros` is the geography validateInventoryRow checks bounds against.
//     Defaulting to PROMOTE_METROS keeps every existing caller and test
//     working unchanged; a caller that has fetched the LIVE wf_promote_metros
//     table (see metrosFromRows above) passes that result here instead, so a
//     metro the database already knows about is never rejected as "unknown"
//     just because the JS fallback constant has not caught up.
export function decidePromotion(place, metroKey, nowIso, adjudicatedSection = null, metros = PROMOTE_METROS) {
  const built = buildInventoryRow(place, metroKey, { nowIso, adjudicatedSection });
  if (!built.row) return { action: "reject", error: `unclassified: ${built.reason}` };
  const row = toWriteRow(built.row, nowIso);
  const v = validateInventoryRow(row, { metroKey, metros, adjudicatedSection });
  if (!v.ok) return { action: "reject", error: v.errors.join("; ").slice(0, 300) };
  return { action: "promote", row, adjudicated: built.adjudicated === true };
}
