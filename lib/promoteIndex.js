...
// 2026-08-22: thirteen Florida boxes added. 6,651 indexed places clear the
// owner's 9.2 Wayfind score without being servable; 3,009 of those are in
// Florida and only 703 sat inside the four original boxes. The other 2,306 were
// unreachable for one reason — no box contained them. Each new box is drawn
// around a MEASURED cluster, with its 9.2+ population counted before the bounds
// were written (see supabase/migrations/20260822_wf_promote_metros_florida.sql).
//
// `clearwater` is the instructive one: 100 places clearing 9.2, sitting 0.05°
// west of the Tampa box's edge. The home market was losing its own beach towns
// to a rounding decision.
//
// The 3,642 places outside Florida stay unreachable ON PURPOSE — that is what
// the 20260813 migration exists for.
export const PROMOTE_METROS = {
  ...
  "miami":             { minLat: 25.55, maxLat: 26.00, minLng: -80.50, maxLng: -80.10 },
  "fort-lauderdale":   { minLat: 26.00, maxLat: 26.45, minLng: -80.35, maxLng: -80.05 },
  "palm-beach":        { minLat: 26.45, maxLat: 26.95, minLng: -80.30, maxLng: -80.02 },
  "naples-fort-myers": { minLat: 26.10, maxLat: 26.75, minLng: -82.05, maxLng: -81.60 },
  "venice-punta-gorda":{ minLat: 26.85, maxLat: 27.15, minLng: -82.50, maxLng: -82.00 },
  "clearwater":        { minLat: 27.88, maxLat: 28.22, minLng: -82.92, maxLng: -82.72 },
  "space-coast":       { minLat: 28.00, maxLat: 28.60, minLng: -80.90, maxLng: -80.55 },
  "orlando-north":     { minLat: 28.75, maxLat: 28.92, minLng: -81.65, maxLng: -81.10 },
  "daytona":           { minLat: 29.00, maxLat: 29.45, minLng: -81.20, maxLng: -80.90 },
  "ocala-gainesville": { minLat: 29.10, maxLat: 29.80, minLng: -82.50, maxLng: -82.00 },
  "st-augustine":      { minLat: 29.75, maxLat: 30.05, minLng: -81.45, maxLng: -81.20 },
  "jacksonville":      { minLat: 30.05, maxLat: 30.55, minLng: -81.90, maxLng: -81.30 },
  "destin":            { minLat: 30.32, maxLat: 30.52, minLng: -86.70, maxLng: -86.30 },
  "pensacola":         { minLat: 30.28, maxLat: 30.62, minLng: -87.45, maxLng: -87.00 },
};
...
// `adjudicatedSection` is the ONLY new input, it is optional, and omitting it
// (every pre-existing caller) reproduces the previous behaviour exactly. It is
// consumed only by classify()'s last net — see lib/scoutAdjudicate.js.
export function decidePromotion(place, metroKey, nowIso, adjudicatedSection = null) {
  ...
  const v = validateInventoryRow(row, { metroKey, adjudicatedSection });
