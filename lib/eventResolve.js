...
import { haversineMi, siteTodayStr } from "./eventsPipeline.js";
import { siteAnchorDate } from "./siteTime.js";
import { sbEnv } from "./serverCache.js";
import { DB_DEADLINE_MS, fetchDeadline } from "./fetchDeadline.js";
...
// v6.03 timezone fix: the Manatee LibCal feed stamps DTSTART in UTC (trailing
// Z, e.g. 20260713T220000Z). The old code copied the raw UTC clock into the
// displayed `time`, so a 6 PM library program (22:00Z) rendered as "22:00" —
// the "10 PM library events" bug. When the stamp is UTC we now derive BOTH the
// displayed date and time from the Gulf-Coast local (America/New_York) wall
// time of that instant, which also corrects the date when the instant crosses
// local midnight (00:30Z on the 14th is really 8:30 PM on the 13th). `dt` stays
// the true UTC instant (used for sorting/window filtering, unchanged). Floating
// (no-Z) and date-only stamps are already local and pass through untouched.
...
export function generateStaples(now = new Date(), opts = {}) {
  ...
  const out = [];
  const d = siteAnchorDate(now); // ET-anchored occurrence math + event-id date (was UTC → off-by-a-day after 8 PM ET)
  const pushOn = (target, ev) => {
    const date = target.getFullYear() + "-" + String(target.getMonth() + 1).padStart(2, "0") + "-" + String(target.getDate()).padStart(2, "0"); // ET day (target derives from siteAnchorDate)
...
// The events feed already stores its post-validation rows in wf_places_cache
// under ev1|... keys. Those rows are the inventory a reader actually saw, so a
// detail click should be able to reuse a still-fresh matching row when a
// provider credential is unavailable. Keep this read narrow and fail closed:
// exact id + provider, a real current date, a usable official URL, and no
// cancelled/postponed state. We never manufacture a row from the URL slug.
function validCachedLocalDate(value) {
...
function validCachedLocalTime(value) {
  if (value == null || value === "") return true;
  const match = String(value).match(/^(\d{2}):(\d{2})(?::(\d{2}))?$/);
  return !!match && Number(match[1]) <= 23 && Number(match[2]) <= 59 && (match[3] == null || Number(match[3]) <= 59);
}

export function cachedTicketmasterEventFromRows(rows, fullId, now = new Date()) {
  const today = siteTodayStr(now);
  for (const row of Array.isArray(rows) ? rows : []) {
    const events = row && Array.isArray(row.v) ? row.v : [];
    const event = events.find((candidate) => candidate && candidate.id === fullId);
    if (!event || event.source !== "Ticketmaster" || !event.name || !validCachedLocalDate(event.date) || !validCachedLocalTime(event.time)) continue;
    if (event.date < today || /cancelled|canceled|postponed/i.test(event.status || "")) continue;
    try {
      const official = new URL(event.url || "");
      if (official.protocol !== "https:") continue;
    } catch { continue; }
    return event;
  }
  return null;
...
export async function resolveCachedTicketmaster(fullId) {
  if (!/^tm_[A-Za-z0-9_.:-]{1,116}$/.test(String(fullId || ""))) return null;
  const server = sbEnv();
  if (!server) return null;
  const query = new URLSearchParams({
    // The primary-key prefix confines the JSON containment check to event-feed
    // rows instead of scanning the shared cache's place/photo population.
    k: "like.ev1|*",
    v: `cs.${JSON.stringify([{ id: fullId }])}`,
    exp: `gt.${new Date().toISOString()}`,
    select: "v,wrote_at",
    order: "wrote_at.desc.nullslast",
    limit: "5",
  });
  try {
    const response = await fetchDeadline(`${server.url}/rest/v1/wf_places_cache?${query}`, {
      headers: { apikey: server.key, Authorization: `Bearer ${server.key}` },
      cache: "no-store",
    }, DB_DEADLINE_MS);
    if (!response.ok) return null;
    return cachedTicketmasterEventFromRows(await response.json(), fullId);
  } catch { return null; }
}

async function resolveTicketmaster(rawId) {
  const fullId = "tm_" + rawId;
  const key = process.env.TICKETMASTER_API_KEY;
  if (!key) return resolveCachedTicketmaster(fullId);
  try {
    const r = await fetchDeadline(`https://app.ticketmaster.com/discovery/v2/events/${encodeURIComponent(rawId)}.json?apikey=${encodeURIComponent(key)}`, { next: { revalidate: 600 } });
    // A definitive provider removal wins over the older feed cache. Only a
    // transient failure may fall back to the exact fresh row the reader saw.
    if (r.status === 404 || r.status === 410) return null;
    if (!r.ok) {
      if (r.status === 408 || r.status === 429 || r.status >= 500) return resolveCachedTicketmaster(fullId);
      return null;
    }
  ...
  } catch { return resolveCachedTicketmaster(fullId); }
}
...
function resolveStaple(fullId) {
  // v5.84: resolve against a window that COVERS the feed's full forward horizon,
  // not a narrower one. The feed emits Sunday staples up to ~20 days out
  // (generateStaples days(0,3), back:0). With only 2 weeks back here, a card for
  // a 15-20-day-out date that was tapped after that date passed (a tab left open,
  // a shared/bookmarked link) fell outside the window and 404'd — the forward/back
  // asymmetry. back:4 (28 days) covers the ~20-day horizon with margin, so a
  // recurring staple's link resolves to its page instead of a dead 404.
  const hit = generateStaples(new Date(), { back: 4 }).find((e) => e.id === fullId);
  if (!hit) return null;
...
async function resolveEventByIdUncached(id) {
...
// generateMetadata and the page body ask for the same event during one server
// render. React's request cache makes that one authoritative lookup, avoiding
// a provider success for metadata followed by a transient body failure. The
// identity fallback keeps this server helper directly runnable in the repo's
// plain Node guards, whose stable React 18 entry does not expose cache().
export const resolveEventById = typeof React.cache === "function"
  ? React.cache(resolveEventByIdUncached)
  : resolveEventByIdUncached;
