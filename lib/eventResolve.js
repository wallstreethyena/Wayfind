...
export function generateStaples(now = new Date()) {
  const out = [];
  const d = now;
  ...
  const days = (dow, weeks) => {
    ...
    for (let w = 0; w < weeks; w++) {
      ...
      if (t.getTime() >= new Date(d.getFullYear(), d.getMonth(), d.getDate()).getTime()) arr.push(t);
    }
    return arr;
  };
...
function resolveStaple(fullId) {
  const hit = generateStaples().find((e) => e.id === fullId);
  if (!hit) return null;
