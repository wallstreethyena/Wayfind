...
import { isDeniedViatorSku, isViatorSearchOrHomeUrl } from "./viatorDenylist.js";
...
/**
 * True only for an exact WeGoTrip PRODUCT page:
 *   /<city-slug>-d<digits>/<product-slug>-p<digits>/
 * A bare city page (e.g. wegotrip.com/miami-d4164138/, no product segment)
 * or the homepage is NOT a product and must never become a redirect
 * destination. This is the reason WeGoTrip was dark for so long: the OLD
 * generic Wayfind link resolved to exactly that kind of homepage. Every
 * PARTNER_OFFER_REGISTRY wegotrip row was hand-verified to end in a real
 * -p<digits> product segment (see the registry's own "soft-404 risk" note),
 * and this check is the runtime enforcement of that same rule — a poisoned
 * or edited registry row pointing at a bare city page fails resolve() rather
 * than redirecting a user to a page with no product on it.
 */
export function isWegotripProductUrl(rawUrl) {
  let u;
  try { u = new URL(String(rawUrl || "").trim()); } catch { return false; }
  if (!/^https?:$/.test(u.protocol)) return false;
  if (!/(^|\.)wegotrip\.com$/i.test(u.hostname)) return false;
  return /^\/[^/]+-d\d+\/[^/]+-p\d+\/?$/.test(u.pathname);
}

/**
 * PURE function PROVIDERS.wegotrip.resolve is BUILT FROM, not a parallel
 * re-implementation of it. Lane F, 2026-09-16: before this existed, nothing in
 * the guard suite called `resolve()` with a registry-row-shaped object whose
 * `destination` was a non-product URL (every real registry row is already a
 * hand-verified product page), so deleting the `isWegotripProductUrl` gate
 * from inside `resolve` would have left check-commerce-redirect green — the
 * gate was only ever exercised indirectly, by calling `isWegotripProductUrl`
 * on its own, never by calling the code path `resolve` actually runs. Pulling
 * the gate out into its own exported function lets the guard call the EXACT
 * logic `resolve` depends on with a fabricated `{ destination }` row (a bare
 * city page, the homepage) and prove the refusal happens on that path, not
 * merely somewhere in the file.
 *
 * @param {{destination?: string|null}|null} row  a PARTNER_OFFER_REGISTRY-shaped
 *   row (or any object carrying `.destination`) — never a raw string, so a
 *   caller cannot accidentally pass an offer id here and get a false negative.
 * @returns {string|null} the destination, only if it is a real wegotrip.com
 *   PRODUCT url; null for a missing row, a missing destination, a bare city
 *   page, the homepage, or any non-wegotrip host.
 */
export function wegotripDestination(row) {
  const dest = (row && row.destination) || null;
  return dest && isWegotripProductUrl(dest) ? dest : null;
}
 ...
 * Klook is enabled below only for exact curated products; there is no
 * provider-wide search or homepage escape hatch.
 *
 * WeGoTrip was deliberately absent for a long time: an OLD generic Wayfind
 * link resolved to a homepage, not a product. It is enabled below ONLY for
 * the 4 hand-verified registry rows, and `resolve` re-checks every
 * destination against isWegotripProductUrl() before it can ever reach
 * finishDest's host gate — a bare city page or the homepage refuses here,
 * not merely by convention in the registry comment.
 */
export const PROVIDERS = {
  viator: {
    ...
    // 2026-08-26 link-health audit: a row the nightly sweep has PROVEN dead
    // (link_ok=false) must not become a redirect — Viator answers a retired
    // product URL with its "similar experiences" search page, which is the
    // owner-reported "lands on search instead of the activity" failure wearing
    // a Book label. Null/never-checked still serves; only proven-dead refuses.
    deadColumn: "link_ok",
  },
  ...
  undercover_tourist: {
    ...
    requireTracking: true,
    // 2026-09-15 affiliate coverage audit: three of the four surfaces that
    // render a UT event ticket (the /florida-events page, guide CTAs, the
    // Events feed) build the CTA from the static registry with no live row in
    // hand, so render-time gating alone could send a click to a row the
    // deals-health cron has PROVEN dead. Same rule as viator above: only
    // link_ok === false refuses; null (unknown, UT blocks bot probes) serves.
    deadColumn: "link_ok",
  },
  ...
  // WeGoTrip — see isWegotripProductUrl() above and the header comment. NOT
  // curatedTpProvider: that helper trusts the registry row's `destination`
  // outright, and this provider needs one extra gate curatedTpProvider does
  // not have — `resolve` re-validates the destination is a real product page
  // before finishDest ever sees it, so a bare city page or the homepage
  // refuses at resolve() rather than merely failing to appear in the registry.
  wegotrip: {
    resolve: (offerId) => wegotripDestination(partnerOfferById(offerId, "wegotrip")),
    hosts: [/(^|\.)wegotrip\.com$/i],
    track: (url) => tpDeepLink("wegotrip", url),
    trackHosts: TP_REDIRECT_HOSTS,
    requireTracking: true,
  },
...
export async function resolveOffer(provider, offerId, deps = {}) {
  ...
  // HOLD denylist is enforced HERE, not in a comment. A catalogue row for
  // 236862P2 / 22211P1 cannot become a Book redirect.
  if (provider === "viator" && isDeniedViatorSku(offerId)) return { error: "denied-sku" };
  ...
  const cols = cfg.deadColumn ? `${cfg.idColumn},${cfg.urlColumn},${cfg.deadColumn}` : `${cfg.idColumn},${cfg.urlColumn}`;
  const url =
    `${s.url}/rest/v1/${cfg.table}?select=${cols}` +
  ...
  // A dead catalogue row fails CLOSED (to our own site, like every other error
  // here) rather than redirecting to a destination we know no longer exists.
  if (cfg.deadColumn && rows[0][cfg.deadColumn] === false) return { error: "offer-link-dead" };
...
function finishDest(raw, cfg, ctx) {
  // A search page or bare homepage is never a Book destination, even when
  // the catalogue row is otherwise well-formed.
  if (isViatorSearchOrHomeUrl(raw)) return { error: "search-is-not-book" };
  ...
  if (!cfg.track) return { dest: raw, sourceUrl: raw };
  ...
  if (dest && isAllowedHost(dest, okHosts)) return { dest, sourceUrl: raw };
