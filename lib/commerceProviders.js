...
export const PROVIDERS = {
  viator: {
    ...
    // Registry-first, table-second. A founder-verified exact product URL
    // (173028P1 — Shell Key clear kayak) lives in partnerOfferRegistry under
    // provider "viator". Product-code hooks that are NOT in that registry
    // (412732P1, …) still resolve against wf_experiences. A resolve miss
    // falls through to the table in resolveOffer — it is not a dead end.
    resolve: (offerId) => partnerOfferById(offerId, "viator")?.destination || null,
    hosts: [/(^|\.)viator\.com$/i],
...
export async function resolveOffer(provider, offerId, deps = {}) {
  ...
  if (typeof cfg.resolve === "function") {
    ...
    if (raw) return finishDest(raw, cfg, ctx);
    // Viator is BOTH registry-backed (exact pinned URLs) and table-backed
    // (wf_experiences product_code). A resolve miss on a table-backed
    // provider is not "offer-not-found" — it is "try the catalogue".
    if (!cfg.table) return { error: "offer-not-found" };
  }
