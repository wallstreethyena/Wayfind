...
// v9.0 — see isBirthdayPlace. Breakfast/coffee/pastry counters, by PRIMARY
// type only; `brunch_restaurant` is deliberately absent (a brunch room is a
// birthday format the owner named).
const MORNING_PRIMARY_RX = /^(breakfast_restaurant|coffee_shop|cafe|bakery|bagel_shop|donut_shop|tea_house|juice_shop|acai_shop)$/;
...
export function isBirthdayPlace(p) {
  ...
  // v9.0 — A MORNING PRIMARY IS NOT A BIRTHDAY PLAN (audit, 2026-09-07).
  // Live at band=night: First Watch (Parrish) and Keke's Breakfast Cafe
  // (Cortez), both `breakfast_restaurant` PRIMARY, ranked in the 9pm
  // "Birthday Plans, Solved" rail — admitted because Google also hangs a
  // SECONDARY `brunch_restaurant` on them and the type test below fires on
  // any entry in the array. Wayfind's own guide copy says of First Watch
  // "they close at 2:30pm — this is a birthday breakfast play, not a dinner
  // one". The primary type decides (lib/placeCategory.js law): a room whose
  // whole identity is breakfast, coffee or pastry is vetoed here even when a
  // birthday-type tag rides in its secondaries. A PRIMARY `brunch_restaurant`
  // (a real brunch room — the owner's garden-brunch shape) still qualifies.
  const primary = String((p.primaryType || p.primary_type) || "").toLowerCase();
  if (MORNING_PRIMARY_RX.test(primary)) return false;
