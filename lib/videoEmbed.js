...
// tokenless embeddable-by-id surface. "Embeddable" means an official iframe is
// available; it does not promise that the provider will play media inline.
// Instagram can render only a native "Watch on Instagram" handoff for a post.
// Facebook /share/r/ reels return null on
...
export function embedSrc(platform, url) {
  try {
    ...
    if (platform === "instagram") {
      ...
      if (!m) return null;
      // Keep /p/ as /p/: those URLs may be stills or carousels, and rewriting
      // one to /reel/ creates a broken player for a perfectly valid post.
