...
import { eventTicketDeal, eventTicketHref, VIA_BY_PROVIDER } from "./eventTicketDeals.js";
...
// Keep source provenance in the public record. Including source_type also
// gives the September 10 publication a new read-cache identity, so a cached
// pre-publication event page cannot retain superseded schedules or photo URLs.
const EVENT_COLUMNS =
  ...
  "editorial_score,uniqueness_score,popularity_score,source_url,event_page_url,link_ok,link_verdict,source_type";
...
/**
 * The organiser's OWN page for the "Official site" button (v8.99) — the same
 * three gates as eventOutboundUrl, but event page first: a reader pressing
 * "Official site" wants the organiser, not a ticket vendor. Ticket URL is the
 * floor only when it is the only link the row holds.
 */
export function eventWebsiteUrl(row) {
  if (!row || row.link_ok === false) return "";
  for (const raw of [row.official_event_url, row.event_page_url, row.official_ticket_url]) {
    const safe = safeUrl(raw);
    if (safe) return safe;
  }
  return "";
...
/**
 * Every displayable curated event.
 *
 * A FAILED READ IS NOT AN EMPTY QUEUE (atlas-build's selector rule, applied
 * here). This used to return [] on !supabase, on any query error, and in
 * catch — the same [] as a healthy empty table. /florida-events then rendered
 * "Nothing verified is coming up right now" with JSON-LD numberOfItems:0 as a
 * successful ISR response (revalidate=3600). Production served that cached
 * empty document while /api/events and individual event pages still had Howl.
 *
 * Runtime failures throw CuratedEventsUnavailableError. Next.js will not
 * treat that as a happy empty page: ISR keeps the last successful document
 * (or 500s if there is none) instead of caching "no events".
 *
 * Two []s that are still honest:
 *   * SSG / next build (`isSsgBuild()`): do not wait on Supabase.
 *     dpl_96WvKb restarted /florida-events/anastasia-manatee-performing-arts-2026
 *     for >60s. generateStaticParams then returns [] — the hub must NOT treat
 *     that skip as a healthy empty (see floridaEventsHubPageModel `ssg`).
 *   * A query that succeeded and returned zero displayable rows.
 *
 ...
 * PostgREST max-rows so a page can never itself be silently clipped. A page
 * that errors mid-walk throws — a partial list is the same silent truncation.
 */
const PAGE = 500;

export class CuratedEventsUnavailableError extends Error {
  constructor(reason, cause) {
    super("curated events unavailable: " + reason);
    this.name = "CuratedEventsUnavailableError";
    this.reason = reason;
    if (cause !== undefined) this.cause = cause;
  }
}

function curatedReadFail(reason, cause) {
  throw new CuratedEventsUnavailableError(reason, cause);
}

function checkedEventRows(data) {
  if (!Array.isArray(data) || data.some((row) => !row || typeof row !== "object" ||
      Array.isArray(row) || typeof row.event_id !== "string" || !row.event_id ||
      typeof row.event_status !== "string")) {
    curatedReadFail("malformed-response");
  }
  return data;
}
...
//
// `db` is a test-only reader injection. Production omits it and uses the
// cached/live client the `fresh` flag selected.
export async function fetchCuratedEvents({ limit = null, signal = AbortSignal.timeout(8000), fresh = false, db: dbOverride } = {}) {
  // SSG / next build: do not wait on Supabase. dpl_96WvKb restarted
  // /florida-events/anastasia-manatee-performing-arts-2026 for >60s.
  // generateStaticParams then returns [] — a skip, not a healthy empty queue.
  if (isSsgBuild()) return [];
  const db = fresh ? supabaseLive : supabase;
  const reader = dbOverride !== undefined ? dbOverride : db;
  if (!reader) curatedReadFail("supabase-unavailable");
  const q = () => {
    const query = reader
    .from("wf_events")
  ...
  try {
    if (Number.isFinite(limit) && limit > 0) {
      ...
      if (error) curatedReadFail("query-error", error);
      return checkedEventRows(data);
    }
    const all = [];
    for (let from = 0; ; from += PAGE) {
      const { data, error } = await q().range(from, from + PAGE - 1);
      if (error) curatedReadFail("query-error", error);
      checkedEventRows(data);
    ...
    if (err instanceof CuratedEventsUnavailableError) throw err;
    curatedReadFail("query-threw", err);
  }
}

export async function fetchCuratedEventBySlug(slug, { db: dbOverride } = {}) {
  if (isSsgBuild()) return null;
  if (!slug) return null;
  const reader = dbOverride !== undefined ? dbOverride : supabase;
  if (!reader) curatedReadFail("supabase-unavailable");
  try {
    const { data, error } = await reader
      .from("wf_events").select(EVENT_COLUMNS).eq("slug", slug)
      .abortSignal(AbortSignal.timeout(8000)).maybeSingle();
    if (error) curatedReadFail("query-error", error);
    if (data === null) return null;
    checkedEventRows([data]);
    if (!DISPLAYABLE_STATUS.has(data.event_status)) return null;
    // /florida-events/[slug] is the Florida-branded single-event page — a
    // non-Florida row is an honest 404 here, not a display bug elsewhere.
    // See isFloridaEvent below for why this lives here and not in isTrusted.
    if (!isFloridaEvent(data)) return null;
    return data;
  } catch (err) {
    if (err instanceof CuratedEventsUnavailableError) throw err;
    curatedReadFail("query-threw", err);
...
export const FLORIDA_EVENTS_HUB_EMPTY_COPY =
  "Nothing verified is coming up right now. Rather than pad this page, we have left it empty.";

/**
 * The hub's publish decision, given a completed list read.
 *
 * `ssg: true` is a skipped read (isSsgBuild), not a healthy empty. It must
 * not produce the empty-success document (empty copy + ItemList:0) that ISR
 * would then cache.
 *
 * A failed read (`ok !== true` or non-array rows) throws — the same error the
 * fetch throws — so a caller cannot launder an outage into "no events".
 */
export function floridaEventsHubPageModel(read, { now = new Date(), railKeys = [], ssg = false } = {}) {
  if (ssg) {
    return { skipped: true, empty: false, rails: [], eligible: [], numberOfItems: null };
  }
  if (!read || read.ok !== true || !Array.isArray(read.rows)) {
    curatedReadFail(read && read.reason ? read.reason : "invalid-read");
  }
  // The hub is branded "Florida Events" in its own title, metadata and
  // JSON-LD — a Chicago or a UK row filtered here can never reach a rail card
  // or the ItemList below it. See isFloridaEvent below for why this check
  // lives on the hub model rather than in isTrusted/isEligible.
  const floridaRows = read.rows.filter(isFloridaEvent);
  const ctx = { now, size: 8 };
  const rails = railKeys.map((k) => buildRail(k, floridaRows, ctx)).filter(Boolean);
  const eligible = floridaRows.filter((e) => isEligible(e, { now }));
  return {
    skipped: false,
    empty: rails.length === 0,
    rails,
    eligible,
...
// 2026-09-06 — THE MISSING GEOGRAPHY GATE. isTrusted/isEligible check status,
// source_tier, verification_confidence, card_hook and city — nothing here
// ever checked STATE. wf_events.state defaults to 'FL' in Postgres, so almost
// every row already carries it; nothing stopped a Chicago or a UK row from
// being trusted and eligible and landing on a hub whose own metadata,
// headline and JSON-LD all say "Florida Events".
//
// Deliberately NOT folded into isTrusted/isEligible: those two are shared by
// app/api/events/route.js and app/api/events/fall/route.js, which both serve
// an arbitrary lat/lng anywhere and already gate distance themselves
// (CURATED_REACH_MI, FALL_RAIL_RADIUS_MI) — for those, a "Florida-only" row
// is not a defect, it is the whole curated inventory. Baking a Florida check
// into the shared trust predicate would silently start dropping legitimately
// nearby rows for a reader standing outside Florida's bounding box, which
// those two routes never asked for. The gate belongs on the two surfaces that
// actually claim to be Florida: the hub (floridaEventsHubPageModel) and the
// single-event page (fetchCuratedEventBySlug), both above.
//
// state IS TRUSTED WHEN PRESENT. A null-state row predates the column's 'FL'
// default (a real, if rare, legacy shape) and must not be silently dropped —
// so a null state falls back to Florida's own bounding box against the row's
// coordinates. A row with neither a state nor coordinates cannot be placed on
// the map at all, and is rejected rather than guessed onto it.
const FLORIDA_BBOX = { minLat: 24.3, maxLat: 31.1, minLng: -87.7, maxLng: -79.7 };

export function isFloridaEvent(e) {
  if (!e) return false;
  if (e.state != null && String(e.state).trim() !== "") {
  ...
  return lat >= FLORIDA_BBOX.minLat && lat <= FLORIDA_BBOX.maxLat &&
    lng >= FLORIDA_BBOX.minLng && lng <= FLORIDA_BBOX.maxLng;
}
...
export function curatedToFeedEvent(row) {
  ...
  return {
    ...
    // via NAMES THE MERCHANT THE ENTRY ACTUALLY SELLS THROUGH — never a
    // hardcoded UT constant. 2026-09-15: an included-with-admission event at a
    // Tiqets/Klook-only merchant (lib/eventTicketDeals.js "zoo-boo-zoo-miami-
    // 2026") must read "Tickets · Tiqets", not silently claim Undercover
    // Tourist sold it.
    ticketVia: VIA_BY_PROVIDER[eventTicketDeal(row.event_id)?.provider] || "",
    ticketProduct: eventTicketDeal(row.event_id)?.product || "",
