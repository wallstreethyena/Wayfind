...
import { keepPhotoCredits } from "./photoCredits";
...
// Cache-only Place Details. NEVER calls Google. Used by loadPlace so an Atlas
// card can render from copy we already hold when the pd1| cache is cold.
export async function peekPlaceDetails(id) {
  if (!id) return null;
  const k = "pd1|" + id;
  const fresh = await cget(k);
  if (fresh && fresh.v && ("photoRef" in fresh.v)) return fresh.v;
  const st = await cget(k, { staleMs: STALE_MAX_MS });
  return (st && st.v) || (fresh && fresh.v) || null;
}
...
export async function getPlaceDetails(id) {
  ...
  try {
    ...
    const norm = normalizeDetails(raw);
    // PHOTO CREDIT (2026-09-23): normalizeDetails keeps only the photo name;
    // keep Google's author credit for it too (from this same response, no new call).
    if (norm) keepPhotoCredits([raw], FRESH_MS);
