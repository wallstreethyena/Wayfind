...
export function recordExposure(ctx, capture, opts) {
  ...
  // Exposure is recorded in a layout effect at mount — before the idle-booted
  // SDK exists — so a live client may be absent. Queue it (lib/browserAnalytics
  // captureOrQueue) instead of silently losing the $feature_flag_called event.
  const emit = capture || ((name, props) => {
    try {
      if (o.posthog) o.posthog.capture(name, props);
      else if (typeof window !== "undefined") captureOrQueue(window, name, props);
    } catch (e) {}
  });
