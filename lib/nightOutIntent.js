// The nine evidence-gated decisions behind the consolidated Night Out tile.
...
//
// ONE ITEM, ONE RAIL (owner, 2026-09-23): every place and every dated event
// lands on exactly one Night Out rail, its canonical identity below. The old
// cross-posting put every concert on Live Music, Shows AND a "Dinner +
// Entertainment" rail, so three shelves repeated the same Ticketmaster cards.
// That rail was removed at the owner's instruction: a genuine dinner show is
// a show, so it lives on Shows. Never re-add cross-posting to fill a shelf.
...
function isDinnerShow(item) {
  return explicit(item, /\b(dinner (?:theater|theatre|show|cabaret)|medieval times|pirates dinner|sleuths mystery dinner|themed dinner|meal and (?:a )?show)\b/);
}
...
// Social Play and a comedy club or dinner show Shows. (A dinner CRUISE is not a
// dinner show: it falls through to Waterfront.)
export function nightOutPlaceRail(place) {
  ...
  if (isDinnerShow(place)) return "shows";
...
export function nightOutPlaceRails(place) {
  // Kept as an array so callers iterate one shape; it never holds more than
  // the canonical rail (see ONE ITEM, ONE RAIL at the top of this file).
...
export function nightOutEventRail(event) {
  ...
  // The event's own ticketing taxonomy outranks words in its title. Without
  // this, "Little Big Town - For The Art Of It Tour" read as an art TOUR and
  // landed on Night Tours, and a band like Arcade Fire would read as an arcade.
  const segment = lower(event?.segment);
  if (isDinnerShow(event)) return "shows";
  if (/\b(?:arts?\s*(?:&|and)\s*theatre|theater|theatre|comedy)\b/.test(segment)) return "shows";
  if (/\b(?:music|concerts?)\b/.test(segment)) return "live-music";
  if (isSocialPlay(event)) return "social-play";
  if (isNightTour(event)) return "night-tours";
  if (isShow(event)) return "shows";
  if (isLiveMusic(event)) return "live-music";
...
export function nightOutEventRails(event) {
  // One dated happening, one shelf: a concert is Live Music, a play or a
  // comedy set is Shows. The event's own taxonomy decides, never its venue.
