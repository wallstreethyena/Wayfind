...
// allowanceFromLedger — pure translation from a raw wf_spend_ledger read
// into the shape lib/photoRepair.js's headroom check and any future
// operator-facing report both want, computed exactly once so neither has to
// re-derive "what phase are we in" from raw numbers.
//
...
// unreadable ledger (a non-2xx response, a thrown fetch, a malformed body)
// is an OPERATOR-VISIBLE unknown, not a silent 0 — every numeric field is
// `null`, and phase is "unknown". This is what makes the symmetric
// fail-closed rule in lib/photoRepair.js possible to state precisely:
// "unknown never blocks AND never releases" reads directly off
// `phase === "unknown"`, never off a fabricated headroom of 0.
//
// THE OCTOBER 1ST FIX (2026-09-09). A THIRD case exists and used to collapse
// into "unknown" alongside the truly-unreadable one: wf_spend_ledger's month
// row for `photos` is created lazily, on the FIRST GRANT of that calendar
// month (spendGate.js's wf_spend_take) — so for a stretch after every
// rollover (measured: 00:50 UTC on the 1st, before any reader has spent) an
// HTTP 200 with a genuinely EMPTY row array is the correct, honest read of a
// month nothing has touched yet. Folding that into "unknown" chained into a
// real incident: scripts/photo-monitor.mjs's rule is "unknown -> queue the
// probe", so a clean 200/[] read filed hundreds of perfectly healthy
// cold-cache probes as `source-unavailable` defects, breached the
// run-over-run growth ratio, and paged the owner over a healthy site. An
// absent row is not an unreadable ledger — it is a MEASURED fact (nothing
// spent yet), read against the operating cap as its cap, same as any other
// real reading. Only a genuinely unreadable response (non-2xx, a thrown
// fetch, a malformed body) may still produce "unknown" — see `rowPresent`
// below.
//
// phase:
//   "unknown"   — the ledger could not be read as real numbers (a non-2xx
//                 response, a thrown fetch, or a malformed body — never an
//                 absent row on an otherwise-successful read, see above).
...
// `rowPresent: false` is the call site's signal for "HTTP 2xx, but the
// month's wf_spend_ledger row for `photos` does not exist yet" — the lazy-
// row-creation gap this fix closes (see the header comment above). It is
// NOT the caller's job to decide the phase from that; the caller only knows
// the row is absent, this function decides what that means. Default `true`
// so every existing caller (which always passed a real {used, cap} or {})
// keeps its exact prior behaviour unchanged — a bare `{}` with no
// `rowPresent` key still reads as "row present, but unmeasured" -> unknown.
export function allowanceFromLedger({ used, cap, rowPresent = true } = {}) {
  let u, c;
  if (rowPresent === false) {
    // Nothing has been taken this month — a real, measured 0, read against
    // the operating cap (there is no `cap` column to read when there is no
    // row at all). Falls through to the SAME phase/headroom arithmetic as
    // any other measured reading below, rather than a hand-written literal,
    // so the two can never drift apart.
    u = 0;
    c = PHOTOS_OPERATING_FREE_CAP;
  } else {
...
// ── Photo-repair burn rate / runway (2026-09-09) ───────────────────────────
//
// WHY THIS LIVES HERE, NOT AT EITHER READER. app/api/health/photos/route.js
// and scripts/os-state.mjs both want "how fast is the photos ledger burning,
// and how many days of headroom does that leave" — and both already read the
// SAME wf_job_pulse rows the OTHER already reads (app/api/cron/photo-repair's
// own pulse note, which has carried `allowance=<used>/<cap>` in every note
// since #1222). Computing the arithmetic twice risks the two disagreeing the
// same way computePhotoCoverage above already exists to prevent for the
// coverage percentage — one function, both callers, or the numbers drift.
//
// PURE, NO I/O: the caller fetches the pulse rows (an ordinary
// `?job=eq.photo-repair&select=note,ran_at&order=ran_at.desc&limit=2` read)
// and passes them in — this module stays free of any Supabase/env access per
// its file header.
//
// parseAllowanceNote reads the exact literal
// app/api/cron/photo-repair/route.js's `allowanceNote` writes
// (`allowance=<used>/<cap>`, e.g. "allowance=968/2000") straight out of the
// note string — never a re-derivation from the ledger itself, which this
// module cannot read anyway.
export function parseAllowanceNote(note) {
  const m = /\ballowance=(\d+)\/(\d+)\b/.exec(String(note || ""));
  if (!m) return null;
...
// computePhotoRunway — burn24h (photos spent per 24h, projected from the gap
// between the two most recent readings) and runwayDays (headroom ÷ burn24h,
// using the NEWER reading's cap so a mid-window cap raise is reflected
// immediately, never averaged against a stale one).
//
// `notes` is the last photo-repair pulses the caller already fetched,
// ordered NEWEST FIRST — the exact shape
// `?order=ran_at.desc&limit=2` returns and the shape
// scripts/photo-monitor.mjs's lastPhotoMonitorPulses already uses for the
// same table — each `{ note, ranAt }` (a `succeeded` field, if present, is
// ignored; unlike pulseVerdict this never filters on it, since a
// budget_blocked/classified run still carries a real allowance reading).
//
// NULL-SAFE BY CONSTRUCTION (AGENTS.md §5's corollary — an unmeasured
// quantity is an operator-visible unknown, never a fabricated 0 or a
// fabricated date): both burn24h and runwayDays come back `null` together
// when any of the following holds —
//   - fewer than two notes carry a parseable `allowance=<used>/<cap>` tag
//     (a fresh deploy with only one photo-repair pulse ever filed, or two
//     pulses that both read "unread"/"unreadable" — see the route's own
//     allowanceNote — never parse)
//   - the two notes' timestamps do not strictly increase (clock skew, a
//     duplicate/replayed pulse, or malformed `ranAt`) — there is no time
//     window to project a rate over
//   - `used` went DOWN between the two readings (a mid-month ledger reset
//     or correction) — the arithmetic no longer means "burn", so this
//     returns null rather than a negative rate
// A genuinely zero burn (used unchanged between the two reads) is a REAL
// measurement, not an unknown — burn24h is 0 and runwayDays is null (there
// is no meaningful "days until exhausted" at zero burn; reporting a
// fabricated infinity would be exactly the kind of confident-but-empty
// number this module's header warns against).
export function computePhotoRunway(notes) {
  const list = Array.isArray(notes) ? notes : [];
  const parsed = [];
  for (const n of list) {
    const allowance = parseAllowanceNote(n && n.note);
    if (!allowance) continue;
    const t = new Date(n && n.ranAt).getTime();
    if (!Number.isFinite(t)) continue;
    parsed.push({ ...allowance, t });
    if (parsed.length >= 2) break;
  }
  if (parsed.length < 2) return { burn24h: null, runwayDays: null };

  const [newer, older] = parsed;
  const deltaMs = newer.t - older.t;
  if (!(deltaMs > 0)) return { burn24h: null, runwayDays: null };

  const usedDelta = newer.used - older.used;
  if (usedDelta < 0) return { burn24h: null, runwayDays: null };

  const burn24h = usedDelta * (86400000 / deltaMs);
  if (!Number.isFinite(burn24h)) return { burn24h: null, runwayDays: null };

  const headroom = Math.max(0, newer.cap - newer.used);
  const runwayDays = burn24h > 0 ? headroom / burn24h : null;
  return { burn24h, runwayDays };
}
