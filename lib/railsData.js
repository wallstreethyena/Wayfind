...
export function slimPlace(p) {
  ...
  const creatorSources = Array.isArray(p._creatorSources)
    ? p._creatorSources
      .filter((source) => source && source.handle)
      .map((source) => ({
  ...
  return {
    ...
    // Creator Picks is split into one shelf per explicitly credited person in
    // the client. Keep the provenance collected from the curated registry;
    // never reconstruct a handle from a place name or a social URL there.
    ...(creatorSources.length ? {
      creatorHandles: creatorSources.map((source) => source.handle),
      creatorSources,
    } : {}),
...
export async function loadPools(citySlug, opts) {
  ...
  if (readerOrigin) {
    ...
    opts?.onStage?.("nearby");
    const built = await Promise.all(nearbyCats.map((c) =>
      buildNearbyPool(readerOrigin, c, { locName, skipEditorial: true, readCache }).catch(() => [])));
...
async function buildCreatorsPool(pools, origin) {
  ...
  const jobs = [];
  // Some curated venues have posts from more than one creator. spotsByCity()
  // intentionally exposes one primary video per registry entry for its browse
  // UI, so build an exact Place-ID index from allCreators() as the provenance
  // source. Entries without a Place ID still keep their explicit spot.video
  // credit below; no venue-name or URL parsing is used to invent a handle.
  const sourcesByPlaceId = new Map();
  const addSource = (placeId, video) => {
    const id = String(placeId || "");
    const handle = String(video && video.creator || "").replace(/^@+/, "").trim();
    if (!id || !handle) return;
    const source = {
      handle,
    ...
    const list = sourcesByPlaceId.get(id) || [];
  ...
  for (const creator of creatorRegistry.creators || []) {
    for (const spot of creator.spots || []) addSource(spot.placeId, spot.video);
  }
  for (const g of groups) {
    ...
    for (const spot of g.spots) {
      ...
      jobs.push((async () => {
        ...
        const direct = directHandle ? [{
          handle: directHandle,
          platform: spot.video.platform || null,
          url: spot.video.url || null,
        }] : [];
  ...
  for (const result of resolved) {
    ...
    if (!merged) {
      merged = { ...row, _creatorSources: [] };
      rowsById.set(row.id, merged);
    }
    for (const source of result.sources || []) {
      if (!source || !source.handle) continue;
      const key = source.handle.toLowerCase();
      if (!merged._creatorSources.some((item) => item.handle.toLowerCase() === key)) {
        merged._creatorSources.push({ ...source });
      }
    }
  }
  ...
  return rows.filter((r) => {
    if (!r || !r.id || !r._creatorSources.length) return false;
