  ...
  try {
    ...
    if (url && anon) {
      ...
      const r = await fetchDeadline(
        `${url}/rest/v1/wf_inventory?select=place_id,name,lat,lng,google_types,primary_type,cuisines,signals,photo_ref,editorial&status=eq.OPERATIONAL&${q}&order=signals->reviews.desc.nullslast&limit=300`,
