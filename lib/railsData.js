...
export async function loadPools(citySlug, opts) {
  ...
  // "creators" and "summer" are not rankedFor categories — each is built from
  // its own curated registry (buildCreatorsPool / buildSummerPool) after the
  // ranked pools exist.
  const cats = [...new Set(Object.values(RAIL_SELECT).flatMap((c) => c.pools))]
    .filter((c) => c !== "creators" && c !== "summer");
...
async function buildSummerPool(pools, origin) {
  ...
  const jobs = entries.map((e) => (async () => {
    ...
    if (!row) return null;
    // Registry marker + the card's editorial line (the entry's own timing /
    // heat guidance — evidence-true by the registry's standard). A pool-reused
    // row is mutated in place, same as _creatorSourced directly above.
    row._summerSourced = true;
    if (!row._summerWhy) row._summerWhy = e.why || null;
    return row;
  })());
  const rows = (await Promise.all(jobs)).filter(Boolean);
  const seen = new Set();
