...
import { getPlaceDetails, normalizeDetails } from "./placeDetails.js";
...
// City-ranked and reader-nearby reads are independent once the optional
// consolidated prime has settled. Keep them in one bounded queue so nearby
// can overlap ranked work without moving all contention into Supabase.
...
export async function loadPools(citySlug, opts) {
  ...
  // Narrow seams keep the scheduling and completeness contract executable
  // offline. Production callers do not provide these.
  ...
  if (readCache) {
    await primeReads(jobs.map(({ cat, city }) => ({ catSlug: cat, city: LANDING_CITIES[city] })), readCache).catch(() => {});
  }
  ...
  const tasks = [
    ...nearbyCats.map((cat) => async () => ({
      kind: "nearby", cat,
      result: await Promise.resolve().then(() => buildNearby(readerOrigin, cat, {
        locName, skipEditorial: true, readCache, includeStatus: true,
      })).catch(() => ({ rows: [], degraded: true })),
    })),
    ...jobs.map(({ cat, city }) => async () => ({
      kind: "ranked", cat, city,
      rows: await Promise.resolve().then(() => rank(cat, city, {
        withPhotos, skipEditorial: true, readCache, inventoryOnly: true,
      })).then((r) => r || []).catch(() => []),
    })),
  ];
  ...
  if (readerOrigin) {
    const built = nearbyResults.map((entry) => entry.result);
    if (built.some((result) => result.degraded)) {
      throw new Error("Nearby inventory reads incomplete");
    }
    nearbyCats.forEach((cat, i) => {
      const near = built[i].rows;
...
async function buildCreatorsPool(pools, origin) {
  ...
  // Creator membership comes from verified posts, not the broad category's
  // top-ranked window or an unrelated Place Details cache. Read exact owned
  // IDs before resolving the registry so lower-volume creator finds survive.
  const localGroups = groups.filter((g) => Number.isFinite(g.distMi) && g.distMi <= CREATOR_FINDS_RADIUS_MI);
  const missingIds = [...new Set(localGroups.flatMap((g) => g.spots.map((s) => s.placeId)).filter((id) => id && !byId.has(id)))];
  ...
  if (missingIds.length) {
    ...
    for (let offset = 0; offset < missingIds.length; offset += 100) {
      const owned = await serveInventoryByPlaceIds(missingIds.slice(offset, offset + 100), origin.lat, origin.lng, CREATOR_FINDS_RADIUS_MI * 1609.34, { failLoud: true, withKnownIds: true });
      for (const id of owned.knownIds) knownInventoryIds.add(id);
      for (const raw of owned.places) {
        const d = normalizeDetails(raw);
        if (!d || !Number.isFinite(d.lat) || !Number.isFinite(d.lng)) continue;
        const row = { id: d.id, name: d.name, rating: d.rating, reviews: d.reviews,
          types: d.types, primaryType: raw.primaryType || null, cuisines: raw.cuisines || [],
          status: d.businessStatus, lat: d.lat, lng: d.lng, priceLevel: raw.priceLevel || null,
          photoRef: d.photoRef, ...(raw.photo_url ? { photo_url: raw.photo_url } : {}), editorial: d.description, city: null,
          distMi: haversineMi(origin.lat, origin.lng, d.lat, d.lng) };
        if (row.distMi > CREATOR_FINDS_RADIUS_MI) continue;
        byId.set(row.id, row); allRows.push(row);
      }
    }
  }
  const jobs = [];
  const sourcesByPlaceId = new Map();
  const sourcesBySpotKey = new Map();
  ...
  for (const creator of creatorRegistry.creators || []) for (const spot of creator.spots || []) {
    addSource(spot.placeId, spot.video);
    const sources = sourcesBySpotKey.get(spot.key) || [];
    if (spot.video?.creator) sources.push({ handle: spot.video.creator.replace(/^@+/, "").trim(), platform: spot.video.platform || null, url: spot.video.url || null });
    sourcesBySpotKey.set(spot.key, sources);
  }
  for (const g of groups) {
    ...
    for (const spot of g.spots) {
      ...
      jobs.push((async () => {
        ...
        if (!row && spot.placeId && !knownInventoryIds.has(spot.placeId)) {
        ...
        return { row, sources: exact.length ? exact : (sourcesBySpotKey.get(spot.key) || direct) };
      })());
    }
  }
...
export async function loadRailPlaces(citySlug, opts) {
  ...
  const [creators, summer, birthday, localpicks, family, events, drive] = await Promise.all([
    buildCreatorsPool(pools, origin), buildSummerPool(pools, origin).catch(() => []),
...
export async function railMenuData(citySlug, opts) {
  ...
  return { places: (data && data.places) || {}, thin: (data && data.thin) || RAILS.filter((r) => r.list).map((r) => r.id),
    citySlug: (data && data.citySlug) || slug, cityLabel: city.name,
    covered: !failed, failed, degraded: failed, complete: !failed,
