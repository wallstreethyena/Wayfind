 ...
 * @param {string} citySlug a LANDING_CITIES key
 */
export async function railMenuData(citySlug) {
  const city = LANDING_CITIES[citySlug] || LANDING_CITIES.sarasota;
  const slug = LANDING_CITIES[citySlug] ? citySlug : "sarasota";
  const data = await loadRailPlaces(slug).catch(() => null);
  return {
