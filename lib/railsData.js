...
import { isDateNightShortlist, DATENIGHT_NEAR_MI, DATENIGHT_TYPES } from "./dateRoom.js";
...
export async function loadPools(citySlug, opts) {
  ...
  const SYNTHETIC = new Set(["creators", "summer", "birthday", "breakfast", "quickeats", "family", "events", "localpicks", "datenight"]);
  ...
  if (readerOrigin) {
    ...
    const built = await Promise.all(nearbyCats.map((c) =>
      buildNearbyPool(readerOrigin, c, { locName }).catch(() => [])));
...
export async function loadRailPlaces(citySlug, opts) {
  ...
  const [creators, summer, birthday, localpicks, family, events, drive, datenight] = await Promise.all([
    ...
    buildBirthdayPool(pools, origin).catch(() => []),
    ...
    buildIdentityPool(pools, origin, isFamilyPlace, FAMILY_NEAR_MI, ["things-to-do"], isStrongFamilyPlace, { typeOv: FAMILY_TYPES }).catch(() => []),
    buildIdentityPool(pools, origin, isStrongTicketedVenue, EVENTS_NEAR_MI, ["things-to-do", "nightlife"], null, { typeOv: TICKETED_TYPES }).catch(() => []),
    ...
    buildDrivePool(pools, origin, cities).catch(() => []),
    // Date Night shortlist at a hard 27 miles — restaurants, clubs, shows,
    // rooftops already in inventory. No Places call; empty stays empty.
    buildIdentityPool(pools, origin, isDateNightShortlist, DATENIGHT_NEAR_MI, ["restaurants", "nightlife", "events"], null, { typeOv: DATENIGHT_TYPES }).catch(() => []),
  ]);
  ...
  const [breakfast, quickeats] = await Promise.all([
    buildIdentityPool(pools, origin, isBreakfastPlace, BREAKFAST_NEAR_MI, ["restaurants", "creators"]).catch(() => []),
    buildIdentityPool(pools, origin, isQuickService, 8, ["restaurants", "creators"], isStrongQuickService).catch(() => []),
  ]);
