...
// The server's own deadline for building one metro's rails. Strictly inside
// DaypartRail's RAILS_LOAD_TIMEOUT_MS (30s) so the SERVER decides what the
// reader is told, not the client's stopwatch. scripts/check-fetch-deadlines.mjs
// pins the ordering between the two.
export const RAILS_SERVER_DEADLINE_MS = 20000;
...
export async function loadPools(citySlug, opts) {
  ...
  const results = await Promise.all(jobs.map(({ cat, city }) =>
    ...
    rankedFor(cat, city, { withPhotos, skipEditorial: true, readCache }).then((r) => r || []).catch(() => [])));
 ...
 */
async function buildDrivePool(pools, origin, pooledCities, readCache) {
  if (!origin || !Number.isFinite(origin.lat) || !Number.isFinite(origin.lng)) return [];
  ...
  // mapper has never carried editorial through either).
  const results = await Promise.all(jobs.map(({ cat, city }) =>
    rankedFor(cat, city, { withPhotos: true, skipEditorial: true, readCache }).then((r) => r || []).catch(() => [])));
  const seen = new Set();
...
export async function loadRailPlaces(citySlug, opts) {
  ...
  // One cached Supabase read covers the entire beach pool. When current
  // samples exist, water quality becomes the Beach Day decision before score.
  pools.beaches = await attachBeachWater(pools.beaches);
...
export async function railMenuData(citySlug, opts) {
  ...
  //
  // WHY 20s. It has to sit strictly INSIDE the client's budget — DaypartRail's
  // RAILS_LOAD_TIMEOUT_MS is 30s — so that the server always answers first and
  // the reader is told the truth by us rather than by a stopwatch. The cost is
  // deliberate and worth naming: a pathological run where every wave burns the
  // full 8s network ceiling (~24s) is given up on at 20s instead of being
  // allowed to finish. A page that takes 24 seconds has already lost the
  // reader; an honest degraded answer at 20 has not.
