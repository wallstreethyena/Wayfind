...
async function buildBirthdayPool(pools, origin) {
  ...
  const jobs = entries.map((e) => (async () => {
    ...
    if (!row) return null;
    // v8.19.1 — CLONE before flagging (same rule as summer/creators): a
    // pool-reused row is shared with the anchor pools, and an in-place stamp
    // would mark those appearances registry-sourced too.
    row = { ...row };
