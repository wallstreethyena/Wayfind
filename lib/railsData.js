...
const OWNED_CATEGORY_FOR_POOL = Object.freeze({
  restaurants: "food",
...
/**
 * Build a narrow synthetic pool without ever capping a broad shelf first.
 *
 * Source 1 reuses already-ranked rows when they happen to qualify. Source 2 is
 * authoritative owned inventory: read deterministically to exhaustion, apply
 * serviceability, exact radius and the real identity predicate, then rank the
 * survivors. The old top-300 review shelf is deliberately gone.
 *
 * This one helper feeds Breakfast, Quick Eats, Family, Events and Birthday, so
 * the ordering law is global for those surfaces instead of five local patches.
 */
async function buildIdentityPool(pools, origin, predicate, radiusMi, sourceCats, widenPredicate, opts) {
  ...
  const categories = [...new Set(sourceCats.map((cat) => OWNED_CATEGORY_FOR_POOL[cat]).filter(Boolean))];
  const exactIdentity = widenPredicate || predicate;
  try {
    if (categories.length) {
      const owned = await fetchOwnedPool(origin.lat, origin.lng, {
        categories,
        radiusMi,
        identity: (place) => Number(place?.reviews || 0) >= 15 && exactIdentity(place),
      });
      for (const place of owned.places || []) {
        if (!place || !place.id || seen.has(place.id)) continue;
        const shaped = { ...place, city: place.city || null };
        const g2 = governedScoreOf(shaped, shaped.city || null);
    ...
    // Source 1 remains a safe fallback. An incomplete owned read never becomes
    // a partial candidate universe because fetchOwnedPool refuses truncation.
  }
