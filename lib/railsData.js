...
// ship the rail with no cards and an honest line. Below this count we do that.
const MIN_CARDS = 3;
const MAX_CARDS = 8;
...
export async function loadPools(citySlug, opts) {
  ...
  const cats = [...new Set(RAILS.filter((r) => r.source).map((r) => r.source.cat))];
...
export async function loadRailPlaces(citySlug, opts) {
  ...
  for (const rail of RAILS) {
    if (!rail.source) continue;                       // the guides rail has no list
    const pool = pools[rail.source.cat] || [];
    const picked = rail.pick ? pool.filter((p) => { try { return rail.pick(p); } catch { return false; } }) : pool;
    if (picked.length < MIN_CARDS) {
      // Honest empty beats a padded list. The tile still renders — an off-peak
      // or thin rail is parked, never hidden — and its drop says so.
      places[rail.id] = [];
      thin.push(rail.id);
      continue;
    }
    places[rail.id] = picked.slice(0, MAX_CARDS).map(slimPlace).filter(Boolean);
  }
