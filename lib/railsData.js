...
export async function loadRailPlaces(citySlug, opts) {
  ...
  // v8.30 — the owner's handpicked board for the reader's own town. Built
  // AFTER the anchor pools so tier 1 can reuse their measured, ranked rows,
  // and before fillRails so every routed rail can see it.
  // userOrigin, NOT origin. The board is a "near you" claim about the reader's
  // own town, so it is built only from the reader's own point — never from a
  // city centroid. Without this the prerendered homepage, which ranks the
  // flagship metro from Sarasota's centre, would hand every cold visitor
  // anywhere in the world the Lakewood Ranch board (8 miles away, inside
  // LOCAL_PICK_MARKET_MI) at first paint. Same rule the near-me guard three
  // blocks up states: no origin -> honest empty, never a city snap.
  pools.localpicks = await buildLocalPicksPool(pools, userOrigin, band).catch(() => []);
...
export async function railMenuData(citySlug, opts) {
  ...
  const data = await loadRailPlaces(slug, { origin: opts && opts.origin, requireOrigin: opts && opts.requireOrigin, band }).catch(() => null);
  return {
