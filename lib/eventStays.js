...
export function selectEventStays(rows, origin, max = 6) {
  ...
  return rows.filter((p) => {
    ...
    // A service business can carry Google's broad lodging/hotel secondary types.
    if (/^(massage|spa|beauty_salon|wellness_center)$/.test(p.primaryType || p.primary_type || "") && !/hotel|resort|inn\b|suites|motel|lodge\b/i.test(p.name)) return false;
