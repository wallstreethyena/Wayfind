...
function eventStayPhoto(p) {
  ...
  return p?.photos?.[0]?._directUri
    || (p?.photos?.[0]?.name ? "/api/photo?ref=" + encodeURIComponent(p.photos[0].name) + "&g=2&w=640" : null);
}
...
export function completePoolPlace(row) {
  ...
  return {
    ...
    photo: isPlaceOwnedPhotoUrl(photoUrl) ? photoUrl
      : (row?.photo_ref ? "/api/photo?ref=" + encodeURIComponent(row.photo_ref) + "&g=2&w=640" : null),
