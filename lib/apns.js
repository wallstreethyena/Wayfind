  ...
  async function worker() {
    while (cursor < list.length) {
      ...
      if (isInvalidPushResult(result)) {
        // AWAITED. A caller's onInvalid is typically an async deleteToken()
        // (see app/api/cron/weekend-picks) — if this only fired the call and
        // moved on, the batch's own returned promise could resolve, and the
        // caller's recordPulse/return could run, before the deletion actually
        // committed. Awaiting here means "sendPushBatch resolved" truthfully
        // implies "every onInvalid it triggered has already finished."
        try { await onInvalid?.(token); } catch (e) {}
      }
