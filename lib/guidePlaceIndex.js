...
export function buildGuidePlaceIndex(guides) {
  ...
  for (const [slug, guide] of Object.entries(guides || {})) {
    ...
    for (const pick of guide.picks) {
      ...
      if (!placeId) continue;
      const seen = byId.get(placeId);
      if (seen) {
        // Already attributed to an earlier guide. Record this one too so the
        // /places page can link back to EVERY guide that features it (a
        // restaurant in both a St. Armands roundup and the Sarasota city
        // guide should credit both), without changing the primary record.
        if (seen.guideSlug !== slug && !seen.alsoIn.some((g) => g.slug === slug)) {
          seen.alsoIn.push({ slug, title: guideTitle });
        }
        continue;
      }
      byId.set(placeId, {
...
/**
 * Every placeId a GUIDES pick makes indexable on its own, deduped, for the
 * sitemap/static-params union. A pick whose blurb is below
 * GUIDE_DETAIL_MIN_BLURB_LEN does not flip hasDetails, so its /places page can
 * still render noindex; listing it would put a noindex URL in the sitemap
 * (Search Console: "Submitted URL marked noindex"). Such an id still reaches
 * the sitemap through the ordinary indexed/Atlas paths when it qualifies.
 */
export function listGuidePlaceIds(index = GUIDE_PLACE_INDEX) {
  const out = [];
  for (const [id, rec] of index) if (guidePlaceHasSubstantiveDetail(rec)) out.push(id);
  return out;
}
