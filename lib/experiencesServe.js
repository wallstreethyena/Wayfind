...
export async function serveExperiences({ metro, city, lat, lng, mi, cat, page, limit } = {}) {
  ...
  // v6.99 (owner: "spa and wellness links make no sense", live screenshot of
  // kayak/manatee/dolphin tours under the Spa & Wellness chip) — an active
  // category that is neither "all" nor a real entry in CATEGORY_BY_KEY used
  // to fall through both branches below and silently return every row
  // UNFILTERED, identical to "all". That is exactly what happened here: the
  // ground-truthed Viator catalog (lib/experiencesData.js CATEGORIES) has no
  // spa/wellness tag today, so "spa" matched neither branch and this table's
  // caller (UnifiedBrowseCommerceRail) never saw the empty result that would
  // have triggered its own live-Viator-search fallback -- it just rendered
  // the generic all-attractions rail and called it "Spa & wellness". An
  // unrecognized category now returns NO rows instead of ALL rows, so the
  // caller's existing "rows.length === 0 -> try a live search" fallback
  // actually fires for every current and future subfilter chip that has no
  // matching entry in this catalog, instead of silently mislabeling
  // unrelated inventory as a match.
  if (active !== "all") {
