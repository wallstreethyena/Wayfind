...
import { filterBrowseBookables } from "./browseBookableMatch.js";

const BROWSE_READ_PAGE = 1000;
const BROWSE_READ_MAX = 10000;
const BROWSE_READ_DEADLINE_MS = 12000;

function dedupeExperienceRows(rows) {
  const seen = new Set();
  return rows.filter((row) => {
    const code = String(row && row.product_code || "").trim();
    if (!code) return true;
...
/** Deterministic, bounded complete read used only by browse-context requests. */
export async function readCompleteBrowseExperiences(env, destIds, opts = {}) {
  const pageSize = Math.min(Math.max(Number(opts.pageSize) || BROWSE_READ_PAGE, 1), 1000);
  const maxRows = Math.max(Number(opts.maxRows) || BROWSE_READ_MAX, pageSize);
  const doFetch = opts.fetchImpl || fetch;
  const deadlineMs = Math.max(Number(opts.deadlineMs) || BROWSE_READ_DEADLINE_MS, 1);
  const deadlineAt = Date.now() + deadlineMs;
  const headers = { apikey: env.key, Authorization: `Bearer ${env.key}` };
  const inList = destIds.map((x) => encodeURIComponent(x)).join(",");
  const base = `${env.url}/rest/v1/wf_experiences?select=*&dest_id=in.(${inList})&order=product_code.asc`;
  const rows = [];
  const fetchPage = (from, to) => {
    const left = deadlineAt - Date.now();
    if (left <= 0) throw new Error("Browse experiences complete read exceeded its deadline");
    const signal = typeof AbortSignal !== "undefined" && typeof AbortSignal.timeout === "function" ? AbortSignal.timeout(left) : undefined;
    return doFetch(base, { headers: { ...headers, Range: `${from}-${to}`, "Range-Unit": "items" }, cache: "no-store", ...(signal ? { signal } : {}) });
  };

  for (let from = 0; from < maxRows; from += pageSize) {
    const to = Math.min(from + pageSize, maxRows) - 1;
    const r = await fetchPage(from, to);
    if (!r.ok) throw new Error(`Browse experiences read returned ${r.status}`);
    const page = await r.json();
    if (!Array.isArray(page)) throw new Error("Browse experiences read returned a non-array body");
    if (page.length > to - from + 1) throw new Error("Browse experiences read ignored its bounded range");
    rows.push(...page);
    if (page.length < to - from + 1) return dedupeExperienceRows(rows);
  }

  // One-row probe distinguishes exactly-full from truncated without admitting
  // an incomplete universe. A binding safety cap fails loudly.
  const probe = await fetchPage(maxRows, maxRows);
  if (!probe.ok) throw new Error(`Browse experiences completeness probe returned ${probe.status}`);
  const tail = await probe.json();
  if (!Array.isArray(tail)) throw new Error("Browse experiences completeness probe returned a non-array body");
  if (tail.length) throw new Error(`Browse experiences exceeded the complete-read ceiling (${maxRows})`);
  return dedupeExperienceRows(rows);
}
...
/** Exact pre-rank/pre-pagination browse gate, exported for executable tests. */
export function prepareBrowseExperienceRows(rows, browseCat, browseSub) {
  return filterBrowseBookables(
    dropDeadLinkRows(rows).filter((r) => r && String(r.image || "").trim() && String(r.product_url || "").trim()),
    browseCat,
    browseSub || "all",
  );
}

/** Browse matching is authoritative; `cat` remains the legacy-only filter. */
export function selectExperienceView(rows, active, browseMode) {
  return browseMode ? (Array.isArray(rows) ? rows : []) : filterByChip(rows, active);
}

export async function serveExperiences({ metro, city, lat, lng, mi, cat, page, limit, browseCat, browseSub, completeCatalog = false } = {}) {
  ...
  const browseMode = browseCat != null || browseSub != null;
  ...
  let rows = [];
  try {
    if (browseMode || completeCatalog) rows = await readCompleteBrowseExperiences(s, destIds);
    else {
    ...
    if (browseMode || completeCatalog) throw e;
  ...
  // Browse asks for cards that can render and convert. Apply these gates to
  // the complete market pool before ranking and slicing, so 50 means up to 50
  // qualifying offers rather than 50 slots padded by broken or irrelevant rows.
  if (browseMode) {
    rows = prepareBrowseExperienceRows(rows, browseCat, browseSub);
  }
  ...
  const view = selectExperienceView(rows, active, browseMode);
