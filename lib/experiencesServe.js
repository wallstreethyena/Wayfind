...
export async function serveExperiences({ metro, city, lat, lng, mi, cat, page, limit, browseCat, browseSub } = {}) {
  ...
  let rows = [];
  try {
    if (browseMode) rows = await readCompleteBrowseExperiences(s, destIds);
    else {
