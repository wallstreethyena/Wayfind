...
// The unified commerce system (lib/commerce.js) records a partner click as
// `commerce_cta_clicked`, not as one of the seven legacy `*_out` events. Paid
// landing surfaces (guides, /go/florida, /florida-events) use ONLY that event,
// so before this rule Google Ads saw zero conversions from every paid click
// even though PostHog recorded them (2026-09-22 audit: $104 spent, 0 Ads
// conversions). It counts as an affiliate conversion ONLY when the caller says
// the click is monetized — an "open in Wayfind" or map CTA on the same card is
// the same event name and must never report as a sale-intent conversion.
// Callers that already fire a legacy `*_out` for the same click must not also
// forward this event (they don't today: emitCommerce never forwards).
export const COMMERCE_CLICK_EVENT = "commerce_cta_clicked";
export function isAffiliateClick(event, params) {
  const e = _s(event);
  if (AFFILIATE_EVENTS.indexOf(e) >= 0) return true;
  return e === COMMERCE_CLICK_EVENT && !!params && params.monetized === true;
}

export function classify(event, params) {
  const e = _s(event);
  if (isAffiliateClick(e, params)) return "primary";
...
export function labelKeyFor(event, params) {
  ...
  if (isAffiliateClick(e, params)) return "affiliate";
