...
// This tag belongs to Google Ads customer 553-622-6713 ("Wayfind"), the account
// that runs every Wayfind campaign. Audit that account, never 505-826-0233.
// See docs/GOOGLE_ADS_ACCOUNT.md.
...
export function isValidConversionLabel(v) {
  ...
  // ONE placeholder list, shared with the money paths (lib/envPlaceholder.js).
  // Without this, "[SENSITIVE]" — the string `vercel env pull` writes for a
  // sensitive var — is rejected only by the charset check below, i.e. only
  // because Vercel happens to use square brackets. That is luck, not a rule.
  if (isPlaceholderCredential(s)) return false;
...
// PARTNER_CLICK_EVENT (2026-09-22 revision) is the ONLY signal that makes an
// Ads conversion. It is sent from exactly one place — CommerceClickBeacon —
// and only after the click has been VERIFIED to land on one of our four
// partner redirect routes (host + pathname, matched against the real anchor).
//
// The earlier rule ("commerce_cta_clicked" + a caller-supplied `monetized:
// true` flag) is gone. It was unsafe in two ways, both found auditing guide
// pages: (1) GuideConversion already calls track("commerce_cta_clicked", {
// monetized: !!cta.monetized }) for every guide CTA, and lib/track.js forwards
// every track() call to forwardToGoogle — so the old rule turned that into a
// SECOND Ads conversion for the same click the beacon also reports, under a
// different dedupe key, i.e. a guaranteed double-count wherever both fire.
// (2) `monetized` is a caller-asserted label, not a verified fact: some
// "courtesy" coupons (lib/guideCta.js) point straight at a partner's own site
// (e.g. ringling.org) with no commission and no redirect, yet are still
// labelled monetized:true (a separate labelling bug) — under the old rule a
// click on a $0 link reported as a paid conversion. A Google Ads conversion
// must mean "this browser was actually handed off through one of our four
// partner redirect routes", decided by the destination, never by a flag.
export const PARTNER_CLICK_EVENT = "partner_click";
export function isAffiliateClick(event) {
  const e = _s(event);
  return AFFILIATE_EVENTS.indexOf(e) >= 0 || e === PARTNER_CLICK_EVENT;
}

// `params` stays in the signature — several existing callers (forwardToGoogle
// below) already pass it, and this keeps them source-compatible — but it is
// deliberately UNUSED for this decision now: whether a click is affiliate is a
// function of the event name alone, never of a field the caller supplied.
export function classify(event, params) {
  const e = _s(event);
  if (isAffiliateClick(e)) return "primary";
...
export function labelKeyFor(event, params) {
  const e = _s(event);
  if (e === "signup_completed") return "signup";
  if (isAffiliateClick(e)) return "affiliate";
...
function dedupeKey(event, params) {
  const e = _s(event);
  const p = params || {};
  // partner_click dedupes on its own click_id when one is present: the same
  // click_id reaching forwardToGoogle twice (StrictMode, a double-tap) is
  // unambiguously the same click, which place_id/id/provider alone cannot
  // guarantee (two different partner clicks on the same place in one session
  // would otherwise collide onto one key).
  if (e === PARTNER_CLICK_EVENT && typeof p.click_id === "string" && p.click_id.trim()) {
    return "partner_click|" + p.click_id.trim();
  }
  const id = p.place_id || p.id || p.provider || "";
  return e + "|" + _s(String(id));
}
...
export function forwardToGoogle(event, params, opts) {
  ...
  report.tier = classify(report.event, params);
  ...
  if (report.tier !== "analytics") {
    const lk = labelKeyFor(report.event, params);
