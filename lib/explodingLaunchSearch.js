...
export async function loadProvidedTrendList({ center, city, fetchImpl = fetch, signal } = {}) {
  ...
  // EVERY trend on the supplied list gets its chance at a card (owner,
  // 2026-08-11: "make sure wayfind has a place card for each of these trending
  // categories… every single one of our categories"). The old head-of-three
  // stopped searching after the first hits, so seventeen licensed trends never
  // even asked whether the town had a verified match. The whole universe is
  // swept in bounded batches; a trend still renders ONLY when a real,
