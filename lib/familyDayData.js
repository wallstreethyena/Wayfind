...
import { FAMILY_DAY_RAILS, familyRailMatches, matchesFamilyFilters } from "./familyDayTaxonomy.js";
...
export function composeFamilyRail(rawRows, { lat, lng, radiusMi, rail, filters = {} }) {
  const seen = new Set();
  const places = [];
...
export async function familyDayAnswer(options) {
  const def = FAMILY_DAY_RAILS.find((r) => r.id === options.rail);
  ...
  const matched = composeFamilyRail(results.flatMap((p) => p.rows), options);
  return { places: matched.slice(0, 24), matched: matched.length, rail: options.rail, radiusMi: options.radiusMi,
    truncated: results.some((p) => p.truncated), more: matched.length > 24 };
}
