...
//
// THE LADDER, in the owner's order:  deal > bookable > delivery > directions
...
//   delivery  the only branch that fires today.
//   directions always available.
//
// MONETIZED IS NOT THE SAME AS WORKING, and the disclosure follows the money.
// NEXT_PUBLIC_UBEREATS_TEMPLATE is unset, so "Order pickup" resolves to a real
// Uber Eats destination but earns nothing. Rendering "We may earn a commission"
// under a link that cannot earn is a FALSE STATEMENT to the user and a false
// signal to us. So `monetized` is computed from whether the link actually earns,
// and the FTC line renders off that flag — never off "this row has a CTA".
 ...
 * @param {string}  [a.deliveryUrl]      resolved delivery href, or null
 * @param {boolean} [a.deliveryEarns]    whether the delivery link is tracked
 * @param {string}  [a.mapsUrl]          directions href
 * @returns {{type:string,label:string,href:string|null,monetized:boolean}}
 */
export function resolveRowCta({ deal, bookingUrl, deliveryUrl, deliveryEarns, mapsUrl } = {}) {
  if (deal && (deal.url || deal.affiliateUrl)) {
    return { type: "deal", label: CTA_LABELS.deal, href: deal.url || deal.affiliateUrl, monetized: true };
  }
  if (bookingUrl) {
    return { type: "bookable", label: CTA_LABELS.bookable, href: bookingUrl, monetized: true };
  }
  if (deliveryUrl) {
    // monetized follows the MONEY, not the presence of a link — see the header.
    return { type: "delivery", label: CTA_LABELS.delivery, href: deliveryUrl, monetized: !!deliveryEarns };
  }
