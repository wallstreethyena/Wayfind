...
export function isUpscaleBirthdayDinner(place) {
  ...
  return priceNumOf(place) >= 3 || /fine_dining_restaurant|steak_house|steakhouse/.test(primary + " " + evidence);
}
...
export function isRooftop(place) {
  ...
  return /\b(rooftop|roof deck|sky bar)\b/.test(evidenceOf(place));
}

export function isBeachfront(place) {
  if (place?._birthdayAttributes?.beachfront === true) return true;
  return /\b(beachfront|on the beach|oceanfront|gulf[- ]front)\b/.test(evidenceOf(place));
}
...
export function isSpeakeasy(place) {
  ...
  return /\b(speakeasy|hidden bar|secret bar)\b/.test(evidenceOf(place));
}
...
function birthdayRank(a, b) {
  const aNear = Number(a?.distMi) <= BIRTHDAY_NEAR_MI ? 1 : 0;
  const bNear = Number(b?.distMi) <= BIRTHDAY_NEAR_MI ? 1 : 0;
...
export function composeBirthdayRails(places, opts = {}) {
  ...
  return {
    rails: BIRTHDAY_RAIL_DEFS.map((definition) => ({
      ...definition,
      places: pool.filter((place) => birthdayRailMembership(definition.id, place)).slice().sort(birthdayRank),
    })),
  };
}
