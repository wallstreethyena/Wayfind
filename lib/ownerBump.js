...
//
// ownerPick on the SERVER path is still env-derived (WF_OWNER_USER_ID). Live
// proved that a missing or mismatched ownerId silently no-ops the bump while
// the founder is signed in as gabrielpereira@me.com. The like path therefore
// also recognizes that email on the signed-in session — optimistic stamp, then
// reconcile. A device that is not that email cannot mint ownerPick.

/** The bump, in INTERNAL points (0-100). 7 internal = 0.7 on the badge. */
export const OWNER_BUMP = 7;

/** The founder's signed-in email. Public, not a secret; the bump is disclosed. */
...
/**
 * Is this the founder's signed-in session?
 *
 * Email only — never WF_OWNER_USER_ID (that stays server env). Other accounts
 * return false, so a client cannot mint the bump for anyone else.
 */
export function isOwnerAccount(user) {
  if (!user || typeof user !== "object") return false;
  const emails = [user.email, user.user_metadata && user.user_metadata.email];
  return emails.some((e) => String(e || "").trim().toLowerCase() === OWNER_ACCOUNT_EMAIL);
}
 ...
 *
 * Idempotent: liking twice does not add 1.4. A null/unrated base stays null
 * (no fake 0.7). `_members.ownerPick` is set so the Curator's pick mark and
