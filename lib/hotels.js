...
// Quality guard: ~10% of the ingested list is not a bookable hotel — bare street
// addresses used as a name ("2903 Avenue B, Anna Maria Island"), or vacation-rental
// listings ("Angel's Vacation Rentals", "…by RedAwning"). Those aren't lodging you
// "book a room" at, and a street address as a place name with a "Check rates" button
// is an honesty/quality defect. Drop them from what we serve (data untouched).
function isJunkHotel(h) {
  ...
  // 2026-09-23, before the app launch: rows that are not a place you can book
  // a room at — a restaurant, a spa, a travel agency, a condo association, a
  // campground, an amenity ("Pool at ...", "Hyatt Pool"), an email address in
  // a name column. Each one is named with its evidence in
  // lib/ownedHotelExclusions.js; they were being served as hotel cards WITH a
  // "Check rates" button, which is a promise we cannot keep.
  return isExcludedOwnedHotel(h);
}
// Served set = the ingested list minus the non-hotel noise above.
const SERVED = OWNED.filter((h) => !isJunkHotel(h));

// Always "configured" — the owned list is bundled, no external key required.
export function hotelsConfigured() { return Array.isArray(SERVED) && SERVED.length > 0; }
...
function toPlace(h, distKm) {
  return {
    ...
    // v6.18: same-origin 30-day cached proxy (/api/photo) instead of a direct
    // referrer-restricted googleapis URL with a key in the <img> src.
    // v8.56.31: the shared hotel resolver (lib/hotelImage.js) — exact
    // property identity, terms-gated partner image, owned copy, then the
    // same-property Google path (ref, else ?place=), else the branded fallback.
    photo: hotelCardImageSrc({ googlePlaceId: h.gpid, photo_ref: h.photo_ref, name: h.name }),
    photos: [],
    ...
    // wfScore is on the app-wide 0–100 contract (toDisplayScore divides by 10 for the
    // /10 badge). ownedHotels.json carries the owner's curated editorial score on a
    // 0–10 scale today, so ×10 (rating fallback 0–5 → ×20). Storing 0–10 here rendered
    // every owned hotel as ~0.9/10 in the red band — the "glass-box" one-score-everywhere
    // promise requires the same 0–100 scale as lib/google.js wayfindScore().
    // SCALE-TOLERANT on purpose: if the source data is ever regenerated to 0–100, a
    // value already >10 passes through unchanged, so this can never double-scale.
    wfScore: h.wfScore != null ? Math.round(h.wfScore <= 10 ? h.wfScore * 10 : h.wfScore)
             : (h.rating != null ? Math.round(h.rating * 20) : null),
    stayType: h.stayType || null,
    vibe: h.vibe || null,
    whyGo: h.whyGo || null,
    knownFor: h.knownFor || null,
    // 2026-09-23 (owner): `lane`, `bookingMove` and `insiderMove` are curator
    // notes ("refresh the direct site before putting this in a top slot").
    // Nothing renders them, and they no longer ride along on the public
    // place object or its API payload.
    blurb: h.whyGo || null,
    owned: true,
  };
}
...
export async function searchHotels({ lat, lng, city, limit = 20, exhaustive = false } = {}) {
  ...
  if (typeof lat === "number" && typeof lng === "number" && isFinite(lat) && isFinite(lng)) {
    ...
    const pool = exhaustive ? near : near.slice(0, Math.max(n * 3, 40));
    pool.sort((a, b) => {
    ...
    return (exhaustive ? pool : pool.slice(0, n)).map(({ h, d }) => toPlace(h, d));
  }
  if (city) {
    const c = String(city).toLowerCase().trim();
    return SERVED.filter((h) => (h.city || "").toLowerCase() === c).slice(0, n).map((h) => toPlace(h, null));
  }
  return SERVED.slice(0, n).map((h) => toPlace(h, null));
}
