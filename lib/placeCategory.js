...
// A place is a beach if a genuine beach TYPE says so, OR its NAME says beach and
// nothing in its Google types contradicts that. The one contradiction we honor:
// a `playground` type with NO beach type is an inland neighborhood park that
// merely has "Beach" in its name — e.g. West Beach Park, Orlando (google_types
// ['park','playground',…], an inland lake with a playground), which was surfacing
// under the home "Beach Day" hero as the metro's only "beach". Real Gulf beaches
// that carry only `park`/`tourist_attraction` (Coquina, Lido, Turtle Beach Park,
// Manatee Beach Park) never carry a `playground` type, so they stay beaches. A
// real beach that also has a playground keeps its `beach` type and is unaffected.
const isRealBeach = (arr, name) =>
  arr.includes("beach") ||
  (/\bbeach\b/i.test(String(name || "")) && !arr.includes("playground"));
