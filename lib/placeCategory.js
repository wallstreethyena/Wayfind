...
export const CATEGORY_SECTION = { food: "Food", nightlife: "Nightlife", hotels: "Hotels", shopping: "Shopping", beach: "Activities", attractions: "Activities", family: "Activities" };

// The sections an adjudicated verdict may name. Declared HERE, with no import,
// so this file stays dependency-free for the client bundle — the same reason
// lib/beaches.js inlines the score constants. scripts/check-scout-law.mjs
// compares it against scoutAdjudicate.SECTIONS and fails the build on drift,
// which is exactly how check-beaches-page pins that other copy.
export const ADJUDICABLE = new Set(["Food", "Nightlife", "Activities", "Hotels", "Shopping"]);
...
// Medical primaryTypes. v8.99 (2026-09-06): the first canary run with database
// access found 24 non-destination rows in a discovery inventory — 20 of them
// chiropractors, medical clinics and doctors that the 2026-09-01 Florida
// coverage ingest had classified as ATTRACTIONS. The route in: Google types a
// chiropractor `[chiropractor, wellness_center, health]`, `chiropractor` was in
// no exclusion list, and `wellness_center` is an Activities type — so the
// type-list fallback in exclusionReason() saw a section and let it through.
// Google's own primaryType said "medical" every time. It is decisive here too.
//
// ONE narrow exemption, precision over recall: a medical primary that ALSO
// carries `tourist_attraction` is a real destination first (The Turtle
// Hospital, Marathon — primaryType `veterinary_care`, a famous Keys stop).
const MEDICAL_PRIMARY = new Set([
  "chiropractor", "medical_clinic", "doctor", "dentist", "dental_clinic", "hospital",
  "physiotherapist", "pharmacy", "drugstore", "veterinary_care", "skin_care_clinic",
  "medical_lab", "psychologist", "psychiatrist", "counselor", "wellness_clinic",
]);
...
// Types the row ALREADY carries. Empty `types[]` / `google_types[]` (the
// wf_inventory default) reuses primaryType — Google's own answer — so a
// classifiable place is not dropped just because the array is blank.
// NEVER invents from a stored category, a list name, or a vibe. No signal
// → empty array (fail closed). Callers that need a category still go
// through classify(), which may recover from the NAME net and flag it.
export function existingTypeSignals(p) {
  if (!p) return [];
  if (Array.isArray(p.types) && p.types.length) return p.types.map(String);
  if (Array.isArray(p.google_types) && p.google_types.length) return p.google_types.map(String);
...
function typeTokens(p) {
  return existingTypeSignals(p).map((t) => String(t).toLowerCase());
}
const primaryOf = (p) => String((p && (p.primaryType || p.primary_type)) || "").toLowerCase();

// Observation / observatory identity. An observatory that also lists cafe or
// restaurant types is not Food — that was One World Observatory classified as
// coffee. Used as a confidence gate before ranking: Food + observation → Activities.
export function observationIdentity(p) {
  const name = String((p && (p.name || p.title)) || "").toLowerCase();
  if (/\bobservatory\b|\bobservation deck\b|\bobservation tower\b/.test(name)) return true;
  const arr = typeTokens(p);
  return arr.some((t) => /observation_deck|observation_tower/.test(String(t)));
}

function gatedSection(p, section) {
  if (section === "Food" && observationIdentity(p)) return "Activities";
  return section;
}
...
export function exclusionReason(p) {
  ...
  // v8.99: a medical primaryType is a service, whatever spa / wellness_center /
  // gym types ride along — unless Google also calls it a tourist_attraction.
  if (pt && MEDICAL_PRIMARY.has(pt) && !arr.includes("tourist_attraction")) return EXCLUSION.SERVICE;
  ...
  if (CHARITY_TYPE.test(joined) || CHARITY_NAME.test(String(p.name || ""))) return EXCLUSION.SERVICE;
...
// A food bank is not a place to eat, and its rating measures how well it serves
// people in need. FOUND 2026-08-22 while unblocking the unclassified bucket:
// FEAST Food Pantry (4.8 / 234) carries the bare type `food` and nothing else,
// so FOOD_TYPE below matched it and it resolved straight into "best places to
// eat". Vetoed here, before any resolver runs, for the same reason the trades
// are: an aid service is not a discovery result. scripts/check-scout-law.mjs
// pins it.
// MEASURED: FEAST's types are ["food","point_of_interest",
// "association_or_organization","service","establishment"] — there is no
// `food_bank` token to match, so the type veto alone CANNOT catch it. The only
// honest signal is the name, and the phrases below are deliberately whole and
// specific: "food pantry", not "pantry" (The Pantry Bistro is a restaurant),
// and "rescue mission", not "mission" (Mission BBQ is a restaurant).
const CHARITY_TYPE = /\b(food_bank|soup_kitchen|homeless_shelter|welfare_office|social_services)\b/;
// Goodwill and Salvation Army are deliberately ABSENT: their thrift stores are
// a real shopping destination and already resolve that way via SPECIFIC_SHOP.
// Only phrases that can mean nothing but aid provision belong here.
const CHARITY_NAME = /\bfood\s+pantry\b|\bfood\s+bank\b|\bsoup\s+kitchen\b|\bhabitat\s+for\s+humanity\b|\brescue\s+mission\b|\bhomeless\s+shelter\b/i;
...
const NIGHTLIFE_TYPE = /night_club|nightclub|\bbar\b|_bar\b|\bpub\b|pub_|brewery|brewpub|taproom|tavern|wine_bar|cocktail|\blounge\b|speakeasy|distillery|karaoke|hookah|dance_club|beer_garden|comedy_club/;
...
const ACTIVITY_TYPE = /tourist_attraction|amusement|theme_park|water_park|aquarium|\bzoo\b|museum|art_gallery|art_studio|\bpark\b|national_park|state_park|dog_park|nature_preserve|natural_feature|botanical|\bgarden\b|\btrail\b|hiking|\bbeach\b|marina|\bpier\b|boardwalk|landmark|historical|monument|memorial|stadium|\barena\b|theater|theatre|performing_arts|concert_hall|live_music_venue|music_venue|movie_theater|cinema|bowling|arcade|casino|winery|golf_course|\bgolf\b|playground|planetarium|science_museum|observation|scenic_spot|event_venue|amphitheater|amphitheatre|fairground|recreation|\bgym\b|fitness_center|\bspa\b|wellness_center|fishing_charter|sports_club|sports_complex|sports_activity_location|athletic_field|tennis_court|swimming_pool|tour_agency|travel_agency|tourist_information_center|visitor_center|water_sports|scuba_diving_center|adventure_sports_center/;
...
function sectionFromPrimary(pt) {
  ...
  // 2026-09-02 (menu taxonomy audit) — acai_shop/pastry_shop/salad_shop/
  // kebab_shop/noodle_shop are real Google primaryTypes this branch never
  // named, so they fell through to the generic "(_store|_shop)$" Shopping
  // branch below (67 live wf_inventory rows measured: acai_shop=41,
  // pastry_shop=17, salad_shop=6, noodle_shop=2, kebab_shop=1 — see
  // scripts/data/menu-taxonomy-repair-2026-09-02.sql for the data repair).
  if (/(^|_)restaurant$|^(cafe|coffee_shop|bakery|deli|delicatessen|sandwich_shop|bagel_shop|donut_shop|ice_cream_shop|dessert_shop|diner|cafeteria|food_court|meal_takeaway|meal_delivery|steak_house|pizzeria|creamery|butcher_shop|juice_shop|tea_house|candy_store|chocolate_shop|grocery_store|supermarket|health_food_store|food_store|farmers_market|market|acai_shop|pastry_shop|salad_shop|kebab_shop|noodle_shop)$/.test(pt)) return "Food";
  if (/^(bar|pub|night_club|nightclub|sports_bar|wine_bar|brewery|brewpub|taproom|tavern|beer_garden|distillery|cocktail_lounge|karaoke_bar|bar_and_grill|dance_hall|comedy_club)$/.test(pt)) return "Nightlife";
  if (/^(marina|beach|park|state_park|national_park|dog_park|museum|art_museum|history_museum|art_gallery|art_studio|aquarium|zoo|botanical_garden|garden|amusement_park|water_park|tourist_attraction|historical_landmark|historical_place|monument|planetarium|performing_arts_theater|cultural_center|movie_theater|bowling_alley|video_arcade|casino|golf_course|miniature_golf_course|stadium|arena|event_venue|concert_hall|live_music_venue|hiking_area|wildlife_park|wildlife_refuge|nature_preserve|natural_feature|observation_deck|scenic_spot|spa|wellness_center|gym|fitness_center|sports_activity_location|sports_complex|athletic_field|tennis_court|swimming_pool|sports_club|fishing_charter|tour_agency|travel_agency|tourist_information_center|visitor_center|amphitheatre|amphitheater|playground|water_sports|scuba_diving_center|adventure_sports_center|farm|ranch|winery|vineyard)$/.test(pt)) return "Activities";
...
export function primaryCategory(p) {
  ...
  return gatedSection(p, sectionFromPrimary(primaryOf(p)) || sectionFromTypes(typeTokens(p)));
}
...
// The beach NAME net. v8.99 (2026-09-06): "\bbeach\b anywhere in the name" put
// 41 non-beaches in the beach list on the first canary run that could see the
// database — Shattered Glass DIY St. Pete Beach (an art studio), Sea Blaster
// Fort Walton Beach (a tour boat), Miami Beach Sign, Madeira Beach Dog Park,
// Cocoa Beach Pier Arch. Florida's towns are NAMED Beach; a name that merely
// mentions one is not a shoreline. Two rules, both structural:
//   1. the word must be the place's own identity — the name ENDS in "Beach",
//      or "Beach" + a shoreline suffix (Park, Access, Pavilion, Shore, State
//      Park…). "Hollywood North Beach Park" yes; "Miami Beach Boardwalk" no
//      (a boardwalk, → Activities); "Skyway Beach" yes; "Miami Beach Sign" no.
//   2. a commercial identity in the type list vetoes the name net outright —
//      a store, studio, restaurant, bar or tour agency named after a beach
//      town is that business, not a beach.
// A real `beach` GOOGLE TYPE still decides on its own, as before.
// A trailing ", <town>" ("South Beach Park, Boca Raton") is a locator, not identity.
const BEACH_NAME = /\bbeach(?:\s+(?:park|access|pavilion(?:\s+\d+)?|preserve|shore|landing|state\s+park|regional\s+access|county\s+park|oceanside\s+park|oceanview\s+park))?\s*(?:,\s*[a-z.' ]+)?$/i;
const BEACH_NAME_VETO = /\b(store|gift_shop|art_studio|art_gallery|clothing_store|home_goods_store|restaurant|food|bar|cafe|tour_agency|travel_agency|lodging|hotel|dog_park|playground|marina|boat_rental|night_club)\b/;
export const isRealBeach = (arr, name) => {
  if (arr.includes("beach")) return true;
  const n = String(name || "");
  if (!BEACH_NAME.test(n)) return false;
  return !BEACH_NAME_VETO.test(" " + arr.join(" ") + " ");
};
...
export function classify(input) {
  ...
  // LAST NET, and only after every signal Google gave us has abstained: a
  // section adjudicated by lib/scoutAdjudicate.js from the editorialSummary
  // prose this file structurally cannot read (Mote Marine Laboratory: types
  // say `research_institute`, Google's own summary says "Aquarium with a
  // stingray touch tank"). Two properties make it safe, and both are
  // STRUCTURAL rather than promised:
  //   1. it cannot reach an EXCLUDED place — exclusionReason() returned above;
  //   2. it cannot override a DECIDED one — every branch above already set
  //      `section`, and this only fires on !section.
  // Landed needs_review like the name net (lib/seedPlaces.js): a model may
  // FLAG, it may never silently decide. scripts/check-scout-law.mjs pins all
  // of this, including that ADJUDICABLE matches scoutAdjudicate's SECTIONS.
  if (!section && ADJUDICABLE.has(p.adjudicatedSection)) { section = p.adjudicatedSection; via = "adjudicated"; }
  const gated = gatedSection(p, section);
  if (gated !== section) { section = gated; }
