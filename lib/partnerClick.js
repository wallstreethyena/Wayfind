...
export function planPartnerClick({ href, owned, locationHost, fallbackSurface, mint }) {
  ...
  // Only http(s), and only our own host. The host is checked on the RESOLVED
  // URL for every href, relative or not: a protocol-relative href
  // ("//evil.example/api/viator/go") has no scheme but still resolves to a
  // foreign host, and a plain relative href resolves to the page's own host,
  // which isOwnHost accepts.
  if (/^https?:$/i.test(url.protocol) === false) return null;
  if (!isOwnHost(url.hostname, locationHost)) return null;
