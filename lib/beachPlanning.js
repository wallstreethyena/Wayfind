...
export const BEACH_PILOT = [
  ...
  { slug: 'lido', name: 'Lido Beach', county: 'Sarasota', lat: 27.3103, lng: -82.5779, stations: ['LIDO CASINO BEACH'], visitBeachId: '4', visitBeachName: 'Lido Key Beach' },
  { slug: 'siesta', name: 'Siesta Beach', county: 'Sarasota', lat: 27.2664, lng: -82.5507, stations: ['SIESTA KEY BEACH'], visitBeachId: '2', visitBeachName: 'Siesta Key Beach' },
];
...
export function forecastPeriods(data, now = Date.now()) {
  ...
  return (Array.isArray(data?.properties?.periods) ? data.properties.periods : [])
    .filter(p => Date.parse(p.endTime) > now && Date.parse(p.endTime) > Date.parse(p.startTime) && Date.parse(p.startTime) < now + 24 * 3600000)
    .map(p => ({ start: p.startTime, end: p.endTime, summary: p.shortForecast || 'Unknown',
      ...
      rain: Number.isFinite(p.probabilityOfPrecipitation?.value) && p.probabilityOfPrecipitation.value >= 0 && p.probabilityOfPrecipitation.value <= 100 ? p.probabilityOfPrecipitation.value : null,
      daytime: p.isDaytime === true, wind: p.windSpeed || 'Unknown' }));
}
export function planningSummary({ samples = [], alerts = null, periods = [], closure = 'unknown', waterClosure = 'unknown', localFlag = 'unknown', redTide = null }) {
  const hazards = [];
  ...
  if (waterClosure === 'closed') hazards.push('The latest local report says the water is closed to the public. Follow posted signs and lifeguard instructions.');
  if (['yellow', 'red', 'purple'].includes(localFlag)) hazards.push(`A ${localFlag} flag was reported locally. Check the posted flag and follow lifeguard instructions.`);
...
// Authority-selected condition pages. These are links, not ingested live flags.
// Never read the color legend on these pages as a current flag report.
export function localBeachSources(beach) {
  if (beach.county === 'Manatee') return {
    title: 'Manatee County Beach Patrol',
    url: 'https://www.mymanatee.org/services-and-amenities/service-listing/service-details/check-beach-conditions',
    conditionsUrl: beach.slug === 'manatee' ? 'https://safebeachday.com/manatee-public-beach/' : 'https://safebeachday.com/coquina-beach-and-cortez-beach/',
    instructions: 'The county directs visitors to Safe Beach Day for reports at 10 a.m. and 3 p.m. Flags at the lifeguard towers override the online report.',
  };
  return { title: 'Sarasota County beach information',
    url: 'https://www.sarasotacountyparks.com/parks-and-facilities/discover-a-park/type-of-park/beaches',
    conditionsUrl: 'https://visitbeaches.org/',
    instructions: 'Check the local beach report and posted signs. Confirm current flags and closures with the lifeguards when you arrive.',
  };
}
