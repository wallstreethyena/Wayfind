 ...
 */
// SERVED SIZE (2026-09-17). Production audit: 109 of 159 vaulted copies were
// over 1 MB and the largest was 14.7 MB, because the vault stored Commons
// ORIGINALS. A card never needs more than 1280 px. When a display width is
// given, the URL goes through Supabase Storage's image transformation
// endpoint (verified live: a 10.7 MB vault copy renders as a 335 KB JPEG at
// width 640), clamped to one of two canonical widths (640 for any card up to
// 800 px, else 1280) so the CDN caches a small, fixed set of variants. With
// no width (provenance, admin, legacy callers) the plain object URL is
// returned exactly as before.
// resize=contain IS REQUIRED (measured 2026-09-17): the endpoint's default
// mode is `cover`, which with only a width keeps the ORIGINAL height and
// returns a centre crop (a 5184x3456 photo came back 640x3456; EPCOT's
// 1280x853 came back 640x853). `contain` scales proportionally (640x427).
export const VAULT_SERVE_WIDTHS = Object.freeze([640, 1280]);
export function vaultServeWidth(width) {
  const w = Number.parseInt(width, 10);
  if (!Number.isFinite(w) || w <= 0) return 640;
  return w <= 800 ? 640 : 1280;
}
export function vaultPublicUrl(storagePath, supabaseUrl, opts = {}) {
  ...
  if (opts && opts.width != null) {
    return `${base}/storage/v1/render/image/public/${PLACE_PHOTOS_BUCKET}/${path}?width=${vaultServeWidth(opts.width)}&resize=contain&quality=75`;
  }
