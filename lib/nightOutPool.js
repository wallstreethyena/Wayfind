...
// v8.98 — ONE definition of the deterministic exhaustive owned read and of the
// row-level serviceability gates, shared with every other surface that has the
// same narrow question. This module keeps its own SHAPE (rowToNightOutPlace)
// and its own LAW (nightOutPlaceRail, exactly 27 miles); what it no longer
// keeps is a private copy of "page wf_inventory in a stable order", which is
// how two readers drift apart. See lib/ownedPool.js.
import { readOwnedCategory, hydratePhotoRefs, isServableRow as isServableOwnedRow, milesBetween as ownedMilesBetween } from "./ownedPool.js";
...
import { NIGHT_OUT_DISTRICT_IDS, nightOutAttributes } from "./nightOutEvidence.js";
...
// Everything the ten predicates read. `editorial` is included ON PURPOSE and
// must stay: isShow/isLiveMusic/isDateDining and the rest match against
// name + types + editorial, so a lean read would convert candidate starvation
// into evidence starvation.
//
// `photo_ref` is deliberately NOT read here (2026-09-09). Measured on the
// Parrish box, it was 59% of every 1,000-row page (469 KB of 970 KB) and no
// predicate, gate or ranking law reads it — the whole pool was hauling ~1.6 MB
// of Google photo names per cold compute so that a few hundred survivors could
// render a thumbnail. On the 2026-09-09 deploy ten cold cells hit /api/night-out
// at once, every page blew its deadline, and all ten served a 503. The card
// still gets its photo: hydratePhotoRefs() fetches the reference AFTER
// admission, for admitted rows only. See lib/ownedPool.js OWNED_POOL_FIELDS.
const FIELDS = "place_id,name,lat,lng,category,secondary_categories,primary_type,google_types,status,excluded,signals,editorial";

const MI = 1609.34;

/** PURE. Great-circle miles — the shared one, re-exported so this module's
 *  callers and guards keep their import and there is still only one formula. */
export const milesBetween = ownedMilesBetween;
 ...
 */
export const isServableRow = isServableOwnedRow;
...
export function rowToNightOutPlace(row, origin, editorialOverride) {
  ...
  return {
    ...
    _nightOutAttributes: nightOutAttributes(id),
    _wfInventory: true,
  };
}
...
export function admitNightOutRows(rows, origin, opts = {}) {
  ...
  const out = [];
  const seen = new Set();
  // Admitted rows, parallel to `out`: the lean read carries no photo_ref, so the
  // fetch hydrates these rows after the cut and rebuilds the places from them.
  const admittedRows = [];
  ...
  for (const row of Array.isArray(rows) ? rows : []) {
    ...
    stats.servable++;
    // EXACTLY 27 miles, MEASURED rather than read off the card (owner review,
    // 2026-09-06). rankInventory's gate is radius x 1.15 (~31mi at 27), which is
    // right for its own general role and wrong here: composeNightOutRails refuses
    // anything past NIGHT_OUT_MAX_MI anyway, so a row between 27 and 31 is a
    // candidate that can only ever be discarded later — and, measured, it is also
    // what made the diagnostic's "route" column larger than its "ALL" column for
    // Cocktails (203 vs 202) and briefly look like negative loss.
    //
    // v8.98h: this tested `place.distMi`, which rowToNightOutPlace rounds to one
    // decimal for the card. A row at 27.0498 miles renders "27" and was admitted.
    // The rail's own copy says "the only place within 27 miles that clears this",
    // so the cut is now taken from the ROW's coordinates and the rounded value is
    // left where it belongs, on the card.
  ...
  return { places: out, stats, admittedRows };
}
...
async function readCategory(env, category, box, opts) {
  return readOwnedCategory(env, category, box, {
    ...opts,
    fields: FIELDS,
    pageSize: NIGHT_OUT_POOL_PAGE,
    maxRows: NIGHT_OUT_POOL_MAX_ROWS,
  });
}

async function readGovernedDistricts(env, opts = {}) {
  ...
  const ids = NIGHT_OUT_DISTRICT_IDS.map(quote).join(",");
  const url = `${env.url}/rest/v1/wf_inventory?select=${FIELDS}&place_id=in.(${encodeURIComponent(ids)})&order=place_id.asc`;
  const res = await doFetch(url, { headers, cache: "no-store" }, deadlineMs);
  if (!res.ok) throw new Error(`Night Out governed district read returned ${res.status}`);
  const rows = await res.json();
  if (!Array.isArray(rows)) throw new Error("Night Out governed district read returned a non-array body");
  return rows;
}
...
export async function fetchNightOutPool(lat, lng, opts = {}) {
  ...
  const allSettled = await Promise.allSettled([
    ...categories.map((c) => readCategory(env, c, box, opts)),
    readGovernedDistricts(env, opts),
  ]);
  const settled = allSettled.slice(0, categories.length);
  const districtRead = allSettled[categories.length];
  const reads = settled.filter((r) => r.status === "fulfilled").map((r) => r.value);
  const categoryFailures = settled.length - reads.length;
  const districtFailures = districtRead?.status === "rejected" ? 1 : 0;
  const failures = categoryFailures + districtFailures;
  ...
  if (!reads.length) {
    ...
    throw new Error(`All Night Out inventory reads failed${why ? `: ${why.reason && why.reason.message ? why.reason.message : why.reason}` : ""}`);
  }
  // An incomplete pool is not a normal answer — see lib/ownedPool.js. Night Out
  // gets the same law rather than a softer one: a partial pool cached for an hour
  // is the silent degradation this whole change removes.
  const truncated = reads.some((r) => r.truncated);
  if (truncated && !opts.allowTruncated) {
    throw new Error(
      `Night Out pool hit NIGHT_OUT_POOL_MAX_ROWS (${NIGHT_OUT_POOL_MAX_ROWS}) — the pool is incomplete, so identity `
      + "ran on a slice. Raise the cap, narrow the read, or pass allowTruncated and say the answer is partial.",
    );
  }
  const districtRows = districtRead?.status === "fulfilled" ? districtRead.value : [];
  const raw = [...reads.flatMap((r) => r.rows), ...districtRows];
  const admitted = admitNightOutRows(raw, origin, { maxMi, editorialOverride: opts.editorialOverride, railOf: opts.railOf });
  // Photos AFTER the cut, for the survivors only (see FIELDS). The rows are
  // hydrated in place and the places rebuilt from them, so `photoRef` on a
  // served place comes from the same mapper as every other field.
  const photoRefs = await hydratePhotoRefs(env, admitted.admittedRows, {
    fetchImpl: opts.fetchImpl,
    deadlineMs: opts.photoDeadlineMs,
  });
  const places = admitted.admittedRows
    .map((row) => rowToNightOutPlace(row, origin, opts.editorialOverride))
    .filter(Boolean);
  return {
    places,
    stats: {
      ...
      photoRefs,
      truncated,
      // Incomplete for ANY reason — see lib/ownedPool.js. One failed category
      // out of three is the commonest incomplete answer there is, and it used to
      // report `degraded: false`. A pool whose photos could not all be fetched is
      // incomplete in the same sense: served now, never cached for an hour.
      degraded: truncated || failures > 0 || photoRefs.failed > 0,
      sourceFailures: failures,
      districtSourceFailures: districtFailures,
      governedDistrictRows: districtRows.length,
