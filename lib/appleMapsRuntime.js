...
export function loadAppleMapKit(token) {
  ...
  if (!token || /placeholder/i.test(token)) return Promise.reject(new Error("NEXT_PUBLIC_APPLE_MAPS_TOKEN is not configured"));
...
export function createAppleMapController({ mapkit, container, venue, picks = [], onSelect, onError, requestTimeoutMs = 15000 }) {
  ...
  picks.filter((p) => p && Number.isFinite(p.lat) && Number.isFinite(p.lng)).forEach((p, i) => add(p, { title: p.name, subtitle: p.cat || "Nearby", color: "#2EC9A6", glyphText: String(i + 1) }, p.id));
  ...
  return { map, routeFromCoordinate, searchAndRoute, clearRoute, destroy };
}
