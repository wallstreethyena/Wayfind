...
import { gateShut, spendAllowSkuTransition, textEnterpriseCap } from "./spendGate.js";
...
async function _searchGoogle(query, city, radiusM, withCityName, key, withPhotos) {
  ...
  try {
    ...
    // null preserves searchOnce's stale-cache fallback. A budget denial must
    // never be cached as a legitimate empty market.
    if (gateShut() || !(await spendAllowSkuTransition("text_pro", "text_enterprise", textEnterpriseCap()))) return null;
