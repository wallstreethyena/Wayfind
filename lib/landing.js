...
// Home markets first (launch prompt 5); Hawaii comes after these prove out.
...
async function searchOnce(query, city, radiusM) {
  ...
  try {
    const r = await fetch("https://places.googleapis.com/v1/places:searchText", {
      ...
      body: JSON.stringify({ textQuery: query + " " + city.name + " " + city.state, maxResultCount: 20, locationBias: { circle: { center: { latitude: city.lat, longitude: city.lng }, radius: Math.min(radiusM, 50000) } } }),
...
export async function rankedFor(catSlug, citySlug) {
  ...
  let pool = null;
  for (const radiusM of [27359, 48280]) {
    const raw = await searchOnce(cat.query, city, radiusM);
    ...
    pool = gated.filter((p) => p.distMi == null || p.distMi <= (radiusM / 1609.34) * 1.3);
