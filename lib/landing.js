...
import CollectionHero, { HeroCta } from "../app/components/CollectionHero";
...
async function _searchGoogle(query, city, radiusM, withCityName, key, withPhotos) {
  try {
    const mask = "places.id,places.displayName,places.location,places.rating,places.userRatingCount,places.formattedAddress,places.types,places.businessStatus,places.priceLevel"
      ...
      + (withPhotos ? ",places.photos,places.regularOpeningHours" : "");
    ...
    return (d.places || []).map((p) => {
      ...
      return {
        ...
        openNow: p.regularOpeningHours && typeof p.regularOpeningHours.openNow === "boolean" ? p.regularOpeningHours.openNow : null,
      };
    }).filter((p) => p.name);
  } catch (e) { return null; }
}
...
export async function LandingPage({ catSlug, citySlug }) {
  ...
  return (
    <main style={S.page}>
      ...
      <CollectionHero
        eyebrow={`${cat.icon} The Wayfind shortlist · ${city.name}, ${city.state}`}
        titleTop={`Best ${cat.label}`}
        titleBottom={`in ${city.name}`}
        subtitle={`Not another endless directory. These are the ${cat.label.toLowerCase()} that earn your time—ranked by review strength, distance, and what makes each one worth choosing.`}
        heroImg={LANDING_HERO[catSlug]}
        height={430}
        radius={28}
        bleed="0 -18px 34px"
        maxWidth={920}
        cta={<HeroCta href="#wayfind-shortlist">See the shortlist ↓</HeroCta>}
      />
