...
async function _cacheRow(ck) {
  ...
  try {
    const r = await fetch(`${s.url}/rest/v1/wf_places_cache?k=eq.${encodeURIComponent(ck)}&select=v,exp`, { headers: { apikey: s.k, Authorization: `Bearer ${s.k}` }, cache: "no-store" });
    if (!r.ok) return null;
...
async function _cachePut(ck, v) {
  ...
  try {
    await fetch(`${s.url}/rest/v1/wf_places_cache`, { method: "POST", headers: { apikey: s.k, Authorization: `Bearer ${s.k}`, "Content-Type": "application/json", Prefer: "resolution=merge-duplicates" }, body: JSON.stringify({ k: ck, v, exp: new Date(Date.now() + 5 * 24 * 3600 * 1000).toISOString() }) });
  } catch (e) {}
}
        ...
        <>
          ...
          {list.map((p, i) => (
            <div key={p.id || i} style={S.card}>
              ...
              <p style={S.why}>{whyLine(p, cat.singular)}</p>
              {insiderByIdx[i] ? <p style={{ fontSize: 13.5, color: "#E6EDF3", margin: "6px 0 0", lineHeight: 1.5 }}>🗝️ <b>Insider:</b> {insiderByIdx[i].tip || insiderByIdx[i].special}</p> : null}
