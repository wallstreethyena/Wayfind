...
const LANDING_TOUR_CITIES = { "manatee-sarasota": ["Sarasota"], tampa: ["Tampa", "St. Petersburg", "Clearwater"], orlando: ["Orlando"] };
async function landingTours(metro) {
  ...
  try {
    const r = await fetch(url + "/rest/v1/wf_experiences?select=product_code,title,city,rating,reviews,from_price,image,product_url&city=in.(" + cities.map((c) => '"' + c + '"').join(",") + ")&order=reviews.desc&limit=60", { headers: { apikey: svc, Authorization: "Bearer " + svc }, next: { revalidate: 21600 } });
    if (!r.ok) return [];
