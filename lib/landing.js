...
export async function rankedFor(catSlug, citySlug, opts) {
  ...
  pool.forEach((p) => {
    ...
    p._s = q == null
      ? -Infinity
      : governedWayfindScore(q, { hasCreatorVideo: hasCreatorVideoAt(p), distanceMi: isFinite(mi) && mi > 0 ? mi : null, trending: !!p.trending }) + (CURATED_NAMES.has(_nn(p.name)) ? 15 : 0) + localCategoryBoost(p);
  });
...
export function whyLine(p, singular) {
  const bits = [];
  // 2026-08-08: mandatory disclosure for the trending rank component — the
  // reason leads the line wherever these rows render (root page, /florida,
  // culture + guide bridges). lib/trendSignal.js reasons are level-honest.
          ...
          {topLevel.map(({ place: p, children }, i) => (
            <div key={p.id || i} style={S.card}>
              ...
              <div style={{ display: "flex", gap: 12, justifyContent: "space-between", alignItems: "flex-start" }}>
                <div>
                  <div style={{ fontSize: 10.5, fontWeight: 900, letterSpacing: "1.7px", textTransform: "uppercase", color: "#FF8A3D" }}>{i === 0 ? "Best match" : "More strong matches"}</div>
                ...
                {(() => {
                  ...
                  const _shown = _q == null ? null : toDisplayScore(governedWayfindScore(_q, { hasCreatorVideo: hasCreatorVideoAt(p), distanceMi: isFinite(_mi) && _mi > 0 ? _mi : null, trending: !!p.trending }));
                  return (
                    <div style={{ minWidth: 58, padding: "9px 8px", borderRadius: 14, background: "#F7F1E7", color: "#0D1724", textAlign: "center", fontWeight: 900, lineHeight: 1 }}>
              ...
              {(() => {
                ...
                const wq = sig && sig.water ? (sig.water.advisory ? { t: "Advisory — check before swimming", c: "#EF4444" } : sig.water.result === "Good" ? { t: "Water quality: Good", c: "#22C55E" } : sig.water.result === "Moderate" ? { t: "Water quality: Moderate", c: "#E8B84B" } : sig.water.result ? { t: "Water quality: Poor", c: "#EF4444" } : null) : null;
