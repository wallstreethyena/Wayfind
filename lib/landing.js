...
import { atlasEditorialForPlace, landingWhyFits, rankingWhyLine } from "./rankingWhy.js";
import { landingCardPhotoSrc } from "./placePhoto.js";
import { fetchDeadline, NET_DEADLINE_MS, DB_DEADLINE_MS } from "./fetchDeadline.js";
import { isSsgBuild, placesCallsForbidden, fetchLandingInventory, landingIdentityOk } from "./landingInventory.js";
...
// Hero chrome only — never a place-card image. A card may only show a photo
// that belongs to THAT place (landingCardPhotoSrc). The v6.57 city+category
// Pexels pool painted real venue signs on the wrong cards (Pangea Alchemy Lab
// wearing Shamrock City Pub, live /nightlife/parrish 2026-08-29). Empty /
// placeholder is correct when we do not hold that place's photo.
const LANDING_HERO = {
...
async function landingEditorials(places) {
  ...
  if (url && anon && ids.length) {
    try {
      const r = await fetchDeadline(url + "/rest/v1/wf_editorial?verified=is.true&select=place_id,hook,why_here,local_tip&place_id=in.(" + ids.map(encodeURIComponent).join(",") + ")", { headers: { apikey: anon, Authorization: "Bearer " + anon }, next: { revalidate: 3600 } });
      if (r.ok) {
          ...
          {topLevel.map(({ place: p, children }, i) => (
            <div key={p.id || i} style={S.card}>
              ...
              <div style={{ flex: "1 1 260px", minHeight: 230, borderRadius: 16, overflow: "hidden", background: "#1C2230" }}>
                {(() => {
                  const src = landingPhoto(p);
                  return src
                    ? <img src={src} alt="" data-wf-place-id={p.id || ""} style={{ width: "100%", height: "100%", minHeight: 230, display: "block", objectFit: "cover" }} />
                    : <div role="img" aria-hidden="true" style={{ width: "100%", height: "100%", minHeight: 230, background: "#1C2230" }} />;
                })()}
              </div>
