 ...
 */
/**
 * Did /api/rails say the RANKING FAILED — as opposed to "this point is not a
 * covered city"? The two are different sentences to a reader: one is "try
 * again", the other is "not live here". The route sets the top-level flag
 * (app/api/rails/route.js, degraded and thrown paths); the two `data.*` reads
 * are defence against any older payload shape that carried the flag inside.
 * A failed answer is never a fact about the reader's town.
 */
export function isFailedRailsResponse(j) {
  if (!j || typeof j !== "object") return false;
  if (j.failed === true) return true;
  const d = j.data;
  return !!(d && typeof d === "object" && (d.failed === true || d.covered === false));
}

export function liveFromRailsResponse(j) {
  if (!j || j.covered !== true || !j.data || isFailedRailsResponse(j)) return emptyRailLive();
  const d = j.data;
  ...
  return {
    places,
    railTotals: d.railTotals && typeof d.railTotals === "object" ? d.railTotals : {},
    railHasMore: d.railHasMore && typeof d.railHasMore === "object" ? d.railHasMore : {},
...
/** Append one ordered /api/rails page without disturbing any sibling rail. */
export function mergeRailPage(previous, response, railId) {
  const incoming = liveFromRailsResponse(response);
  if (!previous || !incoming.covered || !railId) return previous;
  const before = Array.isArray(previous.places?.[railId]) ? previous.places[railId] : [];
  const page = Array.isArray(incoming.places?.[railId]) ? incoming.places[railId] : [];
  const seen = new Set(before.map((row) => row?.id).filter(Boolean));
  const appended = before.slice();
  for (const row of page) {
    if (!row?.id || seen.has(row.id)) continue;
    seen.add(row.id);
    appended.push(row);
  }
  const total = incoming.railTotals?.[railId] ?? previous.railTotals?.[railId] ?? appended.length;
  return {
    ...previous,
    places: { ...(previous.places || {}), [railId]: appended },
    railTotals: { ...(previous.railTotals || {}), [railId]: total },
    railHasMore: { ...(previous.railHasMore || {}), [railId]: appended.length < total },
  };
}
...
/**
 * The LOCALITY a Google Places `formattedAddress` names, as the "City, ST"
 * label the header prints — or "" when the string does not carry one.
 *
 * WHY (location-integrity audit, 2026-09-08). Tapping an autosuggest PLACE
 * moves the ranking centre to that business. The tap handler had no city for
 * the new point without spending a geocode, so it kept the OLD header label
 * whenever centerAgreesWithLabel() said the pair was plausible — and that
 * check is sized for the cross-state corruption it was written for
 * (PAIRING_MAX_MI = 40). Every Gulf Coast town is inside 40 miles of every
 * other, so a Parrish reader who tapped a Cortez restaurant kept "Parrish"
 * in the header while the map and the rails (correctly) moved to Cortez —
 * and the writer effect persisted that pair. The address Google already
 * returned with the place names the city for free:
 *   "12321 Cortez Rd W, Cortez, FL 34215, USA" → "Cortez, FL"
 * US shape only, on purpose: the segment before "ST 12345" is the locality.
 * Anything else returns "" and the caller falls back to "no city" — the
 * honest answer when we do not know (locationHonesty prints nothing rather
 * than a guess).
 */
export function localityFromFormattedAddress(addr) {
  ...
  for (let i = 1; i < parts.length; i++) {
    ...
    // The previous segment must read as a place name, not a street line.
    if (!city || /\d/.test(city) || city.length > 40) return "";
    return `${city}, ${m[1]}`;
  }
  return "";
}

export const STORED_PIN_TTL_MS = 6 * 3600 * 1000;

/**
 * Is a stored wf_center record still young enough to stand in for the
 * reader's location? REQUIRES a finite `ts`. The two readers of this record
 * (the manual-pin hydration and the GPS saved-anchor shortcut) used to accept
 * `!ts ||`, i.e. a record with no timestamp was trusted forever — a
 * pre-`ts` record, or one written by any client that dropped the field, never
 * expired (audit finding, 2026-09-08). A pin whose age we cannot measure is
 * not a pin we can vouch for.
 */
export function storedPinFresh(record, now = Date.now(), ttlMs = STORED_PIN_TTL_MS) {
  if (!record || typeof record !== "object") return false;
  if (!Number.isFinite(Number(record.lat)) || !Number.isFinite(Number(record.lng))) return false;
