...
const FIELDS = "place_id,name,lat,lng,category,primary_type,google_types,status,excluded,signals,editorial,photo_ref,photo_url";
...
export function themeParkRows(rows, mode = "flagship", intent = null) {
  ...
  for (const row of Array.isArray(rows) ? rows : []) {
    ...
    if (!(rating > 0) || !(score > 0) || !row.place_id || (!row.photo_ref && !row.photo_url)) continue;
    const place = {
      ...
      photo: row.photo_url || `/api/photo?ref=${encodeURIComponent(row.photo_ref)}&w=640`,
...
export async function loadThemeParks({ mode = "flagship", intent = null, env = sbEnv(), deadlineMs = DB_DEADLINE_MS } = {}) {
  ...
  const names = aliases.map(quoteIn).join(",");
  const url = `${env.url}/rest/v1/wf_inventory?select=${FIELDS}&name=in.(${encodeURIComponent(names)})&limit=${aliases.length}`;
  ...
  return themeParkRows(await response.json(), mode, intent);
}
