...
import {
  isNotEventArtwork, isPlaceholderTmUrl,
  LIVE_POSTER_RATIO, LIVE_POSTER_MIN_CROP_WIDTH,
} from "./livePosterCandidate.js";
export { isNotEventArtwork, isPlaceholderTmUrl } from "./livePosterCandidate.js";

export const POSTER_RATIO = LIVE_POSTER_RATIO; // 9:16, matches lib/rails.js --wf8-ratio
const MIN_CROP_WIDTH = LIVE_POSTER_MIN_CROP_WIDTH; // below this the crop reads soft at 2x DPR
...
export async function attemptAttentionCrop(buffer, { ratio = POSTER_RATIO } = {}) {
  ...
  // OWNER DECISION 2026-09-17, superseding the earlier reject/fallback rule:
  // "no blurred extended, I need it to be the full picture, try to fit as much
  // as you can, if you can't then it is what it is."
  //
  // A poster is a picture that fills its tile. Letterboxing real event art
  // inside blurry filler looked worse than any crop ever will, so the two
  // checks that used to force that fallback -- subject-survival and the
  // chopped-banner-text heuristic -- no longer REJECT. The attention crop
  // still aims the window at the densest part of the frame, so "as much as we
  // can" is exactly what it keeps; it just no longer refuses the shot when
  // the geometry costs it something. survivalRatio is still returned for
  // observability.
  //
  // What still rejects, because these are about honesty and legibility rather
  // than composition: art that is not this event's (placeholder/venue/stock),
  // a crop too small to render sharp, and a frame with no usable exposure.
...
export async function fitPosterImage(candidates, { ratio = POSTER_RATIO, fetchImage } = {}) {
  ...
  // NO BLURRED-EXTEND FALLBACK (owner, 2026-09-17). A poster either fills its
  // tile with this event's own picture or it does not run. When nothing here
  // can be cropped to fill, the caller walks to the NEXT EVENT down the same
  // ranking (see app/components/useLivePosterTiles.js) rather than
  // letterboxing a photo inside blurry filler.
