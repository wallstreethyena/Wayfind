...
// 2026-09-23 — THE MAP DEFAULT MAY ONLY FILL A BLANK, NEVER OVERRIDE A CHOICE.
// Owner ask (2026-08-03): open the map on Activities near the reader. The old
// test for "the reader has not picked anything" was `cat === "food"`, but
// "food" is ALSO what the reader has when they explicitly chose Food. Measured
// live on production: Parrish Food > Cafés listed 93 places, then the first Map
// tap of the session switched the category to Activities, so the map showed a
// different set of places than the list beside it (Tampa Food > Dinner: 769
// listed, 381 pins, zero overlap). The default now applies only when no
// category or chip was chosen in this session.
export function shouldApplyMapDefault({ cat, chosen } = {}) {
  return !chosen && cat === MAP_DEFAULT_CATEGORY;
}
