...
async function fetchLeadImageFilename(title, doFetch) {
  const url = `https://en.wikipedia.org/w/api.php?action=query&format=json&prop=pageimages&piprop=original&redirects=1&titles=${encodeURIComponent(title)}&origin=*`;
...
export async function findCommonsPhoto(place, deps = {}) {
  const doFetch = typeof deps.fetch === "function" ? deps.fetch : undefined;
  const reject = (reason) => {
    try {
  ...
  try {
    ...
    if (!candidate) return reject("no_wiki_candidate");
    ...
    const image_url = commons.url || lead.fallbackUrl;
