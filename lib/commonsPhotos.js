...
//
// 2026-09-17 ADDITIONS (wrong-place audit). Business-type words (restaurant,
// cafe, bar, ...) and a few more generic venue nouns (cove, zoo, stadium,
// arena) were MISSING here, which let a place like "Lenny's Restaurant" or
// "Ortygia (restaurant)" reduce to a single real distinctive token while
// "restaurant"/"cafe" still counted as if it proved something. Adding them
// here (same treatment as the existing "theatre"/"marina"/"bay" entries)
// shrinks the false sense of multi-token confidence for exactly the shape of
// place this audit found broken.
const GENERIC_PLACE_TOKENS = new Set([
  ...
  "bar", "bars", "grill", "grille", "pub", "pubs", "tavern", "taverns",
  "bistro", "bistros", "diner", "diners", "kitchen", "kitchens",
  "saloon", "saloons", "brewery", "breweries", "bakery", "bakeries",
  "eatery", "eateries", "cove", "coves", "zoo", "zoos",
  "aquarium", "aquariums", "stadium", "stadiums", "arena", "arenas",
  "mall", "malls", "pier", "piers", "boardwalk", "boardwalks",
  "lighthouse", "lighthouses",
]);

// 2026-09-17 (wrong-place audit). A production sample of 23 ACTIVE free
// photos that were plainly another place — Blue Ridge Park served "Blue
// Ridge Parkway shield"; Château 13 served "Château Frontenac"; The Pinery
// (Orlando) served "The Pinery, Colorado"; Congo River Rapids served an
// aerial of the actual Congo river; Dezerland Park Orlando served a
// celebrity red-carpet photo — shared ONE root cause: distinctive-token
// overlap (even a real one) was treated as sufficient EVIDENCE, with no
// check for a plainly CONFLICTING location word sitting right there in the
// same title. "Blue Ridge Parkway" and "Blue Ridge Park" share both
// distinctive tokens; nothing about that overlap flags that a parkway
// running through Virginia/North Carolina is not a park token a Florida (or
// any single-market) inventory can ever legitimately match against.
//
// This is intentionally NOT exhaustive — it is every US state's distinctive
// word plus the specific non-Florida cities/countries this audit's false
// positives actually named (Toronto, Portland, the Bronx, Lansing, Quebec,
// Kisangani, Colorado, Georgia, ...). A token here NEVER blocks a match by
// itself: it only fires when the place's OWN name/city tokens do not
// already contain it (see conflictingLocationToken below) — a place that is
// genuinely named "Georgia's Diner" or sits in a city on this list is never
// penalised for its own name or its own home town.
const US_STATE_TOKENS = [
  "alabama", "alaska", "arizona", "arkansas", "california", "colorado",
  "connecticut", "delaware", "georgia", "hawaii", "idaho", "illinois",
  "indiana", "iowa", "kansas", "kentucky", "louisiana", "maine", "maryland",
  "massachusetts", "michigan", "minnesota", "mississippi", "missouri",
  "montana", "nebraska", "nevada", "hampshire", "jersey", "mexico", "york",
  "carolina", "dakota", "ohio", "oklahoma", "oregon", "pennsylvania",
  "rhode", "tennessee", "texas", "utah", "vermont", "virginia",
  "washington", "wisconsin", "wyoming",
];
const OTHER_CONFLICTING_LOCATION_TOKENS = [
  "canada", "ontario", "quebec", "toronto", "congo", "kisangani", "brazil",
  "greece", "kastellorizo", "italy", "sicily", "cuba", "jamaica", "bahamas",
  "england", "scotland", "ireland", "france", "germany", "china", "japan",
  "australia", "denver", "seattle", "chicago", "boston", "phoenix",
  "dallas", "houston", "memphis", "nashville", "richmond", "columbus",
  "cleveland", "pittsburgh", "baltimore", "philadelphia", "vegas",
  "angeles", "francisco", "diego", "antonio", "lansing", "bronx",
  "brooklyn", "manhattan", "atlanta", "portland",
];
export const CONFLICTING_LOCATION_TOKENS = new Set([...US_STATE_TOKENS, ...OTHER_CONFLICTING_LOCATION_TOKENS]);

// Files that are never a photo OF the place, whatever the name overlap:
// road/heraldic insignia, flags, maps, philatelic/numismatic images, and
// photographs whose actual subject is a PERSON (portrait/headshot/mugshot,
// or an album/band photo). "Blue_Ridge_Parkway_shield" and a park's Wikidata
// article using a person's Wikipedia infobox photo (John Williams Park /
// "John_Williams_Walker") are exactly this shape.
export const COMMONS_NON_PLACE_MEDIA_RX =
  /\b(shield|logo|seal|flag|coat[_\s-]?of[_\s-]?arms|road[_\s-]?sign|route[_\s-]?map|locator[_\s-]?map|portrait|headshot|mugshot|album[_\s-]?cover|postage[_\s-]?stamp|mascot)\b/i;

// Commons' own biographical categories ("Living people", "Deaths", a
// nationality+occupation category) are how the site marks a file as being
// ABOUT A PERSON, not a place — checked against title/description/
// categories together, never against the narrow title-only name-token hay,
// because this is exactly the kind of context a category (not a title) is
// meant to carry.
export const COMMONS_PERSON_CATEGORY_RX =
  /\bliving people\b|\b(?:19|20)\d{2}\s+(?:births|deaths)\b|\bpeople from\b|\balumni\b|\bpoliticians?\b|\bactors?\b|\bactresses\b|\bmusicians?\b|\b(?:football|baseball|basketball|rugby(?:\s+union|\s+league)?)\s+players?\b/i;
 ...
 */
// Venue-TYPE words (a subset of GENERIC_PLACE_TOKENS — every entry except
// the pure address/administrative/direction ones). "Al Lopez Park" and
// "Al_Lopez_Field" share their one real distinctive token ("lopez") and can
// sit a few hundred feet apart in the real world, so neither the name-token
// check nor a tight geo radius catches this shape of miss — the file is
// calling itself a different KIND of venue than the place's own name does.
const NON_VENUE_TYPE_GENERIC_TOKENS = new Set([
  "national", "state", "community", "county", "city", "florida", "usa", "united", "states",
  "road", "street", "avenue", "drive", "trail", "highway",
  "rd", "st", "ave", "blvd", "dr", "trl", "hwy", "fl", "us",
  "north", "south", "east", "west", "point", "key", "bay", "river",
  "creek", "inlet", "harbor", "harbour", "yacht", "golf", "rv",
  "resort", "works", "lounge", "goods", "sporting",
]);
const VENUE_TYPE_WORDS = new Set([...GENERIC_PLACE_TOKENS].filter((t) => !NON_VENUE_TYPE_GENERIC_TOKENS.has(t)));

function venueTypeWordsIn(text) {
  return normTokens(text).filter((t) => VENUE_TYPE_WORDS.has(t));
}

// Pure. A venue-type word (park/field/stadium/bar/museum/...) the FILE's
// title uses that the place's own full display name never uses at all.
// Generous on the place side (the FULL name, not just the primary entity)
// so a legitimate location-suffix venue word ("... - Nathan Benderson
// Park") still counts as the place's own. Returns the conflicting word, or
// null when the file uses no venue word the place does not already share
// (including when the file names no venue-type word at all).
function conflictingVenueType(place, file) {
  const placeWords = new Set(venueTypeWordsIn((place && place.name) || ""));
  const fileWords = venueTypeWordsIn(file.title || file.filename || "");
  for (const w of fileWords) {
    if (!placeWords.has(w)) return w;
  }
  return null;
}

// Pure. A token in CONFLICTING_LOCATION_TOKENS that appears in the file's
// TITLE/ObjectName and is NOT already part of the place's own name or its
// own (explicit or inferred) city. That last exclusion is what keeps this
// from ever penalising a place for its own name ("Georgia's Diner") or its
// own home town — it can only fire on a location word the PLACE does not
// own. Narrow hay (title+objectName) on purpose: a category or caption
// mentioning an unrelated state is a weaker signal than the file's own
// title naming one, and the narrow-hay name-token check right above this
// already treats categories as non-authoritative for the same reason.
// Returns the conflicting token, or null.
function conflictingLocationToken(place, file, titleHay) {
  const ownToks = new Set([
    ...distinctivePlaceTokens(splitPlacePrimaryName(place && place.name)),
    ...distinctivePlaceTokens(inferPlaceCity(place) || ""),
  ]);
  for (const t of titleHay) {
    if (CONFLICTING_LOCATION_TOKENS.has(t) && !ownToks.has(t)) return t;
  }
  return null;
}

// 2026-09-17 (wrong-place audit). Run immediately before EVERY accept path
// below — never earlier — so an existing, already-correct rejection (geo
// mismatch, branch mismatch, city mismatch, no evidence at all) keeps its
// own specific reason. This only ever turns a case the OLDER logic would
// have accepted into a rejection; it can never soften an existing refusal.
function guardAgainstWrongEntity(place, file, titleHay) {
  const broadText = [
  ...
  if (COMMONS_NON_PLACE_MEDIA_RX.test(broadText) || COMMONS_PERSON_CATEGORY_RX.test(broadText)) {
    return "identity_non_place_media";
  }
  const conflict = conflictingLocationToken(place, file, titleHay);
  if (conflict) return "identity_conflicting_location";
  const venueConflict = conflictingVenueType(place, file);
  if (venueConflict) return "identity_conflicting_venue_type";
  return null;
}
...
export function verifyCommonsFileIdentity(place, file) {
  ...
  // NARROW HAY — title + ObjectName ONLY (2026-09-17: previously included
  // description and categories here, which is exactly what let a file
  // merely CATEGORISED under the place's home city/state complete an
  // otherwise-unmet token requirement without its TITLE ever depicting the
  // place — e.g. a file titled for an unrelated subject but filed under
  // "Category:Eureka Springs, Arkansas"). This matches the module's own
  // stated design ("Distinctive PRIMARY-entity tokens must appear in the
  // file TITLE or ObjectName — not only in a caption or category") that the
  // title gate above already followed but this full check did not.
  const hay = new Set(normTokens([file.title || file.filename || "", file.objectName || ""].join(" ")));
  ...
  if (havePlace && haveFile) {
    ...
    // 2026-09-17: this used to read `!titleHasCity && dist > BRANCH_GEO_MI` —
    // a short/generic name (<=2 distinctive tokens: "Al Lopez Park" ->
    // "lopez") whose file merely mentioned the RIGHT city earned the loose
    // 15mi metro gate instead of the tight campus radius. That is precisely
    // "same city, different venue, coincidentally close" (Al Lopez Park vs.
    // the old Al Lopez Field). A short/generic name now always needs the
    // tight radius when it is relying on geo at all — sharing the place's
    // own city is not itself proof of which building in that city this is.
    if (!branchToks.length && shortBrand && dist > COMMONS_BRANCH_GEO_MI) {
      return { ok: false, reason: "identity_branch_mismatch", dist };
    }
    const wrongEntity = guardAgainstWrongEntity(place, file, hay);
  ...
  if (city) {
    ...
    if (cityOk) {
      const wrongEntity = guardAgainstWrongEntity(place, file, hay);
      if (wrongEntity) return { ok: false, reason: wrongEntity };
      return { ok: true, confidence: 0.85, reason: "city_and_name" };
    }
...
export async function findCommonsPhoto(place, deps = {}) {
  ...
  try {
    ...
    // PHOTO IDENTITY IS STRICTER THAN PAGEVIEW IDENTITY. A generic/short
    // Wikipedia article with no coordinates can legitimately be "a place"
    // without being THIS place. The production repair queue exposed the exact
    // failure: Wayfind "Wishing well" in Los Angeles matched a generic
    // "Wishing well" article whose lead image depicted Fleetwood, UK.
    //
    // For <=2 distinctive primary-name tokens, a coordinate-less article must
    // prove the Wayfind city in its own title/shortdesc/extract/categories.
    // If Wayfind cannot infer/provide a city, or the article cannot prove it,
    // do not trust the lead image. Fall through to direct Commons, whose file
    // verifier requires file-level geo/city proof.
    const pageCoords = page && page.coordinates && page.coordinates[0];
    const wikiPrimaryTokens = distinctivePlaceTokens(splitPlacePrimaryName(place.name));
    if ((!pageCoords || pageCoords.lat == null || pageCoords.lon == null) && wikiPrimaryTokens.length <= 2) {
      const photoCity = inferPlaceCity(place);
      const cityTokens = distinctivePlaceTokens(photoCity);
      const articleHay = new Set(normTokens([
        verified.finalTitle || candidate.title || "",
        pageprops["wikibase-shortdesc"] || "",
        page.extract || "",
        (page.categories || []).map((row) => row && row.title || "").join(" "),
      ].join(" ")));
      const cityProven = cityTokens.length > 0 && cityTokens.every((token) => tokenMatches(token, articleHay));
      if (!cityProven) return await tryDirect("identity_photo_generic_without_geo");
    }
    ...
    // 2026-09-17 (wrong-place audit). The Wikipedia lead-image path trusts
    // whatever image an already-identity-verified ARTICLE's infobox carries
    // — verifyWikiIdentity proves the ARTICLE is the right subject, not that
    // its lead image is a photo of the place at all. A road's Wikipedia
    // article legitimately has a lead image that is its route shield; a
    // person's article (when it slips past the article-level checks) has a
    // portrait. Treated exactly like "no usable lead image" — a legitimate
    // empty result for THIS path, not an identity mismatch — so a real photo
    // of the same place still gets a chance via Commons-direct.
    if (COMMONS_NON_PLACE_MEDIA_RX.test(lead.filename)) {
      return await tryDirect("no_lead_image");
    }
