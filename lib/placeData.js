...
import { atlasPlaceFor, mergePlacePage, shouldCallGooglePlaceDetails } from "./atlasPlaceAllowlist";
...
// Atlas-card ids never call getPlaceDetails (no Google). Indexed ids peek the
// pd1| cache and only spend Places when that cache is cold AND there is no
// Atlas copy to prefer.
export const loadPlace = cache(async (id) => {
  if (!id) return null;
  const skel = await getSkeleton(id);
  const atlas = atlasPlaceFor(id);
  if (!skel && !atlas) return null;
  const cached = skel ? await peekPlaceDetails(id) : null;
  let details = cached;
  if (shouldCallGooglePlaceDetails({ skel, cached, atlas })) {
