...
export const loadPlace = cache(async (id) => {
  ...
  // An editorial page, like an Atlas page, takes its identity from the
  // wf_inventory row we already hold (every verified editorial id has one),
  // so it opens even when no search has written wf_place_ids yet.
  const inv = (atlas || editorial) ? await getInventoryIdentity(id) : null;
  ...
  let details = cached;
  // A verified-editorial page never spends a Google Places Details call: it
  // is listed in the sitemap, so every crawl would otherwise be a potential
  // metered call. It already carries Wayfind's own copy + inventory identity,
  // and a warm pd1| cache still enriches it.
  if (!editorial && shouldCallGooglePlaceDetails({ skel: indexed, cached, atlas })) {
...
export function cityOf(address) {
  ...
  // nothing usable.
  // "street, city" (two segments, no state or country): the city is last.
  if (parts.length === 2 && !/\d/.test(parts[1]) && parts[1].length <= 40) return parts[1];
...
export async function placePageMetadata({ params }) {
  ...
  const title = `${p.name}${city ? " in " + city : ""} | Wayfind`;
