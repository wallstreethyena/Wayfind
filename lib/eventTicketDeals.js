...
export const SEPARATELY_TICKETED = Object.freeze([
  ...
  // 2026-09-15 affiliate coverage audit: two more Zoo Miami nights, own tickets
  // required beyond regular admission — confirmed on the organizer's site,
  // distinct from Zoo Boo (which IS included, see the Tiqets mapping above).
  "roars-smores-snores-spooktacular-campout-zoo-miami-2026", // overnight campout, own ticket required
  "zoo-miami-monster-masquerade-2026",                        // ticketed evening event, not included with admission
]);
...
export function eventTicketCta(eventId, { surface = "event", liveDeal = undefined } = {}) {
  ...
  return {
    ...
    label: entry.product === "park-admission" ? `Park tickets · ${via} ↗` : `Tickets · ${via} ↗`,
  };
}
