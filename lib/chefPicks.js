...
export function chefPickPlaces(c) {
  ...
  return c.entries.map((e) => ({
    ...
    // v8.95 — THE PHOTO IS THE PLACE ID, and that is the whole fix. Every
    // place card must carry an image (owner, v8.13.3: "I don't want any of the
    // place cards not to have an image"); these seven shipped blank for four
    // days because this file promised "`photo` self-heals once refs are
    // harvested" and no harvest could ever have run — the seven are in five
    // metros wf_inventory does not cover.
    //
    // `id` is already the Google place id, and lib/placePhoto cardImageSrc
    // turns it into /api/photo?place=<id>, which the SERVER resolves against
    // wf_inventory and then lib/curatedPhotoRefs. Carrying the resource name
    // here instead was measured and reverted: it put 5KB of gzip-hostile
    // base64 into the homepage client bundle and took check-bundle red.
