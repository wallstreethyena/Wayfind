...
function shape(row, providers) {
  ...
  return {
    ...
    // v8.29.4 — THE VENUE'S OWN SCORE, when we have it (owner, 2026-08-20:
    // "these places still don't have the wayfind score"). A Viator card in
    // this rail carries rating/reviews and shows the Wayfind chip; the
    // Undercover Tourist cards next to it carried neither, so LEGOLAND and
    // Busch Gardens rendered with an empty metadata row — the two biggest
    // attractions in the state, looking less trustworthy than a kayak tour.
    // Stamped by mergeVenueScores() below from wf_inventory, via the deal's
    // own maps_to. Null when we cannot confirm the venue; never invented.
    rating: row.venue_rating != null ? Number(row.venue_rating) : null,
    reviews: row.venue_reviews != null ? Number(row.venue_reviews) : null,
...
// v8.29.4 — a deal's venue score, resolved from the one place it can honestly
// come from: wf_inventory, the same table every place card on this site reads.
// wf_deals.maps_to already names the attraction ("busch gardens tampa bay");
// nothing else in the deal row does.
//
// THE MATCH IS CONSERVATIVE ON PURPOSE. ilike on the stem returns near-misses
// — "legoland florida" also surfaces LEGOLAND Discovery Center Arizona, and
// "busch gardens tampa bay" also surfaces its Howl-O-Scream event and a Red
// Roof Inn named after it. A row is only accepted when the venue name contains
// EVERY word of maps_to; an exact name match wins outright, otherwise the most-
// reviewed survivor does. No confirmed row → no score, and the card says
// "Bookable" instead of borrowing a number that is not about it.
async function mergeVenueScores(s, h, rows) {
  const stems = [...new Set(rows.map((r) => String(r.maps_to || "").trim().toLowerCase()).filter((v) => v.length >= 4))];
  if (!stems.length) return;
  const byStem = new Map();
  await Promise.all(stems.map(async (stem) => {
    try {
      const u = `${s.url}/rest/v1/wf_inventory?select=name,signals&status=eq.OPERATIONAL&name=ilike.${encodeURIComponent("%" + stem + "%")}&limit=8`;
      const r = await fetch(u, { headers: h, cache: "no-store" });
      if (!r.ok) return;
      ...
      for (const row of await r.json()) {
        ...
        const exact = name === stem;
        if (!best || (exact && !best.exact) || (exact === best.exact && reviews > best.reviews)) best = { rating, reviews, exact };
      }
      if (best) byStem.set(stem, best);
    } catch { /* a deal with no confirmable venue simply keeps no score */ }
  }));
  for (const row of rows) {
    const hit = byStem.get(String(row.maps_to || "").trim().toLowerCase());
    if (hit) { row.venue_rating = hit.rating; row.venue_reviews = hit.reviews; }
  }
}
...
// v6.94 (owner: "we are offering expensive orlando trips that are not even
// nearby... this is the type of common sense i am asking [for]" — a Parrish
// user, ~91mi from Orlando, was shown Walt Disney World / Universal /
// Gatorland tickets under the header "Bookable highlights NEAR Parrish").
// 150mi technically excluded Anaheim/ski (the guard's own fixtures), so it
// was never "no gate" — it was just loose enough that a claim as strong as
// "NEAR {city}" (the literal copy in both IntentPartnerPick.js and home.js's
// unified rail) stopped being true a long time before 150mi did. Tightened
// to match the mi=60 radius the SAME rail's Viator query already uses
// (UnifiedBrowseCommerceRail, app/home.js) — one rail, one honest "near"
// radius, not two different ones depending on which provider answered.
const DEAL_RADIUS_MI = 60; // must match "near {city}" honestly — see UnifiedBrowseCommerceRail's mi=60 Viator search in app/home.js
...
export async function serveDeals(category, lat, lng) {
  ...
  try {
    ...
    await mergeVenueScores(s, h, data);
