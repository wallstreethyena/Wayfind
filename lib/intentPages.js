...
import { RAIL_ART_V } from "./rails.js";
...
import { SERVICE_RX } from "./placeFilter.js";
...
export const INTENT_PAGES = {
  ...
  "date-night": {
    ...
    card: {
      ...
      accent: "#F472B6",
      art: "/cards/date-night-owner.png",
    },
    eyebrows: ["Date night, decided", "Two people, one good call"],
    accent: "#F472B6",
    // v8.93 — THE PAGE HERO IS VERSIONED; the share card above deliberately is
    // NOT (its own note requires identical bytes on every deploy, behind an
    // immutable cache). CollectionHero renders heroImg as a bare <img src>, so
    // a swapped poster under a fixed filename is invisible to every phone
    // holding the 30-day CDN copy — the v8.15 bug that made the owner say "I
    // still see the old card", here on the landing hero instead of the rail
    // tile. RAIL_ART_V is the counter the tile ladder already uses, so one bump
    // moves both surfaces together.
    art: `/cards/date-night-owner.png?v=${RAIL_ART_V}`,
  ...
  tonight: {
    ...
    card: {
      ...
      promise: "Bars, clubs, live shows and after-dark attractions, ranked for this hour.",
      accent: "#818CF8",
    ...
    art: "/cards/tonight-alfonso-scarpa-unsplash.jpg",
    floor: { rating: 4.4, reviews: 150 },
    // Tonight means nightlife and after-dark activities, never a breakfast,
    // cafe, or generic dinner list wearing a nighttime heading.
    compose: {
      sections: ["Nightlife", "Activities"],
      maxPerSection: 6,
    ...
    //
    // v7.22 — and the RANKING has to believe that too. Measured live at 10:30am
    // in Parrish: six of the seven cards printed "Closed", because openWeight
    // was demoting every bar and music room for being shut RIGHT NOW on a list
    // about TONIGHT. worth-the-drive already had this exemption; this one did
    // not, so the comment above was true of the query bank and false of the
    // sort. planAhead is a FUNCTION here rather than the flat `true`
    // worth-the-drive uses, because after dark "closed" is real evidence again:
    // at 11pm a shut bar is genuinely a bad answer and openWeight should bite.
    planAhead: (ctx) => !ctx || ctx.timeBucket !== "night",
    queries: () => [
      { cat: "nightlife", q: "speakeasy cocktail bar" },
      { cat: "nightlife", q: "night club dance club" },
      { cat: "nightlife", q: "live music bar tonight" },
      { cat: "attractions", q: "candlelight concert immersive dining tonight" },
    ],
    titles: [
  ...
  "worth-the-drive": {
    ...
    card: {
      ...
      promise: "Springs, landmarks and day trips inside 50 miles. The strictest list we run.",
      accent: "#38BDF8",
    ...
    // A card closer than this belongs in a nearby list, not a list whose very
    // promise is that the extra drive is worth making. Strictly greater than 17.
    minDistanceMi: 17,
    ...
    // v7.11 (owner, 2026-08-11, from the live rail: "there is no results for
    // worth the drive and this is the one where we can provide the most because
    // we can expand the radius"): 30mi gave this list a 17-30mi eligible band —
    // thirteen miles of Florida — and St Pete showed two cards. 50 miles is the
    // band the trend-intelligence brief codifies for drive-framed surfaces
    // (15-50mi), and minDistanceMi 17 still guarantees nothing "nearby" wears
    // the day-trip label.
    radiusM: 80467,
    subs: [
  ...
  budget: {
    ...
    queries: (c) => {
      ...
      const food = x.timeBucket === "morning" ? { cat: "food", q: "cheap breakfast highly rated" }
        : x.timeBucket === "afternoon" ? { cat: "food", q: "smash burger cheap eats highly rated" }
  ...
  // v7.15 — "The 30-Minute Break" (owner ask, 2026-08-11: lunch is a
  // fast-food moment and the menu had no answer for it). The promise is
  // QUICK-SERVICE FORMAT, never speed-in-minutes: counter-serve / fast-casual
  // / takeaway identities, ranked by the same governed score as everything
  // else. National chains are neither banned nor favored — the score decides,
  // and brand dedupe keeps one brand from filling the rail. NO wait-time or
  // "ready in X minutes" claims anywhere in this copy: we hold no
  // service-time data. "30 minutes" is the READER's constraint, named as such.
  "quick-bite": {
    headerDeck: (city) => "The counter-serve side of " + city + " — quick-format food that still earns its rating.",
    // SHARE CARD (v6.72). The OG route reads ONLY from `card` — same contract
    // as every intent page; a missing card is a build failure (check-intent-cards).
    card: {
      eyebrow: "The 30-Minute Break",
      line1: "Short break, real lunch",
      promise: "The best quick, counter-serve food near you — ranked by score, not by brand.",
      accent: "#F97316",
      art: "/cards/food-choices-adobestock-301125732.jpeg",
    },
    eyebrows: ["The 30-Minute Break", "Short break, real lunch"],
    accent: "#F97316",
    art: "/cards/food-choices-adobestock-301125732.jpeg",
    floor: { rating: 4.3, reviews: 120 },
    compose: {
      sections: ["Food"],
      // v7.16 — quick is an IDENTITY, not a query flavor: every row must pass
      // lib/quickService.isQuickService (quick TYPE or counter-serve name
      // evidence; slow formats vetoed; a generic restaurant with no quick
      // evidence is refused — that row belongs on Actually Worth Eating).

      identity: "quickbite",
      excludeTypes: ["fine_dining_restaurant", "steak_house", "buffet_restaurant"],
    },
    // A 30-minute break can't spend itself driving: the free radius is tighter
    // than any other page and the deduction starts sooner. Reorders only —
    // the distancePenalty never excludes (same contract as tonight's).
    distancePenalty: { freeMi: 5, per: 3, deduct: 0.3 },
    queries: (c) => {
      const x = asCtx(c);
      const byBucket = x.timeBucket === "morning"
        ? [{ cat: "food", q: "quick breakfast counter serve grab and go" }, { cat: "food", q: "best breakfast sandwich bagel shop" }]
        : x.timeBucket === "afternoon"
        ? [{ cat: "food", q: "best quick lunch fast casual counter serve" }, { cat: "food", q: "best fast food highly rated local favorite" }]
        : [{ cat: "food", q: "quick dinner takeout counter serve" }, { cat: "food", q: "best fast casual dinner spot" }];
      return [{ cat: "food", q: "fast casual quick eats highly rated" }, { cat: "food", q: "drive thru takeout counter service food" }].concat(byBucket);
    },
    titles: [
      (c, city) => "Only got 30 minutes?",
      (c, city) => "Short break, real lunch",
      (c, city) => "Fast food, the good kind",
    ],
    subs: [
      (city) => "Counter-serve and fast-casual spots in " + city + ", ranked by the Wayfind Score — a beloved local counter outranks a big brand here. We left off anything under 120 reviews.",
      (city) => "Quick-service food in " + city + ", ranked by the Wayfind Score, never by brand. We left off anything under 120 reviews.",
      (city) => "Good food for short breaks in " + city + ", ranked by the Wayfind Score. We left off anything rated under 4.3.",
    ],
  },
  "hidden-gems": {
    // SHARE CARD (v6.72). The OG route reads ONLY from `card` — it never touches
    ...
    queries: (c) => {
      ...
      const byBucket = x.timeBucket === "morning"
        ? [{ cat: "food", q: "unique cafe tucked away" }, { cat: "food", q: "hidden gem bakery kunafa dubai chocolate dessert" }]
        : x.timeBucket === "afternoon"
        ? [{ cat: "food", q: "unique cafe hojicha matcha tucked away" }, { cat: "attractions", q: "small museum lesser known" }]
...
export function sectionAllowed(r, compose) {
  // v7.16 — a compose may name an IDENTITY the row must prove. "quickbite" is
  // the quick-service test (lib/quickService.js): without it the 30-Minute
  // Break converges with Actually Worth Eating, because text queries cannot
  // separate quick from merely good.
  if (compose && compose.identity === "quickbite" && !isQuickService(r)) return false;
...
/**
 * v7.22 — ONE resolver for `planAhead`, shared by the home rail and the intent
 * page, because an intent may now decide it per hour rather than for all time.
 *
 * `worth-the-drive` is `true` always: a landmark you drive an hour for is a
 * plan whatever the clock says. `tonight` is a predicate — before dark it is a
 * plan (and "Closed" is not evidence against a bar at 10am), after dark it is
 * not (a shut bar at 11pm is genuinely the wrong answer). Both call sites go
 * through here so the rail and the page can never disagree about which one a
 * list is; scripts/test-intent-composition.mjs pins exactly that.
 */
export function resolvePlanAhead(def, ctx) {
  const p = def && def.planAhead;
...
export function rankRows(rows, floor, opts) {
  ...
  const minDistanceMi = opts && Number.isFinite(opts.minDistanceMi) ? opts.minDistanceMi : null;
  ...
  // THE SERVICE BACKSTOP (owner, 2026-08-11). Callers gate with
  // lib/placeFilter.placeAllowed against their query's own category; this is
  // the last line for any path that reaches the ranker some other way. A
  // service-business NAME is disqualifying everywhere ("names of service shops
  // are reliable" — placeFilter's own doctrine), and it is an identity test,
  // never a score test.
  const withDist = (rows || []).filter(Boolean).filter((r) => !SERVICE_RX.test(String(r.name || ""))).map((r) => {
  ...
  }).filter((r) => minDistanceMi == null || (Number.isFinite(r.distMi) && r.distMi > minDistanceMi));
  ...
  //
  // which has NO creator-video term at all — so the creator lift that made Ryan's a
