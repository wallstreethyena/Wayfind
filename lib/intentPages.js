...
export const INTENT_PAGES = {
  ...
  tonight: {
    ...
    art: "/cards/tonight-alfonso-scarpa-unsplash.jpg",
    floor: { rating: 4.4, reviews: 150 },
    // v7.09 — "perfect for tonight" is a mood too. Mixed is correct here; four
    // restaurants and a bar is not a night out, it is a dinner list.
  ...
  budget: {
    ...
    subs: [
      // v7.09 (owner: "the menu wording talks about Google… we are Wayfind").
      // Naming the supplier in our own promise line put a competitor's brand on
      // our differentiator. It was also no longer TRUE: floor.freeOutdoor now
      // admits the unpriced outdoors, so "we left off anything with no price on
      // record" described a filter that had stopped existing. The claim below is
      // the one this list actually enforces.
      (city) => "The cheap half of " + city + ": free parks and beaches, plus the two lowest price bands. Ranked by the Wayfind Score, same as everything else.",
