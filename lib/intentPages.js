...
export const INTENT_PAGES = {
  ...
  nearby: {
    ...
    card: {
      eyebrow: "Know what is around you",
  ...
  seasonal: {
    ...
    // A missing or incomplete `card` is a BUILD FAILURE (check-intent-cards).
    card: {
      eyebrow: "Built for the season you are in",
  ...
  tonight: {
    ...
    card: {
      ...
      promise: "The highest-scoring places near you for tonight.",
      accent: "#818CF8",
    ...
    queries: (c) => {
      const x = asCtx(c);
      if (x.timeBucket === "night") {
        return [{ cat: "nightlife", q: "live music tonight" }, { cat: "food", q: "dinner open late" },
          { cat: "nightlife", q: "cocktail bar lounge" }, { cat: "attractions", q: "evening activity things to do tonight" }];
      }
      if (x.timeBucket === "morning") {
        return [{ cat: "food", q: "breakfast brunch highly rated" }, { cat: "food", q: "best coffee" },
          x.outdoorOK ? { cat: "attractions", q: "morning walk park market" } : { cat: "attractions", q: "indoor activity morning" },
          { cat: "attractions", q: "things to do today" }];
      }
      return [{ cat: "attractions", q: "things to do today" }, { cat: "food", q: "lunch highly rated" },
        { cat: "attractions", q: x.outdoorOK ? "afternoon outdoor activity" : "indoor activity air conditioned" },
        { cat: "nightlife", q: "early evening drinks" }];
    },
    titles: [
      (c, city) => { const b = asCtx(c).timeBucket; return b === "night" ? "Tonight, ranked" : b === "morning" ? "This morning, ranked" : "This afternoon, ranked"; },
  ...
  "worth-the-drive": {
    ...
    card: {
      ...
      promise: "The strictest list we run — day trips and landmarks that earn it.",
      accent: "#38BDF8",
    ...
    queries: (c) => {
      const x = asCtx(c);
      if (x.timeBucket === "night") {
        return [{ cat: "attractions", q: "evening show live entertainment worth the drive" }, { cat: "food", q: "destination restaurant dinner" }];
      }
      const base = [{ cat: "attractions", q: "day trip worth the drive" }, { cat: "attractions", q: "iconic landmark tradition" }];
      return x.outdoorOK
        ? base.concat([{ cat: "attractions", q: "state park springs natural attraction" }])
    ...
    // every intent page searches the same 32km radius, rankRows has no distance
    // predicate, and this entry deliberately has no distancePenalty. What
    // actually distinguishes this page is the FLOOR — 4.6/300 is the strictest
    // in INTENT_PAGES — plus the day-trip queries. Say that instead.
    subs: [
  ...
  budget: {
    ...
    card: {
      ...
      promise: "Google\u2019s two lowest price bands, still ranked by the Wayfind Score.",
      accent: "#34D399",
    ...
    // it could not keep.
    floor: { rating: 4.4, reviews: 100, maxPrice: 2 },
    distancePenalty: { freeMi: 17, per: 5, deduct: 0.2 },
  ...
  "hidden-gems": {
    ...
    card: {
      eyebrow: "Hidden gems",
      line1: "Hidden gems",
      promise: "The spots locals keep to themselves — loved, not overrun.",
      accent: "#A78BFA",
...
export function rankRows(rows, floor, opts) {
  ...
  //
  // v6.72 adds three conditions terms ON TOP, all commensurate on the /10 scale:
  //   conditionsAdjust  the existing lib/ranking weather+daypart deltas (0-100
  //                     scale there, so scaled by /10 to land on this one)
  //   bucketWeight      morning quiet+close, night energy+open-late
  //   openWeight        a MULTIPLIER — closed and closing-soon cannot win
  const key = (r) => {
    const q = (bayes(r.rating, r.reviews) / 5) * 10 - r.deduction;
    if (!ctx) return q;
    ...
    return (q + cond + bucketWeight(r, ctx)) * openWeight(r, nowMs);
  };
  return withDist
    ...
    .filter((r) => r.rating >= floor.rating && r.reviews >= floor.reviews && (floor.maxReviews == null || r.reviews <= floor.maxReviews) && (floor.maxPrice == null || (r.priceLevel != null && r.priceLevel <= floor.maxPrice)))
