...
export const INTENT_PAGES = {
  ...
  nearby: {
    ...
    queries: (h) => [
      { cat: "attractions", q: "top things to do" },
      { cat: "food", q: "best restaurants" },
      { cat: "nightlife", q: "best bars" },
      { cat: "attractions", q: "parks outdoors" },
    ],
    titles: [
      (h, city) => "What's actually worth your time in " + city,
      (h, city) => "You have driven past most of these",
      (h, city) => "Everything good near you, ranked",
    ],
    subs: [
  ...
  "date-night": {
    ...
    titles: [
      (h, city) => (h >= 5 && h < 10.5 ? "Morning date" : h < 14 ? "Lunch date" : h < 18 ? "Afternoon date" : "Tonight"),
      (h, city) => "Stop asking each other where to go",
      (h, city) => "The night you will both remember",
    ],
    subs: [
  ...
  family: {
    ...
    titles: [
      (h, city) => "Family day",
      (h, city) => "Where \"I am bored\" becomes \"can we go back?\"",
      (h, city) => "The day they will ask to repeat",
    ],
    subs: [
  ...
  seasonal: {
    ...
    titles: [
      (h, city) => SEASON_META[currentSeason()].label + " picks near you",
      (h, city) => "What actually works in " + city + " right now",
      (h, city) => "The season, handled",
    ],
  ...
  tonight: {
    ...
    titles: [
      (h, city) => (h >= 15 || h < 4) ? "Tonight, ranked" : "Today, ranked",
      (h, city) => "Do not waste a good night",
      (h, city) => "Tonight, handled",
    ],
  ...
  "best-of": {
    ...
    titles: [
      (h, city) => "The highest-scoring places in " + city,
      (h, city) => "The list that does not change with the weather",
      (h, city) => "What " + city + " is actually known for",
    ],
    subs: [
  ...
  "worth-the-drive": {
    ...
    titles: [
      (h, city) => "Worth planning a drive around",
      (h, city) => "Worth the tank of gas",
      (h, city) => "The days you will drive for it",
    ],
  ...
  budget: {
    ...
    titles: [
      (h, city) => "$ and $$ only, still rated 4.4+",
      (h, city) => "Nights you will remember that your wallet will not",
      (h, city) => "Cheap is the filter, not the standard",
    ],
  ...
  "hidden-gems": {
    ...
    titles: [
      (h, city) => "Hidden gems",
      (h, city) => "The places locals quietly hope you never find",
      (h, city) => "The spot you will swear you found first",
    ],
...
export const intentTitle = (def, h, city, v = 0) => at(def.titles, v)(h, city);
