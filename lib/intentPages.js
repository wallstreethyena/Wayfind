...
export function rankRows(rows, floor, opts) {
  ...
  const key = (r) => {
    // 2026-08-07: + the trending component on this surface's /10 scale
    // (TRENDING_BONUS/10 = +0.6), from lib/trendSignal.js — attached by the
    // caller BEFORE ranking, real demand data only, and disclosed on the card
    // (IconicPlaceCard renders the 🔥 reason). No signal → no term.
    const q = (bayes(r.rating, r.reviews) / 5) * 10 - r.deduction + (r.trending ? TRENDING_BONUS / 10 : 0);
    if (!ctx) return q;
