...
export const INTENT_PAGES = {
  ...
  "date-night": {
    ...
    // Owner (2026-08-30): Date Night is hard-capped at 27.0 miles from the
    // page center. The landing no longer searches Places; these queries are
    // a dead fallback and must not hunt attractions (that is the Skyway leak).
    distancePenalty: { freeMi: 27, per: 5, deduct: 0.2 },
    maxMi: 27,
    queries: (c) => {
      const x = asCtx(c);
      if (x.timeBucket === "morning") {
        return [{ cat: "food", q: "wine bar brunch" }, { cat: "food", q: "best restaurant date" },
          { cat: "food", q: "speakeasy cocktail" }, { cat: "nightlife", q: "wine bar" }];
      }
      if (x.timeBucket === "afternoon") {
        return [{ cat: "food", q: "romantic restaurant lunch" }, { cat: "food", q: "wine tasting" },
          { cat: "nightlife", q: "afternoon wine bar" }, { cat: "nightlife", q: "rooftop bar" }];
      }
      return [{ cat: "food", q: "fine dining restaurant" }, { cat: "nightlife", q: "wine bar cocktail lounge" },
        { cat: "food", q: "candlelit dinner" }, { cat: "nightlife", q: "comedy club theater" }];
    },
    titles: [
