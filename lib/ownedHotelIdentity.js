...
export async function runOwnedHotelIdentityBackfill({
  ...
  for (const row of candidates) {
    ...
    let already = null;
    try { already = await readMark(gpidKey); } catch { already = null; }
    if (already && already.gpid) { counts.skipped++; continue; }

    let miss = null;
    try { miss = await readMark(missKey); } catch { miss = null; }
...
export async function defaultSearchIds(query, row) {
  ...
  if (Number.isFinite(lat) && Number.isFinite(lng)) {
    body.locationRestriction = { circle: { center: { latitude: lat, longitude: lng }, radius: SEARCH_RADIUS_M } };
  }
