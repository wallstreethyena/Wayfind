...
import { isExcludedOwnedHotel } from "./ownedHotelExclusions.js";
 ...
 *
 * A row the site no longer serves is not worth a lookup (2026-09-23): the 58
 * rows in lib/ownedHotelExclusions.js are salons, restaurants, campgrounds and
 * condo associations that will never be a hotel card, and 56 of them were
 * still sitting in this queue. Asking Google about them spends a daily search
 * allowance that the real hotels are waiting on, and any answer would be
 * written to a card nobody renders.
 */
export function unresolvedOwnedHotels(list) {
  return (Array.isArray(list) ? list : []).filter((h) => h && !h.gpid && !isJunkOwnedHotel(h) && !isExcludedOwnedHotel(h));
}
...
export function verifyCandidate(row, details) {
  return explainCandidate(row, details) === VERIFY_OK;
}

// WHY A REASON CODE (2026-09-23, owner: "for every hotel that still has no
// photo, categorize the reason"). A boolean answers "did this match" and
// nothing else, so a hotel with no photo was indistinguishable from a hotel
// Google has never heard of, and the only honest report was "199 blank". The
// rule below is byte-for-byte the rule that was already here — same 200 m,
// same lodging types, same street number, same fail-closed order — it just
// SAYS which test ended it. verifyCandidate is now defined in terms of this
// one, so there is still exactly one rule and no second place to loosen.
export const VERIFY_OK = "ok";
export const VERIFY_REASONS = Object.freeze([
  VERIFY_OK,
  "no-location",      // Google returned a place with no coordinates
  "row-no-coords",    // OUR row has no coordinates — a data problem, not Google's
  "too-far",          // a real place, but past the 200 m line
  "no-types",         // Google returned no type list at all
  "not-lodging",      // a real place at the right spot that is not somewhere you sleep
  "street-mismatch",  // right area, wrong building — our street number vs theirs
]);

/** Which test ended the match, or VERIFY_OK. Pure; never widens the rule. */
export function explainCandidate(row, details) {
  if (!row || !details) return "no-location";
  const loc = details.location;
  if (!loc || typeof loc.latitude !== "number" || typeof loc.longitude !== "number") return "no-location";
  // Number(null) is 0, and 0,0 is a real point in the Atlantic — a row with no
  // coordinates would have been silently measured against Null Island and
  // reported "too-far" (2026-09-23). An absent coordinate is absent.
  const rowLat = coord(row.lat), rowLng = coord(row.lng);
  if (!Number.isFinite(rowLat) || !Number.isFinite(rowLng)) return "row-no-coords";
  const distM = haversineMeters(rowLat, rowLng, loc.latitude, loc.longitude);
  if (!(distM <= VERIFY_DISTANCE_M)) return "too-far";
  const types = Array.isArray(details.types) ? details.types : [];
  if (!types.length) return "no-types";
  if (!types.some((t) => ACCEPTED_LODGING_TYPES.has(t))) return "not-lodging";
  const rowNum = leadingStreetNumber(row.address);
  if (rowNum) {
    const detNum = leadingStreetNumber(details.formattedAddress);
    if (detNum !== rowNum) return "street-mismatch";
  }
  return VERIFY_OK;
}

// The order a run reports when several candidates each failed differently: the
// CLOSEST-to-matching reason wins, because that is the one that tells the
// owner what to fix. "street-mismatch" means we found the right block and our
// own address may be stale; "no-candidate" means Google has nothing here at all.
export const REASON_RANK = Object.freeze({
  "street-mismatch": 1, "not-lodging": 2, "no-types": 3, "too-far": 4,
  "no-location": 5, "row-no-coords": 6, "no-candidate": 7, "search-empty": 8,
});
export function worstOf(reasons) {
  let best = null;
  for (const r of reasons || []) {
    if (!r || r === VERIFY_OK) continue;
    if (!best || (REASON_RANK[r] || 99) < (REASON_RANK[best] || 99)) best = r;
  }
  return best || "no-candidate";
}
...
const ZERO_COUNTS = Object.freeze({ attempted: 0, resolved: 0, missed: 0, skipped: 0, searchDenied: 0, detailsDenied: 0, searchFailed: 0 });
...
export async function runOwnedHotelIdentityBackfill({
  ...
  const counts = { ...ZERO_COUNTS, reasons: {} };
  ...
  for (const row of candidates) {
    ...
    let ids = null;
    try { ids = await searchIds(identityQuery(row), row); } catch { ids = null; }
    if (!Array.isArray(ids)) {
      // The request failed (network, 4xx/5xx, a daily quota refusing us).
      // Do NOT write a miss marker — that would hide this hotel for 14 days
      // over our own outage — and stop the run rather than spending the rest
      // of the budget failing the same way, the same posture a denied ledger
      // grant takes above.
      counts.attempted--;
      counts.searchFailed++;
      break;
    ...
    for (const id of ids.slice(0, SEARCH_MAX_RESULT_COUNT)) {
      ...
      const why = explainCandidate(row, details);
      if (why === VERIFY_OK) { winner = id; break; }
      whys.push(why);
    }
    ...
    if (winner) {
      ...
      counts.missed++;
      // The miss marker now carries WHY (2026-09-23). Only our own verdict
      // code — never a place id, a name or an address from Google, which the
      // ToS does not let us keep past 30 days and which this row has no reason
      // to hold anyway. The 14-day TTL is unchanged.
      const why = ids.length ? worstOf(whys) : "no-candidate";
      counts.reasons[why] = (counts.reasons[why] || 0) + 1;
      try { await writeMark(missKey, { at: now(), why }, MARK_TTL_MS_MISS); } catch { /* best effort */ }
    }
  }
...
export async function defaultSearchIds(query, row) {
  ...
  // A missing key is OUR problem too (2026-09-23): reported as an empty answer
  // it would have written off all 353 hotels as "Google has never heard of
  // them" in a couple of runs, on an environment that simply was not
  // configured. null stops the run instead.
  if (!key) return null;
  ...
  // NULL MEANS "WE NEVER GOT AN ANSWER", [] MEANS "GOOGLE HAS NOTHING"
  // (2026-09-23). These were the same value, and the difference is the whole
  // report: at 02:10 resolutions stopped dead and 79 hotels in a row were
  // filed as "no-candidate" — the marker for "Google has never heard of this
  // property" — when a refused request looks exactly like an empty one from
  // in here. A hotel must never be written off for 14 days because WE could
  // not complete a call.
  if (!r) return null;
  let json;
