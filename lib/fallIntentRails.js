...
export const FALL_INTENT_RAIL_DEFS = Object.freeze([
  ...
  { id: "photos", title: "Florida Fall Photo Spots", deck: "A precise viewpoint, a visible subject, the right light and current access guidance. No vague Instagram bait." },
...
// "Fall in Florida" is a statewide collection, but proximity still matters.
// Each shelf fills from its intent-specific local ring first. Only a thin
// shelf widens to verified Florida inventory, with distance retained on every
// card so a destination never masquerades as nearby.
export const FALL_MIN_RESULTS = 8;
export const FALL_STATEWIDE_MAX_MI = 400;
...
export function composeFallIntentRails(events, places, { lat = null, lng = null, today = "", now = new Date() } = {}) {
  ...
  for (const event of Array.isArray(events) ? events : []) {
    ...
    if (rail && candidates[rail] && miles != null && miles <= FALL_STATEWIDE_MAX_MI) {
  ...
  const rails = fallRailOrder(today).map((id) => {
    const ranked = uniqueCards(candidates[id], ctx);
    const local = ranked.filter((card) => card.distMi <= FALL_RAIL_RADIUS_MI[id]);
    const cards = local.length >= FALL_MIN_RESULTS
      ? local
      : ranked.map((card) => ({ ...card, fallbackTier: card.distMi <= FALL_RAIL_RADIUS_MI[id] ? 0 : 1 }));
    return { ...byId.get(id), cards, fallbackUsed: local.length < FALL_MIN_RESULTS && cards.length > local.length };
  });
