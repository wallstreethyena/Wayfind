...
export function nextFallOccurrence(item, today) {
  if (!item || item.kind === "place" || !/^\d{4}-\d{2}-\d{2}$/.test(String(today))) return null;
  const start = String(item.start_date || "");
  const end = String(item.end_date || "");
  if ((end || start) && (end || start) < today) return null;
  if (Array.isArray(item.occurrence_dates)) {
...
export function hasUpcomingFallOccurrence(item, today) {
  ...
  return !Array.isArray(item?.occurrence_dates) || nextFallOccurrence(item, today) != null;
}
