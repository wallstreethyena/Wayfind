 ...
 */
export function dealTiers(coupons, todayIso) {
  ...
  // Soonest-ending first inside each tier, open-ended last. Ordering is derived
  // from the data's own expiry, so no hand-sort can creep in.
  const byExpiry = (a, b) => String(a.expires || "9999-12-31").localeCompare(String(b.expires || "9999-12-31"));
  featured.sort(byExpiry);
  ledger.sort(byExpiry);
