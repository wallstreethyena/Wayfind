...
const METRO_TOWNS = Object.freeze({
  ...
  manatee: "sarasota",
  // 2026-08-07, Clipp per-merchant harvest — every town below is used by a row
  // in lib/clippOffers.js CLIPP_MERCHANT_OFFERS. Same rule as above: ONLY areas
  // the registry actually uses, so an unrecognised area still fails loudly.
  // Each mapping MUST agree with what nearestMetro() returns for a viewer
  // standing in that town — the gate compares deal-metro to VIEWER-metro, so a
  // town mapped anywhere else makes its deals invisible to exactly the people
  // who live nearest them. Locked by scripts/test-coupon-geo.mjs
  // (town-coordinate consistency, on the real resolvers).
  //
  // v8.13.2 (owner screenshot, 2026-08-18): parrish and ruskin used to read
  // "stpete" because the OLD single-centroid nearestMetro put them closer to
  // the St. Pete centroid — straight across Tampa Bay. That alignment made the
  // table agree with a wrong viewer model instead of fixing it: a Parrish
  // reader (Manatee County, 11mi from Bradenton by land) was gated to St. Pete
  // inventory, and with the St. Pete Clipp harvest expired, /coupons served
  // ZERO while Bradenton offers sat live next door. nearestMetro now measures
  // against per-metro ANCHORS (lib/orderInFeatured.js) — Bradenton anchors
  // sarasota, Riverview anchors tampa — so both sides of the gate now say what
  // a resident would: parrish -> sarasota, ruskin -> tampa.
  parrish: "sarasota",
  palmetto: "sarasota",
  osprey: "sarasota",
  englewood: "sarasota",
  clearwater: "stpete",
  "pinellas park": "stpete",
  "tierra verde": "stpete",
  brandon: "tampa",
  riverview: "tampa",
  lithia: "tampa",
  ...
  "land o lakes": "tampa",
  "plant city": "tampa",
  // 2026-08-23, Orlando Clipp harvest. Same rule as every entry above: each town
  // is used by a row in CLIPP_MERCHANT_OFFERS, and each one is what
  // nearestMetro() ALREADY returns for a viewer standing there — verified on the
  // real resolver before these were written, not assumed:
  //   Zellwood 28.7286,-81.6001 · Casselberry 28.6778,-81.3278 · Ocoee 28.5692,-81.5434
  //   Oviedo 28.6700,-81.2081 · Winter Garden 28.5653,-81.5862 · Lake Mary 28.7589,-81.3178
  // all resolve to "orlando", so the deal side and the viewer side of the gate
  // agree and none of these coupons is hidden from the people nearest them.
  // (Orlando itself needs no entry — it is a METROS label.)
  zellwood: "orlando",
  casselberry: "orlando",
  ocoee: "orlando",
  oviedo: "orlando",
...
export function dealNetwork(c) {
  ...
  if (c.commerce && c.commerce.provider) {
    return providerLabel(c.commerce.provider);
  }
  const u = String(c.url || "");
  // OUR OWN REDIRECT names its provider in the query string, and this branch
  // must come FIRST and read the PARAMETER rather than sniff a host.
  //
  // 2026-08-02 (audit F5): once a coupon moves behind /api/commerce/go the
  // partner host is gone from the markup BY DESIGN — that is the entire point
  // of moving it. Every host-matching branch below therefore stops matching and
  // dealNetwork returned null, which does not merely lose a label: it flips
  // dealDisclosure() to "Not an affiliate offer — just a good one." on a card
  // that very much is one. A commission link rendering an explicit
  // NON-affiliate disclosure is a worse FTC position than the raw link it
  // replaced. check-citypass caught it because it asserts the disclosure names
  // the network, instead of trusting the URL to keep a particular shape.
  if (u.startsWith("/api/commerce/go?")) {
    const provider = new URLSearchParams(u.slice(u.indexOf("?") + 1)).get("provider");
...
// Card artwork is EXPLICIT only. Broad intent tags are useful for routing a deal
// into a list, but they are not photographic evidence: a baseball promotion tagged
// `nightout` inherited a close-up restaurant photo, and a museum could inherit a
// café. A plausible but unrelated image is worse than a clearly branded coupon
// tile. Each deal therefore opts into one of OUR committed assets on its own row;
// otherwise the UI renders an unmistakable Wayfind coupon mark.
...
const GENERATED_ART = new Set([
  ...
  // v8.24 (owner: "I never want to see this image ever again") — the AI
  // neon-concert composite (night-out.jpg) is BANNED and deleted; real
  // concert-crowd photo instead. Locked by check-banned-art.
  "/cards/tonight-alfonso-scarpa-unsplash.jpg",
  "/cards/outdoors.jpg",
...
export function dealArtwork(c) {
  ...
  // happened"). The intent-art fallback died for honesty; THIS is the honest
  // replacement: the venue's OWN current photo, keyed on its stable placeId
  // through our proxy (/api/photo?place= — Place Details first photo, 24h
  // cached, self-consistent with the v8.17 ref self-heal). A Ringling deal
  // wears the Ringling; nothing plausible-but-unrelated can render.
  if (typeof c.venuePlaceId === "string" && /^[A-Za-z0-9_-]{10,}$/.test(c.venuePlaceId)) {
    return "/api/photo?place=" + encodeURIComponent(c.venuePlaceId) + "&g=2&w=800";
  }
  return null;
}

export const DEAL_SHEET_INTERNALS = { CADENCE, GENERATED_ART };
