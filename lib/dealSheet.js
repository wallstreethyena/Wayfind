...
export function dealTiers(coupons, todayIso, viewerCenter) {
  const live = (Array.isArray(coupons) ? coupons : []).filter((c) => couponIsLive(c, todayIso));
  const featured = [];
  const ledger = [];
  for (const c of live) {
  ...
  // (~75mi, true great-circle). null when we have no center or the user is outside
  // every covered metro — dealLocalityRank handles that honestly rather than
  // defaulting to a home city. There are two candidate default centers in this app
  // and picking one here would silently make one of them "local" for everybody.
  const viewerMetro = viewerCenter && isFinite(viewerCenter.lat) && isFinite(viewerCenter.lng)
    ? nearestMetro(viewerCenter.lat, viewerCenter.lng)
    : null;

  // TWO KEYS, both derived from the data — no hand-sort can creep into either.
  //   1. locality: deals good where the viewer is, then national, then elsewhere
...
const INTENT_ART = {
  eatnow: "/cards/where-to-eat.jpg",
  datenight: "/cards/date-night-dining-hero.jpg",
  familyfun: "/cards/family-fun.jpg",
  outdoors: "/cards/outdoors.jpg",
  cozyindoor: "/cards/cozy-indoor-v1.png",
  hiddengems: "/cards/hidden-gems.jpg",
  nightout: "/cards/night-out.jpg",
};
...
export function dealArtwork(c) {
  ...
  if (typeof c.image === "string" && c.image.startsWith("/")) return c.image;
  for (const key of Array.isArray(c.intents) ? c.intents : []) {
    if (INTENT_ART[key]) return INTENT_ART[key];
  }
  return null;
}

export const DEAL_SHEET_INTERNALS = { CADENCE, INTENT_ART };
