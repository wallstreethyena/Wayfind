...
export function captureAttribution(search, opts) {
  // NATIVE SHELL: capture NOTHING (2026-08-05, App Store readiness ticket 4).
  //
  // gclid/gbraid/wbraid are Google Ads click ids. Storing them and joining them
  // to a user is third-party advertising attribution, which is exactly what
  // Apple's privacy questionnaire means by "used to track you" — and there is
  // no App Tracking Transparency prompt in this build. Shipping the capture
  // while declaring "no tracking" misrepresents the privacy label; declaring
  // tracking without an ATT prompt is a 5.1.2 rejection. Neither is acceptable.
  //
  // Gated HERE rather than at the call site because there are THREE callers —
  // GoogleTags, PaidLanding and ExploreBridge — and gating only the first would
  // leave two live. One hop is not proof.
  //
  // Nothing of value is lost: a native app launch has no gclid to capture. Web
  // attribution is untouched.
  if (isNative()) return readAttribution();
