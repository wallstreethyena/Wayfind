...
export function addressLine(x) {
  ...
  // v8.99 (owner, 2026-09-06, on the 3 Daughters Oktoberfest page: "the
  // events page does not have the address"). It did — "222 22nd St S" — and
  // that is not an address anyone can drive to, because the row stores the
  // STREET in `address` and the town in `city`. Measured against live
  // wf_events: 103 of 133 street lines carry no city. So the human line is
  // street + town whenever the street does not already name the town; a row
  // that stores the full postal line ("…, Sarasota, FL 34243") is left alone.
  if (!city || address.toLowerCase().includes(city.toLowerCase())) return address;
  return [address, cityState].filter(Boolean).join(", ");
}

/**
 * The venue's own website, or null. ONE gate for both event surfaces so the
 * "Official site" button cannot point somewhere lib/links.safeUrl would refuse.
 * Curated rows pass the already-gated eventOutboundUrl() result as `url`;
 * aggregator rows pass the provider's `url`. This only checks shape.
 */
export function websiteUrl(x) {
  const raw = x && (x.website || x.url);
  if (!raw) return null;
  try {
...
/** Hostname for the button caption ("3dbrewing.com"), or "". */
...
export function directionsUrl(x) {
  if (!x) return null;
  // The FULL line (street + town) — a bare "222 22nd St S" geocodes to
  // whichever 22nd Street Google likes best, and there are hundreds.
  const address = str(x.address) ? addressLine(x) : "";
...
/**
 * The same ladder, addressed to APPLE MAPS. Event pages only (2026-09-08).
 *
 * Owner, 2026-09-08: "in the events page switch to Apple Maps permanently,
 * not temporarily." The in-page venue map has been Apple MapKit since #1144;
 * this makes the OUTBOUND navigation link on those pages Apple too, so the
 * event surface is one map vocabulary end to end — the reader looks at an
 * Apple map, plans an Apple route, and opens Apple Maps to drive it. On an
 * iPhone (the audience) a maps.apple.com link opens the native Maps app
 * directly; elsewhere it opens Apple Maps on the web. No SDK, no key, no
 * spend: this is a URL.
 *
 * Same rungs and the same honesty rule as directionsUrl: exact address first,
 * then the coordinate, then venue + town, and NULL when the row cannot name a
 * destination — so the page renders no button rather than a pin in the
 * middle of a city. Google place ids mean nothing to Apple, so that rung is
 * skipped rather than translated.
 *
 * URL form is Apple's long-standing Map Links scheme (`daddr` + `dirflg=d`),
 * which both the native app and maps.apple.com on the web honour; `q` labels
 * the destination card with the venue name instead of a bare address.
 *
 * @returns {string|null} an absolute maps.apple.com directions URL, or null
 */
export function appleDirectionsUrl(x) {
  if (!x) return null;
  const address = str(x.address) ? addressLine(x) : "";
  const venue = str(x.venue || x.name);
  const city = str(x.city);
  ...
  // Hand-encoded rather than URLSearchParams: Apple's Map Links reference
  // reads a coordinate pair by its literal comma, and an address string is
  // safest with its commas as %2C. encodeURIComponent gives the second; the
  // coordinate rung is written with the comma left in on purpose.
  const daddr = hasCoords && destination === `${lat},${lng}` ? `${lat},${lng}` : encodeURIComponent(destination);
  const label = venue ? "&q=" + encodeURIComponent(venue) : "";
  return "https://maps.apple.com/?daddr=" + daddr + "&dirflg=d" + label;
}
