...
//   4. RECONCILE A ROW THAT IS ALREADY FREE TO CLOSE, regardless of its own
//      schedule (2026-09-23 Lane L2b fix — see THE 2026-09-23 FIX below).
//
// THE 2026-09-23 FIX (measured in production, Lane L2b follow-up audit). 24
// open rows already had an ACTIVE wf_place_photo (a real, free, servable
// photo) sitting right next to them — never closed, because a vault entry
// going active is an EVENT, not a clock tick, and runRepair only ever looks
// at a row when its OWN next_attempt_at backoff (built to throttle PAID
// attempts) comes due. Separately, 58 open rows were still carrying the
// exact next_attempt_at=2026-10-01T00:00:00Z calendar pin the 2026-09-09 fix
// below was supposed to have retired — #1222's own data-fix migration
// matched only failure_reason='spend-restricted' (173 rows) and missed the
// other 234 rows that day carried failure_reason='source-unavailable' with
// the SAME pin (git log 1a715283's own measurement). Both are the same root
// failure: something a REAL request would serve for free (cache or vault)
// sitting behind a schedule built for money. fetchDueRows now runs a THIRD,
// unconditional select — wf_photo_queue_reconcilable
// (20260923_wf_photo_queue_reconcile.sql) — that finds every open/
// budget_blocked row with an active vault row or a fresh exact-ref cache hit
// RIGHT NOW, and processes it THAT drain regardless of next_attempt_at,
// status, or the ledger. A companion one-time data-fix UPDATE in that same
// migration un-pins any OPEN row scheduled past its own honest 7-day backoff
// ceiling — keyed to the INVARIANT the bug violated (backoffMs's own max),
// not to the literal date, so it also catches any future recurrence of this
// exact class.
//
...
//     ledger read at all.
//
// ORDER: usable same-place cache and verified free photos come first,
// independent of inventory's Google ref. Only an actual absence of all
// sources is no-source. Stale-reference and headroom checks follow, so
// a budget never hides a data fact or prevents a free recovery.
...
//      release first.
//   3. wf_photo_queue_reconcilable (2026-09-23 Lane L2b fix, sibling to #2
//      above) — every open/budget_blocked row that already has a FREE
//      recovery sitting right now: an active wf_place_photo vault row, or a
//      fresh exact-ref cache hit for the place's current photo_ref (see
//      20260923_wf_photo_queue_reconcile.sql). NEVER gated on next_attempt_at
//      OR the ledger — closing a row that is already free to close was never
//      supposed to wait on a backoff clock built to throttle PAID attempts,
//      and this is exactly how a real vault entry going active (or a cache
//      row warming up from an ordinary reader hit) sat invisible to the
//      worker for up to 7 more days, and how a leftover pre-#1222
//      next_attempt_at pin (58 rows measured 2026-09-23, the
//      source-unavailable half of the 234 that slipped past #1222's own
//      spend-restricted-only WHERE clause) blocked a free check that never
//      needed the pin's protection in the first place. Best-effort: any
//      failure reading this view (including the view not existing yet on an
//      environment the migration hasn't reached) is swallowed and this
//      drain simply proceeds without it — see fetchReconcileCandidates.
// A failure on select 1 aborts immediately without ever attempting select 2
// or 3 (fail-soft callers must see exactly one queue failure, not a second
// unexplained endpoint) — see case 11.
//
// ONE SHARED BUDGET, NOT TWO/THREE (2026-09-09 fix, extended 2026-09-23,
// measured in production). Each select used to be issued with
// `limit=<limit>` and then simply concatenated: a `--limit=25` drain
// reported attempted=50, and one pulse note showed attempted=423. A
// `?limit=500` cron could attempt 1,000 rows against this route's 60s
// ceiling, time out mid-batch, and file NO pulse at all — job-watch then
// reads a dead run as silence, not failure. Every select still asks the
// server for at most `limit` rows (cheap, ordinary indexed reads), but the
// ROWS THIS DRAIN ACTUALLY PROCESSES are capped to `limit` in total:
// reconcile fills FIRST (these are guaranteed free wins — closing them is
// strictly better than spending the same row-budget on a due row that may
// still classify as unresolved), budget_blocked fills next (oldest
// blocked_since — the longest-waiting rows release first), and whatever
// budget remains is topped up from the ordinary open queue.
async function fetchReconcileCandidates(s, limit) {
  try {
    const r = await fetch(
      `${s.url}/rest/v1/wf_photo_queue_reconcilable?select=${SELECT}&limit=${Math.max(1, limit)}`,
    ...
    return []; // best-effort — never blocks the ordinary due/blocked selects
  }
}
...
async function fetchDueRows(s, limit, hasHeadroom) {
  ...
  const reconcile = await fetchReconcileCandidates(s, cappedLimit);
  const reconcileSlice = reconcile.slice(0, cappedLimit);
  const reconcileSeen = new Set(reconcileSlice.map((r) => r.place_id));
  ...
  const remainingAfterReconcile = Math.max(0, cappedLimit - reconcileSlice.length);
  if (!blockedCandidates.length || !(await hasHeadroom())) {
    // nothing to gate, or measured 0/unknown headroom: never release a
    // budget_blocked row — but reconcile candidates are free and always kept.
    const openSlice = dueOpen.filter((r) => !reconcileSeen.has(r.place_id)).slice(0, remainingAfterReconcile);
    return reconcileSlice.concat(openSlice);
  }

  const blockedSlice = blockedCandidates.filter((r) => !reconcileSeen.has(r.place_id)).slice(0, remainingAfterReconcile);
  const seen = new Set([...reconcileSeen, ...blockedSlice.map((r) => r.place_id)]);
  const remaining = Math.max(0, remainingAfterReconcile - blockedSlice.length);
  const openSlice = dueOpen.filter((r) => !seen.has(r.place_id)).slice(0, remaining);
  return reconcileSlice.concat(blockedSlice).concat(openSlice);
}
...
//
// THE OCTOBER 1ST FIX (2026-09-09). wf_spend_take (lib/spendGate.js) creates
// the month's wf_spend_ledger row lazily, on the FIRST GRANT of that
// month — so right after a monthly rollover, an HTTP 200 with a genuinely
// EMPTY rows array is the correct read of "nothing spent yet this month",
// not a failed read. This function used to return the SAME `{}` for that
// case and for "the row exists but a field is garbage" — both fed
// allowanceFromLedger({}) and came back phase:"unknown", which
// hasHeadroomForClassification()/canReleaseBlocked() below then treat as
// "never release". An absent row on Oct 1 does not deserve that: it is a
// real, measured "nothing spent", not an unknown. So this function now
// returns a shape allowanceFromLedger can tell apart at the call site:
//   - a real row                         -> {used, cap} (unchanged)
//   - HTTP 2xx, zero rows                -> { rowPresent: false } — absent,
//     not unreadable; getAllowance() below passes this straight through and
//     allowanceFromLedger reads it as used=0 against the operating cap (see
//     that function's own comment).
//   - non-2xx, or a malformed (non-array) body -> THROWS, same shape as
//     before, so the caller's catch produces {} -> allowanceFromLedger's
//     genuine phase:"unknown" (AGENTS.md §5's corollary: an unmeasured
//     budget is an operator-visible unknown, never a fabricated 0 — and, as
//     of this fix, never a fabricated unknown either, when the read
//     actually succeeded).
export async function readPhotosAllowanceRaw(s) {
  ...
  const rows = await r.json();
  if (!Array.isArray(rows)) throw new Error("wf_spend_ledger read failed: malformed body (not an array)");
  if (!rows.length) return { rowPresent: false };
  const row = rows[0];
  return { used: row.used, cap: row.cap };
}
