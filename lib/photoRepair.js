...
export async function runRepair({ limit = DEFAULT_LIMIT, dryRun = false, sbEnv, findSamePlace = findSamePlaceCachedPhoto } = {}) {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      if (livePhotoRef) {
        // OPTIONS OBJECT, NOT POSITIONAL. findSamePlaceCachedPhoto's signature
        // is ({ placeId, width, now, fetchImpl, env }) — calling it as
        // (placeId, width) destructures `placeId` off a STRING, gets
        // undefined, fails its own PLACE_ID_RX and returns null WITHOUT EVER
        // QUERYING. The worker would then recover nothing, ever, and file
        // every recoverable place as spend-restricted instead. Same shape as
        // the recordPulse(db, "inventory-refresh", …) incident
        // (scripts/check-job-pulse-contract.mjs): an injected dep's fake
        // accepted the wrong shape, so the tests could not see it. Case 10
        // now calls the REAL function against a stub fetch to lock the
        // signature.
        recovery = await findSamePlace({ placeId: row.place_id, width: 640 });
