...
//      release first.
// Both bounded by `limit`. A failure on select 1 aborts immediately without
// ever attempting select 2 (fail-soft callers must see exactly one queue
// failure, not a second unexplained endpoint) — see case 11.
async function fetchDueRows(s, limit, hasHeadroom) {
  const nowIso = new Date().toISOString();
  const dueOpen = await getQueueRows(
    s,
    `status=eq.open&next_attempt_at=lte.${encodeURIComponent(nowIso)}&select=${SELECT}&order=next_attempt_at.asc&limit=${Math.max(1, limit)}`
  );
  const blockedCandidates = await getQueueRows(
    s,
    `or=(status.eq.budget_blocked,and(status.eq.open,failure_reason.eq.spend-restricted))&select=${SELECT}&order=blocked_since.asc.nullslast&limit=${Math.max(1, limit)}`
  );
  ...
  const seen = new Set(dueOpen.map((r) => r.place_id));
  return dueOpen.concat(blockedCandidates.filter((r) => !seen.has(r.place_id)));
}
