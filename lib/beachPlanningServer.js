...
import { fetchVisitBeachesReport, refreshVisitBeachesReport } from './beachLocalConditions.js';
...
const localResults = unstable_cache(async slug => recorded(async () => {
  const beach = BEACH_PILOT.find(candidate => candidate.slug === slug && candidate.visitBeachId);
  if (!beach) throw new Error('No configured local beach feed');
  return fetchVisitBeachesReport(beach);
}), ['beach-pilot-local-v1'], { revalidate: 900 });

export async function getBeachPlanning(slug) {
  const beach = BEACH_PILOT.find(b => b.slug === slug);
  if (!beach) return null;
  const [water, tide, weather, alert, local] = await Promise.all([
    countyResults(beach.county), tideResults(), weatherResults(beach.lat,beach.lng), alertResults(beach.lat,beach.lng),
    beach.visitBeachId ? localResults(beach.slug) : Promise.resolve({ data: null, retrievedAt: null, status: 'linked-only' }),
  ]);
  const now = Date.now();
  ...
  let localConditions = { flag: 'unknown', flagLabel: 'Unknown', waterClosure: 'unknown', reportedAt: null, age: 'unknown', reportId: null };
  if (local.data) {
    try { localConditions = refreshVisitBeachesReport(local.data, now); }
    catch { localStatus = 'unavailable'; }
  }
  const waterClosure = localConditions.age === 'current' ? localConditions.waterClosure : 'unknown';
  return { beach, samples, periods, alerts, redTide, localConditions, closure: 'unknown', waterClosure, flags: localConditions.flag,
    sources: { water: { status: water.status, retrievedAt: water.retrievedAt }, redTide: { status: tide.status, retrievedAt: tide.retrievedAt }, weather: { status: weather.status, retrievedAt: weather.retrievedAt, issuedAt: weather.data?.properties?.updateTime || null, age: evidenceAge(weather.data?.properties?.updateTime, 6, now) }, alerts: { status: alerts === null ? 'unavailable' : 'available', retrievedAt: alert.retrievedAt }, local: { status: localStatus, provider: beach.visitBeachId ? 'Mote BCRS / Visit Beaches' : 'County-selected report link', retrievedAt: local.retrievedAt, reportedAt: localConditions.reportedAt, age: localConditions.age } },
    ...planningSummary({ samples, alerts, periods, redTide, closure: 'unknown', waterClosure, localFlag: localConditions.flag }), generatedAt: new Date(now).toISOString() };
}
