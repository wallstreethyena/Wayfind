...
const alertResults = unstable_cache(async (lat,lng) => recorded(async () => {
  const d = await (await request(`https://api.weather.gov/alerts/active?point=${lat},${lng}`)).json();
  if (!Array.isArray(d.features)) throw new Error('Invalid alert response');
  return d.features.map(f => f.properties).filter(Boolean).map(p => ({ event: p.event, headline: p.headline, issuedAt: p.sent, expiresAt: p.expires, instruction: p.instruction }));
}), ['beach-pilot-alerts-v1'], { revalidate: 60 });
...
export async function getBeachPlanning(slug) {
  ...
  return { beach, samples, periods, alerts, redTide, closure: 'unknown', flags: 'unavailable',
    sources: { water: { status: water.status, retrievedAt: water.retrievedAt }, redTide: { status: tide.status, retrievedAt: tide.retrievedAt }, weather: { status: weather.status, retrievedAt: weather.retrievedAt, issuedAt: weather.data?.properties?.updateTime || null }, alerts: { status: alert.status, retrievedAt: alert.retrievedAt } },
