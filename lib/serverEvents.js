...
// OUR OWN SYNTHETIC MONITOR MUST NEVER COUNT AS A HUMAN.
// scripts/run-synthetic-monitor.mjs self-identifies with a fixed UA
// ("WayfindSyntheticMonitor/1.0 …") on every request, including a real,
// unrefused hit to /api/{viator,commerce,ticketmaster}/go (its "book-links"
// scenario needs a REAL partner redirect — see lib/crawler.js's
// isSyntheticMonitor doc for why it is never folded into isCrawler()). That
// means the redirect route still emits provider_redirect_started/failed for
// it unless something stops it here — this is that something, at the single
// chokepoint every server-side capture already goes through, so no go route
// has to remember to check it individually.
...
import { isSyntheticMonitor } from "./crawler.js";
...
/**
 * Read one request header. Accepts a Fetch Headers instance or any `{ get }`.
 * Missing / unreadable headers are empty string — never an invented value.
 */
...
/**
 * PostHog visitor-identity props derived from the incoming request.
 *
 * WHY THESE LIVE HERE, NOT IN commercePayload.
 * commercePayload is a CONTEXT_FIELDS whitelist. $raw_user_agent and $ip are
 * PostHog *system* properties, not commerce schema fields — stuffing them into
 * ctx would be silently dropped. Attach them AFTER the whitelist, here.
 *
 * $raw_user_agent is what PostHog uses (with $user_agent from the JS SDK) to
 * compute the query-time virtual $virt_is_bot. An empty UA falls through to
 * Automation / no_user_agent → every server event looks like a bot. We forward
 * the visitor UA when the request has one; we never invent a browser UA, and
 * we never set $virt_is_bot ourselves.
 */
function visitorPropsFromHeaders(headers) {
  const out = {};
  const ua = headerGet(headers, "user-agent").slice(0, UA_MAX);
  if (ua) out.$raw_user_agent = ua;
  const xff = headerGet(headers, "x-forwarded-for");
  const firstHop = xff ? xff.split(",")[0].trim() : "";
  const ip = (firstHop || headerGet(headers, "x-real-ip")).slice(0, IP_MAX);
  if (ip) out.$ip = ip;
  return out;
}
 ...
 *
 * `headers` is the incoming request Headers (or `{ get }`). When present, the
 * visitor User-Agent and first forwarded IP are copied onto PostHog system
 * props. Pass headers here — never through commercePayload.
 *
 * @returns {Promise<boolean>} true only when PostHog accepted the event.
 */
export async function captureServer(event, { distinctId, properties, headers } = {}) {
  ...
  // Checked before the key too: our own synthetic monitor is not analytics
  // suppression (it must still get a real redirect elsewhere in the route),
  // it is "never a human" — see the module comment above.
  if (isSyntheticMonitor(headerGet(headers, "user-agent"))) return false;
  const key = phKey();
  ...
  try {
    const r = await fetch(HOST + "/capture/", {
      ...
      body: JSON.stringify({
        ...
        properties: {
          ...(properties || {}),
          ...visitorPropsFromHeaders(headers),
          $lib: "wayfind-server",
        },
      }),
    });
