...
const PLACE_PHOTO_PROXY_RX = /\/api\/photo\?[^#]*\bplace=([A-Za-z0-9_-]+)(?:&|$)/;
...
function isRealPhotoSrc(src) {
  ...
  if (s.includes("/api/photo")) {
    ...
    if (proxy && hasPlacePhotoRef(decodeRef(proxy[1]))) return true;
    // Owned-inventory deep links cannot carry Google's photo resource name.
    // They use the guarded proxy's exact place-id lookup instead. This is
    // still a real photo of the named place, not a shared stock fallback.
    return PLACE_PHOTO_PROXY_RX.test(s);
  }
...
function proxyFromRef(ref) {
  return hasPlacePhotoRef(ref) ? ("/api/photo?ref=" + encodeURIComponent(ref) + "&g=2&w=640") : null;
}
