...
// lib/creatorVideos.js. Every function below runs on the client (this file
// is imported eagerly on "/" via lib/lawfulOrder.js -> lib/ranking.js and
// lib/sources.js — the ranked-list machinery, not homepage-specific) and
// needs only .length and .reach off each video, never the url/caption/
// address. lib/creatorSignals.js answers that from a ~17KB-gz lean mirror of
// the curated registry instead of pulling in the full ~56KB-gz
// lib/creatorVideos.js — see that file's header for the full reasoning.
import { creatorVideosFor } from "./creatorSignals.js";
 ...
 */
export function displayedWfScore(place, locName, trace) {
  const q = num(place && place.wfScore);
  if (q == null) return null;
  // THE GOVERNING LAW (owner, 2026-08-07 — lib/wayfindScore.js): a creator
  // video is worth a flat +2 (0.2 on the shown scale), full stop. The
  // reach-scaled evidence curve above no longer sets the displayed number —
  // "if there is an influencer video, I want that to add a zero point seven."
  // The 4.2★/30-review floor no longer gates the DISPLAY term either: the
  // flat +2 is small enough that it cannot lift a mediocre place over an
  // excellent one (the failure the floor existed to stop was a +45), and the
  // owner's rule has no carve-out. Distance is the card's own context: the
  // −2 past 17 miles applies where the caller knows the distance
  // (distMi/distance_mi ride the place object on every ranked surface).
  const dist = num(place && place.distMi) != null ? num(place.distMi)
    : num(place && place.distance_mi) != null ? num(place.distance_mi) : null;
  // 2026-08-08: the trending component (lib/wayfindScore TRENDING_BONUS) rides
  // the place object — lib/trendSignal.js attachTrendSignals sets `trending`
  // only from real demand data, and every surface that decorates its pool
  // also renders the 🔥 reason (check-score-law.mjs). Undecorated objects are
  // unchanged: no flag, no bump.
  return governedWayfindScore(q, { hasCreatorVideo: hasCreatorVideoAt(place, locName), distanceMi: dist, trending: !!(place && place.trending) }, trace);
}
