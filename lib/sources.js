...
export function junkGate(categoryId, p, subId) {
  // Specific chips skip the curated-name bypass. Ca' d'Zan is a first-party
  // pick and used to skip placeAllowed — that is how a mansion led Beaches
  // and Family → Rainy day.
  if (!(subId && subId !== "all") && p && p.name && CURATED_NAMES.has(_nn(p.name))) return true;
