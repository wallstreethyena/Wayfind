"use client";
// v4.86 — Multi-source place aggregation. The app used to be Google-only for
// venue lists; this module is the single entry point that fans out to every
// available place source in parallel, normalizes to the app's common shape,
// dedupes across sources, and hands the merged pool to the EXISTING ranking
// engine unchanged (same _sortScore math + near-first rule as the Google
// path, so ranking behavior is identical — just with a bigger honest pool).
//
// Sources handled HERE (venue-shaped places):
//   • Google Places — primary (client SDK or server proxy, cached)
//   • Foursquare    — second source via /api/fsq/search (fail-soft)
//   • NPS + RIDB (Recreation.gov) + OpenStreetMap — outdoor/recreation places
//     via the server-only /api/outdoors route (keys never reach the browser);
//     scoped to Beach day, Things to do, and the outdoor vibes so they can't
//     pollute food or nightlife
// Sources that stay on their own purpose-built surfaces, by design:
//   • Curated picks — injected + boosted downstream in the vibe loader
//   • Viator        — bookable products (not venues) on the vibe/browse rails
//   • Ticketmaster/SeatGeek/PredictHQ/Eventbrite — events, on the Events tab
// Merging products/events into venue lists would fake venue semantics
// (address/hours they don't have) — they enrich views instead of the pool.
//
// Dedupe rule: same venue = normalized-name match (equal, or one contains the
// other at ≥5 chars) AND coordinates within 250 m. On a match the Google
// record stays canonical and Foursquare fills only the gaps (rating, review
// count, price, photo, open-now, extra categories) — never overwrites.
import { searchPlaces as searchGooglePlaces, queryFor, vibeFor, wayfindScore, prominenceScore, distMeters, DEFAULT_RADIUS_M } from "./google";
import { CURATED } from "./curated";
import { placeAllowed } from "./placeFilter";
// The 23-mile beach rule. searchPlaces is the ONE door every venue list in the
// app walks through — Google, Foursquare, NPS/RIDB/OSM, every category, every
// surface — so vetting here is what makes "nothing beach-shaped is recommended
// unless it is actually near you" a property of the system instead of a patch
// on one card. It is pure arithmetic over coordinates the rows already carry:
// no extra request, no extra cost. See lib/beaches.js for the rule itself.
import { vetBeachDistance, BEACH_NEAR_MI } from "./beaches";
import { isOpenNow } from "./businessStatus";
import { lawfulSort } from "./lawfulOrder.js";

const _nn = (s) => String(s || "").toLowerCase().replace(/&/g, "and").replace(/[^a-z0-9]/g, "");

// v4.94 — the gate now lives in lib/placeFilter.js (ONE module, imported by
// every result path and executed directly by the check-gate build guardrail).
// This wrapper only adds the curated bypass: first-party picks always pass.
const CURATED_NAMES = new Set(CURATED.map((c) => _nn(c.name)));
export function junkGate(categoryId, p, subId) {
  // A curated name is not a chip identity. Ca' d'Zan is a first-party
  // pick and used to skip placeAllowed, which is how a Ringling mansion
  // became a Beaches card and a Family → Rainy day card.
  const sub = subId || null;
  if (sub && sub !== "all") return placeAllowed(categoryId, sub, p);
  if (p && p.name && CURATED_NAMES.has(_nn(p.name))) return true;
  return placeAllowed(categoryId, sub, p);
}

const POI_RX = /park|beach|preserve|trail|garden|pier|marina|monument|landmark|memorial|boardwalk|island|natural_feature|playground|springs?\b|national_/;
export function qualityFloor(p) {
  if (!p) return false;
  if (p.name && CURATED_NAMES.has(_nn(p.name))) return true;
  if (p.status && p.status !== "OPERATIONAL") return false;
  if (p.rating != null && (p.reviews || 0) >= 15) return true;
  return POI_RX.test((((p.types || []).join(" ")) + " " + (p.name || "")).toLowerCase());
}

function sameVenue(a, b) {
  if (!a || !b || a.lat == null || b.lat == null) return false;
  const na = _nn(a.name), nb = _nn(b.name);
  if (!na || !nb) return false;
  const nameHit = na === nb || (na.length >= 5 && nb.includes(na)) || (nb.length >= 5 && na.includes(nb));
  if (!nameHit) return false;
  return distMeters({ lat: a.lat, lng: a.lng }, { lat: b.lat, lng: b.lng }) <= 250;
}

// ONE REQUEST PER QUERY, NOT EIGHT.
//
// A single "Beach day" tap fans out into two parallel searchPlaces calls (the
// category and its second sub-filter), and both satisfy wantOutdoors, so both
// call this with byte-identical arguments. Then the adaptive-radius ladder in
// app/home.js re-invokes the whole fetch per rung when a thin area comes back
// short — up to four more times. Measured: eight identical
// /api/outdoors?lat=..&lng=..&radius=.. requests from one tap, each holding a
// 6s abort against a route whose own header documents 24s OSM waits.
//
// The route's server-side memo cannot help: every one of those requests is in
// flight before any of them has populated it. The dedupe has to live on the
// client, keyed on the exact query, and it must be the PROMISE that is shared —
// caching the result would also cache a transient failure.
const _outdoorsInFlight = new Map();
async function outdoorsSearch(center, radiusMeters) {
  if (typeof window === "undefined" || !center) return [];
  const qs = new URLSearchParams({ lat: Number(center.lat).toFixed(4), lng: Number(center.lng).toFixed(4), radius: String(Math.round(radiusMeters)) }).toString();
  const hit = _outdoorsInFlight.get(qs);
  if (hit) return hit;
  const run = (async () => {
    try {
      // v4.97: the outdoors route can wait up to ~24s on OSM timeouts — that
      // was the Beach day hang. 6s and we move on without it (cached later).
      const ctrl = new AbortController();
      setTimeout(() => ctrl.abort(), 6000);
      const r = await fetch("/api/outdoors?" + qs, { signal: ctrl.signal });
      if (!r.ok) return [];
      const d = await r.json();
      return Array.isArray(d.places) ? d.places : [];
    } catch (e) { return []; }
  })();
  _outdoorsInFlight.set(qs, run);
  // Cleared on settle, not cached: a later tap at the same coordinates should
  // be free to retry, and the route has its own memo for the repeat case.
  run.then(() => _outdoorsInFlight.delete(qs), () => _outdoorsInFlight.delete(qs));
  return run;
}

async function fsqSearch(q, center, radiusMeters, limit = 30) {
  if (typeof window === "undefined" || !q || !center) return [];
  try {
    const qs = new URLSearchParams({ q, lat: Number(center.lat).toFixed(4), lng: Number(center.lng).toFixed(4), radius: String(Math.round(radiusMeters)), limit: String(limit) });
    const r = await fetch("/api/fsq/search?" + qs.toString());
    if (!r.ok) return [];
    const d = await r.json();
    return Array.isArray(d.places) ? d.places : [];
  } catch (e) { return []; }
}

// v6.63: the 1.3-pts-per-mile decay that used to live here is gone. Distance
// is priced once, visibly, at a flat -0.2 past 17 miles inside the governed
// Wayfind Score (lib/wayfindScore.js); charging it a second time invisibly, at
// fifteen times the weight, is what reordered this pool against its own chips.

// Drop-in replacement for google.searchPlaces — same signature, merged pool.
export async function searchPlaces(categoryId, subId, center, radiusMeters = DEFAULT_RADIUS_M, vibeId = "all", keyword = "") {
  let fq = queryFor(categoryId, subId);
  try { const vq = vibeFor ? vibeFor(categoryId, vibeId) : ""; if (vq) fq = vq + " " + fq; } catch (e) {}
  if (keyword) fq = keyword + " " + fq;
  const wantOutdoors = categoryId === "beach" || categoryId === "attractions"; // v4.90: NPS/RIDB/OSM only where they belong
  // v6.44 (owner, the 23-mile rule) — RECOMMENDATION vs REQUEST.
  // A user who taps "Beach day" has ASKED for beaches: suppressing results
  // inside the radius they themselves searched would be a regression, not a
  // fix. So an explicit beach browse honors its own radius (never TIGHTER than
  // the rule, so the coastal default still behaves), while every other
  // category — where a beach would be an UNSOLICITED recommendation — holds to
  // BEACH_NEAR_MI. That is exactly the owner's line: a beach must not be
  // recommended unless one is within 23 miles.
  const beachMax = categoryId === "beach" ? Math.max(BEACH_NEAR_MI, radiusMeters / 1609.34) : BEACH_NEAR_MI;
  const [g, f0, o0] = await Promise.all([
    searchGooglePlaces(categoryId, subId, center, radiusMeters, vibeId, keyword).catch(() => []),
    fsqSearch(fq, center, radiusMeters).catch(() => []),
    wantOutdoors ? outdoorsSearch(center, radiusMeters).catch(() => []) : Promise.resolve([]),
  ]);
  const f = [...(f0 || []), ...(o0 || [])];
  const out = (g || []).slice();
  if (!f || !f.length) return _rank(vetBeachDistance(out.filter((p) => junkGate(categoryId, p, subId) && qualityFloor(p)), center, beachMax));
  const gate = radiusMeters * 1.15; // same hard distance gate as the Google path
  for (const fp of f) {
    if (fp.distMi == null && fp.lat != null) fp.distMi = distMeters(center, { lat: fp.lat, lng: fp.lng }) / 1609.34;
    const twin = out.find((gp) => sameVenue(gp, fp));
    if (twin) {
      if (twin.rating == null && fp.rating != null) { twin.rating = fp.rating; twin.wfScore = wayfindScore(fp.rating, fp.reviews || 0); }
      if ((!twin.reviews || twin.reviews === 0) && fp.reviews) twin.reviews = fp.reviews;
      if (twin.price == null && fp.price != null) { twin.price = fp.price; twin.priceNum = fp.priceNum; }
      if (!twin.photo && fp.photo) { twin.photo = fp.photo; twin.photos = fp.photos || []; twin.photoAttr = ""; }
      // v6.34: hours evidence travels TOGETHER. Copying a bare openNow boolean
      // gave twins an "Open now" pill with no hours behind it and no freshness
      // stamp (the Escape Reality contradiction) — carry the structured hours
      // bundle and the capture stamp with the boolean.
      if (twin.oh == null && fp.oh != null) { twin.oh = fp.oh; if (twin.utcOffset == null && fp.utcOffset != null) twin.utcOffset = fp.utcOffset; }
      if (twin.openNow == null && fp.openNow != null) { twin.openNow = fp.openNow; if (twin.hoursAsOf == null && fp.hoursAsOf != null) twin.hoursAsOf = fp.hoursAsOf; }
      if (Array.isArray(fp.types) && fp.types.length) twin.types = Array.from(new Set([...(twin.types || []), ...fp.types]));
      twin.sources = Array.from(new Set([...(twin.sources || ["google"]), fp.src || "fsq"]));
      continue;
    }
    if (fp.distMi != null && fp.distMi * 1609.34 > gate) continue;
    // v4.96 ghost guard: a commercial place (food/nightlife/stays/shopping)
    // that exists ONLY in a secondary source with zero rating and zero
    // reviews is more likely stale or fake than real — drop it. Outdoor POIs
    // (parks, piers, beaches) legitimately lack ratings and stay.
    if (!wantOutdoors && fp.rating == null && !(fp.reviews > 0)) continue;
    fp.wfScore = wayfindScore(fp.rating, fp.reviews || 0);
    fp.wfProm = prominenceScore(fp.rating, fp.reviews || 0); // ordering only; never displayed
    // v4.94: unknown quality must not outrank known quality — an unrated
    // secondary-source venue caps well below any decently rated place.
    // v6.34: an unrated row gets NO score — never a manufactured 0 (which
    // rendered as a red "0.0/10" badge on real places like De Soto National
    // Memorial). Unrated rows already sort last via (p.wfScore || 0) below.
    if (fp.rating == null) { fp.wfScore = null; fp.wfProm = null; }
    fp.sources = [fp.src || "fsq"];
    out.push(fp);
  }
  // v4.89: hard category gate on the whole merged pool (both sources) —
  // junk out BEFORE ranking, so service businesses can't outrank real spots.
  // v4.97 quality floor (audit rule 3): must be OPERATIONAL and carry a real
  // rating with >=15 reviews — OR be a curated pick — OR be a true outdoor
  // POI (parks, beaches, piers legitimately have no ratings). Kills the
  // nameless/generic zero-review listings that were ranking #1.
  // v6.44 (owner): ...and the 23-mile beach rule on top. Non-beach rows are
  // untouched; anything that SAYS beach must prove it is within reach of the
  // user or it never reaches a card. Fail-closed on missing coordinates.
  const gated = vetBeachDistance(out.filter((p) => junkGate(categoryId, p, subId) && qualityFloor(p)), center, beachMax);
  return _rank(gated);
}

function _rank(out) {
  // v6.34: normalize the openNow FIELD through the one status source before
  // anything downstream reads it. Dozens of call sites (filters, sorts, hero
  // logic, map marker colors, LLM payloads) read p.openNow directly — after
  // this, every one of them inherits the freshness rule (stale/unverifiable
  // snapshot → null), and rows with structured hours carry a value that is
  // true for THIS render, not for fetch time.
  out.forEach((p) => { try { p.openNow = isOpenNow(p); } catch (e) {} });
  // Re-rank the merged pool exactly like lib/google.js does its own list —
  // which, as of v6.63, means the governed Wayfind Score first and proximity
  // only as a tie-breaker. See the long note at the matching block in
  // lib/google.js: the old copy of this (1.3 pts/mile past 4, capped 30, plus a
  // near-first reshuffle) reordered the merged Google+Foursquare pool against
  // the number printed on the card, and this pool is what `suggested` — the
  // home feed — is built from.
  return lawfulSort(out, (p) => -(Number(p.distMi) || 0));
}
