...
// v4.97 kept the whole route from hanging; v6.50 makes the budget depend on
// what outdoors IS for the category being searched. See OUTDOORS_BUDGET_MS.
const OUTDOORS_HANG_MS = 6000;   // beach: outdoors IS the inventory — wait for it
const OUTDOORS_ASSIST_MS = 1800; // elsewhere: it only tops up Google + Foursquare

async function outdoorsSearch(center, radiusMeters, budgetMs = OUTDOORS_HANG_MS) {
...
export async function searchPlaces(categoryId, subId, center, radiusMeters = DEFAULT_RADIUS_M, vibeId = "all", keyword = "") {
  ...
  const [g, f0, o0] = await Promise.all([
    ...
    // v6.50 PERF — the outdoors budget depends on what outdoors IS here.
    //
    // All three providers are awaited together, so this merge is only ever as
    // fast as its slowest leg. Measured on production 2026-07-28: Google
    // answered in ~1.5s and /api/outdoors took 4285ms, so the homepage's "things
    // to do" rail sat waiting ~2.8s on data it does not depend on. The 6s cap
    // stopped the route HANGING; it never stopped it being the long pole.
    //
    // For `beach`, outdoors (NPS/RIDB/OSM) IS the inventory — the 23-mile rule
    // and the beach pages are built on it, so it keeps the full 6s. Anywhere
    // else it only tops up a pool Google and Foursquare have already filled, so
    // it gets 1800ms and we move on without it.
    //
    // Dropping it is safe rather than lossy: /api/outdoors writes through the
    // shared server cache (lib/serverCache), so the request that timed out here
    // still warms the pool and the next load gets those places instantly. That
    // is the same "cached later" contract v4.97 already relied on — this only
    // changes how long the user waits before taking it.
    wantOutdoors ? outdoorsSearch(center, radiusMeters, categoryId === "beach" ? OUTDOORS_HANG_MS : OUTDOORS_ASSIST_MS).catch(() => []) : Promise.resolve([]),
  ]);
  const f = [...(f0 || []), ...(o0 || [])];
  const out = (g || []).slice();
