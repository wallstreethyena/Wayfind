 ...
 *
 * NOT DISPLAYED, AND NO LONGER THE PRIMARY ORDER (v6.63). Nothing here is
 * shown to a reader, and since the governing law this is only ever a
 * TIE-BREAKER between rows whose displayed Wayfind Score is identical — see
 * byPlaceScore() below and lib/lawfulOrder.js. It used to say "ORDER-ONLY: the
 * Wayfind Score a reader SEES is never this number", which is precisely the
 * split that let a card showing 10.0 render beneath a card showing 9.3.
 *
 * @param {object} parts
 ...
 * @param {number} [parts.trend]               TRENDING_BONUS when lib/trendSignal.js
 *                                          marked the row trending (disclosed 🔥);
 *                                          0/absent otherwise
 * @returns {number}
 */
 ...
 *
 * v6.63 — THE LAW OUTRANKS THE MODEL. placeScore's own header above still says
 * "ORDER-ONLY. The Wayfind Score a reader SEES is never this number." That
 * sentence was the bug, and it is why the owner photographed a 10.0 sitting
 * under a 9.3 (2026-08-08). `curated` alone is worth +15 here — more than twice
 * the entire creator-video bonus — and faveTier is a private per-user term, so
 * this score could and did move a row several visible points against its own
 * chip.
 *
 * placeScore is NOT deleted, because what it models is real: affinity, holiday
 * fit, moment fit, curation, evidence. It is demoted to a TIE-BREAKER. The
 * governed Wayfind Score decides the order; placeScore decides which of two
 * rows showing the same number a given reader sees first, which is exactly the
 * job personalisation should have on a surface that prints a merit score.
 */
export function byPlaceScore(partsFor) {
  return lawfulComparator((r) => placeScore(partsFor(r)));
}
