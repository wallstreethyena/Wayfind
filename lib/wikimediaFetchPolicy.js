...
//
// 2026-09-09 — COMMONS AND UPLOAD JOINED THE POLICY. isWikimediaUrl matched only
// en.wikipedia.org and wikimedia.org (the pageviews REST host); the new free
// permanent-photo lane (lib/commonsPhotos.js) queries commons.wikimedia.org's
// Action API for imageinfo/extmetadata and reads image bytes from
// upload.wikimedia.org. Neither host was covered, so every one of those
// requests took the `return baseFetch(input, init)` early exit below and
// bypassed the semaphore, the Retry-After backoff AND maxlag entirely — the
// same 12:23 failure shape this file exists to prevent, just against a
// different Wikimedia host. Both hosts are added to isWikimediaUrl so they are
// gated exactly like en.wikipedia.org is today; actionApiInput's maxlag
// injection is extended to commons.wikimedia.org's /w/api.php too (upload.
// wikimedia.org serves raw file bytes, never Action API, so it never matches
// that path and never gets a maxlag param — nothing to add there). No other
// behaviour changes: en.wikipedia.org and wikimedia.org keep doing exactly
// what they did before this commit.
...
const ACTION_API_HOSTS = new Set(["en.wikipedia.org", "commons.wikimedia.org"]);
...
function actionApiInput(input) {
  ...
  try {
    const u = new URL(raw);
    if (!ACTION_API_HOSTS.has(u.hostname) || u.pathname !== "/w/api.php") return input;
