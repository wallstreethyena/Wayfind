...
export const FAMILY_DAY_RAILS = Object.freeze([
  ...
  { id: "indoor", title: "Museums, Indoor Play & Discovery", description: "Museums, indoor play, science, aviation and space discoveries.", cats: ["attractions"] },
...
const LEGACY_RAIL_ALIASES = Object.freeze({ space: "indoor" });
...
const NAMES = {
  ...
  space: /\b(space center|space museum|planetarium|observatory|science (center|museum)|museum of science|aviation museum|air (?:and space )?museum|aerospace museum)\b/i,
...
function railForExactType(type) {
  if (!type) return null;
  for (const railId of Object.keys(EXACT)) if (EXACT[railId].has(type)) return railId;
  return null;
}
...
function railIdentity(place) {
  ...
  if (!name) return null;
  for (const railId of Object.keys(NAMES)) if (NAMES[railId].test(name)) return railId;
  return null;
}

export function canonicalFamilyRailId(railId) {
  const requested = norm(railId);
  const canonical = LEGACY_RAIL_ALIASES[requested] || requested;
  return RAIL_IDS.has(canonical) ? canonical : null;
}
...
export function familyRailMatches(place, railId) {
  const id = canonicalFamilyRailId(railId);
  if (!id) return false;
  const primary = primaryOf(place);
  ...
  const verified = familyFilterFacts(place?.place_id || place?.id || "");
  const acceptedIds = id === "indoor" ? ["indoor", "space"] : [id];
  if (!hardVeto && Array.isArray(verified?.rail_types) && acceptedIds.some((railType) => verified.rail_types.includes(railType))) return true;
  return acceptedIds.includes(railIdentity(place));
}
