...
const NAMES = {
  ...
  space: /\b(space center|space museum|planetarium|observatory|science (center|museum))\b/i,
  active: /\b(bowling|mini(ature)? golf|go[ -]?karts?|trampoline|skating rink|sports complex|adventure course)\b/i,
...
function railIdentity(place) {
  ...
  // may admit them.
  if (/\b(nude|nudist|adults? only|dog park|axe(?: throwing)?)\b/i.test(name)
      && !familyFilterFacts(place?.place_id || place?.id || "")) return null;
  ...
  if (primary === "amusement_center" && typesOf(place).some((type) => type === "amusement_park" || type === "theme_park")) return "attractions";
  ...
  for (const type of typesOf(place)) {
