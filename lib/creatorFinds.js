...
/**
 * SHOWN == SORTED, AND NOTHING SHOWS WITHOUT A SCORE. PURE.
 *
 * Owner, 2026-08-09: "make sure the Wayfind Score is always displayed in every
 * one of those cards, and that they are always displayed from highest to lowest
 * score."
 *
 * This row broke both halves of the score law at once. mergeCreatorInventory
 * returns pool rows first and then registry spots by CITY nearness — a sound
 * CANDIDATE order and a meaningless DISPLAY order, because the two halves are
 * ranked by different things entirely. And a registry spot whose Google lookup
 * had not landed rendered with no badge: the owner photographed P J's Sandwich
 * Shop sitting in the rail with no Score at all.
 *
 * One rule fixes both, and it is the rule the rest of the app already runs on:
 * a card carries a real Score or it is not a card.
 *
 *   · every entry resolves to ONE number, via the caller's scoreOf — the same
 *     function that draws the badge, so badge and position cannot disagree;
 *   · an entry with no number is HELD BACK. Not shown scoreless, and not given
 *     an invented one. A spot mid-hydration appears the moment its lookup
 *     lands; one Google cannot match never appears. That keeps "no match, no
 *     score" and the owner's rule in the same list rather than trading one off
 *     against the other;
 *   · then a plain descending sort, ties broken by review depth — the same
 *     tiebreak rankRows uses, so two 9.2s are not in arbitrary order.
 */
export function orderCreatorCards(inventory, scoreOf, depthOf) {
  const score = typeof scoreOf === "function" ? scoreOf : () => null;
  const depth = typeof depthOf === "function" ? depthOf : () => 0;
  return (Array.isArray(inventory) ? inventory : [])
    .filter(Boolean)
    .map((entry) => {
      let sc = null, dp = 0;
      try { sc = Number(score(entry)); } catch (e) { sc = null; }
      try { dp = Number(depth(entry)) || 0; } catch (e) { dp = 0; }
      return { entry, score: Number.isFinite(sc) ? sc : null, depth: dp };
    })
