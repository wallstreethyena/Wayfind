...
// v8.19 — THE FACILITY VETO (owner screenshot: the summer registry's
// "Coquina Beach" resolved to "Coquina Beach Parking" and a parking lot wore
// a 9.5 place card). Token-boundary containment treats "X" and "X Parking"
// as one venue, and they never are: the leftover suffix names a SUB-FACILITY
// of the venue, not the venue. If the extra tokens are facility words, the
// match is refused — the class, not the instance.
const FACILITY_RX = /\b(parking|garage|lot|restroom|bathroom|boat ramp|launch|lifeguard|tower|entrance|gate|ticket booth|box office|bus stop|trailhead|kiosk|pavilion #?\d*)\b/i;
export function sameVenueName(a, b) {
  ...
  if (longer.startsWith(shorter + " ") || longer.endsWith(" " + shorter)) {
    const rest = longer.startsWith(shorter + " ")
      ? longer.slice(shorter.length + 1)
      : longer.slice(0, longer.length - shorter.length - 1);
    return !FACILITY_RX.test(rest);
  }
  return false;
}
