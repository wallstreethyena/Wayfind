...
const CAPS = {
  text_pro: 4800,
  details_enterprise: 950,
  details_enterprise_atmosphere: 950,
  details_pro: 4800,
  photos: 950,
  nearby_pro: 4800,
  nearby_enterprise: 950,
  details_ids_only: 9500,
  autocomplete: 10000,
};

// Billing-class corrections must not mint a second allowance in the month they
// ship. Callers that historically debited the wrong SKU first debit that legacy
// row at a conservative 950 ceiling, then the real row at no more than 950.
// Therefore an already-exhausted legacy row still blocks the provider call even
// when the corrected row is new/empty. Both grants are atomic; a denial is
// fail-closed and may sacrifice ledger headroom, never create provider spend.
const SKU_TRANSITIONS = new Set([
  "text_pro:text_enterprise",
  "nearby_pro:nearby_enterprise",
  "details_pro:details_enterprise_atmosphere",
  "details_enterprise:details_enterprise_atmosphere",
]);
export async function spendAllowSkuTransition(legacySku, actualSku, actualCap = 950) {
  if (!SKU_TRANSITIONS.has(`${legacySku}:${actualSku}`) || legacySku === actualSku) return false;
  const legacyPublishedCap = CAPS[legacySku];
  const legacyCap = Math.min(950, Number.isFinite(legacyPublishedCap) ? legacyPublishedCap : 950);
  const correctedCap = Math.min(950, Math.floor(Number(actualCap)));
  if (!Number.isFinite(correctedCap) || correctedCap <= 0) return false;
  if (!(await spendAllowCapped(legacySku, legacyCap))) return false;
  return spendAllowCapped(actualSku, correctedCap);
}
...
export function effectiveCap(sku, cap) {
  ...
  if (mode === "free") {
    if (free) return Math.min(n, free);
    // Rich Text Search is a configured-only path, but its free-mode ceiling is
    // still the known 1,000-event Enterprise allowance with a 5% buffer.
    if (sku === "text_enterprise") return Math.min(n, 950);
    return CONFIGURED_CAP_SKUS.has(sku) ? n : null;
  }
  return n;
}
