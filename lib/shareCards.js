...
// ── THE ART LAYER IS GONE — 2026-08-12 ────────────────────────────────────
// SHARE_VISUALS, VISUAL_ALIASES, visualKey(), intentArtFor() and
// shareVisualFor() lived here to answer one question: which photograph does
// this share surface get? There is one share card now (app/api/og/card.jsx),
// the owner chose a typographic direction, and it draws no photograph at all —
// so nothing asks that question any more.
//
// Every art value in this file was already null after the deletion, which means
// the whole resolver — including the exact-key-beats-fuzzy-match rule that took
// a real bug to find — was a hundred lines of machinery for returning null. It
// is deleted rather than left exported, along with scripts/check-share-art-
// source.mjs, which proved that two null values agreed with each other.
//
// The `art: null` fields on the SHARE_CARDS rows above STAY. They are the
// record of a decision, and scripts/check-share-card-art.mjs asserts they can
// never hold a path again.
