...
// v6.72: delegates to the one source. This was a third independent ET-hour
// reader (siteTime.siteTodayStr, nowContext.siteHourFloat, and this). The
// integer floor is kept because MORNING_CUTOFF is an integer hour and callers
// compare against it; the tz parameter is honoured for the non-default case.
export function localHour(now = new Date(), tz = SITE_TZ) {
  if (tz === SITE_TZ) return Math.floor(siteHourFloat(now));
  try {
    const p = new Intl.DateTimeFormat("en-US", { timeZone: tz, hour: "2-digit", hourCycle: "h23" }).formatToParts(now); // one-clock-ok: honours an explicitly-passed non-default timezone; the default path delegates to siteHourFloat above
    return Number(p.find((x) => x.type === "hour").value);
  } catch { return Math.floor(siteHourFloat(now)); }
}
