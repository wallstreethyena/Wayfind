...
// Exported for consumers that must MATCH stored vector keys (applyAffinity's
// tag fold): vector values are written through norm(), so any reader that
// normalizes differently — e.g. plain toLowerCase, which leaves the
// underscores in "mexican_restaurant" — silently matches nothing. That is the
// dead-dimension class of bug this file's 2026-08-07 fix exists to end; one
// normalizer, both paths.
export const tasteNorm = norm;
...
/**
 * Does this user have enough learned taste for personalization to DO anything?
 * 2026-08-07 root-cause: the feed gated on `_vec.category` alone, and the
 * owner's own vector was 2 category dims + 17 tag dims — the toggle read as
 * dead because the discriminating dimensions never counted (or applied).
 * The gate now matches what applyAffinity actually consumes: any learned
 * category OR tag dim, or >=2 explicit session reactions.
 */
export function hasLearnedTaste(vec, activeSignalCount = 0) {
  if (Number(activeSignalCount) >= 2) return true;
  const v = vec || {};
  return Object.keys(v.category || {}).length > 0 || Object.keys(v.tag || {}).length > 0;
}
