...
// ── primary_type is a SERVING key, so it may never be blank or a broad parent ──
// v8.99 (2026-09-06). The first database canary run found 80 OPERATIONAL rows
// with NULL primary_type and 41 `beach` rows typed `park`/`tourist_attraction`.
// Both came straight out of this file: extractPlaceFields copies Google's
// `primaryType` verbatim, and Google omits it for many rides, galleries and
// beaches. The read path's allowlist-first category rule is keyed on
// primary_type, so an untyped row vanishes from the very category built to
// feature it (2026-08-20: The Ringling, Mote SEA — the BEST rows in a market),
// and a beach typed `park` drops out of the beach gate (Manatee Public Beach,
// 9,074 reviews). Normalising at ingest is the fix that survives every future
// seed, promote and scout run — the same three faults were repaired by hand on
// 2026-08-20 and came straight back with the next ingest.
//
// Rules, in order — never invents from a vibe:
//   1. a `beach` category whose primary is missing or a broad parent → `beach`
//      (the classifier's own verdict outranks the provider's superset)
//   2. Google's primaryType, when it gave one
//   3. the first SPECIFIC google type (not point_of_interest/establishment/…)
//   4. the category's canonical type — the honest "we know the list, not the
//      exact kind" answer, and still a value the category allowlists accept
const BROAD_PARENT = new Set(["tourist_attraction", "point_of_interest", "establishment", "park"]);
const GENERIC_TYPE = new Set(["point_of_interest", "establishment", "service", "food", "store", "health", "association_or_organization", "educational_institution", "event_venue"]);
const CANONICAL_PRIMARY = { food: "restaurant", nightlife: "bar", attractions: "tourist_attraction", beach: "beach", hotels: "lodging", shopping: "store" };
export function normalizePrimaryType(primary, types, category) {
  const pt = primary ? String(primary) : null;
  if (category === "beach" && (!pt || BROAD_PARENT.has(pt))) return "beach";
  if (pt) return pt;
  const specific = (Array.isArray(types) ? types : []).map(String).find((t) => !GENERIC_TYPE.has(t));
  if (specific) return specific;
  return CANONICAL_PRIMARY[category] || null;
}
...
export function buildInventoryRow(place, metroKey, { anchor = null, nowIso, adjudicatedSection = null } = {}) {
  ...
  if (anchor) {
    return {
      row: {
        ...
        primary_type: normalizePrimaryType(f.primary_type, f.google_types, anchor.category),
  ...
  const { category, tags, via } = classifyPlace(f.google_types, f.primary_type, f.name, adjudicatedSection);
  if (!category) return { row: null, reason: "unclassified (no type/name signal)", place: f };
  // A model-adjudicated category is UNTRUSTED for exactly the same reason a
  // name-recovered one is: it recovered an identity Google's structured fields
  // never asserted. Same review queue, same null last_verified_at, and a
  // distinct `source` so the provenance is legible in the row itself.
  const adjudicated = via === "adjudicated";
  const nameRecovered = via === "name";
  const untrusted = nameRecovered || adjudicated;
  return {
    row: {
      ...f, metro: metroKey, category, tags, anchor: false,
      primary_type: normalizePrimaryType(f.primary_type, f.google_types, category),
      source: adjudicated ? "scout_adjudicated" : "google_type", via,
      // Untrusted-provenance OR non-operational -> review queue, never silently trusted.
      needs_review: untrusted || !operational,
      last_verified_at: untrusted || !operational ? null : nowIso,
      signals: { rating: f.rating, reviews: f.reviews, price: f.price, priceNum: f.priceNum },
    },
    reason: null, nameRecovered, adjudicated, nonOperational: !operational,
  };
}
