...
export async function fetchPartnerInventory(city, intent, opts = {}) {
  ...
  const params = new URLSearchParams({ query: request.query, region: request.region, mode: "city", count: String(opts.count || 12) });
  params.set("q", request.query);
  params.delete("query");
  if (request.destId) params.set("destId", request.destId);
  const curatedParams = new URLSearchParams({ city: String(city || ""), intent: String(intent || "") });
  try {
    ...
    // curated call is caught separately rather than failing the pair.
    const exactPromise = doFetch("/api/viator/curated?" + curatedParams.toString())
      .then((r) => (r && r.ok ? r.json() : null))
      .catch(() => null);
    const r = await doFetch("/api/viator/tours?" + params.toString());
    const [j, exact] = await Promise.all([r && r.ok ? r.json() : null, exactPromise]);
    const items = (j && Array.isArray(j.items)) ? j.items : ((j && Array.isArray(j.tours)) ? j.tours : (Array.isArray(j) ? j : []));
    return mergePartnerInventory(items, exact && exact.items);
  } catch (e) {
    return [];
  }
}
