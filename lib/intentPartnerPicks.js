...
const LOCAL_INTENT_COPY = Object.freeze({
  food: Object.freeze({ query: "food tour culinary tasting cooking class", eyebrow: "A local food experience worth booking", reason: (city) => `A verified food-led experience in ${city} that belongs beside the local restaurant shortlist.` }),
...
// Restaurant surfaces have a narrower promise than the general date-night
// sheet. The owned experience cache is destination-scoped but intentionally
// unfiltered, so a Sarasota/Bradenton read can begin with kayaks. Search terms
// alone cannot be the serving gate: every candidate must carry positive food
// identity before it can become a restaurant-page card.
//
// Prefer explicit taxonomy when a provider supplies it, while retaining a
// conservative title fallback for the normalized cards currently returned by
// both /api/experiences and /api/viator/tours. The phrases are deliberately
// food-led ("food tour", "wine tasting", "cooking class"), rather than loose
// meal words such as lunch/dinner that commonly describe inclusions on an
// otherwise unrelated boat or kayak trip.
const FOOD_EXPERIENCE_PHRASES = Object.freeze([
  "food tour", "culinary tour", "culinary walk", "culinary experience",
  "cooking class", "cookery class", "chef experience", "chef's table",
  "food tasting", "wine tasting", "wine tour", "winery tour", "vineyard tour",
  "brewery tour", "beer tasting", "cocktail class", "mixology class",
  "dining experience", "restaurant tour",
]);

const FOOD_EXPERIENCE_TAXONOMY = /^(?:food(?:[-_ ]?(?:and|&)?)?[-_ ]?(?:drink|wine)|food[-_ ]?tour|culinary|cooking[-_ ]?class|wine[-_ ]?(?:tour|tasting)|dining[-_ ]?experience)$/i;

export function partnerInventoryEligibleForIntent(row, intent) {
  if (intent !== "food") return true;
  if (!row) return false;
  const taxonomy = []
    .concat(row.category || [], row.categories || [], row.tags || [], row.subcategory || [], row.subcategories || [])
    .map((value) => String(value || "").trim())
    .filter(Boolean);
  if (taxonomy.some((value) => FOOD_EXPERIENCE_TAXONOMY.test(value))) return true;
  const title = String(row.title || "").toLowerCase().replace(/[^a-z0-9'&]+/g, " ").replace(/\s+/g, " ").trim();
  return FOOD_EXPERIENCE_PHRASES.some((phrase) => title.includes(phrase))
    || /\bfood(?:\s+[a-z0-9'&]+){0,4}\s+tour\b/.test(title);
}
...
export function inventoryPartnerPick(city, intent, inventory) {
  ...
  const item = Array.isArray(inventory)
    ? inventory.find((row) => row && partnerInventoryEligibleForIntent(row, intent) && String(row.code || "").trim() && String(row.title || "").trim())
    : null;
...
export function resolvedIntentPartnerPicks(city, intent, inventory, limit = 4) {
  ...
  intentPartnerPicks(city, intent).filter((row) => partnerInventoryEligibleForIntent(row, intent)).forEach(add);
  if (cleanCity && cleanCity !== "your town" && copy && Array.isArray(inventory)) {
    for (const row of inventory) {
      if (chosen.length >= limit) break;
      if (!partnerInventoryEligibleForIntent(row, intent)) continue;
...
export async function fetchPartnerInventory(city, intent, opts = {}) {
  ...
  try {
    ...
    return qualifyPartnerInventory(merged).filter((row) => partnerInventoryEligibleForIntent(row, intent)).slice(0, plan.candidateCount);
  } catch (e) {
    return [];
  }
}
