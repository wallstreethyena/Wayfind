...
// v8.7: "sandwich(es)" joined the evidence list — a place NAMED a sandwich
// shop is counter-serve the same way a "subs"/"deli" name is (P J's Sandwich
// Shop, 0.3mi from the owner, was refused while carrying the promise in its
// own name). Table-service places do not name themselves after a sandwich.
const NAME_QUICK = /\b(express|drive[\s-]?thr(u|ough)|drive[\s-]?in|to[\s-]?go|takeout|take[\s-]?away|counter|shack|stand|truck|cart|walk[\s-]?up|window|deli|taqueria|pizzeria|slice|bagels?|donuts?|doughnuts?|juice|smoothie|bowls?|poke|wings|subs|sandwich(es)?|hoagie|cheesesteak|gyros?|shawarma|falafel|burritos?|tacos?|hot ?dogs?)\b/i;
...
/**
 * v8.18 — the STRONG form, for candidate sets that were not pre-targeted.
 * isQuickService() was tuned for rows arriving from a "quick casual eats"
 * SEARCH; run over raw inventory it over-admits, because Google hangs
 * `meal_takeaway`/`bakery`/`deli` as SECONDARY types on sit-down rooms and
 * supermarkets — executed against live Parrish inventory, plain
 * isQuickService admitted two Publix, a Walmart Supercenter and three grill
 * & pub dining rooms. The strong form keeps the same vetoes but only admits
 * on the PRIMARY type or the name evidence: what the place IS, not what it
 * also offers. Used by the rails' inventory-widened quickeats pool
 * (lib/railsData.js buildIdentityPool); /quick-bite keeps the plain form
 * over its targeted candidates.
 */
export function isStrongQuickService(r) {
  if (!isQuickService(r)) return false;
  const pt = String((r && (r.primaryType || r.primary_type)) || "").toLowerCase();
  if (QUICK_TYPES.has(pt)) return true;
  return NAME_QUICK.test(String((r && r.name) || "").toLowerCase());
}
