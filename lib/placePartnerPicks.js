...
// HOLD SKUs — same codes as lib/viatorDenylist.js. Inlined so the
// homepage chunk does not pull the denylist module. test-viator-integrity-lock
// CALLs the denylist and fails if a later pin uses a denied code.
function placePickIsLive(row) {
  if (!row) return false;
  if (row.provider !== "viator") return true;
  const id = String(row.offerId || "").trim().toUpperCase();
  return id !== "236862P2" && id !== "22211P1";
}
...
export function placePartnerPick(place) {
  ...
  // Denylisted Viator SKUs cannot become a Book pin, even if a later edit
  // puts them back in PLACE_PARTNER_PICKS. Comments are not a lock.
  if (!row || !placePickIsLive(row)) return null;
  return row;
}
...
export function nearbyTourListAllowed(place) {
  return !placePartnerPick(place);
}
