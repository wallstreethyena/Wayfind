...
export const PLACE_PARTNER_PICKS = Object.freeze([
  ...
  // RETIRED 343215P2 — catalogue link_ok=false, verified 2026-09-11.
...
// ── RETIRED PINS — absent products or confirmed dead catalogue links ─────
//
...
//
// UNKNOWN IS NOT DEAD. Entries are catalogue-absent unless their explicit
// reason records a confirmed dead link. A product whose health probe merely failed
// (403, timeout, rate limit) is unknown, not dead, and must never land here.
export const RETIRED_VIATOR_PINS = Object.freeze([
  { offerId: "343215P2", names: ["Rainbow Springs State Park"], reason: "retired-catalogue-link-dead", checkedAt: "2026-09-11" },
...
const RETIRED_CODES = new Map(RETIRED_VIATOR_PINS.map((r) => [String(r.offerId).toUpperCase(), r.reason || "retired-absent-from-catalogue"]));
...
export function pinServeability(row, catalog) {
  ...
  if (RETIRED_CODES.has(code)) return { serveable: false, reason: RETIRED_CODES.get(code) };
