...
export function drainPendingActions(placeId, run) {
  if (!placeId || typeof run !== "function") return 0;
  let q = null;
  try { q = typeof window !== "undefined" ? window[PENDING_ACTIONS_KEY] : null; } catch (e) { q = null; }
  if (!q || !q.length || typeof q.splice !== "function") return 0;
  const mine = [];
...
export function useActionBridge(placeId, run, ready) {
  ...
  useIsoLayoutEffect(() => {
    ...
    drainPendingActions(placeId, (action) => runRef.current(action));
    return undefined;
  }, [placeId, ready]);
  return rootRef;
}
