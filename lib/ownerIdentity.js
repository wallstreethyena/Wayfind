...
/**
 * Is this verified session the founder?
 *
 * Server-only. Either door is enough: the session email, or the session
 * user-id matching WF_OWNER_USER_ID. Used so the likes route can tell the
 * signed-in owner "this session is you" without putting the email or UUID
 * in the client bundle.
 */
export function isOwnerSession(sessionUser, envOwnerId) {
  if (!sessionUser || !sessionUser.id) return false;
  if (isOwnerEmail(sessionUser.email)) return true;
  const env = String(envOwnerId || "").trim();
  return !!(env && String(sessionUser.id).trim() === env);
}
