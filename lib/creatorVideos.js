...
const CURATED = [
  ...
  { key: "nueva-cantina-tampa", match: { name: "Nueva Cantina", city: "Tampa" },
    category: "Night out",
    // Downtown Tampa (903 N Franklin St) is not yet its own locked wf_inventory
    // row — no Google Places credentials in this session to resolve it — so
    // this entry keeps matching by name+city, which already reaches the
    // Downtown Tampa location. The reel below is the venue's own account and
    // covers all of Nueva Cantina's locations per its caption.
  ...
  // parent via the owner's browser. Only the certain matches ship here;
  // Rosallie Le French Cafe, Haraz House and All Fired Up need a fresh
  // wf_inventory resolution this session could not run (no Google Places
  // API credentials in this sandbox) and are held for a credentialed run.
