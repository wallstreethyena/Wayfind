// lib/birthdayUniverse.js — THE OWNER'S BIRTHDAY SEED LIST.
//
...
//
// v8.26 — NOT THE UNIVERSE. Serving only this shortlist at 45 miles is what
// put Bulla Gastrobar Tampa on the Birthday Plans rail from Parrish. The
// rail now ranks nearby wf_inventory (lib/birthdayPlace.js +
// lib/railsData.js buildBirthdayPool). Entries here are a SEED / editorial
// boost: a matching nearby inventory or ranked-pool row may carry the why
// line. They never become a card on their own, never hydrate via Place
// Details, and never stretch a market. Rail distance is BIRTHDAY_NEAR_MI.
//
// SAME FAIL-CLOSED RULE AS lib/summerUniverse.js: every entry is a REAL
// Google place; placeIds resolved 2026-08-18 from the permanent wf_place_ids
// index; unresolved entries ship placeId:null and seed nothing until
// scripts/resolve-birthday-place-ids.mjs fills lib/birthdayPlaceIds.js.
// Rank is CURATION order only — display order on every surface is the
// governed Wayfind Score, highest first (the owner's global rule).
//
...
// `destination: true` marks the owner's Orlando destination-birthday tier
// ("Orlando is where I would go if the goal is a destination birthday").
// It is a LABEL, not a wider radius — those entries seed only when the
// visitor is actually near them.
