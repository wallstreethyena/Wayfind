...
import { isGroceryMarket, isExcluded } from "./placeCategory.js";
...
export function composeSummerPickRails(places, tours, limit = SUMMER_PICK_LIMIT) {
  const placeRows = (Array.isArray(places) ? places : []).filter((place) => place?.id && place?.name && summerPlaceHasRealPhoto(place) && !isGroceryMarket(place) && !isExcluded(place));
