...
export function knownForLine(row) {
  ...
  const why = toSentences(row && row.why_here, MAX);
  const lead = (hook && isUsableCardHook(hook, name) ? hook : "")
    || (why && isUsableCardHook(why, name) ? why : "");
  if (!lead) return null;
  const tip = String((row && row.local_tip) || "").replace(/\s+/g, " ").trim();
  if (tip && isUsableCardHook(tip, name) && lead.length + 1 + tip.length <= MAX) return lead + " " + tip;
  return lead;
}
