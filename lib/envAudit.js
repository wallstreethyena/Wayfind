...
const OPTIONAL = [
  { keys: ["GOOGLE_MAPS_SERVER_KEY"], feature: "server Places proxy + shared cache (absent = owned inventory only)" },
  // Public MapKit JS token for the event-page venue map (#1144). Absent =
  // the Where block renders the address + Apple link with the map fallback.
  // Must be the NON-EXPIRING, domain-restricted kind — see
  // docs/apple-event-maps.md; /api/health/apple-maps reads its lifetime.
  { keys: ["NEXT_PUBLIC_APPLE_MAPS_TOKEN"], feature: "event-page Apple map (MapKit JS)" },
  ...
  // iOS push notifications (owner Gabe, 2026-09-23). Absent = every push send
  // in lib/apns.js degrades explicitly (apnsConfigured() false -> 503
  // "unconfigured" from app/api/push/test, jobCannotRun from
  // app/api/cron/weekend-picks) rather than throwing or silently no-opping.
  { keys: ["APNS_TEAM_ID"], feature: "iOS push notifications" },
  { keys: ["APNS_KEY_ID"], feature: "iOS push notifications" },
  { keys: ["APNS_AUTH_KEY"], feature: "iOS push notifications" },
  { keys: ["APNS_TOPIC"], feature: "iOS push notifications" },
  { keys: ["APNS_ENV"], feature: "iOS push notifications" },
  // Weekend Picks push (app/api/cron/weekend-picks) — deliberately absent by
  // default. WEEKEND_PICKS_PUSH_ENABLED must be the literal "1" or the cron
  // returns {skipped:"disabled"} before touching the database or APNs at
  // all; this is a product safety gate (never mass send unless explicitly
  // turned on), not a missing-integration warning, so it is listed here for
  // visibility only, never treated as required.
  { keys: ["WEEKEND_PICKS_PUSH_ENABLED"], feature: "Weekend Picks push" },
  { keys: ["WEEKEND_PICKS_ALLOWLIST"], feature: "Weekend Picks push" },
  // Sign in with Apple revoke on account deletion (owner Gabe, 2026-09-23).
  // Absent = lib/appleAuth.js's appleConfigured() is false, and
  // app/api/account/delete/route.js records revoked:"skipped_unconfigured"
  // rather than throwing — account deletion itself never depends on these.
  { keys: ["APPLE_TEAM_ID"], feature: "Sign in with Apple revoke (account deletion)" },
  { keys: ["APPLE_SIWA_KEY_ID"], feature: "Sign in with Apple revoke (account deletion)" },
  { keys: ["APPLE_SIWA_PRIVATE_KEY"], feature: "Sign in with Apple revoke (account deletion)" },
  { keys: ["APPLE_SIWA_CLIENT_ID"], feature: "Sign in with Apple revoke (account deletion)" },
];
...
export const VALUE_OVERRIDES = [
  ...
  {
    key: "WF_ALERT_FROM",
    fallback: "Wayfind Alerts <alerts@gowayfind.com>",
    shape: /^[^<>@]*<?[^\s@<>]+@[^\s@<>]+\.[a-z]{2,}>?$/i,
    feature: "Resend alert/digest sender identity",
