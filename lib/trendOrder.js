...
//
// It is a SHADOW value. It has not been validated against a real snapshot,
// because rights are unconfirmed and no real snapshot has been imported. It must
// not be raised without the shadow report the spec requires.

import { snapshotFreshness, mayInfluencePublicRanking } from "./trendRights.js";
...
export function trendOrderBoost(input) {
  const {
    ...
    rightsMode, shadow = false, manualState = null,
  } = input || {};
  ...
  // 1. LICENCE. A boost that reaches a user is a public ranking signal, and
  //    that needs commercial_approved. Shadow mode computes the number for the
  //    owner's report without it ever reaching a reader — that is the whole
  //    point of shadow mode, and it is how this system can be evaluated before
  //    it is licensed.
  if (!shadow && !mayInfluencePublicRanking(rightsMode)) {
    return zero(`rights mode "${rightsMode}" does not permit influencing public ranking — boost is zero (shadow evaluation is still available)`);
  }

  // 2. Manual override. An owner denial outranks every computed factor.
  if (manualState === "deny") return zero("manually denied by the owner");

  // 3. FRESHNESS. Stale ⇒ zero boost AND (elsewhere) no label. One fact.
  ...
  // 5. Strength must be a real 0–1. Out-of-range is a BUG upstream, and clamping
