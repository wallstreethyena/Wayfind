...
function loadOnce(key, loader) {
  ...
  const held = { pending: null, started: now, loaderStarted: null, loaderMs: null };
  const pending = Promise.resolve().then(() => {
    held.loaderStarted = Date.now();
    return loader();
  }).finally(() => {
    if (held.loaderStarted != null) held.loaderMs = Math.max(0, Date.now() - held.loaderStarted);
...
function safeTimingName(name) {
  const value = typeof name === "string" ? name : "";
  return /^[a-z][a-z-]{0,47}$/.test(value) ? value : "rail-answer";
}

function logTiming(started, name, cacheState, outcome, extra = {}) {
  try {
    console.info("[rail-answer] timing", {
      name: safeTimingName(name),
      cacheState,
      outcome,
      elapsedMs: Math.max(0, Date.now() - started),
      ...extra,
    });
  } catch (_) { /* telemetry must never affect the rail answer */ }
}
...
export async function fastCachedRail(key, loader, { name = "rail-answer", usable = Boolean } = {}) {
  ...
  if (good && Date.now() - Number(held.savedAt || 0) < FRESH_MS) {
    logTiming(started, name, "hit", "returned", { usable: true });
    return { value: held.value, state: "hit" };
  }
  if (good) {
    const refresh = loadOnce(key, loader).pending
    ...
    logTiming(started, name, "stale", "returned", { usable: true });
    return { value: held.value, state: "stale" };
  }
  const load = loadOnce(key, loader);
  const build = load.pending.then((value) => {
    const isUsable = !!usable(value);
    if (isUsable) scheduleWrite(key, value, name);
    return { answer: { value, state: "miss" }, loaderMs: load.loaderMs, usable: isUsable };
  });
  ...
  const delayedHit = read.then((entry) => valid(entry)
    ? { answer: { value: entry.value, state: "late-hit" }, usable: true }
    : new Promise(() => {}));
  try {
    const result = await Promise.race([build, delayedHit]);
    const extra = { usable: result.usable };
    if (Number.isFinite(result.loaderMs)) extra.loaderMs = result.loaderMs;
    logTiming(started, name, result.answer.state, "returned", extra);
    return result.answer;
  } catch (error) {
    const extra = {};
    if (Number.isFinite(load.loaderMs)) extra.loaderMs = load.loaderMs;
    logTiming(started, name, "miss", "failed", extra);
    throw error;
  }
}
