...
export function composeWorthEatingRails(places) {
  ...
  const rank = (a, b) => (wayfindScore(b.rating, b.reviews) - wayfindScore(a.rating, a.reviews)) || ((a.distMi ?? 99) - (b.distMi ?? 99));
  return WORTH_EATING_RAILS.map((rail) => ({ ...rail, places: buckets.get(rail.id).sort(rank) }));
}
