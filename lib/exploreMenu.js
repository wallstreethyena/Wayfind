// lib/exploreMenu.js — the "Explore near you" home menu (v5.84; tiles v6.37).
//
// Four tiles, fixed benefit copy (NO live stats/claims — this replaces the
...
//
// v6.37 — "Shop Local" retired (owner call: the tile under-performed and its
// results were unreliable). Its slot became "Order In" (Uber Eats delivery).
// 2026-08-26 — "Order In" retired WITH Uber Eats (owner directive: untracked
// affiliate handoff, deleted permanently; see lib/affiliates.js REMOVED note).
// The menu is four tiles now; do not re-add a delivery tile without a tracked
// provider behind /api/commerce/go.
...
const ORDER_DAY = ["today", "food", "stay", "night"];
const ORDER_EVENING = ["today", "night", "food", "stay"];
...
// The four tile keys in display order.
...
export function orderExploreMenu(now, tzOffsetMinutes) {
  ...
  if (typeof tzOffsetMinutes === "number" && isFinite(tzOffsetMinutes)) {
    ...
    minutes = loc.getUTCHours() * 60 + loc.getUTCMinutes(); // one-clock-ok: reads a date already shifted by a REAL venue tz offset, which is more precise than the site default
  } else {
    // v6.72: venue-local ET, not the device clock. The tzOffsetMinutes branch
    // above is preferred and unchanged — it uses a REAL nearby-place offset,
    // which is more precise than the site default.
    minutes = Math.round(siteHourFloat(now) * 60);
  }
