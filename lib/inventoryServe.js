...
//
// 2026-09-23 — no longer also the exhaustive bulk read's field list (see
// EXHAUSTIVE_INVENTORY_FIELDS below, added in the same day's fix round):
// lib/inventoryBoxBatch.js and lib/familyDayData.js are the callers this
// constant still serves, byte-for-byte unchanged.
...
// 2026-09-23 (fix round) — the EXHAUSTIVE bulk read's OWN field list, WITHOUT
// `photo_ref`. Measured by lib/ownedPool.js's header: photo_ref was ~59% of a
// 1,000-row page's bytes (Parrish food, page 0: 1,000 rows = 970 KB, photo_ref
// alone 469 KB), and nothing in admission — no predicate, no radius law, no
// servability gate — ever reads it. The exhaustive read can run to thousands
// of rows (Tampa food: 4,482+) before the rank cap throws almost all of them
// away, so hauling every row's photo reference here paid that exact cost a
// second time, for a file whose own header already names ownedPool's fix as
// the model to follow. Hydrated back AFTER ranking, for the SERVED rows only
// — see the hydratePhotoRefs call in serveFromInventoryUncached below, which
// reuses lib/ownedPool.js's existing hydratePhotoRefs (the same "admit before
// the cost, hydrate after" pattern, not a second copy of it). This is a
// SEPARATE constant from LEAN_INVENTORY_FIELDS on purpose: inventoryBoxBatch.js
// and familyDayData.js import that one directly and must stay byte-for-byte
// unaffected by this split.
export const EXHAUSTIVE_INVENTORY_FIELDS = "place_id,name,lat,lng,category,secondary_categories,primary_type,google_types,cuisines,status,excluded,signals";
...
//
// 2026-09-23 (fix round, item 8) — THE RETRY MUST NOT OVERSHOOT THE TOTAL
// BUDGET. This used to reuse the page's own `ms` unchanged for the retry, so
// a page whose first attempt consumed most of its allotment before failing
// could spend up to a SECOND full `ms` on the retry — on top of what earlier
// pages already spent — overshooting INVENTORY_SERVE_TOTAL_BUDGET_MS/
// options.totalDeadlineMs, the very ceiling `deadlineAt` exists to enforce.
// `deadlineAt` (the same absolute timestamp readOwnedCategory's own
// pageBudget() clamps against) is threaded in here so the retry gets AT MOST
// whatever is left of the total budget, never the page's original ceiling.
async function fetchPageWithRetry(doFetch, url, init, ms, deadlineAt) {
  const remainingMs = (fallback) => {
    if (!Number.isFinite(deadlineAt)) return fallback;
    const left = deadlineAt - Date.now();
    return Math.max(0, Math.min(fallback, left));
  };
  let res;
  try {
    res = await doFetch(url, init, ms);
  } catch {
    const retryMs = remainingMs(ms);
    if (retryMs <= 0) throw new Error("Inventory page retry had no total budget left");
    return doFetch(url, init, retryMs); // one retry, clamped; a second failure propagates
  }
  if (res && res.ok) return res;
  if (res && res.status >= 500) {
    const retryMs = remainingMs(ms);
    if (retryMs <= 0) return res; // no budget left for a retry — surface the failed response as-is
    try { return await doFetch(url, init, retryMs); } catch { return res; }
  }
  return res;
}
...
async function readExhaustiveRows(s, physical, cat, virtual, box, options) {
  const doRead = async () => {
    ...
    const result = await readOwnedCategory(s, physical, box, {
      fields: EXHAUSTIVE_INVENTORY_FIELDS,
      primaryOnly: options.primaryOnly,
      pageSize,
      maxRows,
      deadlineMs: perPageMs,
      deadlineAt,
      fetchImpl: (url, init, ms) => fetchPageWithRetry(baseFetch, url, init, ms, deadlineAt),
    });
  ...
  if (options.readCache && typeof options.readCache.get === "function") {
    ...
    const key = `rows:${physical}:${virtual ? cat : "-"}:${box.minLat.toFixed(4)},${box.maxLat.toFixed(4)},${box.minLng.toFixed(4)},${box.maxLng.toFixed(4)}:${options.primaryOnly ? 1 : 0}:leanNoPhoto`;
...
async function serveFromInventoryUncached(cat, physical, virtual, lat, lng, radiusM, n, sub, options, s) {
  ...
  // 2026-09-23 (fix round, item 7) — photo_ref, hydrated AFTER the rank, for
  // the SERVED rows only — see EXHAUSTIVE_INVENTORY_FIELDS above for why the
  // bulk read no longer carries it. Reuses lib/ownedPool.js's hydratePhotoRefs
  // (the SAME "admit before the cost, hydrate after" pattern that file's
  // header describes) against synthetic {place_id, photo_ref:null} rows built
  // from the served places, rather than a second implementation of "how a
  // served row gets its photo back". Every served place object ends up in the
  // exact same shape invRowToPlace already produced when photo_ref rode along
  // on the box read — `photos: [{name}]` plus a `photo_ref` mirror — so this
  // is invisible to every existing caller. Unconditional (not gated by
  // options.skipEditorial): skipEditorial only ever meant "the caller hydrates
  // EDITORIAL text itself"; every caller, skipEditorial or not, has always
  // received photo_ref on served rows, and this keeps that true.
  // photosIncomplete (2026-09-23 re-audit): a hydration request that FAILED
  // (not a place that simply has no photo) is reported in meta, so the route
  // can refuse to edge-cache a page whose photos went missing for a moment.
  let photosIncomplete = false;
  if (served.length) {
    try {
      const { hydratePhotoRefs } = await import("./ownedPool.js"); // lazy: matches this file's own convention
      const photoRows = served.map((p) => ({ place_id: p.id, photo_ref: null }));
      const photoStats = await hydratePhotoRefs(s, photoRows, {
        fetchImpl: options.fetchImpl,
        deadlineMs: Number.isFinite(options.deadlineMs) && options.deadlineMs > 0 ? options.deadlineMs : DB_DEADLINE_MS,
      });
      const photoById = new Map(photoRows.map((r) => [r.place_id, r.photo_ref]));
      for (const place of served) {
        const ref = photoById.get(place.id);
        if (ref) { place.photos = [{ name: ref }]; place.photo_ref = ref; }
      }
      if (photoStats && photoStats.failed > 0) photosIncomplete = true;
    } catch {
      // photo_ref is enrichment, never a reason to blank a served row — but it
      // is a reason not to let a CDN hold this photo-less page for a day.
      photosIncomplete = true;
    }
  }

  if (options.withMeta) {
    return { places: served, meta: { eligible, served: served.length, offset, truncated, pages, photosIncomplete } };
  }
  return served;
}
