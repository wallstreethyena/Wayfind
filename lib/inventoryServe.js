...
export function invRowToPlace(r) {
  ...
  const out = {
    id: r.place_id,
    displayName: { text: placeDisplayName(r, r.name) },
...
export async function serveFromInventory(cat, lat, lng, radiusM, n, sub, options = {}) {
  ...
  if (options.readCache && typeof options.readCache.get === "function") {
    const key = `srv:${physical}:${Number(lat).toFixed(4)}:${Number(lng).toFixed(4)}:${radiusM}:${n}:${subIdEarly}:${options.primaryOnly ? 1 : 0}:${options.skipEditorial ? 1 : 0}`;
...
async function serveFromInventoryUncached(cat, physical, virtual, lat, lng, radiusM, n, sub, options, s) {
  ...
  const _box = boxForRadius(lat, lng, radiusM);
  const box = `&lat=gte.${_box.minLat.toFixed(4)}&lat=lte.${_box.maxLat.toFixed(4)}`
    + `&lng=gte.${_box.minLng.toFixed(4)}&lng=lte.${_box.maxLng.toFixed(4)}`;
  // Fetch only fields a card or identity contract can consume. The prior
  // select=* transferred operational metadata and large enrichment columns
  // on every reader request, multiplying the cost across concurrent rails.
  //
  // WO8 (2026-09-02) — options.skipEditorial drops `editorial` from a BULK
  // pool read. `editorial` is a long free-text column and this read is
  // limit=1000: measured, up to ~1MB of it crosses the wire for rows that are
  // then thrown away by rankInventory's own top-N cut — of a 1000-row box read
  // maybe 20 rows ever ship. The caller that opts in (railsData.js's pool
  // builders) re-hydrates editorial with ONE place_id=in.(...) read for only
  // the rows that actually made a shipped rail — see hydrateEditorialFor
  // below. Default (no option) is BYTE-FOR-BYTE the prior behavior: every
  // other caller of serveFromInventory (category pages during a Google
  // outage, etc.) is unaffected.
  const fields = options.skipEditorial
    ? LEAN_INVENTORY_FIELDS
  ...
  const base = `${s.url}/rest/v1/wf_inventory?select=${fields}&limit=1000${box}`;
  ...
  const deadlineMs = Number.isFinite(options.deadlineMs) && options.deadlineMs > 0
    ...
    : DB_DEADLINE_MS;
  try {
    let r = await fetchDeadline(options.primaryOnly ? plain : withSecondary, { headers: h, cache: "no-store" }, deadlineMs);
    if (!options.primaryOnly && !r.ok) r = await fetchDeadline(plain, { headers: h, cache: "no-store" }, deadlineMs); // pre-migration
    if (!r.ok) {
      if (options.failLoud) throw new Error(`Inventory ${physical} read returned ${r.status}`);
      return [];
    }
