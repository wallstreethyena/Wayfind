...
const REGION_COORDS = {
  ...
  // ⏳ SEASONAL (sunset 2026-08-30): regions for the summer-2026 city guides.
  // Coords only — REGION_CITY stays untouched, so every inventory-backed tier
  // still fails soft for these; this just turns on the ISR weather block.
  Miami: { lat: 25.76, lng: -80.19 },
  "Fort Lauderdale": { lat: 26.12, lng: -80.14 },
  "Key West": { lat: 24.56, lng: -81.78 },
  "St. Augustine": { lat: 29.90, lng: -81.31 },
  "Daytona Beach": { lat: 29.21, lng: -81.02 },
  "Panama City Beach": { lat: 30.18, lng: -85.80 },
...
export async function guideWeather(region, revalidateSeconds = 900) {
  ...
  if (!c) return null;
  // `next build` must not wait on Open-Meteo. ISR fills this in later.
  if (process.env.NEXT_PHASE === "phase-production-build") return null;
  try {
    const { fetchDeadline, NET_DEADLINE_MS } = await import("./fetchDeadline.js");
    ...
    const r = await fetchDeadline(u, { next: { revalidate: revalidateSeconds } }, NET_DEADLINE_MS);
    if (!r.ok) return null;
    return await r.json();
  } catch (e) {
    return null;
