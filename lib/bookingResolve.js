...
export function bookingTargets(detail, kind, topItem, locName, opts) {
  ...
  // Hotels leave through the same tracked server path on every surface.
  const hotelTarget = Aff.hotelGoUrl(detail, bcity);
  if (hotelTarget) return { verifiedUrl: null, goFallback: null, tk: null, tu: hotelTarget, fallbackSuppressed: null };
  const tu = tk;
