...
export const ESTABLISHED_CREATOR_FOLLOWER_MIN = 10000;
...
export function creatorQualified(creator, platform, handle, now) {
  ...
  const creatorHandle = String(creator?.handle || "").trim().replace(/^@/, "").toLowerCase();
  const postHandle = String(handle || "").trim().replace(/^@/, "").toLowerCase();
  return !!(creator && creator.platform === platform && creatorHandle && creatorHandle === postHandle
...
export function qualifySocialPost(post, { creator = null, now } = {}) {
  ...
  const reviewed = creatorQualified(creator, post?.platform, post?.handle, now);
  const followers = observedCount(post?.creator_follower_count);
  // Follower qualification is accepted only from Meta Business Discovery,
  // where the count and handle arrive together from the official API. A value
  // copied from a caption, hashtag result or model output cannot qualify anyone.
  const established = post?.platform === "instagram" && post?.source === "business_discovery"
    && followers !== null && followers >= ESTABLISHED_CREATOR_FOLLOWER_MIN;
  const trusted = reviewed || established;
  const base = { policy: SOCIAL_POLICY_VERSION, evidence, likes, creator_followers: followers,
    creator_qualified: trusted, creator_qualification: reviewed ? "reviewed_registry" : established ? "official_followers" : null,
    publish: false };
  ...
  return { ...base, eligible: true,
    reason: reviewed ? "qualified_creator" : established ? "established_creator_followers" : "likes_over_1000",
    next: "verify_florida_destination" };
}
...
export async function safeSocialJson(url, { fetcher = fetch, timeoutMs = 8000, headers = {} } = {}) {
  try {
    const response = await fetcher(url, { cache: "no-store", headers, signal: AbortSignal.timeout(timeoutMs) });
