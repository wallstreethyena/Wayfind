...
// carried +0.7 for a creator video, −0.2 past 17 miles and +0.6 trending, none
...
export function gateOutdoor(places, now) {
  if (!now || now.outdoorOK) return (places || []).filter(Boolean);
  return (places || []).filter(Boolean).filter((p) => {
