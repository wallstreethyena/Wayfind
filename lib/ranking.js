...
// First-party member signal. Silent below 3 distinct authors so one person
// can never move a score; capped under a point so Google-scale evidence still
// anchors the number. Warnings pull down, everything else pushes up.
export function memberDelta(sig) {
  if (!sig || !sig.authors || sig.authors < 3) return 0;
  const up = Math.min(0.75, 0.15 * sig.authors);
