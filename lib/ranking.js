...
const KW = {
  ...
  // v7.26 — `amusement` was a SUBSTRING and it caught `amusement_center`, which
  // is the arcade / escape-room / family-entertainment type, not a theme park.
  // Found by diffing this classifier against 80 real DB rows before and after
  // v7.22: "The Great Escape Room St Pete" and "Dare 2 Escape" were being
  // SUPPRESSED on a thunderstorm day — an escape room is the single best
  // rainy-afternoon answer this app has, and the gate was deleting it. The
  // mirror error rode along with it: `roller_coaster` matched nothing and an
  // outdoor coaster survived the same storm.
  //
  // `mobile_home_park` is excluded for the same substring reason — it matched
  // `park` and read as outdoor recreation, which a residential co-op is not.
  outdoor: ["amusement_park", "theme_park", "roller_coaster", "zoo", "stadium", "campground", "golf", "botanical", "preserve", "nature", "bridge", "adventure_sports", "hiking", "trail", "garden", "playground", "park"],
  ...
  // v7.26 — the indoor half of the amusement fix. These are the types a reader
  // wants BECAUSE it is raining or 103°, so getting them wrong is expensive in
  // both directions: suppressed when they are the answer, and never boosted
  // when the gate shuts. `amusement_center` covers arcades, escape rooms and
  // family entertainment centres; it must be matched BEFORE the outdoor list,
  // which venueLean already does (indoor is tested first).
  indoor:  ["museum", "art_gallery", "aquarium", "movie", "theater", "theatre", "performing_arts", "bowling", "spa", "casino", "library", "church", "place_of_worship", "gym", "fitness", "hall", "convention", "school", "university", "hospital", "amusement_center", "arcade", "escape_room", "trampoline", "indoor", "planetarium", "observatory"],
  // Lodging is neither indoor recreation nor an outdoor outing — it is a place
  // to sleep, and it must not be weather-gated as if it were a day out.
  lodging: ["mobile_home_park", "rv_park", "lodging", "hotel", "motel", "inn", "resort_hotel", "bed_and_breakfast", "extended_stay"],
};
...
export function venueLean(place) {
  ...
  const observation = /\bobservatory\b|\bobservation deck\b|\bobservation tower\b/.test(name) || hasKw(ts, ["observation_deck", "observation_tower"]);
  ...
  // Confidence gate: incidental cafe/restaurant types on an observatory are
  // not a coffee/indoor-food read (One World Observatory).
  if (foodBarShop && !observation) return { lean: "indoor", water: false };
  // v7.26 — LODGING IS NOT AN OUTING. Before the outdoor list, because
  // `mobile_home_park` and `rv_park` both substring-match `park` and were
  // reading as outdoor recreation. Neutral, not indoor: a resort is not an
  // indoor answer to bad weather either.
  if (hasKw(ts, KW.lodging)) return { lean: "neutral", water: false };
...
export function coarseCat(place) {
  ...
  const ts = typeList(place);
  const name = ((place && (place.name || place.title)) || "").toLowerCase();
  if (/\bobservatory\b|\bobservation deck\b|\bobservation tower\b/.test(name) || hasKw(ts, ["observation_deck", "observation_tower"])) return "Activities";
