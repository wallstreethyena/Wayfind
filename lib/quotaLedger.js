...
function emptyDay() { return { spent: { searchNearby: 0, searchText: 0 }, reservations: {}, observed429: {} }; }
...
export function read(now = Date.now()) {
  ...
  return { day, spent: { ...emptyDay().spent, ...rec.spent }, reservations: live, observed429: rec.observed429 || {}, corrupt: false };
}
...
function write(rec, now = Date.now()) {
  ...
  const payload = { [rec.day]: { spent: rec.spent, reservations: rec.reservations, observed429: rec.observed429 || {} } };
...
// ── CAP-DRIFT DETECTION (#461 interim) ────────────────────────────────────
//
// CAPS is a hardcoded record of what was set in the console. It can be wrong:
// the change may not have applied, another key on the same GCP project shares
// the quota (it is per-PROJECT, not per-key), or somebody changed it and did not
// update WF_CAP_*. A wrong cap makes the ledger CONFIDENTLY wrong — it grants
// against headroom that does not exist, and the green preflight then vouches for
// the 429 that follows. That is worse than no preflight.
//
// Reading the true limit needs Cloud Monitoring and credentials the repo does
// not have (#461). This is the interim that needs neither: when Google 429s, the
// call count at which it fired is EVIDENCE about the real cap, and we were going
// to eat that 429 anyway. Record it and clamp the rest of the day to it.
//
// The clamp is a LOWER BOUND, not a measurement: our observed count is this
// machine's spend, and another key on the project may have spent more. So the
// real cap is AT MOST what we observed — clamping down is the safe direction.
export function effectiveCap(sku, rec = read()) {
  const obs = rec.observed429 && rec.observed429[sku];
  const seen = obs && Number.isFinite(obs.calls) ? obs.calls : null;
  return seen == null ? CAPS[sku] : Math.min(CAPS[sku], seen);
}

/** Warnings where an observed 429 contradicts the configured cap. */
export function capWarnings(rec = read()) {
  const out = [];
  for (const sku of Object.keys(CAPS)) {
    const obs = rec.observed429 && rec.observed429[sku];
    if (!obs || !Number.isFinite(obs.calls)) continue;
    if (obs.calls < CAPS[sku]) {
      out.push(`CAP DRIFT ${sku}: Google 429'd after ~${obs.calls} calls today but WF_CAP says ${CAPS[sku]}.` +
        ` The real cap is AT MOST ${obs.calls} — off by >=${CAPS[sku] - obs.calls}.` +
        ` Today's effective cap is clamped to ${obs.calls}. Fix the configured cap, or another key on the project is sharing this quota.`);
    }
  }
  return out;
}
...
/**
 * Record that Google 429'd on a SKU, with our best estimate of the call count at
 * which it fired. Estimate = ledger spend so far + this sweep's calls; it is a
 * lower bound because other keys on the project are invisible to us.
 */
export function noteQuotaExhausted(sku, observedCalls, now = Date.now()) {
  const rec = read(now);
  if (rec.corrupt) return { recorded: false, why: "ledger corrupt" };
  if (!Object.prototype.hasOwnProperty.call(CAPS, sku)) return { recorded: false, why: `unknown sku ${sku}` };
  const calls = Math.max(0, Number(observedCalls) || 0);
  const prev = rec.observed429[sku];
  // Keep the LOWEST observation — the tightest evidence about the real ceiling.
  if (!prev || calls < prev.calls) rec.observed429[sku] = { calls, at: now, capAtTime: CAPS[sku] };
  write(rec, now);
  return { recorded: true, sku, calls, effectiveCap: effectiveCap(sku, read(now)), warnings: capWarnings(read(now)) };
}
...
export function remaining(rec = read()) {
  ...
  return {
    searchNearby: Math.max(0, effectiveCap("searchNearby", rec) - c.searchNearby),
    searchText: Math.max(0, effectiveCap("searchText", rec) - c.searchText),
  };
}
...
export function reserve(req, now = Date.now()) {
  ...
  if (short.length) {
    return {
      ...
      detail: [
        ...short.map((sku) => `${sku}: needed ${calls[sku]}, ${avail[sku]} remaining (cap ${effectiveCap(sku, rec)}${effectiveCap(sku, rec) !== CAPS[sku] ? ` — CLAMPED from ${CAPS[sku]} by an observed 429` : ""}, spent ${rec.spent[sku]}, reserved by others ${committed(rec)[sku] - rec.spent[sku]})`),
        ...capWarnings(rec),
      ],
...
export function describe(rec = read()) {
  ...
  return [
    ...
    ...Object.keys(CAPS).map((sku) => `  ${sku.padEnd(13)} cap ${String(effectiveCap(sku, rec)).padStart(5)}${effectiveCap(sku, rec) !== CAPS[sku] ? ` (CLAMPED from ${CAPS[sku]})` : ""}  spent ${String(rec.spent[sku]).padStart(5)}  reserved ${String(c[sku] - rec.spent[sku]).padStart(5)}  remaining ${String(rem[sku]).padStart(5)}`),
    `  outstanding reservations: ${Object.keys(rec.reservations).length || "none"}`,
    ...capWarnings(rec).map((w) => `  *** ${w}`),
  ].join("\n");
}
