...
// try to be shareLink(): no Capacitor branch, no rating hook.
//
// "No Capacitor branch" used to be justified with "a guide is never inside the
// native wrapper". That stopped being true: the iOS app is a remote-URL shell
// that opens EVERY page, and a push notification lands readers straight on
// these ones. It is still the right call, for a measured reason rather than an
// assumed one: inside the app WKWebView implements navigator.share by
// presenting the real iOS share sheet (UIActivityViewController). Verified in
// the iOS 26.5 simulator on 2026-09-23 by a UI test that opened a guide from a
// notification, tapped its Share button, found the system sheet's Copy action
// and read the shared URL back off the pasteboard (destination share audit).
// If that ever regresses, add the Capacitor branch HERE, once, rather than in
// each page.
//
// What it DOES have to match is the one thing that is easy to get wrong and
