...
let chooserModulePromise = null;
export function queueShareChooser(payload, onCopied, loadChooser) {
  if (typeof document === "undefined" || !document.body) return false;
  const load = typeof loadChooser === "function" ? loadChooser : () => import("./shareChooser.js");
  chooserModulePromise ||= load();
  const pending = chooserModulePromise;
  pending.then(
    (module) => { module.openShareChooser(payload, onCopied); },
    () => { if (chooserModulePromise === pending) chooserModulePromise = null; },
  );
  return true;
}
    ...
    try {
      ...
      if (pr && typeof pr.then === "function") {
        ...
        pr.then(function () {}, function (e) {
          ...
          queueShareChooser(p, onCopied);
        });
      }
      return "native";
    } catch (e) {
      if (queueShareChooser(p, onCopied)) return "chooser";
      return "failed";
    }
  }
  if (queueShareChooser(p, onCopied)) return "chooser";
