...
import { commerceHref } from "./commerce";
...
export async function PlacePage({ id }) {
  ...
  const partner = placePartnerPick({ name: p.name, id });
  const bookHref = partner
    ? commerceHref({ provider: partner.provider, offerId: partner.offerId, surface: "place_page", contentId: id })
    : null;
  ...
  return (
    <div style={S.wrap}>
      ...
      <div>
        {bookHref ? (
          <a href={bookHref} rel="sponsored noreferrer" style={S.cta}>Tickets · {partner.merchant} ↗</a>
        ) : null}
        <a href={openHref} style={bookHref ? S.cta2 : S.cta}>Open in Wayfind →</a>
