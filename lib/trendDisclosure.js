 ...
 *
 * Returns null — meaning render NOTHING, not a placeholder — when freshness or
 * the match do not support a label. A stale snapshot removes the
 * label by the same code path that removes the boost, so the two can never
 * disagree on screen.
 */
export function trendLabel(match, opts) {
  const { form = "rising" } = opts || {};
