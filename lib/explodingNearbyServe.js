...
//   · The owner list is the FLOOR. A snapshot-path failure still falls through
//     to it, and readable inventory may return cards or an honest empty.
//   · Missing/invalid Supabase configuration or a failed inventory read is an
//     operator failure, never an inventory-empty product state.
//   · A true empty (metro unsupported, or no verified local match) is
//     honest empty — never "temporarily unavailable".
//   · That sentence is lawful only when the owner list itself is gone or its
//     required inventory read was attempted and failed.
...
export const PARTIAL_UNAVAILABLE_COPY = "Some trend details couldn’t load. The ranked places below are still available.";
...
/**
 * Paint law for the homepage rail. A failed primary walk consults the owner
 * list before it can paint an error. Once that floor request itself fails,
 * though, the failure must stay distinguishable from a verified empty result.
 * Execute this; do not re-derive it from JSX.
 */
export function explodingUiStatus({ status, trends, error, ownerFloorFailed = false, universe = EXPLODING_NEARBY_UNIVERSE } = {}) {
  ...
  // The in-repo list is a fallback opportunity, not proof that its backing
  // inventory was readable. Only the caller that actually attempted the
  // floor may set this flag. Without it, the historical owner-list fail-soft
  // remains intact and the floor still gets its chance to return cards or an
  // honest 200/no_verified_inventory response.
  if (ownerFloorFailed) {
    return {
      status: status === "trend_configuration_error" ? status : "trend_data_error",
      trends: [],
...
/** The specialized module stays silent when the parent drop already has cards. */
export function shouldPaintExplodingEmpty({ status, hasRankedFallback = false } = {}) {
  return status === "no_verified_inventory" && !hasRankedFallback;
}

/** Keep provider/configuration details in the API response, not reader copy. */
export function explodingUiErrorCopy({ error, ownerFloorFailed = false, hasRankedFallback = false } = {}) {
  if (ownerFloorFailed) return hasRankedFallback ? PARTIAL_UNAVAILABLE_COPY : UNAVAILABLE_COPY;
  return error || UNAVAILABLE_COPY;
}
...
function failedFloor(metro, error, universe = EXPLODING_NEARBY_UNIVERSE) {
  if (!ownerListExists(universe)) {
    return payload({ status: "trend_data_error", error: UNAVAILABLE_COPY }, 502);
  }
  if (error instanceof TrendConfigError) {
    return payload({
      status: "trend_configuration_error",
      metro,
      trends: [],
      error: error.message,
      basis: "owner_list",
 ...
 * PostgREST read. Snapshot failures still fall through to the owner list.
 * If the inventory read itself fails, the response stays distinguishable
 * from a successful read that found no verified local match.
 */
export async function serveExplodingNearby({ lat, lng, readRows, universe = EXPLODING_NEARBY_UNIVERSE } = {}) {
  ...
  try {
    ...
    return failedFloor(metro, e, universe);
  }
