...
export function isPlaceOwnedPhotoUrl(value) {
  ...
  // A Commons URL on a place row is a hotlink; the vault copy is served via
  // the place's ref / ?place= path instead (lib/imageHostPolicy.js).
  if (isFreeSourceHotlink(s)) return false;
  return true;
}
...
/** True only when the row carries validated, same-place stored image proof. */
export function hasStoredPlacePhoto(place) {
  return !!(ownedUrlOf(place) || ownedRefOf(place));
}
...
export function landingCardPhotoSrc(place) {
  ...
  if (ref) return "/api/photo?ref=" + encodeURIComponent(ref) + "&g=2&w=1200";
  return "";
}
...
export function ownedPlacePhotoSrc(placeId, w = 640) {
  ...
  return "/api/photo?place=" + encodeURIComponent(id) + "&g=2&w=" + (Number(w) || 640);
}
...
export function cardImageSrc(row, w = 640) {
  ...
  if (ref) return "/api/photo?ref=" + encodeURIComponent(ref) + "&g=2&w=" + (Number(w) || 640);
