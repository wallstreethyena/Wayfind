...
export async function resolvePlacePhoto(input, deps) {
  ...
  const cacheLookup = async (photoRef) => {
    ...
    // A small thumbnail can reuse the place's already-cached 640px card image.
    // This is a read only and does not extend the source's 30-day lifetime.
  ...
  if (useRef && !gateShut) {
    ...
    const spendAllowed = typeof input.authorizeSpend === "function"
      ...
      : !!input.spendAllowed;
    if (!spendAllowed) return emptyPhotoResult("spend-denied");
  ...
  return emptyPhotoResult(useRef && gateShut ? "gate-shut" : "no-photo");
}
