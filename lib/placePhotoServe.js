...
export function isOwnedPhotoUrl(value) {
  ...
  // A shared partner/stock host is rejected above.
  return true;
}
...
async function defaultFetchOwnedUri(ref, w, key) {
  ...
  const media = "https://places.googleapis.com/v1/" + ref + "/media?maxWidthPx=" + w + "&skipHttpRedirect=true&key=" + key;
  const r = await fetch(media, { cache: "no-store" });
  if (!r.ok) return null;
  const j = await r.json();
  const uri = j && j.photoUri;
  return isOwnedPhotoUrl(uri) ? String(uri) : null;
}
