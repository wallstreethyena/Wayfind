...
import { canonicalPhotoWidth, photoCacheCandidateWidths } from "./photoCacheRecovery.js";
import {
  PHOTO_URI_ALIVE,
  PHOTO_URI_DEAD,
    ...
    if (placeId && (await grant("details_ids_only"))) {
      ...
      try {
        ...
        if (d && d.ok) {
          ...
          if (name && PHOTO_REF_RX.test(name)) fresh = name;
        } else if (d) {
      ...
      if (fresh) {
        ...
        if (f.uri) return flush({ uri: f.uri, upstream: "ok", retried: f.retried });
...
export async function resolvePlacePhoto(input, deps) {
  ...
  // HERMETIC BREAKER (2026-09-17). The live breaker lives in the production
  // Supabase cache. A caller that injects its OWN Google (deps.fetchOwnedUri or
  // deps.fetchImpl) is a test or a harness, never the /api/photo route, and it
  // must not read or trip production's breaker: the Vercel preview build of
  // v8.56.20 runs the guard suite with production Supabase env present, saw
  // the real breaker still open from the 2026-09-16 quota day, and failed
  // check-no-imageless-card on a quota-open that had nothing to do with the
  // code under test. Production never injects a fetcher, so its breaker
  // behaviour is unchanged. An explicit deps.breakerOpen/tripBreaker still wins.
  ...
  const remember = async (photoRef, uri) => {
    ...
    // SPEND EFFICIENCY (2026-09-16): written at the CANONICAL width, never
    // the raw requested width — this is the write-side half of "one paid
    // fetch per photo": a card asking for 480 and a card asking for 720 both
    // land on the SAME photo|{ref}|640 row, so the SECOND card's request is
    // a free cache hit (see cacheLookup / photoCacheCandidateWidths above)
    // instead of its own paid fetch.
    try { await cacheSet(photoCacheKey(photoRef, canonicalPhotoWidth(w)), { uri: String(uri), vok: now }); } catch { /* cache write is best-effort */ }
  };
  ...
  if (placeId) {
    ...
    // PLACE-ONLY DISCOVERY (2026-09-17, production-measured). 211 visible
    // places were served as `/api/photo?place=<id>` with no inventory photo
    // name (most are not in wf_inventory at all), and every one resolved to
    // "no-photo" without ever asking Google. Opt-in (input.discoverPlace, set
    // by app/api/photo/route.js) so direct resolver callers keep their shape.
    // From here the ordinary gated path runs: cache, probe, breaker, negative
    // cache, one ledger grant, a free IDs-only lookup, one media call.
    if (!useRef && input && input.discoverPlace && GOOGLE_PLACE_ID_RX.test(placeId)) {
      useRef = placeDiscoveryRef(placeId);
    }
  }
  ...
  if (useRef && !gateShut) {
    ...
    // SINGLEFLIGHT (2026-09-16). Everything from here through the Google
    // fetch, remember(), and the quota-breaker trip is the "paid section" —
    // the part that may spend a ledger grant. Keyed on the SAME ref at its
    // CANONICAL width (canonicalPhotoWidth — matches remember()'s write key
    // above), so two concurrent callers asking for different card sizes of
    // the SAME photo share the SAME in-flight attempt, not two. The first
    // caller to reach this key creates `run`; every other caller for the
    // same key in this same lambda instance, arriving before `run` settles,
    // gets back the IDENTICAL promise — same result object, same redirect,
    // ZERO extra grants and ZERO extra Google calls. A probe can never join
    // or create an entry here: it always returns above, before this line.
    ...
    const run = (async () => {
      try {
        ...
        // NEGATIVE CACHE (2026-09-16): a ref that JUST proved (this same 24h
        // window) it has no fetchable photo — see NEGATIVE_CACHEABLE_UPSTREAM
        // and the write site below — skips the ledger and Google entirely.
        // defaultCacheGet reads with a 30-day STALE grace (THIRTY_DAYS_MS),
        // so a row physically past its own 24h ttl still comes back non-null
        // with `stale:true` for up to 30 days; that grace is right for a real
        // photo redirect, but WRONG for a 24h negative marker, so `stale` is
        // read here as "treat this as gone" — only a row still inside its
        // true 24h window (`stale:false`) counts as a hit. A missing row
        // reads as `null` either way (see defaultCacheGet's own doc comment).
        let negHit = null;
        try { negHit = await cacheGet(photoNegativeKey(useRef)); } catch { negHit = null; }
        if (negHit && !negHit.stale) {
          return { type: "miss", location: null, cacheControl: "private, no-store", reason: "negative-cached", ref: useRef, upstream: "negative-cached", retried: false, refunded: 0 };
        }
        ...
        // record refunds without a network call. The width asked of Google is
        // the CANONICAL width (canonW), never the raw `w` — a card at 480 and
        // a card at 720 fetch the SAME 640px photo once, not twice.
        const discovering = isPlaceDiscoveryRef(useRef);
        const rawFetched = await fetchOwnedUri(useRef, canonW, serverKey, authorizeMore, d.fetchImpl, d.retryDelayMs, d.refund, { freshFirst: discovering || !!(input && input.freshFirst), placeOnly: discovering });
        ...
        // NEGATIVE CACHE, written (2026-09-16): only for the upstream classes
        // in NEGATIVE_CACHEABLE_UPSTREAM — an answer that means "no photo is
        // coming back on an immediate retry", never a transient/rejection
        // class (quota/key-denied/server/network/denied/stale-heal-denied)
        // that a moment later, or a refund, might resolve differently. Never
        // written for a probe (probe is always false here, but stated anyway
        // — the same defence-in-depth posture remember() above already uses)
        // and always best-effort: a failed write only means the NEXT request
        // pays for the same "no" again, never a broken response now.
        if (!probe && NEGATIVE_CACHEABLE_UPSTREAM.has(fetched.upstream)) {
          try { await cacheSet(photoNegativeKey(useRef), { cls: fetched.upstream, at: now }, NEGATIVE_CACHE_TTL_MS); } catch { /* best-effort */ }
        }
        ...
        // Cleanup happens exactly once, when `run` itself settles, no matter
        // how many followers are awaiting the SAME promise — the next
        // request for this ref+width after this one starts a fresh attempt.
