...
export async function resolvePlacePhoto(input, deps) {
  ...
  const cacheLookup = async (photoRef) => {
    ...
    // Nearby thumbnails can reuse the same place's larger main-card image.
    // This is a read only: never renew the source's expiry by copying it.
    for (const width of w < 640 ? [w, 640] : [w]) {
      try {
  ...
  // Check the cache before reserving spend; a denied budget must never
  // trigger a paid request, even for a catalogued photo.
  if (useRef && !gateShut) {
    const spendAllowed = typeof input.authorizeSpend === "function"
      ? await input.authorizeSpend()
      : !!input.spendAllowed;
    if (!spendAllowed) return emptyPhotoResult();
