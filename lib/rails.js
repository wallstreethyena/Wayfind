// lib/rails.js — homepage daypart rails.
//
...
// reaches a reader again.
  { id: "season", title: "Summer Picks", axis: "summer utility — ten answers for heat, rain, school break and vacation mode",
    emptyWhy: "not enough nearby places have both summer fit and a verified real photo yet",
    short: "Best summer ever", sub: "Ten ways to do Florida summer",
    cta: "See all ten summer rails", art: "season", href: "/summer-picks", list: true },

  // 2026-09-04 — the owner's supplied Lunch in My City photograph, promoted
  // from the Shortcuts sheet into the homepage poster rail. This is a launch
  // card, not a ranked-list rail: a plain tap opens the location-aware lunch
  // reveal. The originally supplied place URL resolves to Gatorland, which is
  // not a lunch restaurant and caused the poster to reopen the wrong detail.
  // `opensPage` is explicit so DaypartRail does not intercept the anchor and
  // open an empty generic drop.
  { id: "lunchcity", posterHidden: true, title: "Lunch in My City", axis: "challenge — accept one lunch prompt wherever the reader is now",
    emptyWhy: "the lunch challenge is temporarily unavailable",
    short: "Tap the box to accept the challenge", sub: "One lunch place around you, wherever you are now",
    cta: "Accept the challenge", art: "lunchcity", href: "/?go=lunch", opensPage: true },

  { id: "today", title: "Today's Best Options", axis: "one decision surface — the best verified options across ten distinct ways to spend today",
    emptyWhy: "we could not verify a best-in-category discovery near you yet",
    short: "Do something better", sub: "Ten ranked ways to make today count",
    cta: "Show today's best", art: "today", href: "/things-to-do", list: true },
  ...
  // v8.94 — THE TITLE FOLLOWS THE POSTER TOO (owner, 2026-08-30). v8.93.1 moved
  // the poster's SUB into the copy and left the NAME behind, so the reader
  // still met "Exploding Trends Near You" in the three places a rail's title
  // is actually rendered — the "Showing <rail> near <city>" drop bar, the tile
  // alt/aria-label, and the <h1> on /r/trending — while the picture above all
  // three said TRENDING NEAR YOU. That is the same art-vs-copy split this rail
  // already paid for on 2026-08-16, just narrowed to one field.
  //
  // It is also the honest direction, not only the tidy one. "Exploding" is a
  // VELOCITY claim; "Trending" is what the drop can evidence — the owner's
  // curated trends lead it, and the places behind them are gated on the real
  // TREND_THRESHOLD spike flag and creator evidence. Dropping the louder word
  // costs nothing true and means test-buzz's SPIKE_WORDS contract is satisfied
  // by the copy itself rather than by its two backings, which is strictly
  // stronger. `cta` loses "exploding" for the same reason.
  //
  // NOT renamed: the rail id, /trending, the selector, lib/explodingNearby*.
  // This is what the card SAYS, not what it does.
  { id: "trending", title: "Trending Near You", axis: "signal — the owner's curated trends, creator posts, and genuine demand surges",
    emptyWhy: "no trend, creator post or demand surge is live near you yet",
    short: "What people are searching and experiencing right now", sub: "And where to try each one, near you",
    cta: "See what's trending", art: "trending", href: "/trending", list: true },
  ...
  // TITLE FOLLOWS THE ART (owner, 2026-09-10). His replacement poster reads
  // "FAMILY DAY, SOLVED." and there is no HTML text overlay on a tile any more
  // (v8.1 — the tile IS the picture), so the baked word is what a reader sees.
  // "Decided" survived only in the places the picture is NOT: this title, the
  // /family page <title>, and the intent card line. All three moved together;
  // a tile that says SOLVED opening a page that says decided is the seam
  // check-rail-art-matches-copy exists to catch.
  { id: "family", title: "Family Day, Solved", axis: "kids in tow",
    ...
    cta: "Plan the day", art: "family", href: "/family", opensPage: true,
    regional: { orlando: "family-orlando", fl: "family-fl" }, list: true },

  // TITLE FOLLOWS THE ART (owner, 2026-09-10). His replacement poster reads
  // "Creators Pick", and with no HTML overlay on a tile (v8.1) the baked
  // words are what a reader sees. The axis below is UNCHANGED and still
  // governs what may appear here: a real person went and posted it. The
  // poster renames the shelf, it does not loosen the sourcing rule — the
  // cards behind it still come from the creator library (lib/railsData.js
  // THE CREATORS POOL), each carrying the name of the creator who posted.
  // app/components/BestNearby.js renders the same pool in its own section
  // and was renamed in the same commit; one pool must not wear two names.
  { id: "locals", title: "Creators Pick", axis: "source — a real person went and posted it",
  ...
  //
  // v8.94 — `opensPage`: THIS TILE NAVIGATES. Owner, 2026-08-30: "the page for
  // cindy when you click on it is not showing up either — let's add a
  // professional page for Cindy Selects." It was not a routing bug;
  // /creators/cindy.selects has been live and returning 200 the whole time.
  // The tile simply never went there: tileClick preventDefaults every anchor
  // and opens the in-rail drop instead, which is right for the fourteen rails
  // whose answer IS a shelf of place cards and wrong for the one rail whose
  // answer is a page — her map, her summary, her videos, her share button.
  // A tile that promises a person and delivers a shelf is a smaller version of
  // the same product.
  { id: "cindy", title: "Your Next Coffee Spot", axis: "one named creator — every café @cindy.selects filmed herself",
    ...
    cta: "Find your café", art: "cindy-selects-type", href: "/creators/cindy.selects", list: true, opensPage: true },

  { id: "drive", posterHidden: true, title: "Worth the Drive", axis: "distance — worth leaving town for",
  ...
  { id: "tonight", title: "Night Out", axis: "the intent — ten exclusive ways to spend the night",
    emptyWhy: "nothing within 27 miles has enough evidence for this night-out intent",
    short: "Tonight, actually sorted", sub: "Clubs, cocktails, live music, shows, dining & experiences — ranked near you",
    cta: "Plan a night out", art: "tonight", href: "/tonight", list: true },
  ...
  { id: "events", title: "What's Happening Near You", axis: "the date — it happens once, then it is gone",
    ...
    cta: "See what's on", art: "events", href: "/events", list: true, retiredInto: "tonight" },
  ...
  // Duprat sheet in place, augtober opens the statewide fall surface in place
  ...
  // POSTER HIDDEN (owner, 2026-09-10). The tile comes off the homepage track;
  // the record stays, exactly as `lunchcity` and `drive` did in #1196. That
  // is deliberate and not the same as deleting the rail: RAIL_IDS keeps its
  // length for the ten-rail API contract, the id stays stable for analytics,
  // lib/summerPicks.js keeps pulling "chef" in its placeSources, and Ron's
  // seven picks stay addressable — nothing about the testimony changes, only
  // whether the poster is one of the tiles on the home track.
  { id: "chef", title: "Chef Ron Duprat's Top 7", axis: "testimony — a Top Chef's own list, in his order",
    ...
    cta: "See Ron's Top 7", art: "chef", drop: true, posterHidden: true },

  // The internal id stays stable for analytics and the ten-rail API contract;
  // the customer-facing October/Augtober campaign retired with its poster.
  { id: "augtober", title: "Fall in Florida", axis: "the season — warm-weather Florida fall, while it lasts",
    emptyWhy: "no fall events are live near you yet",
    short: "Pumpkins, festivals & fall escapes", sub: "Florida fall plans worth showing up for",
    cta: "Find your best fall", art: "augtober", drop: true },
];
...
export const RAIL_TINT = {
  ...
  lunchcity: "linear-gradient(165deg,#D7C7A9,#7B694F 55%,#18351E)",
  ...
  // v8.15 — breakfast warms toward the tile's yellow.
  breakfast: "linear-gradient(165deg,#4A3A05,#241C0B 55%,#0B0E1A)",
  // v8.95 — sampled from the owner's purple "YOUR BEST DAY EVER" poster,
  // so the decode placeholder feels like the artwork instead of flashing the
  // old mint Birthday Plans card underneath it.
  birthday: "linear-gradient(165deg,#7758A8,#4B347A 55%,#171127)",
  blog: "linear-gradient(165deg,#0C3C57,#0D2233 55%,#0B0E1A)",
  chef: "linear-gradient(165deg,#1E2A22,#101710 55%,#0B0E1A)",
  // Sampled from the owner's blue-sky / pumpkin-field fall poster so the
  // decode placeholder already belongs to the artwork.
  augtober: "linear-gradient(165deg,#075A9C,#0B3B61 55%,#132610)",
};
...
// v17 (2026-08-31): the Birthday tile is the owner's purple "YOUR BEST DAY
// EVER" poster, replacing the mint T-rex artwork under the same five
// filenames. The bump is required so returning phones receive the new cover.
// v18 (2026-09-01): Date Night becomes the owner's pink LOVE poster under the
// same five filenames. The source is a true 9:16 frame, so it fills the shared
// tile without a crop or letterbox; the bump forces returning phones past the
// cached v17 poster.
// v19 (2026-09-01): the expired Augtober campaign becomes the owner's blue-sky
// "Florida's Best Fall, Found" poster. Same five filenames, so the version
// changes to make returning phones receive the seasonal replacement now.
// v20 (2026-09-01): Tonight's Move becomes NIGHT OUT — the consolidated ten-
// intent collection that now owns both nightlife venues and dated happenings.
// The new 9:16 poster replaces all five existing `tonight` derivatives.
// v21 (2026-09-02): Fall becomes the owner's new FALL IN FLORIDA scarecrow
// poster under the same five `augtober` filenames. The 941×1672 source is a
// true 9:16 frame; the bump makes returning phones receive it immediately.
// v22 (2026-09-03): Summer Picks becomes the owner's BEST SUMMER EVER poster
// under the existing five `season` filenames. The version must change with
// the bytes or returning phones keep the previous Made for Summer artwork.
// v24 (2026-09-10): Family Day becomes the owner's FAMILY DAY, SOLVED poster
// under the existing five `family` filenames. Same names, new bytes — so the
// version MUST move or every phone holding the 30-day copy keeps showing the
// previous artwork, which is the v8.15 bug this counter exists for.
// v25 (2026-09-10): two more owner posters under existing filenames — BEST
// BLOGS on `blog` and CREATORS PICK on `locals`. Same names, new bytes.
// v26: polished Family Day poster, including both regional variants.
export const RAIL_ART_V = "26";
