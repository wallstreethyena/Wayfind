...
// v6.9x — the two regexes above only see a role word directly touching one
// name, or a name directly touching a verb. Confirmed live on 2026-08-01
// (Max's Table, Tacos My Guey, Eden Bar — all three of that day's cached
// dining "why" paragraphs) that real model output also names staff via a
// comma-separated list ("Julia, Jenna, Tammi, and Patty B are named
// specifically") or a "like NAME[, NAME]" construction ("servers like Andy
// and Noah", "Service standouts like Alejandro") — neither shape touches a
// role word directly against a single name, so both slipped past every
// existing check into production. Two more nets, same drop-the-sentence
// policy. Still regex, not real NLP: a sentence naming multiple capitalized
// common nouns right before "are/is/were/was" (e.g. an unusual "The Grouper,
// the Snapper, and the Redfish were fresh") could still false-positive — an
// acceptable tradeoff given the confirmed alternative (real staff names
// shipping to the site) is worse.
const NAME_LIST_RX = /\b(?:[A-Z][a-z]{2,}\.?(?:\s[A-Z]\.?)?\s*,\s*){1,}(?:and\s+)?[A-Z][a-z]{2,}\.?(?:\s[A-Z]\.?)?\s+(?:are|is|were|was)\b/;
const LIKE_NAME_RX = /\b(?:named|ask for|request|shout[- ]?out to|standouts?\s+like|servers?\s+like|bartenders?\s+like|hosts?\s+like|staff\s+like)\b\s*[^.?!]{0,4}?\b[A-Z][a-z]{2,}\b/i;

function sentenceNamesStaff(s) {
  return STAFF_ROLE_RX.test(s) || NAME_THEN_SERVICE_VERB_RX.test(s) || NAME_LIST_RX.test(s) || LIKE_NAME_RX.test(s);
}
...
function stripStaffNameSentences(text) {
  ...
  const kept = sentences.filter((s) => !sentenceNamesStaff(s));
  return kept.join(" ").trim();
}

// v6.9x — nothing previously stopped the model from writing "one reviewer
// said... another visitor noted..." attribution, which is what makes copy
// read as a review digest instead of an edited recommendation (confirmed in
// the same 2026-08-01 Max's Table paragraph: "one picky eater was shocked...
// another reviewer had the best gyro of their life"). Same drop-the-sentence
// policy as staff names — the rest of a paragraph can still be good.
const ANECDOTE_RX = /\b(one|another|a)\s+(reviewer|visitor|customer|guest|couple|family|patron)\b/i;

function stripAnecdoteSentences(text) {
  const t = String(text || "");
  if (!t) return t;
  const sentences = t.split(/(?<=[.!?])\s+/);
  const kept = sentences.filter((s) => !ANECDOTE_RX.test(s));
  return kept.join(" ").trim();
}
...
export function validateWhyParagraph(text, place, opts = {}) {
  ...
  why = stripAnecdoteSentences(why);
  if (!why) return fail("entirely single-reviewer anecdotes");
