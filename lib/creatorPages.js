...
function spotCard(s, platformColor, premium = false) {
  ...
  if (premium) return <article key={s.key + v.url} className="wfcr-card">
    {embeddable ? <CreatorPlaybackDetails details={premiumDetails} buttonStyle={{ color: "#e7c899" }}>
      <VideoFacade platform={v.platform} url={v.url} label={label} coverTitle={s.name} coverCity={s.city} fallbackPoster="/cards-v8/cindy-selects-portrait-380.webp" poster={s.placeId ? `/api/photo?place=${encodeURIComponent(s.placeId)}&w=640` : null} />
