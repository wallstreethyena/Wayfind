...
function spotCard(s, platformColor, premium = false) {
  ...
  if (premium) return <article key={s.key + v.url} className="wfcr-card">
    {isEmbeddable(v.platform, v.url) && <VideoFacade platform={v.platform} url={v.url} label={label} coverTitle={s.name} coverCity={s.city} fallbackPoster="/cards-v8/cindy-selects-portrait-380.webp" poster={s.placeId ? `/api/photo?place=${encodeURIComponent(s.placeId)}&w=640` : null} />}
    <div className="wfcr-card-body"><h3 style={S.place}>{s.name}</h3><div style={S.addr}>{s.address || s.city}</div>
  ...
  return (
    <div key={s.key + v.url} className={premium ? "wfcr-card" : undefined} style={premium ? undefined : S.card}>
      ...
      {isEmbeddable(v.platform, v.url) && <VideoFacade platform={v.platform} url={v.url} label={label} fallbackPoster={premium ? "/cards-v8/cindy-selects-portrait-380.webp" : null} poster={premium && s.placeId ? `/api/photo?place=${encodeURIComponent(s.placeId)}&w=640` : null} />}
      <div>
        ...
        <a href={v.url} target="_blank" rel="noopener" style={{ ...S.credit, color: pl.color || platformColor }} aria-label={nativeAria}>
          {nativeAction}
        </a>
