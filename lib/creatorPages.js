...
import { captionFor } from "./creatorCaptions";
...
import CreatorPlaybackDetails from "../app/components/CreatorPlaybackDetails";
import CreatorAvatar from "../app/components/CreatorAvatar";
import CreatorShareButton from "../app/components/CreatorShareButton";
import CreatorVideoRail from "../app/components/CreatorVideoRail";
import CreatorMapPanel from "../app/components/CreatorMapPanel";
import { creatorMapRows } from "./creatorPlaces";
...
export function creatorProfile(handle) {
  ...
  return {
    handle: c.handle,
    // The curated spots themselves, in curation order. The MAP needs the
    // placeIds and it must not re-walk CURATED to get them: a second reader of
    // the same source is a second opinion waiting to happen (the page would
    // list a spot the map silently skipped, or the reverse, and neither side
    // could prove which was right).
    spots: c.spots,
    platform,
...
export function creatorMetadata(handle) {
  ...
  return { title, description, alternates: { canonical: url }, ...socialMeta({ title, description, url, ...(p.handle === "cindy.selects" ? { image: `${SITE_URL}/api/og/rail?id=cindy&creator=1&v=23` } : {}) }) };
}
...
function spotCard(s, platformColor, premium = false) {
  const v = s.video;
  const pl = PLATFORM[v.platform] || {};
  const label = `${s.name} on ${pl.label || v.platform}`;
  // Instagram /p/ links can be stills or carousels. "View post" is true for
  // those and for reels, while "Watch" is not guaranteed by the URL.
  const instagramPost = v.platform === "instagram";
  const nativeAction = instagramPost ? `View post on ${pl.label || v.platform} ↗` : `Watch on ${pl.label || v.platform} ↗`;
  const nativeAria = instagramPost ? `View ${label} (opens in a new tab)` : `Watch ${label} (opens in a new tab)`;
  ...
  const embeddable = isEmbeddable(v.platform, v.url);
  const premiumDetails = <div className="wfcr-card-body"><h3 style={S.place}>{s.name}</h3><div style={S.addr}>{s.address || s.city}</div>
    {captionFor(v) ? <p>{captionFor(v)}</p> : null}
    <div className="wfcr-card-actions"><a href={openHref}>Explore place →</a><a href={v.url} target="_blank" rel="noopener" aria-label={nativeAria}>{nativeAction}</a></div>
  </div>;
  if (premium) return <article key={s.key + v.url} className="wfcr-card">
    {embeddable ? <CreatorPlaybackDetails details={premiumDetails} buttonStyle={{ color: "#e7c899" }}>
      <VideoFacade platform={v.platform} url={v.url} label={label} coverTitle={s.name} coverCity={s.city} fallbackPoster="/cards-v8/cindy-selects-portrait-380.webp" poster={s.placeId ? `/api/photo?place=${encodeURIComponent(s.placeId)}&g=2&w=640` : null} />
    </CreatorPlaybackDetails> : premiumDetails}
  </article>;
  const details = <>
    <h3 style={S.place}>{s.name}</h3>
    <div style={S.addr}>{s.address || s.city}</div>
    {captionFor(v) ? <p style={{ fontSize: 14, color: "#CBD5E1", lineHeight: 1.55, margin: "0 0 14px" }}>{captionFor(v)}</p> : null}
    <div>
      {/* FOLLOWED, deliberately — no nofollow, no noreferrer. */}
      <a href={v.url} target="_blank" rel="noopener" style={{ ...S.credit, color: pl.color || platformColor }} aria-label={nativeAria}>{nativeAction}</a>
      <a href={openHref} style={S.open}>Open {s.name} in Wayfind →</a>
    </div>
  </>;
  return (
    <div key={s.key + v.url} className={premium ? "wfcr-card" : undefined} style={premium ? undefined : S.card}>
      {embeddable ? <CreatorPlaybackDetails details={details} contentStyle={{ paddingTop: 12 }}>
        <VideoFacade platform={v.platform} url={v.url} label={label} />
      </CreatorPlaybackDetails> : details}
    </div>
  );
}

// One sentence of real context under the map heading, built from the rows the
// map actually drew — never from the curated count. The two differ whenever a
// spot has no placeId yet or wf_inventory has not ingested it, and claiming
// eleven pins over a map showing nine is the exact species of small lie that
// makes a reader stop trusting the honest numbers elsewhere on the page.
export function mapIntro(profile, rows) {
  const n = (rows || []).length;
  if (!n) return "";
  const cities = Array.from(new Set((rows || []).map((r) => r && r.city).filter(Boolean)));
  const where = cities.length === 1 ? cities[0]
    : cities.length === 2 ? `${cities[0]} and ${cities[1]}`
    : cities.length > 2 ? `${cities.slice(0, -1).join(", ")} and ${cities[cities.length - 1]}`
    : "";
  const total = profile && profile.placeCount ? profile.placeCount : n;
  const gap = total > n ? ` (${total - n} more ${total - n === 1 ? "spot is" : "spots are"} listed below but not yet mapped)` : "";
  return `${n} of them are on the map${where ? `, across ${where}` : ""}${gap}.`;
}

// ASYNC ON PURPOSE (v8.94). The map's coordinates are a server-side join from
// each spot's placeId into wf_inventory (lib/creatorPlaces.js) — inventory
// Wayfind already owns, so the page costs zero metered Places calls no matter
// how often it rebuilds. It is awaited HERE rather than fetched in the client
// panel for two reasons: a creator page is a crawlable SEO surface and its
// content must exist in the HTML, and a client fetch would put a Supabase read
// on the reader's connection on every visit instead of once per revalidate.
//
// creatorMapRows() fails closed to [] — unconfigured env, a 400, a timeout —
// and CreatorMapPanel renders null on an empty list. So a database that is
// down costs the page its map and nothing else.
//
// `mapPlaces` is an INJECTION SEAM, the same one lib/landingInventory.js uses
// (`opts.inventoryRows`): pass rows and the join is skipped. It exists because
// a bare-node guard has no Supabase env, so without it the only renderable
// state is the empty one — and a test that can only ever see the empty state
// proves the map is absent, never that it is right. The route never passes it.
export async function CreatorPage({ handle, mapPlaces: injectedMapPlaces }) {
  const p = creatorProfile(handle);
  if (!p) {
    return (
      <div style={S.wrap}>
  ...
  const premium = p.handle === "cindy.selects";
  const url = `${SITE_URL}/creators/${encodeURIComponent(p.handle)}`;
  ...
  const mapPlaces = Array.isArray(injectedMapPlaces) ? injectedMapPlaces : await creatorMapRows(p.spots);

  return (
    <div className={premium ? "wfcr-page" : undefined} style={{ ...S.wrap, ...(premium ? { maxWidth: 1180 } : {}) }}>
      {premium ? <style dangerouslySetInnerHTML={{ __html: CREATOR_PREMIUM_CSS }} /> : null}
      {jsonLd(breadcrumb)}
      {jsonLd(profile)}
      {jsonLd(itemList)}
      {premium ? <div className="wfcr-topbar"><a href="/">← Back to Wayfind</a><span>CINDY SELECTS</span><a href="#cindy-films">The films ↓</a></div> : null}
      <nav style={S.crumb}><a href="/" style={S.crumbLink}>Wayfind</a> › <a href="/creators" style={S.crumbLink}>Creators</a> › @{p.handle}</nav>

      {premium ? <div className="wfcr-hero"><img src="/cards-v8/cindy-selects-portrait-760.jpg" alt="Cindy enjoying coffee at a colorful café" width="760" height="1350" fetchpriority="high" /><div><span className="wfcr-eyebrow">THE CINDY SELECTS EDIT</span><h2>Good coffee.<br />Beautiful places.<br /><em>Your next little escape.</em></h2><p>A personal collection of cafés, bites and places worth slowing down for.</p><a href="#cindy-films">Explore the collection ↓</a></div></div> : null}
      <div className={premium ? "wfcr-profile" : undefined} style={S.hero}>
        <span style={{ ...S.ring, background: `linear-gradient(150deg, ${p.platformColor}, rgba(255,255,255,.28) 58%, ${p.platformColor})` }}>
          {premium ? <img src="/cards-v8/cindy-selects-portrait-380.webp" alt="Cindy Selects" width="72" height="72" style={{ borderRadius: "50%", objectFit: "cover", objectPosition: "50% 32%" }} /> : <CreatorAvatar handle={p.handle} platform={p.platform} size={72} color={p.platformColor} />}
        </span>
      ...
      <p style={S.lede}>
        {premium ? "From Bradenton and Parrish to Sarasota and Orlando. Explore Cindy’s reviewed places by city, watch each visit, and save your next stop." : <>{p.summary ? p.summary + " " : ""} Every place @{p.handle} has featured that Wayfind lists is below, organised by city — with the original {p.platformLabel} video and a way straight into the place for hours, directions and what's worth pairing it with nearby.</>}
      </p>
      ...
      {/* Follow HER, then share the page. Both on one row because they are the
          two things this page exists to cause, and burying the share control at
          the bottom is how a page a creator was asked to post never gets
          posted. */}
      <p style={{ margin: "0 0 6px", display: "flex", flexWrap: "wrap", alignItems: "center", gap: 10 }}>
        {p.profileUrl ? (
          /* Followed, on purpose. See the file header. */
          <a href={p.profileUrl} target="_blank" rel="noopener" style={{ ...S.follow, color: premium ? "#e7c899" : p.platformColor }}>
            Follow @{p.handle} on {p.platformLabel} ↗
          </a>
        ) : null}
        <CreatorShareButton handle={p.handle} title={`Every place @${p.handle} has featured, on Wayfind`} />
      </p>

      <CreatorMapPanel handle={p.handle} places={mapPlaces} intro={mapIntro(p, mapPlaces)} />
      ...
      {p.cities.map((g) => {
        ...
        return (
          <section key={g.city}>
            ...
            {premium ? <CreatorVideoRail label={`${g.city} videos`}>{g.spots.map((s) => spotCard(s, p.platformColor, true))}</CreatorVideoRail> : g.spots.map((s) => spotCard(s, p.platformColor))}
      ...
      {p.noCity.length ? (
        <section>
          ...
          {premium ? <CreatorVideoRail label="More videos">{p.noCity.map((s) => spotCard(s, p.platformColor, true))}</CreatorVideoRail> : p.noCity.map((s) => spotCard(s, p.platformColor))}
        </section>
      ) : null}
  ...
  return (
    <div style={S.wrap}>
      ...
      <div style={{ margin: "0 0 18px" }}>
        <ShareButton url={pageShareUrl("/creators")} title="The local creators behind the finds"
          text="The independent local creators behind the places on Wayfind, and what each one has filmed."
          label="Share" tone="dark" event="page_share" meta={{ surface: "creators_index", placement: "header" }} />
      </div>
      {creators.map((c) => {
...
const CREATOR_PREMIUM_CSS = `
.wfcr-hero{position:relative;display:grid;grid-template-columns:1fr 1fr;min-height:420px;border-radius:28px;overflow:hidden;background:#291b1a;margin-bottom:22px;border:1px solid #583a35}
.wfcr-hero>img{width:52%;height:100%;position:absolute;inset:0;object-fit:cover;object-position:50% 30%}
.wfcr-hero>div{position:relative;grid-column:2;padding:36px 36px;background:linear-gradient(90deg,rgba(48,23,25,.85),#301719 12%);display:flex;flex-direction:column;justify-content:center}
.wfcr-eyebrow{font-size:11px;font-weight:800;letter-spacing:.2em;color:#eed4a6}
.wfcr-hero h2{font-family:Georgia,serif;font-size:clamp(32px,4vw,50px);font-weight:400;line-height:1.06;letter-spacing:-.04em;margin:24px 0;color:#fff9ed}
.wfcr-hero em{color:#e7c899}.wfcr-hero p{max-width:300px;line-height:1.6;color:#e5e1d9;font-size:14px}
.wfcr-hero a{align-self:flex-start;border-radius:999px;padding:13px 20px;background:#f0dcba;color:#301719;text-decoration:none;font-size:13px;font-weight:800;margin-top:16px}
.wfcr-topbar{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:18px}.wfcr-topbar a{color:#f0dcba;text-decoration:none;font-size:13px;font-weight:700;padding:10px 0}.wfcr-topbar span{font-size:10px;letter-spacing:.2em;color:#bca399}.wfcr-profile{margin-top:8px}
.wfcr-card{flex:0 0 272px;scroll-snap-align:start;border-radius:20px;overflow:hidden;background:linear-gradient(145deg,#2e2020,#171314);border:1px solid #503734;box-sizing:border-box}
.wfcr-card-body{padding:16px}.wfcr-card-body h3{font-family:Georgia,serif;font-weight:400!important;font-size:21px!important}.wfcr-card-body>p{display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;font-size:12px;line-height:1.5;color:#c9b9af;margin:10px 0 14px}
.wfcr-card-actions{display:flex;gap:12px;align-items:center;justify-content:space-between}.wfcr-card-actions a{color:#efcfaa;font-size:11px;font-weight:700;text-decoration:none}.wfcr-card-actions a:last-child{color:#c3aaa2}
.wfcr-page .wfcm-side{background:#21191a;border-color:#503734}.wfcr-page .wfcm-row[aria-pressed="true"]{background:#4a282c;border-color:#916153;color:#f0dcba}
.wfcr-page a:focus-visible,.wfcr-page button:focus-visible{outline:3px solid #eed4a6;outline-offset:4px}
@media(max-width:640px){.wfcr-hero{min-height:480px;display:flex;align-items:flex-end;border-radius:22px}.wfcr-hero>img{width:100%;object-position:40% 35%}.wfcr-hero>div{width:100%;padding:100px 24px 24px;background:linear-gradient(0deg,rgba(40,17,20,.98),rgba(40,17,20,.72) 68%,transparent)}.wfcr-hero h2{font-size:38px;margin:14px 0}.wfcr-hero p{margin:0}.wfcr-card{flex-basis:82vw;max-width:300px}}
`;
