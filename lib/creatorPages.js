...
function spotCard(s, platformColor, premium = false) {
  ...
  return (
    <div key={s.key + v.url} className={premium ? "wfcr-card" : undefined} style={premium ? undefined : S.card}>
      ...
      {isEmbeddable(v.platform, v.url) && <VideoFacade platform={v.platform} url={v.url} label={label} poster={premium && s.placeId ? `/api/photo?place=${encodeURIComponent(s.placeId)}&w=640` : null} />}
      <div>
