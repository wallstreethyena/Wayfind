...
const FIT = {
  ...
  worldcup: (p) => {
    ...
    let s = 0;
    if (/sports ?bar|watch party|tavern|brewery|beer garden|cantina|gastropub/.test(hay)) s += 14;
    if (/bar|restaurant|grill|lounge/.test(types)) s += 6;
    if (/rooftop|screens|stadium|social|sky bar|watch bar/.test(name)) s += 8;
    if (/brazil|brasil|latin|irish|english pub/.test(name)) s += 6;
