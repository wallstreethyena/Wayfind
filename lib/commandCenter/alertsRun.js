...
export async function gatherAlerts(now = new Date()) {
  ...
  return buildAlertsReport({ fractionOfDay, dailyHist, todayK, signupHist, signupToday,
...
export function buildAlertsReport({ fractionOfDay, dailyHist, todayK, signupHist, signupToday,
  cwvField, lab, err24, boundary1h, sentry, syn, deploys, tpToday, freshness }) {
  ...
  return {
    alerts,
    sources,
    baselineNote: "Baselines estimate today’s pace by scaling the median of complete days by the elapsed share of today. Rules stay silent until 7 days of history and minimum volume exist — no false alarms off tiny numbers.",
  };
}
