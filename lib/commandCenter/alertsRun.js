...
export async function gatherAlerts(now = new Date()) {
  ...
  const [dailyHist, todayK, signupHist, signupToday, cwvField, lab, err24, sentry, syn, deploys, tpToday, freshness] = await Promise.all([
  ...
  const sources = [
    todayK.source, cwvField.source, lab.source, err24.source, sentry.source, syn.source, deploys.source, tpToday.source, freshness.source,
  ].filter(Boolean);
