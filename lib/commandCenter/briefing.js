...
import { trafficCountsContradict, trafficTrackingState, VISIT_TRACKING_REPAIRED_AT } from "./trafficDataQuality.js";
...
// A review trigger only, never a conversion threshold: place-page opens do not
// prove that an eligible partner action rendered or was seen.
export const PARTNER_PATH_REVIEW_TRIGGER_OPENS = 30;
...
function affiliateUnavailable(source, confirmedBookings) {
  const clickNote = "Partner clicks are not used as a substitute.";
  if (source.reason === "not_configured") return {
    title: "Travelpayouts reporting is not configured",
    detail: `${source.nextStep || "Add TRAVELPAYOUTS_TOKEN to the server environment."} ${clickNote}`,
  };
  if (source.reason === "error") return {
    title: "Travelpayouts reporting failed",
    detail: `The provider request failed${source.note ? `: ${String(source.note).replace(/[.]+$/, "")}` : ""}. ${clickNote}`,
  };
  if (confirmedBookings !== null) return {
    title: "Paid earnings are unknown",
    detail: `Travelpayouts confirmed ${integer(confirmedBookings)} booking${confirmedBookings === 1 ? "" : "s"}, but did not return a usable paid commission total. ${clickNote}`,
  };
  return {
    title: "Confirmed bookings and paid earnings are unknown",
    detail: `Travelpayouts did not provide provider-confirmed totals. ${clickNote}`,
  };
}
...
export function buildOwnerBriefing({ now = new Date(), period, results = {}, operations = null } = {}) {
  ...
  const pendingFieldVerified = !!(tp && Array.isArray(tp.fields_used) && tp.fields_used.includes("profit_usd"));
  ...
  const trafficState = trafficTrackingState(dateKey, now);
  const metrics = {
    ...
    affiliate: {
      ...
      pendingEarningsUsd: affiliateResult.source.connected === true && pendingFieldVerified ? number(tp && tp.revenue_pending_usd) : null,
  ...
  const failures = [];
  const missing = [];
  const trafficContradiction = trafficCountsContradict(metrics.traffic.deviceCount, metrics.traffic.sessions);
  ...
  } else if (trafficState === "historical_repaired" || trafficState === "active_known_incident") {
    const repaired = trafficState === "historical_repaired";
    const visitEvidence = metrics.traffic.sessions === 0
      ? "No visit events were recorded, but this must not be read as zero actual visits."
      : `${integer(metrics.traffic.sessions)} visit events were recorded, but the total is incomplete and must not be treated as the day's full visit count.`;
    missing.push(item("historical-session-gap", repaired ? "Yesterday includes a repaired visit-tracking gap" : "Yesterday overlaps the known visit-tracking incident",
      `${integer(metrics.traffic.deviceCount)} browsers and devices were recorded during the known session-writer incident. ${visitEvidence}`,
      repaired ? `Keep the device and action counts as historical context only. Visit recording was repaired in production on ${new Date(VISIT_TRACKING_REPAIRED_AT).toLocaleString("en-US", { timeZone: SITE_TZ, month: "short", day: "numeric", hour: "numeric", minute: "2-digit", timeZoneName: "short" })}.` : "Do not judge traffic from visit totals until the production repair is live.",
      kpis.source, "traffic", "unknown"));
  } else if (trafficContradiction) {
    const missingVisits = metrics.traffic.deviceCount > 0;
    failures.push(item(missingVisits ? "traffic-session-gap" : "traffic-device-gap", "Traffic counters need attention",
      missingVisits
        ? `${integer(metrics.traffic.deviceCount)} browsers and devices were recorded, but no visit events were recorded. This does not establish that people made no visits.`
        : `${integer(metrics.traffic.sessions)} visit events were recorded, but no browser or device identifiers were counted. Sessions with missing device identifiers cannot support a reliable audience count.`,
      "Open Visitors and check both visit recording and device identifiers before judging audience quality.", kpis.source, "traffic"));
  ...
  if (metrics.affiliate.confirmedBookings === null || metrics.affiliate.paidEarningsUsd === null) {
    const unavailable = affiliateUnavailable(affiliateResult.source, metrics.affiliate.confirmedBookings);
    missing.push(item("missing-affiliate", unavailable.title, unavailable.detail, "Check the earnings connection in Places & tickets.", affiliateResult.source, "places", "unknown"));
  ...
  const trafficEvidence = trafficState === "comparable_repaired" && !trafficContradiction && metrics.traffic.deviceCount > 0 && metrics.traffic.sessions > 0
    ? item("traffic-denominator-recorded", "Visit events were recorded on a comparable completed day", `${integer(metrics.traffic.deviceCount)} browsers and devices and ${integer(metrics.traffic.sessions)} visit events were recorded on a complete repaired-era day. Their presence makes the day eligible for comparison; it does not prove every visit was captured.`, "Count this as one eligible comparison day; wait for seven internally consistent completed days before judging a traffic trend.", kpis.source, "traffic", "routine")
    : null;
  let partnerEvidence = null;
  if (!engagementValues.some((value) => value === null)) {
    if (metrics.engagement.detailOpens < PARTNER_PATH_REVIEW_TRIGGER_OPENS && metrics.engagement.outClicks === 0) {
      partnerEvidence = item("partner-path-sample-small", "Partner-click evidence is too thin to judge", `${integer(metrics.engagement.detailOpens)} place-page opens and 0 partner clicks were recorded. Eligible partner-action exposure is not measured, so this is not evidence of a conversion failure.`, `Wait for ${PARTNER_PATH_REVIEW_TRIGGER_OPENS} measured opens before even reviewing the path, then verify eligible action exposure first.`, kpis.source, "journey", "routine");
    } else if (metrics.engagement.detailOpens >= PARTNER_PATH_REVIEW_TRIGGER_OPENS && metrics.engagement.outClicks === 0) {
      partnerEvidence = item("partner-path-review-trigger", "The partner path has enough opens for a routine inspection", `${integer(metrics.engagement.detailOpens)} place-page opens and 0 partner clicks crossed the ${PARTNER_PATH_REVIEW_TRIGGER_OPENS}-open review trigger. Opens include places without an eligible partner action, and action exposure is not measured, so this is not a conversion failure.`, "Review whether eligible partner actions rendered and were exposed before considering any conversion change.", kpis.source, "journey", "routine");
    } else {
      partnerEvidence = item("partner-path-measured", "Partner-path counters were recorded", `${integer(metrics.engagement.detailOpens)} place-page opens and ${integer(metrics.engagement.outClicks)} partner click${metrics.engagement.outClicks === 1 ? "" : "s"} were recorded. These independent counters do not establish that an open produced a click, and eligible action exposure is not measured.`, "No partner-path repair is indicated by these counters; verify eligible exposure before interpreting a conversion rate.", kpis.source, "journey", "routine");
    }
  }
  const intentTotal = engagementValues.slice(1, 4).every((value) => value !== null)
    ? metrics.engagement.saves + metrics.engagement.shares + metrics.engagement.directions : null;
  const intentEvidence = intentTotal === null ? null : item("intent-actions-measured",
    intentTotal > 0 ? "High-intent actions were measured" : "No high-intent action was measured",
    `${integer(metrics.engagement.saves)} saves, ${integer(metrics.engagement.shares)} shares, and ${integer(metrics.engagement.directions)} direction taps were recorded.`,
    intentTotal > 0 ? "No intent-tracking repair is indicated by this report." : "Treat this as an observed zero, not a defect; wait for a multi-day baseline before judging demand.",
    kpis.source, "places", "routine");
  const routine = [trafficEvidence, partnerEvidence, intentEvidence].filter(Boolean);
  ...
  const affiliateOnlyMissing = !failures.length && missing.length === 1 && missing[0].id === "missing-affiliate" && metrics.traffic.deviceCount !== null;
  const summary = {
    status,
    headline: status === "attention" ? "A measured issue needs attention" : needsChanges.some((row) => row.id === "historical-session-gap") ? "Yesterday includes a repaired visit-tracking gap" : affiliateOnlyMissing ? "Traffic is reporting. Earnings need attention." : status === "limited" ? "Some owner metrics are unavailable" : "Yesterday's report is complete",
