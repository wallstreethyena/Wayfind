...
export function buildOwnerBriefing({ now = new Date(), period, results = {}, operations = null } = {}) {
  ...
  if (metrics.traffic.deviceCount === null || metrics.traffic.sessions === null) {
    ...
    failures.push(item("traffic-session-gap", "Visit tracking needs attention",
      `${integer(metrics.traffic.deviceCount)} browsers and devices were recorded, but no visits were recorded. This does not establish that people made no visits.`,
      "Open Visitors and check visit tracking before judging audience quality.", kpis.source, "traffic"));
  } else if (metrics.traffic.deviceCount > 0 || metrics.traffic.sessions > 0) {
  ...
  const routine = [
    item("routine-traffic", "Review yesterday's audience quality", `${integer(metrics.traffic.deviceCount)} browsers and devices and ${integer(metrics.traffic.sessions)} recorded visits.`, "Open Visitors and compare how people arrived with what they did.", kpis.source, "traffic", "routine"),
  ...
  ].filter((row) => row.id !== "routine-traffic" || !failures.some((failure) => failure.id === "traffic-session-gap"));
  ...
  const summary = {
    status,
    headline: status === "attention" ? "A measured issue needs attention" : affiliateOnlyMissing ? "Traffic is reporting. Earnings need attention." : status === "limited" ? "Some owner metrics are unavailable" : "Yesterday's report is complete",
