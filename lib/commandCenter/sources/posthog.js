    ...
    const r = await fetchImpl(`${c.host}/api/environments/${encodeURIComponent(c.project)}/query/`, {
      ...
      body: JSON.stringify({
        query: {
          kind: "HogQLQuery",
          query,
          ...(opts.filterTestAccounts ? { filters: { filterTestAccounts: true } } : {}),
        },
      }),
      cache: "no-store",
      signal: ctrl.signal,
...
async function run(cacheKey, ttlMs, query, opts) {
  ...
  try {
    ...
    const note = opts && opts.filterTestAccounts
      ? "project test-account filters applied; no bot filter available in this SDK config"
      : "no bot filter available in this SDK config";
...
FROM events WHERE event = 'web_vitals' AND ${win(from, to)} AND {filters} GROUP BY metric, device ORDER BY metric, device`;
  return run(`ph:cwv:${+from}:${+to}`, 10 * 60 * 1000, q, { ...(opts || {}), filterTestAccounts: true }).then((r) => {
...
FROM events WHERE event = 'web_vitals' AND properties.metric = 'LCP' AND ${win(from, to)} AND {filters} GROUP BY route HAVING samples >= 5 ORDER BY lcp_p75 DESC LIMIT 10`;
  return run(`ph:cwvroute:${+from}:${+to}`, 10 * 60 * 1000, q, { ...(opts || {}), filterTestAccounts: true });
}
