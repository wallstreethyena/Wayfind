...
export async function tpStats(from, to, opts = {}) {
  ...
  try {
    const data = await memTTL(`tp:${day(from)}:${day(to)}`, 15 * 60 * 1000, async () => {
      // SELF-HEALING FIELD LIST: the live API 400s on any field it doesn't
      // know, naming it ("wrong field: profit_usd", "wrong field:
      // campaign_name" — both observed live). Rather than guess their exact
      // vocabulary, start from the documented core set and drop whichever
      // field the error names, retrying up to twice. Core money/state fields
      // are never dropped silently — if THEY are rejected the panel shows the
      // provider error honestly.
      let fields = ["action_id", "date", "state", "price_usd", "paid_profit_usd", "campaign_id"];
      let d = null;
      for (let attempt = 0; attempt < 3; attempt++) {
        ...
        try {
          ...
          if (!r.ok) {
            ...
            const bad = r.status === 400 ? (t.match(/wrong field:?\s*([\w.]+)/i) || [])[1] : null;
            const droppable = bad && !["action_id", "date", "state"].includes(bad) && fields.includes(bad);
            if (droppable && attempt < 2) { fields = fields.filter((f) => f !== bad); continue; }
      ...
      const rows = Array.isArray(d) ? d : (d && (d.data || d.results || d.rows)) || [];
      ...
      for (const row of rows) {
        ...
        const camp = row.campaign_name ? String(row.campaign_name)
          : row.campaign_id != null ? "program " + row.campaign_id : "(all programs)";
