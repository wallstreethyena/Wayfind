...
import { trafficCountsContradict } from "./trafficDataQuality.js";

export const MIN_BASELINE_DAYS = 7;

// Boundary-burst thresholds. Exported so the guard pins the NUMBERS, not just the
// rule: these were chosen against a real incident (2026-07-30, 14 errors / 3
// people / one hour on v6.56), and a later "tidy up" that raised them past that
// shape would silently restore the blind spot.
export const BOUNDARY_MIN_ERRORS = 5;   // with 2+ people, this is systemic
export const BOUNDARY_MIN_PEOPLE = 2;   // 1 person retrying is not a page
export const BOUNDARY_FLOOD = 25;       // enough that even one person's loop matters

// Self-checks whose failure is a page, not a note. `home` was always here.
// `api_rails` joins it (v8.74) because it is the source of EVERY place card on
// the homepage: when it fails, the reader taps a rail and is shown nothing,
// which the owner has now reported twice, both times from his own phone,
// because nothing else was watching. The default 3s slow budget stays for
// ordinary endpoints; targets that cold-build carry their own (`slowMs`).
export const CRITICAL_CHECKS = ["home", "api_rails"];
...
//   trafficHistory       repaired-era subset eligible for traffic comparison
//   trafficTracking      comparable_repaired | historical_repaired | active_known_incident | pre_filter_incomparable | unknown
...
export function computeAlerts(bundle) {
  ...
  const trafficHist = Array.isArray(b.trafficHistory) ? b.trafficHistory : hist;
  const trafficContradiction = !!(b.today && trafficCountsContradict(b.today.devices, b.today.sessions));
  const currentTrafficUnreliable = trafficContradiction || b.trafficTracking === "historical_repaired" || b.trafficTracking === "active_known_incident";
  ...
  if (b.synthetic && Array.isArray(b.synthetic.checks)) {
    ...
    if (bad.length) {
      const critical = bad.filter((c) => CRITICAL_CHECKS.includes(c.key));
      out.push(alert("site_check_failed", critical.length ? "critical" : "warn",
        critical.length ? `Production is failing a critical self-check: ${critical.map((c) => c.label).join(", ")}` : "An endpoint is failing its self-check",
        bad.map((c) => `${c.label}: ${c.error || "HTTP " + c.status} (${c.ms}ms)`).join(" · "),
        { anchor: "health", current: `${bad.length} failing`, baseline: "all checks passing" }));
    }
    // Per-check budget, falling back to the shared one. /api/rails cold-builds
    // a whole metro and is legitimately slower than /api/geo; holding every
    // target to 3s would page hourly for correct behaviour, and an alarm that
    // fires on correct behaviour is the one people learn to ignore.
    const slow = (b.synthetic.checks || []).filter((c) => c.ok && c.ms > (Number(c.slowMs) || SLOW_MS));
    if (slow.length) {
      out.push(alert("site_check_slow", "warn", "Slow responses from production",
        slow.map((c) => `${c.label} ${c.ms}ms (budget ${Number(c.slowMs) || SLOW_MS}ms)`).join(" · "),
        { anchor: "health", current: `${slow[0].ms}ms`, baseline: `< ${Number(slow[0].slowMs) || SLOW_MS}ms` }));
    }
  }

  // — Visit tracking quality —
  if (b.today && ((b.today.devices != null && Number(b.today.devices) > 0) || (b.today.sessions != null && Number(b.today.sessions) > 0))) {
    if (b.trafficTracking === "historical_repaired") {
      const visits = b.today.sessions == null ? "an unavailable visit total" : Number(b.today.sessions) === 0 ? "no recorded visit events" : `${num(b.today.sessions)} recorded visit events`;
      out.push(alert("visit_tracking_history_repaired", "info", "Today includes the repaired visit-tracking gap",
        `${num(b.today.devices)} active devices and ${visits} were measured on an ET day affected by the known session-writer incident. The visit total is incomplete historical data, not a current outage or a complete audience count.`,
        { anchor: "traffic", current: `${num(b.today.devices)} devices / incomplete visit total`, baseline: "repair live Sep 19, 9:35 PM ET" }));
    } else if (trafficContradiction && b.trafficTracking === "active_known_incident") {
      out.push(alert("visit_tracking_gap_active", "warn", "Visit tracking is in the known incident window",
        "The device and visit counters contradict each other during the known incident window. The measurement repair is not yet production-ready.",
        { anchor: "traffic", current: `${num(b.today.devices)} devices / ${num(b.today.sessions)} visits`, baseline: "both counters should agree" }));
    } else if (trafficContradiction && b.trafficTracking === "comparable_repaired") {
      const missingVisits = Number(b.today.devices) > 0;
      out.push(alert(missingVisits ? "visit_tracking_gap" : "device_tracking_gap", "warn", "Traffic measurement may have regressed after repair",
        missingVisits
          ? "Active devices are arriving on a repaired-era day while no visits are being recorded. Check the session writer before interpreting audience or conversion rates."
          : "Visit events are arriving on a repaired-era day while no browser or device identifiers are being counted. Check device identification before interpreting audience or conversion rates.",
        { anchor: "traffic", current: `${num(b.today.devices)} devices / ${num(b.today.sessions)} visits`, baseline: "both counters should agree" }));
    }
  }

  // — Traffic vs baseline —
  if (!currentTrafficUnreliable && trafficHist.length >= MIN_BASELINE_DAYS && b.today) {
    const base = median(trafficHist.map((d) => d.devices));
    if (base != null && base >= 10) {
      ...
      if (expected >= 5 && cur < expected * 0.5) {
        out.push(alert("traffic_drop", "warn", "Traffic is materially below baseline",
          `Active devices today are tracking at ${cur} vs ~${Math.round(expected)} expected by this time (median of last ${trafficHist.length} comparable repaired-era days, scaled by elapsed share of day; an estimate).${cur > 0 && b.today.sessions === 0 ? " No visits were recorded despite active devices. Check visit tracking before treating this as a fall in actual traffic." : ""}`,
  ...
  // — Boundary-error burst, PER BUILD, PER HOUR —
  //
  // The 24h rule above could not have caught 2026-07-30: a ReferenceError in
  // v6.56 crashed every bookable place sheet and produced 14 errors from 3 users
  // in one afternoon — under the 20-in-24h threshold, and diluted by an otherwise
  // quiet day. The first report came from the owner's phone.
  //
  // TWO PEOPLE IS THE SIGNAL, NOT THE ERROR COUNT. One user can generate a dozen
  // errors by retrying, so a single flaky device must not page anyone. Two
  // distinct people on the same build in the same hour is systemic — that is what
  // separates "someone's browser is unhappy" from "we shipped a crash".
  if (b.boundaryErrors && Array.isArray(b.boundaryErrors.byBuild)) {
    for (const row of b.boundaryErrors.byBuild) {
      const errors = num(row && row.errors);
      const people = num(row && row.people);
      const build = (row && row.build) || "unknown build";
      const systemic = errors >= BOUNDARY_MIN_ERRORS && people >= BOUNDARY_MIN_PEOPLE;
      const flood = errors >= BOUNDARY_FLOOD;
      if (!systemic && !flood) continue;
      out.push(alert("boundary_errors_" + String(build).replace(/[^\w.-]/g, ""),
        people >= 3 || flood ? "critical" : "warn",
        `Build ${build} is crashing for real users`,
        `${errors} error-boundary crash${errors === 1 ? "" : "es"} from ${people} ${people === 1 ? "person" : "people"} in the last hour on ${build}. ` +
        "The boundary records message, stack and build on every one — read app_error in PostHog for the exact throw.",
        { anchor: "health", current: `${errors} crashes / ${people} people`, baseline: "zero" }));
    }
  }
