...
// v8.93 (owner, 2026-08-30): "make sure that the Date Night card is next to
// Tonight's Move, to the right of Tonight's Move."
//
// It is the pairing that makes sense of the two: Tonight's Move is what is ON,
// Date Night is who you are going with.
//
// v8.93.1, same night, later: "I want the position of this to be to the right
// of Tonight's Move" — of TRENDING, with its new poster. So the immediate-right
// slot is trending and Date Night follows it, which keeps his first
// instruction intact (Date Night sits with Tonight's Move, not off in the
// category tail) while honouring the newer one about which card owns slot two.
// The three of them are one thought in order: what is on, what people are
// actually doing, and who you are going with. season gives up one more place;
// it still never leads, and test-seasonal-picks holds that rule.
// The consolidated Today surface must remain in the first visible desktop
// group.  It replaced three separate posters; parking it at slot 13 made the
// consolidation technically present but functionally invisible.
const NIGHT_ORDER = ['augtober', 'tonight', 'trending', 'datenight', 'season', 'today', 'lunchcity', 'events', 'eat', 'chef', 'locals', 'drive', 'birthday', 'family', 'beach', 'break', 'breakfast', 'cindy', 'blog'];
...
export const DAYPARTS = {
  ...
  morning: {
    ...
    why: 'Fall in Florida leads, then breakfast and what is moving.',
    order: ['augtober', 'breakfast', 'trending', 'season', 'today', 'lunchcity', 'eat', 'cindy', 'chef', 'beach', 'birthday', 'family', 'drive', 'locals', 'break', 'datenight', 'tonight', 'events', 'blog'],
  },
  lunch: {
    label: 'Lunch', from: 11.5, to: 13,
    why: 'Fall in Florida leads, then a quick lunch and the afternoon.',
    order: ['augtober', 'break', 'trending', 'season', 'today', 'lunchcity', 'eat', 'beach', 'chef', 'cindy', 'family', 'birthday', 'locals', 'drive', 'events', 'tonight', 'datenight', 'breakfast', 'blog'],
  },
  afternoon: {
    label: 'Afternoon', from: 13, to: 17.5,
    why: 'Fall in Florida leads, with tonight right behind it.',
    order: NIGHT_ORDER,
  },
  night: {
    label: 'Night', from: 17.5, to: 6,
    why: 'Fall in Florida leads, with tonight and dinner behind it.',
    order: NIGHT_ORDER,
  },
};
...
/**
 * Homepage Date Night poster → the qualified-intent page, not the in-rail drop.
 * City / lat / lng ride as query params the page already reads. Never invent a city.
 */
export function dateNightIntentHref({ href, cityLabel, lat, lng } = {}) {
  const dest = href || "/date-night";
