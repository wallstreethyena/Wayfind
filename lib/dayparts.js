...
// order the FIRST poster differently. Every `from`/`to` is a BUCKET_EDGES
// value — no private hour lives here. BAND_TO_BUCKET states the mapping and
// scripts/test-dayparts.mjs proves it holds for all 1,440 minutes of the day.
...
import { BUCKET_EDGES } from "./nowContext.js";
...
export const DAYPARTS = {
  ...
  morning: {
    label: 'Morning', from: BUCKET_EDGES.morningStart, to: BUCKET_EDGES.afternoonStart,
  ...
  lunch: {
    label: 'Lunch', from: BUCKET_EDGES.afternoonStart, to: BUCKET_EDGES.lunchEnd,
  ...
  afternoon: {
    label: 'Afternoon', from: BUCKET_EDGES.lunchEnd, to: BUCKET_EDGES.nightStart,
  ...
  evening: {
    label: 'Evening', from: BUCKET_EDGES.nightStart, to: BUCKET_EDGES.lateNightStart,
    why: 'Dinner leads, and tonight is one swipe away.',
    // 17:30–22:00 is when people eat. tonight sits fourth — nearby, not
    // first. The night band takes the lead at 22:00, when people start
    // preparing for nighttime.
    order: ['eat', 'trending', 'season', 'tonight', 'datenight', 'events', 'augtober', 'chef', 'best', 'locals', 'gems', 'drive', 'birthday', 'family', 'today', 'beach', 'break', 'breakfast', 'cindy', 'blog'],
  },
  night: {
    label: 'Night', from: BUCKET_EDGES.lateNightStart, to: BUCKET_EDGES.morningStart,
