...
//
// `fetchImpl` is an optional 5th hook (2026-09-09), added so wikiOpensearch/
// wikiPageInfo below can be reused hermetically by lib/commonsPhotos.js's
// guard (scripts/test-commons-photos.mjs) without a real network call. It
// defaults to the global fetch — every existing call site omits it and is
// byte-for-byte unchanged.
const jf = async (url, init, diagSrc, onFail, fetchImpl) => {
  const doFetch = fetchImpl || fetch;
  const ctrl = new AbortController();
  const t = setTimeout(() => ctrl.abort(), T);
  try {
    const r = await doFetch(url, { ...init, signal: ctrl.signal });
...
// ── FOURSQUARE: PARKED 2026-09-09 (owner decision, dormant NOT deleted) ─────
//
// WHY. Measured over the full recorded pulse history, this provider has never
// produced a single wf_place_popularity row. Two independent reasons, and the
// second is the one that matters:
//
//   1. VOLUME. Foursquare's pricing (effective 2026-06-01) grants 500 free Pro
//      calls per month. SOURCE_CAPS.foursquare is 30 per run and the popularity
//      cron runs every 2h, so 12 x 30 = 360 calls/DAY, about 10,800/month —
//      roughly 21x the free allowance. The allowance is spent in ~1.4 days,
//      which is exactly the length of the one window it answered (2026-09-01 to
//      09-03, right after the monthly reset). Everything else is HTTP 429.
//
//   2. THE FIELD ISN'T IN OUR PLAN. During that working window every single
//      successful response came back WITHOUT `popularity` — the pulse note was
//      `no_field`, 15 to 21 of each 30-call run. Foursquare documents
//      `popularity` as a Premium attribute while the Places endpoints return
//      Pro fields by default. So paying the ~$155/month the volume above would
//      cost buys MORE CALLS THAT STILL OMIT THE ONE FIELD WE ASK FOR. This
//      fetcher deliberately refuses to manufacture a score when the field is
//      absent (see the `typeof popularity !== "number"` return below), which is
//      correct and is why the row count is zero rather than fabricated.
//
// WHY PARKED AND NOT DELETED. `/api/fsq/search` and `/api/sources/compare` are
// separate, working Foursquare surfaces and are untouched. FSQ_BREAKER stays
// wired. fetchFoursquare's body stays intact. If the owner later buys Premium,
// re-enabling is deleting the one constant below — not rebuilding an
// integration.
//
// WHY THE REASON STRING MATTERS. It must NOT carry the `billing:`/`quota:`
// prefix: lib/jobPulse.classifyHealth escalates that prefix to an incident on
// the FIRST dead run. A deliberately parked provider is not an incident. It
// records attempted=0/succeeded=0/failed=0, which wf_job_health's
// `dead = ((attempted > 0 OR failed > 0) AND succeeded = 0)` rule reads as IDLE.
// Verified live 2026-09-09: tripadvisor, parked the same way since its
// 2026-08-31 sunset, sits at consecutive_zero=0 while foursquare and yelp both
// sat at 23 and were paging.
export const FOURSQUARE_PARKED_REASON = "parked_premium_field_20260909";

// ── YELP: PARKED 2026-09-09 (owner decision, dormant NOT deleted) ───────────
//
// WHY. The Yelp Fusion developer console for the Wayfind app reads, verbatim:
// "Your free trial ended. Start a paid subscription to continue your access."
// (observed 2026-09-09 in the owner's own browser). So the `bad_key_format`
// outcome that paged job-watch 23 runs in a row was a symptom, not the cause:
// the stored key is 127 characters against the 128 Fusion issues, but a
// perfect key would be refused just the same — there is no plan behind it.
//
// OWNER'S FREE-FIRST RULE (2026-09-04): a provider that has become a bill is
// not re-enabled by spending on it by default. Yelp was the only non-Wikipedia
// popularity source for food and nightlife, so this IS a real loss of signal —
// recorded plainly, not hidden — and it is a purchasing decision, not a bug.
//
// WHY PARKED AND NOT DELETED. fetchYelp's body, YELP_KEY_RE, the cuisine
// restriction and SOURCE_CAPS.yelp all stay intact. Re-enabling after a paid
// plan is: repaste a full 128-char key in Vercel, then delete the constant
// below. Nothing else.
//
// WHY THE REASON STRING MATTERS. Same rule as FOURSQUARE_PARKED_REASON: no
// `billing:`/`quota:` prefix (lib/jobPulse.classifyHealth escalates that on the
// first dead run), attempted=0/succeeded=0/failed=0 so wf_job_health files it
// as IDLE, not dead. A provider that is off on purpose is not an incident.
export const YELP_PARKED_REASON = "parked_fusion_trial_ended_20260909";
...
export function popularityAvailability(source, env) {
  ...
  if (source === 'yelp') {
    // Parked BEFORE the key check, exactly like foursquare below: the plan is
    // what is missing, so a configured (even well-formed) key must not re-open
    // the tap and must not page as a bad key.
    if (YELP_PARKED_REASON) return { ready: false, failure: false, reason: YELP_PARKED_REASON };
  ...
  // Parked BEFORE the key check: the credential is fine, the PLAN is what is
  // missing, so a configured key must not re-open the tap. failure:false keeps
  // job-watch quiet about a provider that is off on purpose.
  if (source === 'foursquare') return { ready: false, failure: false, reason: FOURSQUARE_PARKED_REASON };
...
export async function fetchYelp(place) {
  // Second layer, same as fetchFoursquare: the preflight gates the CRON, this
  // gates every OTHER caller, before any network call. Body intact for re-enable.
  if (YELP_PARKED_REASON) { notePop("yelp", YELP_PARKED_REASON); return null; }
...
export async function fetchFoursquare(place) {
  // Second layer, deliberately not redundant: popularityAvailability gates the
  // CRON, this gates every OTHER caller (a backfill script, a future rail) so
  // parking cannot be bypassed by calling the fetcher directly. Returns before
  // any network call. Everything below stays intact for re-enable.
  if (FOURSQUARE_PARKED_REASON) { notePop("foursquare", FOURSQUARE_PARKED_REASON); return null; }
...
// Exported (2026-09-09) so lib/commonsPhotos.js's own commons.wikimedia.org
// imageinfo call identifies itself the same way, per Wikimedia's UA policy —
// one identity for every Wayfind Wikimedia request, not a second string that
// can drift from this one.
export const WIKI_UA = { headers: { "user-agent": "WayfindBot/1.0 (https://www.gowayfind.com; hello@gowayfind.com)", "api-user-agent": "WayfindBot/1.0 (https://www.gowayfind.com; hello@gowayfind.com)" } };
...
export function stripWikiQualifier(name) {
  let n = String(name || "");
...
// Exported (2026-09-09) so lib/commonsPhotos.js reuses this SAME candidate
// picker — the "which opensearch hit is even worth checking" half of the
// matcher — rather than re-deriving its own nameSim loop. See the export note
// on jf() above and on wikiOpensearch/wikiPageInfo below for why this is safe
// to reuse hermetically.
export function bestWikiTitle(query, titles) {
...
// Exported (2026-09-09), with an optional injectable fetchImpl, so
// lib/commonsPhotos.js's photo resolver can run the IDENTICAL opensearch step
// fetchWikipedia uses below — never a second, looser candidate lookup — and
// so scripts/test-commons-photos.mjs can prove it hermetically. Every
// existing call in this file omits fetchImpl and is unchanged.
export async function wikiOpensearch(q, fetchImpl) {
  return jf(`https://en.wikipedia.org/w/api.php?action=opensearch&search=${encodeURIComponent(q)}&limit=5&namespace=0&format=json&origin=*`, WIKI_UA, "wikipedia", undefined, fetchImpl);
}
...
// Exported (2026-09-09), same reasoning as wikiOpensearch above: this is the
// exact call verifyWikiIdentity's inputs come from, so reusing it is what
// makes lib/commonsPhotos.js's identity check the SAME check, not a parallel
// one that can drift.
export async function wikiPageInfo(title, fetchImpl) {
  return jf(`https://en.wikipedia.org/w/api.php?action=query&format=json&prop=pageprops%7Ccoordinates%7Ccategories%7Cextracts&clshow=%21hidden&cllimit=50&exintro=1&explaintext=1&exsentences=2&redirects=1&titles=${encodeURIComponent(title)}&origin=*`, WIKI_UA, "wikipedia", undefined, fetchImpl);
}
...
export async function fetchWikipedia(place) {
  ...
  const found = await wikiSearchCandidate(place.name);
  ...
  if (!m) { if (found.sawResponse) notePop("wikipedia", "no_match"); return null; }
