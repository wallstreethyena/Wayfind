...
export async function fetchTripadvisor(place) {
  ...
  const s = await jf(`https://api.content.tripadvisor.com/api/v1/location/search?key=${key}&searchQuery=${encodeURIComponent(place.name)}&latLong=${place.lat},${place.lng}&radius=2&radiusUnit=mi&language=en`, undefined, "tripadvisor");
  ...
  const det = await jf(`https://api.content.tripadvisor.com/api/v1/location/${m.cand.id}/details?key=${key}&language=en`, undefined, "tripadvisor");
