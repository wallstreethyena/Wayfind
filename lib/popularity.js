...
export const SOURCE_CAPS = { tripadvisor: 20, foursquare: 30 }; // per cron run
// v8.29.14 — SITE_URL backs the Referer header TripAdvisor's domain restriction
// requires. Same constant the canonical URLs are built from, so the header can
// never disagree with the domain the key is registered against.
import { SITE_URL } from "./site.js";   // .js extension: scripts/test-popularity.mjs loads this as raw ESM, where extensionless resolution does not apply
...
const jf = async (url, init, diagSrc) => {
  const ctrl = new AbortController();
  const t = setTimeout(() => ctrl.abort(), T);
  try {
    const r = await fetch(url, { ...init, signal: ctrl.signal });
    if (!r.ok) { if (diagSrc) notePop(diagSrc, "http_" + r.status); return null; }
    return await r.json();
...
export async function fetchFoursquare(place) {
  ...
  let d = key.startsWith("fsq3")
    ? await jf(`https://api.foursquare.com/v3/places/search?${q}&fields=fsq_id,name,geocodes,popularity`, { headers: { Authorization: key } }, "foursquare_v3")
    : null;
  if (!d) d = await jf(`https://places-api.foursquare.com/places/search?${q}`, { headers: { Authorization: "Bearer " + key, "X-Places-Api-Version": "2025-06-17", Accept: "application/json" } }, "foursquare");
...
export async function fetchTripadvisor(place) {
  ...
  if (!key) { notePop("tripadvisor", "no_key"); return null; }
  // v8.29.14 — THE REFERER HEADER. TripAdvisor's Content API requires every key to
  // be restricted to either an IP allowlist or a DOMAIN, and their own docs say
  // that for callers whose IPs are not published — "these services don't publish
  // the IPs of the machines issuing requests" — you must use domain restriction,
  // where "all API requests must include the HTTP Referer Header with an URL
  // containing a restricted domain name."
  //
  // Vercel's egress is exactly that case, and these two calls sent NO headers at
  // all. Production got http_403 on every one for seven days: 1,680 attempts,
  // zero successes, zero rows ever written. A dummy key returns the byte-identical
  // 403 ("explicit deny in an identity-based policy"), which is why this read as a
  // dead credential rather than a missing header.
  //
  // NOTE FOR THE OWNER: the key must ALSO be set to domain restriction
  // `gowayfind.com` in the TripAdvisor developer portal. The header alone is half
  // the contract; an IP-restricted key will keep returning 403 no matter what we send.
  const TA_HEADERS = { headers: { Referer: SITE_URL + "/", Origin: SITE_URL, Accept: "application/json" } };
  const s = await jf(`https://api.content.tripadvisor.com/api/v1/location/search?key=${key}&searchQuery=${encodeURIComponent(place.name)}&latLong=${place.lat},${place.lng}&radius=2&radiusUnit=mi&language=en`, TA_HEADERS, "tripadvisor");
  const cands = (s && s.data || []).slice(0, 3).map((r) => ({ id: r.location_id, name: r.name, lat: null, lng: null }));
  const m = bestMatch(place, cands);
  if (!m) { if (s) notePop("tripadvisor", "no_match"); return null; }
  const det = await jf(`https://api.content.tripadvisor.com/api/v1/location/${m.cand.id}/details?key=${key}&language=en`, TA_HEADERS, "tripadvisor");
  const n = det && det.num_reviews != null ? parseInt(det.num_reviews, 10) : null;
  if (!Number.isFinite(n)) { if (det) notePop("tripadvisor", "no_field"); return null; }
  notePop("tripadvisor", "ok");
...
export async function fetchWikipedia(place) {
  ...
  const s = await jf(`https://en.wikipedia.org/w/api.php?action=opensearch&search=${encodeURIComponent(place.name)}&limit=1&namespace=0&format=json&origin=*`, WIKI_UA);
  ...
  if (!title || nameSim(place.name, title) < CONFIDENCE_FLOOR) { if (s) notePop("wikipedia", "no_match"); return null; }
  const end = new Date(); end.setUTCDate(end.getUTCDate() - 1);
  const start = new Date(end); start.setUTCDate(start.getUTCDate() - 30);
  const fmt = (dt) => dt.toISOString().slice(0, 10).replace(/-/g, "");
  const pv = await jf(`https://wikimedia.org/api/rest_v1/metrics/pageviews/per-article/en.wikipedia/all-access/user/${encodeURIComponent(title.replace(/ /g, "_"))}/daily/${fmt(start)}/${fmt(end)}`, WIKI_UA);
