...
// ── THE SWITCH ─────────────────────────────────────────────────────────────
// ONE constant, read by BOTH layers below. Flip it to false and Foursquare
// popularity is fully live again: the cron selects work and the fetcher issues
// requests. That is the whole recovery procedure.
//
// It is a BOOLEAN and not the reason string for a measured reason. The first
// version of this park used the reason string as the switch, and the two layers
// disagreed about what "off" meant:
//   - setting it to ""  -> availability STILL parked (it never consulted the
//                          constant, it parked unconditionally) while the
//                          fetcher's `if (REASON)` went falsy and resumed
//                          hitting the API. Two layers, opposite states.
//   - deleting the line -> ReferenceError at the fetcher's first call.
// Both were reproduced before this rewrite. A recovery mechanism that has to be
// performed exactly right, and crashes or half-applies otherwise, is not a
// recovery mechanism.
export const FOURSQUARE_PARKED = true;
...
// `opts.parked` defaults to the FOURSQUARE_PARKED constant, so every production
// caller (the cron passes exactly two arguments) is unchanged. It exists so the
// guard can prove BOTH states by CALL — parked and enabled — without mutating
// this module or reading the environment.
export function popularityAvailability(source, env, { parked = FOURSQUARE_PARKED } = {}) {
  ...
  // job-watch quiet about a provider that is off on purpose. Flip
  // FOURSQUARE_PARKED to false and the original key-based behaviour below
  // resumes untouched.
  if (source === 'foursquare' && parked) return { ready: false, failure: false, reason: FOURSQUARE_PARKED_REASON };
...
// Second parameter mirrors popularityAvailability's: production calls this with
// one argument (app/api/cron/popularity/route.js does `FETCHERS[src](p)`), so
// the default applies everywhere real, and the guard can drive the enabled path.
export async function fetchFoursquare(place, { parked = FOURSQUARE_PARKED } = {}) {
  ...
  // any network call. Everything below stays intact for re-enable, and reads
  // the SAME switch the availability layer does — they cannot disagree.
  if (parked) { notePop("foursquare", FOURSQUARE_PARKED_REASON); return null; }
