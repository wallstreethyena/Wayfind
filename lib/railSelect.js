...
import { rankBeachesForToday } from "./beachDecision.js";
...
export const RAIL_SELECT = {
  ...
  datenight: { pools: ["restaurants", "summer"],
    identity: (p) => isDateRoom(p) && (!p._summerSourced || summerOn(p, "datenight")),
    pick: (p) => (p._summerSourced && summerNear(p))
...
export function fillRails(pools, slim = (p) => p, ctx) {
  ...
  for (const rail of RAILS) {
    ...
    if (rail.id === "beach") picked = rankBeachesForToday(picked);
    if (ctx && Number.isFinite(ctx.nearMi)) {
      ...
      const near = rail.id === "beach" ? BEACH_NEAR_MI : rail.id === "drive" ? DRIVE_REACH_MI : rail.id === "birthday" ? BIRTHDAY_NEAR_MI : ctx.nearMi;
      const widen = rail.id === "beach" ? BEACH_NEAR_MI : rail.id === "drive" ? DRIVE_REACH_MI : rail.id === "birthday" ? BIRTHDAY_NEAR_MI : ctx.widenMi;
