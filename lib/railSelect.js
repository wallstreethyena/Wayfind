...
import { isDateNightShortlist, DATENIGHT_NEAR_MI } from "./dateRoom.js";
...
import { isShowVenue } from "./showVenue.js";
...
export const RAIL_SELECT = {
  ...
  // v8.93 — THE POSTER IS THE CONTRACT. The owner's Date Night tile names
  // speakeasies, fancy restaurants, clubs and rooftops and says we will take
  // you up to 27 miles. The room rule (`isDateRoom`) still decides a table
  // for two; `isDateNightShortlist` is that rule PLUS the existing club /
  // show / rooftop / street-event classifiers so the drop matches the art.
  // A summer tag still cannot exempt a park. Distance is DATENIGHT_NEAR_MI
  // in fillRails — hard 27, no 17→25 widen.
  datenight: { pools: ["datenight", "restaurants", "nightlife", "events", "summer"],
    identity: (p) => isDateNightShortlist(p) && (!p._summerSourced || summerOn(p, "datenight")),
    pick: (p) => (p._summerSourced && summerNear(p))
      || (priceNum(p) || 0) >= 2 || nameHas(p, ROOM_WORDS)
      || isDateNightShortlist(p) },
...
export function fillRails(pools, slim = (p) => p, ctx) {
  ...
  for (const rail of RAILS) {
    ...
    if (ctx && Number.isFinite(ctx.nearMi)) {
      ...
      // Date Night is the same shape at 27.0 — the poster states the
      // radius, so a 28-mile steakhouse is out even when 17 cannot fill.
