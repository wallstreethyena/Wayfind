...
import { CREATOR_FINDS_RADIUS_MI } from "./creatorFinds.js";
...
export function fillRails(pools, slim = (p) => p, ctx) {
  ...
  for (const rail of RAILS) {
    ...
    if (ctx && Number.isFinite(ctx.nearMi)) {
      ...
      // The creator library has its own 25-mile coverage contract. Three
      // closer finds must not hide other verified creators inside that area.
      const near = rail.id === "locals" ? CREATOR_FINDS_RADIUS_MI : rail.id === "beach" ? BEACH_NEAR_MI : rail.id === "drive" ? DRIVE_REACH_MI : rail.id === "birthday" ? BIRTHDAY_NEAR_MI : ctx.nearMi;
      const widen = rail.id === "locals" ? CREATOR_FINDS_RADIUS_MI : rail.id === "beach" ? BEACH_NEAR_MI : rail.id === "drive" ? DRIVE_REACH_MI : rail.id === "birthday" ? BIRTHDAY_NEAR_MI : ctx.widenMi;
