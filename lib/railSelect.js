...
export const RAIL_SELECT = {
  ...
  family: { pools: ["things-to-do", "summer"],
    pick: (p) => hasAny(p, FAMILY_TYPES) || (summerOn(p, "family") && summerNear(p)) },
  ...
  tonight: { pools: ["nightlife", "summer"],
    pick: (p) => !p._summerSourced || (summerNear(p) && (summerOn(p, "tonight") || summerOn(p, "datenight"))) },
  ...
  // date on it.
  events: { pools: ["things-to-do", "nightlife", "summer"],
    pick: (p) => hasAny(p, TICKETED_TYPES) || (summerOn(p, "events") && summerNear(p)) },
  ...
  break: { pools: ["restaurants", "creators"], pick: (p) => isQuickService(p) && (p.distMi || 0) <= 8 },
  ...
  breakfast: { pools: ["restaurants", "creators"],
...
export function fillRails(pools, slim = (p) => p, ctx) {
  ...
  for (const rail of RAILS) {
    ...
    if (ctx && Number.isFinite(ctx.nearMi)) {
      const near = rail.id === "beach" ? BEACH_NEAR_MI : ctx.nearMi;
      const widen = rail.id === "beach" ? BEACH_NEAR_MI : ctx.widenMi;
      picked = pickNearThenWiden(picked, near, widen, MIN_CARDS);
    }
    if (picked.length < MIN_CARDS) { places[rail.id] = []; thin.push(rail.id); continue; }
    // v8.10 — the no-place-leads-twice lead swap is GONE. It was a
    // merchandising reorder, and the owner's global rule (2026-08-18) is
    // absolute: highest displayed score first, every rail, no exceptions. If
    // the same place genuinely tops two axes, it leads both — being right
    // beats being novel, and the chip must read in order.
    const rows = picked.slice(0, MAX_CARDS);
