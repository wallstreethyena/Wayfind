...
const HIDDEN_CTA = Object.freeze({
  kind: "none", href: null, label: null, sponsored: false, monetized: false, deal: null, place: null, exact: false,
});

// Search dests never paint as Book / Tickets / Viator. bookQuery is a
// search query, not a product. The Crystal River guide leaked
// /api/viator/go?q=…&intent=search as a Book hop onto searchResults.
export function paintGuideCta(cta) {
  if (!cta) return HIDDEN_CTA;
  if (cta.kind === "tour" && !cta.exact) return HIDDEN_CTA;
  if (isSearchAsBookHref(cta.href)) return HIDDEN_CTA;
  return cta;
}
...
function resolvePrimaryCta(g, todayIso) {
  ...
  // v9.0 — AN EVENT'S OWN TICKET. A guide ABOUT an event (the fall-events
  ...
  // A guide that is plainly about one admission park may name that exact
  // place here. Resolution stays in the same curated place-partner map used
  // by Detail cards, including its retired-product gate. No name fragment,
  // search result, or partner URL can become a ticket button through this
  // rung. Event admission above remains more specific and therefore wins.
  if (g && g.parkTicketPlace) {
    const place = String(g.parkTicketPlace).trim();
    const partner = placePartnerPick({ name: place });
    const href = partner && commerceHref({
      provider: partner.provider,
      offerId: partner.offerId,
      surface: "guide",
      contentId: "guide:" + place,
    });
    if (partner && href) {
      return {
        kind: "deal", href, sponsored: true, monetized: true, exact: true,
        provider: partner.provider, offerId: partner.offerId,
        place, deal: null, label: `Tickets for ${place}`,
      };
    }
  }

  if (intent === "hotel") {
    // Reuse the tracked hotel redirect shared by Stays and event pages.
    ...
    const t = bookingTargets(
      ...
      "hotels", null, region);
    // Keep raw booking.com links out of earning CTAs.
    if (t.tu && !/booking\.com/i.test(String(t.tu))) {
      return { kind: "hotel", href: t.tu, label: "Check rates & availability", sponsored: true, monetized: true, deal: null, place: pick.name, exact: false };
    }
  }
  ...
  if (intent === "tour") {
    ...
    // Search dests (bookQuery → experienceGoUrl?intent=search) are not Book —
    // skip them. Hide, do not pin a Crystal River SKU, do not invent 236862P2.
    for (const p of g.picks) {
      ...
      const href = (exact && p.viatorUrl)
        ...
        : t.tk;
      if (!exact || isSearchAsBookHref(href)) continue;
      const venue = pickVenueLabel(p.name);
      return {
        kind: "tour", href, sponsored: true, monetized: true, deal: null, place: p.name, exact: true,
        label: venue ? `See tickets for ${venue}` : "See tickets & availability",
      };
    }
  }
...
export function guidePrimaryCta(g, todayIso) {
  ...
  const labeled = (!cta || !cta.label) ? cta : { ...cta, label: namedStepLabel(cta) };
  return paintGuideCta(labeled);
}
