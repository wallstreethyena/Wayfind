...
export function guidePrimaryCta(g, todayIso) {
  ...
  if (intent === "hotel") {
    ...
    const t = bookingTargets(
      ...
      "hotels", null, region);
    if (t.tu) return { kind: "hotel", href: t.tu, label: "Check rates & availability", sponsored: true, monetized: true, deal: null, place: pick.name, exact: true };
  }
  ...
  if (intent === "tour") {
    ...
    for (const p of g.picks) {
      ...
      return {
        kind: "tour", href: t.tk, sponsored: true, monetized: true, deal: null, place: p.name, exact,
        label: exact ? `See tickets for ${p.name}` : `Find tours in ${region}`,
      };
    }
  }
