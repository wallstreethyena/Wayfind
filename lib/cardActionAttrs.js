...
export const ACTION_ATTR = "data-wf-act";     // "like" | "dislike" | "save" | "share"
...
//   state actions   tapping twice removes the queued intent and un-paints, so
//                   the queue and the pixels can never disagree
//   share once      Share is an action, not a pressed state. Repeated early
//                   taps keep one queued share and never paint aria-pressed.
...
export function cardActionBridgeScript() {
  return `(function(){try{var Q=window.${PENDING_ACTIONS_KEY}=window.${PENDING_ACTIONS_KEY}||[];document.addEventListener('click',function(e){try{var t=e.target;if(!t||!t.closest)return;var el=t.closest('[${ACTION_ATTR}]');if(!el)return;if(el.closest('[${LIVE_ATTR}="1"]'))return;var id=el.getAttribute('${PLACE_ATTR}');var a=el.getAttribute('${ACTION_ATTR}');if(!id||!a)return;e.preventDefault();e.stopPropagation();var k=a+':'+id,at=-1;for(var i=0;i<Q.length;i++){if(Q[i]&&Q[i].k===k){at=i;break}}if(a==='share'){if(at<0&&Q.length<${MAX_PENDING})Q.push({k:k,action:a,id:id});return}var on;if(at>=0){Q.splice(at,1);on=false}else if(Q.length<${MAX_PENDING}){Q.push({k:k,action:a,id:id});on=true}else{return}if(!el.hasAttribute('${WAS_ATTR}'))el.setAttribute('${WAS_ATTR}',el.getAttribute('aria-pressed')==='true'?'true':'false');el.setAttribute('aria-pressed',on?'true':'false');if(el.classList)el.classList[on?'add':'remove']('is-active')}catch(_){}} ,true)}catch(_){}})();`;
}
