...
// below, or in DELIBERATELY_UNPLACED with a reason. Placement here closed 4
...
// miami-pass-gocity-all-inclusive); the remainder are named in
// DELIBERATELY_UNPLACED with the reason a live image/coordinate could not be
// sourced without guessing, per this lane's standing instruction never to do
// that.
...
export const MENU_PARTNER_OFFERS = Object.freeze([
  ...
  // Undercover Tourist admission rows — same physical venue as the tiqets/
  // klook row named in each comment; no separate UT product photo exists in
  // this repo's JS (wf_deals is DB-only), so the image is reused, never a
  // different venue's photo.
  offer("6", "undercover_tourist", "Universal Orlando Resort", "Universal Orlando Resort (Undercover Tourist)", "Orlando", 28.5383, -81.3792, "https://upload.wikimedia.org/wikipedia/en/d/d0/Universal_Orlando_Resort_logo_2023.png", ["attractions:themeparks", "attractions:family", "family:all", "attractions:all"]), // reuses orlando-klook-universal-admission's image
  offer("7", "undercover_tourist", "SeaWorld Orlando", "SeaWorld Orlando (Undercover Tourist)", "Orlando", 28.5383, -81.3792, "https://aws-tiqets-cdn.imgix.net/images/content/089ce275d9e24a53814738dda3a3eb45.jpg?auto=format%2Ccompress&fit=crop&q=70", ["attractions:themeparks", "attractions:family", "family:all", "family:kids", "attractions:all"]), // reuses orlando-hook-seaworld's image
  offer("13", "undercover_tourist", "Gatorland", "Gatorland (Undercover Tourist)", "Orlando", 28.5383, -81.3792, "https://aws-tiqets-cdn.imgix.net/images/content/740cc2ae6a03412bb1da2e85f8c9539a.jpg?auto=format%2Ccompress&fit=crop&q=70", ["attractions:themeparks", "attractions:family", "family:all", "family:kids", "attractions:all"]), // reuses orlando-hook-gatorland's image
  offer("14", "undercover_tourist", "Discovery Cove", "Discovery Cove (Undercover Tourist)", "Orlando", 28.5383, -81.3792, "https://aws-tiqets-cdn.imgix.net/images/content/e0df5df4b2fa4addbc5c2609e9cbc468.jpg?auto=format%2Ccompress&fit=crop&q=70", ["attractions:themeparks", "family:all", "family:adults", "attractions:all"]), // reuses orlando-hook-discovery-cove's image
  offer("15", "undercover_tourist", "Busch Gardens Tampa Bay", "Busch Gardens Tampa Bay (Undercover Tourist)", "Tampa", 27.9506, -82.4572, "https://aws-tiqets-cdn.imgix.net/images/content/5685889f3bbc4e94878ede256a13d2cb.jpg?auto=format%2Ccompress&fit=crop&q=70", ["attractions:themeparks", "attractions:family", "family:all", "family:kids", "attractions:all"]), // reuses tampa-hook-busch-gardens's image
  offer("16", "undercover_tourist", "LEGOLAND Florida Resort", "LEGOLAND Florida Resort (Undercover Tourist)", "Winter Haven", 28.0222, -81.7328, "https://aws-tiqets-cdn.imgix.net/images/content/c19276f32cff4745bc4479e12d301099.jpg?auto=format%2Ccompress&fit=crop&q=70", ["attractions:themeparks", "attractions:family", "family:all", "family:kids", "attractions:all"]), // reuses winterhaven-hook-legoland's image
  offer("17", "undercover_tourist", "Kennedy Space Center Visitor Complex", "Kennedy Space Center Visitor Complex (Undercover Tourist)", "Merritt Island", 28.5721, -80.648, "https://thumb.wikimedia.org/wikipedia/commons/thumb/e/ed/KSC_Visitor_Complex_logo.svg/960px-KSC_Visitor_Complex_logo.svg.png", ["attractions:themeparks", "attractions:landmarks", "family:all", "attractions:all"]), // reuses merritt-island-klook-kennedy-admission's image
  offer("18", "undercover_tourist", "Peppa Pig Theme Park", "Peppa Pig Theme Park (Undercover Tourist)", "Winter Haven", 28.0222, -81.7328, "https://aws-tiqets-cdn.imgix.net/images/content/2d18daffc6384925ac956493c2704000.jpeg?auto=format%2Ccompress&fit=crop&q=70", ["attractions:themeparks", "family:all", "family:toddlers", "family:kids", "attractions:all"]), // reuses winterhaven-hook-peppa-pig's image
  ...
  // for a product photo. Only ONE Go City pass per market is placed — the
  // others are duplicate passes for an already-covered market (see
  // DELIBERATELY_UNPLACED).
...
// Registry keys the owner asked us to close but for which no coordinate or
// image could be verified without guessing (this lane's standing rule).
// Every id here is a PARTNER_OFFER_REGISTRY key with no other rendering
// surface (PP/VO/IP/CPN/AL/ETD) — see the dead-inventory audit in
// /home/claude/orch/affiliate-inventory.md section I / SUMMARY.
export const DELIBERATELY_UNPLACED = Object.freeze({
  // Provider is dark: PROVIDERS.wegotrip does not exist in
  // lib/commerceProviders.js (lib/commerceProviders.js's own header: "WeGoTrip
  // remains deliberately absent"), so these four can never resolve a redirect.
  "miami-tour-art-deco-south-beach": "wegotrip is not a live PROVIDERS entry (lib/commerceProviders.js) — no redirect can ever resolve for this offer",
  "miami-tour-downtown-audio": "wegotrip is not a live PROVIDERS entry (lib/commerceProviders.js) — no redirect can ever resolve for this offer",
  "keywest-tour-old-town-audio": "wegotrip is not a live PROVIDERS entry (lib/commerceProviders.js) — no redirect can ever resolve for this offer",
  "orlando-tour-echoes-of-history": "wegotrip is not a live PROVIDERS entry (lib/commerceProviders.js) — no redirect can ever resolve for this offer",
  ...
  "miami-pass-gocity-explorer": "duplicate Go City pass for Miami; miami-pass-gocity-all-inclusive already covers this market and gocity.com blocks non-browser fetches, so no distinct product image exists to tell the two cards apart",
  "orlando-pass-gocity-essentials": "duplicate Go City pass for Orlando; orlando-best-gocity-pass already covers this market and gocity.com blocks non-browser fetches, so no distinct product image exists to tell the two cards apart",
  "orlando-pass-gocity-explorer": "duplicate Go City pass for Orlando; orlando-best-gocity-pass already covers this market and gocity.com blocks non-browser fetches, so no distinct product image exists to tell the two cards apart",

  // No fare/rental chip exists anywhere in lib/google.js SUBFILTERS — trip
  // support, not a browse-menu category.
  "orlando-airport-rentcars": "no browse-menu chip fits airport car rental (trip-support kind, not attractions/food/nightlife/family/hotels/shopping)",
  "tampa-airport-rentcars": "no browse-menu chip fits airport car rental",
  "sarasota-airport-rentcars": "no browse-menu chip fits airport car rental",
  ...
  // Bundle of two venues already placed individually (orlando-hook-gatorland,
  // merritt-island-klook-kennedy-admission / orlando-drive-kennedy-explore) —
  // no og:image on its own Tiqets bundle page, and no single photo can
  // honestly represent two different physical places.
  "orlando-best-gatorland-kennedy": "no og:image on its Tiqets bundle page; the two venues it bundles (Gatorland, Kennedy Space Center) are already placed individually under their own offer ids with verified imagery",
});
 ...
 *   `radiusMi` (default 60) miles, deduped on `${provider}:${offerId}`.
 *   Returns [] immediately for every chip the law forbids (food/hotels/
 *   shopping, attractions:spa, nightlife:speakeasy|karaoke) and for a
 *   missing/invalid center — this function never guesses a location.
 */
