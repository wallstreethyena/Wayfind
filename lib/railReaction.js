...
export function placeRouteBackPlan(loc) {
  ...
  return {
    ...
    // An external /p/{id} launch has no in-app page to return to. Close it
    // onto "/" whether or not it carried a legacy action. Mobile Safari
    // restores the last tab URL when Wayfind opens; leaving /p/{id} active
    // made the same place sheet reappear on every launch after it was closed.
    replaceHomeOnClose: onPlace && !sameOriginPrev,
  };
}
