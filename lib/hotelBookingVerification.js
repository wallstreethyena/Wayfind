...
// The runtime allow-list is a plain module, not a JSON import: this file is
// pulled into the client bundle through lib/affiliates.js, so it ships only
// the 40 verified rows (4 KB) rather than the full 394-row audit record, and
// it avoids an import attribute the JSX toolchain cannot carry through its
// temp-file rewrite. The full evidence stays in
// lib/hotelBookingVerification.json for the guard, which pins the two
// together so this list can never quietly drift from what was audited.
import { VERIFIED_HOTEL_BOOKINGS } from "./hotelBookingVerificationData.js";

const BY_KEY = new Map();
const BY_PLACE = new Map();
for (const r of Array.isArray(VERIFIED_HOTEL_BOOKINGS) ? VERIFIED_HOTEL_BOOKINGS : []) {
  if (!r || !r.placeId) continue;
  if (typeof r.key === "string" && r.key) BY_KEY.set(r.key, r);
  BY_PLACE.set(r.placeId, r);
}

/** The verified allow-list, for guards and audits. Never mutate. */
export function hotelBookingRecords() {
  return Array.isArray(VERIFIED_HOTEL_BOOKINGS) ? VERIFIED_HOTEL_BOOKINGS : [];
}

/**
 * The id a verification record is looked up by. Owned-hotel cards carry the
 * card key as `place.id`; inventory-backed rails (the stays rail beside an
 * event) carry the GOOGLE PLACE ID instead. Both address the same property, so
 * both resolve — the evidence itself is keyed to the place id either way,
 * which is what makes it identity and not a display string.
 */
...
export function hotelBookingVerification(place) {
  ...
  if (!key) return null;
  return BY_KEY.get(key) || BY_PLACE.get(key) || null;
}
...
export function isHotelBookingVerified(place) {
  ...
  return Boolean(rec && rec.placeId);
}
