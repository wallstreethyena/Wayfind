...
// Naive eTLD+1: the last two dot-separated labels. This is a diagnostic
// aid, not a public-suffix-list lookup, so a host under a multi-part suffix
// (e.g. "foo.co.uk") reports "co.uk" rather than "foo.co.uk". That is an
// acceptable trade for this use: the goal is naming which unexpected
// provider domain a run saw, not exact registrable-domain correctness.
...
// invariant failed without leaking a token, path, query, or full response body.
// The one exception is the host: when it falls outside the tp.st/tpx.lu
// allowlist, the registrable domain (never the full subdomain, path, or
// query) is included so an operator can tell which provider domain broke
// the allowlist without re-running provisioning by hand — the same gap that
// made the 2026-09-22 "host-other" failure require a manual investigation
// (see wallstreethyena/Wayfind#1442) to name tpx.lu in the first place.
