...
export function validTpShortUrl(value) {
  try {
    ...
    // Travelpayouts currently appends an ERID disclosure token to some tp.st
    // short links. Keep the allowlist narrow: ERID is the only accepted query
...
function decodeProviderLinks(payload, destinations, marker, trs) {
  ...
  for (const row of result.links) {
    ...
    seen.add(row.url);
    if (row.code === "success") {
      if (!validTpShortUrl(row.partner_url)) throw new Error("travelpayouts_short_url_invalid");
