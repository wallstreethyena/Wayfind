...
// tonight-alfonso-scarpa-unsplash.jpg, beach-adobestock-216195684.jpeg) reused for the hero AND
...
//
// THE FIX: Pexels (pexels.com/api) — an optional stock source whose request
// volume is bounded by PEXELS_MONTH_CAP, unlike the paid Google Places Photos
// SKU (the reason withPhotos stayed paid-
...
// v1.01 (2026-08-08): was 10. lib/landing.js renders up to 15 ranked cards
// (rankedFor()'s pool.slice(0, 15)) PLUS the hero, so a page can need 16
// distinct slots. At 10, cards past #10 wrapped and repeated an earlier
// card's photo on the SAME page — a milder rerun of the exact bug this file
// was built to fix. Confirmed live on /things-to-do/sarasota and
// /things-to-do/kihei (both routinely render 15 cards). 24 covers every
// route's max with headroom; Pexels per_page=24 is one call, still cached
// 21 days, no cost or quota change (free tier either way).
const POOL_SIZE = 24;
...
export async function stockPhotoPool(query) {
  // `next build` must not wait on Pexels. Static category images already exist.
  if (process.env.NEXT_PHASE === "phase-production-build") return [];
  const key = pexelsKey();
  if (!key || !query) return [];
  // v2 prefix: bumped alongside the POOL_SIZE increase above so every
  // already-cached (and now too-small) 10-item pool is treated as a cache
  // miss and re-fetched at 24 immediately, instead of silently serving the
  // old undersized pool for up to the existing 21-day TTL.
  const ck = "wfstock2|" + String(query).toLowerCase().trim();
  try {
    const cached = await cget(ck);
    if (cached && Array.isArray(cached.v)) return cached.v;
  } catch (e) {}
  try {
    if (!(await providerSpendAllow("pexels"))) return [];
    const r = await fetch(
