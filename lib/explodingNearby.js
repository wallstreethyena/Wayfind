...
export function selectExplodingNearby({ topics, matches, inventory, center, maxTrends = EXPLODING_NEARBY_MAX_TRENDS }) {
  ...
  return result
    .sort((a, b) => (b._moduleScore - a._moduleScore) || a.label.localeCompare(b.label))
    .slice(0, Math.max(0, Math.min(Number(maxTrends) || EXPLODING_NEARBY_MAX_TRENDS, EXPLODING_NEARBY_MAX_TRENDS)))
    .map(({ _moduleScore, trendStrength, ...publicGroup }) => publicGroup);
}
