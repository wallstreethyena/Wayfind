...
const inFlight = new Map(); // ref -> Promise, including batches already dispatched
let batchTimer = null;
let batchOpenedAt = 0;
function arm() {
  const now = Date.now();
...
async function flush() {
  ...
  take.forEach((ref, i) => {
    ...
    // Missing or malformed scores are unknown, never evidence of a clean shot.
    if (raw && typeof raw.people === "boolean" && typeof raw.aesthetic === "number" &&
        Number.isFinite(raw.aesthetic) && raw.aesthetic >= 0 && raw.aesthetic <= 1) {
      const v = { people: raw.people, aesthetic: raw.aesthetic };
      remember(ref, v);
      waiters[i].forEach((fn) => fn(v));
    } else {
      waiters[i].forEach((fn) => fn(null));
    }
    inFlight.delete(ref);
  });
}
...
export function useBestPhoto(primary, candidates) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const seen = new Set([primaryRef]);
      const alts = (Array.isArray(candidates) ? candidates : [])
        .map((u) => ({ url: u, ref: refOf(u) }))
        .filter((x) => x.ref && !seen.has(x.ref) && seen.add(x.ref)).slice(0, 5);
      if (!alts.length) return;
      const rated = await Promise.all(alts.map((x) => scoreRef(x.ref).then((v) => v ? { ...x, ...v } : null)));
      if (dead) return;
      const ranked = rated.filter((x) => x && !x.people && (pv.people || x.aesthetic > pv.aesthetic)).sort((a, b) => (Number(a.people) - Number(b.people)) || (b.aesthetic - a.aesthetic));
      const winner = ranked[0];
      // swap only to a genuinely better, people-free shot
      if (winner && !winner.people && winner.url !== primary) setBest(winner.url);
    })();
    return () => { dead = true; };
  }, [primary, JSON.stringify(Array.isArray(candidates) ? candidates : [])]);
  return best;
}
...
// unavailable refs remain unknown; only rated refs can win as clean. If none
// is known clean, retain the first supplied ref as an explicit fallback.
export async function pickPeopleFreeRef(refs, opts) {
  ...
  for (const ref of list) {
    let v;
    try { v = await scoreRef(ref); } catch { v = null; }
    if (!v) continue;
