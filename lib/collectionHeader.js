...
const INTENT_EDITORIAL = {
  ...
  "best-of": {
    ...
    deck: (city) => `A focused shortlist of the places that give ${city} its identity`,
    imageKicker: "THE WAYFIND CITY EDITION",
    imageTitle: (city) => `${city}, through a local lens.`,
    dekLead: "The local essentials, with enough context to choose well.",
  },
  "hidden-gems": {
...
// A local color fact (TOWN_PROFILES[town].one in culture.js, or a seasonal
// headline in areaSeasonalContext.js) is already ONE complete, hand-authored
// sentence — see those files' own header comments. It is not a two-clause
// "setup + discardable payoff" pair, so trimming it must never guess which
// half is "the point" and drop the other. That guess (the removed
// culturalCounterpoint() and the old clause-splitting shortLocalFact())
// produced two live, non-sequitur intent-page headers (owner review, real
// 390px screenshots, 2026-08-02): for Parrish's `nearby` page, dropping "A
// tiny old rail town where you don't watch the trains" left "you climb
// aboard" meaning nothing on its own ("Close to home: you climb aboard");
// for Parrish's `seasonal` page, keeping only the first clause of "Storms
// build over the river most afternoons, and the railroad museum's shaded
// platform and the Fort Hamer ramp are what locals plan around them." left
// an unexplained caution glued onto an unrelated sentence about the season's
// appeal. Both authored sentences were already fine — the bug was in the
// extraction, not the content. The only safe transform is (a) convert an
// internal em-dash to a semicolon so the sentence still reads as one
// flowing clause once it's embedded inside another, and (b) trim by total
// word count ONLY if the sentence is genuinely longer than fits — cut from
// the end, never the middle, and only past a generous cap so this is rare
// in practice (the longest hand-authored entries across both files run
// ~25 words).
function trimLocalFact(value, maxWords) {
  const clean = String(value || "").trim().replace(/[.!?]+$/, "").replace(/\s+[—–]\s+/g, "; ");
  const words = clean.split(/\s+/);
  return words.length > maxWords ? words.slice(0, maxWords).join(" ") : clean;
}
function shortLocalFact(value, maxWords = 32) {
  return trimLocalFact(value, maxWords);
}
function compactLocalFact(value, maxWords = 32) {
  return trimLocalFact(value, maxWords);
}

function localIntentLens(intent, place, localContext) {
  const { known, seasonal } = contextFields(localContext);
  const local = compactLocalFact(known);
  ...
  const lenses = {
    ...
    "best-of": `Locally, ${local.charAt(0).toLowerCase() + local.slice(1)}`,
