...
export async function fetchBeachAlerts(lat, lng) {
  try {
    ...
    if (!Array.isArray(j.features)) throw new Error("Invalid NWS alert response");
  ...
  } catch { return { alerts: [], hasUnsafe: null }; }
}
...
// v6.97: the tide-window date is the VENUE's calendar day (ET), never the
// server's. Vercel runs in UTC, so the old local-parts read rolled to
// tomorrow at ~8 PM ET and the "today's tides" list silently showed
// tomorrow's highs and lows every evening — the exact class lib/siteTime.js
// exists for. Exported for scripts/test-marine.mjs.
export function tideDateYmd(d = new Date()) { return siteTodayStr(d).replace(/-/g, ""); }
const ymd = tideDateYmd;
...
export function scoreBeachDay(conditions, distanceMi, cfg = {}) {
  ...
  if (conditions.hasUnsafe !== true && (conditions.hasUnsafe !== false || !Number.isFinite(conditions.airTempMaxF) || !Number.isFinite(conditions.precipProbMaxPct))) {
    return { show: false, status: "unknown", reasons: ["Weather or alert information unavailable"], conditions };
  }
...
// ── lite: per-beach chips (water temp, waves, wind) — spec v6.54 ─────────────
// One marine + one forecast call; wind direction becomes a compass label.
const DIRS = ["N","NNE","NE","ENE","E","ESE","SE","SSE","S","SSW","SW","WSW","W","WNW","NW","NNW"];
export async function getBeachLiteConditions(lat, lng) {
  try {
    const [marine, wx] = await Promise.all([
      fetchMarine(lat, lng),
      getJSON(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&current=wind_speed_10m,wind_direction_10m&wind_speed_unit=mph&timezone=auto`, { revalidate: 900 }).catch(() => ({})),
    ]);
    const cur = (wx && wx.current) || {};
    const deg = cur.wind_direction_10m;
    return {
      waterTempF: marine.waterTempF ?? null,
      waveHeightFt: marine.waveHeightFt ?? null,
      windMph: cur.wind_speed_10m != null ? Math.round(cur.wind_speed_10m) : null,
      windDir: deg != null ? DIRS[Math.round(deg / 22.5) % 16] : null,
