...
import { observedCount, seasonalEvidence } from "./socialQualification.js";

// Meta's current Graph release when this adapter was reviewed (2026-09-06).
// Keep this in one place so a version migration cannot leave half the calls on
// a retired version.
...
// The idle pulse note the scout records while it has no credentials. No
// billing:/quota: prefix on purpose — lib/jobPulse.classifyHealth escalates that
// prefix on the first dead run, and an unconfigured scout is idle, not dead.
export const IG_UNCONFIGURED_REASON = "parked_no_instagram_credentials";

export function igConfigured() { return !!(igToken() && igUserId()); }

const MEDIA_FIELDS = "id,username,caption,media_type,media_url,permalink,thumbnail_url,timestamp,like_count,comments_count";
...
export function hashtagIdUrl(tag) {
  ...
  return `${GRAPH}/ig_hashtag_search?user_id=${encodeURIComponent(user)}&q=${encodeURIComponent(clean)}`;
}
...
export function hashtagMediaUrl(hashtagId, edge = "top_media") {
  ...
  return `${GRAPH}/${encodeURIComponent(hashtagId)}/${edge}?user_id=${encodeURIComponent(user)}&fields=${MEDIA_FIELDS}`;
}
...
export function businessDiscoveryUrl(handle, limit = 12) {
  ...
  return `${GRAPH}/${encodeURIComponent(user)}?fields=${encodeURIComponent(fields)}`;
}
...
export function captionSignals(caption) {
  ...
  return {
    fall: FALL_WORDS.test(text) || seasonalEvidence(text).length > 0,
...
export function toCandidate(media, { source, handle = null, followers = 0, tag = null } = {}) {
  ...
  return {
    ...
    handle: handle || media.username ? String(handle || media.username).toLowerCase() : null,
    ...
    like_count: observedCount(media.like_count),
    comments_count: observedCount(media.comments_count),
