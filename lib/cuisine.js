...
// Google's types[] collapses some of the distinctions Florida diners care most
// about — but NOT as many as I first claimed, and the correction matters.
//
// Measured against the live inventory: `cuban_restaurant`, `colombian_restaurant`,
// `caribbean_restaurant`, `peruvian_restaurant` and `argentinian_restaurant` all
// EXIST and are in TYPE_CUISINE below. What genuinely has no type is
// puerto_rican_restaurant, venezuelan_restaurant, dominican_restaurant,
// salvadoran_restaurant and haitian_restaurant — those land in
// `latin_american_restaurant` or plain `restaurant`, and names are the only route
// to them.
//
// I originally asserted the absent list was much longer. The cost of being wrong
// was that places carrying a real 0.9 type signal were scored 0.55 off an
// editorial guess instead, which nearly dropped Cuban below Tampa's chip floor.
//
...
export const TYPE_CUISINE = Object.freeze({
  ...
  // I ASSERTED IN THREE FILES THAT THESE DO NOT EXIST. They do. Measured against
  // the live inventory 2026-07-30:
  //   colombian_restaurant   10 rows
  //   cuban_restaurant        8 rows  (4 Tampa, 2 Orlando)
  //   caribbean_restaurant    4 rows
  //   peruvian_restaurant     2 rows
  //   argentinian_restaurant  1 row
  // The consequence was not cosmetic: places carrying a real 0.9 type signal were
  // scored 0.55 off an editorial guess instead, and Latin Touch Sandwich Shop —
  // which literally carries `cuban_restaurant` — sat in the weak tier. Note the
  // spelling: Google uses `argentinian_restaurant`, not `argentine`.
  cuban_restaurant: "cuban", colombian_restaurant: "colombian",
  caribbean_restaurant: "caribbean", peruvian_restaurant: "peruvian",
...
export const NAME_CUISINE = Object.freeze([
  { cuisine: "cuban", rx: /\b(cubana|cubano|cuban|kubana|kubano|habana|havana|ropa vieja|lechon|palomilla|medianoche|croqueta|croquetas|pastelito|vaca frita)\b/i },
...
// SIGNAL 4 — review synthesis. Reviews are read, counted and DISCARDED; only the
// derived count is used, and raw review text is NEVER stored (spec).
//
// What it tests is DISHES, not the word "Cuban". This is what separates a Cuban
// kitchen from a shop that pours one Cuban-style shot: Noble Brew's own editorial
// calls its menu "a coffee atlas — Cuban-style shots next to Middle Eastern
// pours", and zero of its five reviews mention a Cuban dish. That is the
// Havana-Nights failure mode in subtler form, and a name or word match cannot see
// it. Kubana Kafe, by contrast, had two dish mentions in five reviews.
export const CUISINE_DISH_RX = Object.freeze({
  cuban: /\b(ropa vieja|lechon|medianoche|cubano|cuban sandwich|croqueta|croquetas|pastelito|pastelitos|picadillo|vaca frita|palomilla|masitas|congri|cortadito)\b/i,
  "puerto-rican": /\b(mofongo|pernil|alcapurria|tostones|arroz con gandules|jibarito|piragua)\b/i,
  colombian: /\b(bandeja paisa|arepa|arepas|ajiaco|lechona|sancocho|chuzo)\b/i,
  venezuelan: /\b(cachapa|cachapas|tequeno|tequenos|pabellon|reina pepiada|patacon)\b/i,
});

/**
 * How many of a place's reviews mention an actual DISH of this cuisine. Takes
 * text in, returns a COUNT — the caller must not retain the text.
 */
export function dishMentions(cuisine, reviewTexts) {
  const rx = CUISINE_DISH_RX[cuisine];
  if (!rx || !Array.isArray(reviewTexts)) return 0;
  let n = 0;
  for (const t of reviewTexts) if (typeof t === "string" && rx.test(t)) n++;
  return n;
}
...
// ── metro resolution for the chooser sheet ────────────────────────────────
// The cuisine sheet exists for the three metros with real food inventory:
// Tampa 296 places, Manatee-Sarasota 261, Orlando 243. Every other metro sits at
// exactly 40, which is a seed and not coverage.
//
// Deliberately NOT reusing lib/orderInFeatured.METROS: its slugs are delivery
// slugs (sarasota, stpete) and do not match the inventory metros
// (manatee-sarasota, tampa). Mapping one onto the other silently would send a
// St Petersburg user a Sarasota chip list.
//
// Pure: haversine only, no I/O.
export const CUISINE_METROS = Object.freeze({
  orlando: { label: "Orlando", lat: 28.5384, lng: -81.3789 },
  tampa: { label: "Tampa Bay", lat: 27.9506, lng: -82.4572 },
  "manatee-sarasota": { label: "Sarasota & Bradenton", lat: 27.3364, lng: -82.5307 },
});
...
/**
 * Which cuisine sheet, if any, serves this location. Null outside ~75mi of all
 * three — and null is the RIGHT answer there. Routing a Miami user to Orlando's
 * chip list would show them counts for restaurants 200 miles away, which is the
 * same category of lie as widening a radius to pad a list.
 */
export const CUISINE_METRO_RADIUS_MI = 75;
export function cuisineMetroFor(lat, lng) {
  if (!isFinite(lat) || !isFinite(lng)) return null;
  let best = null, bestD = Infinity;
  for (const k of Object.keys(CUISINE_METROS)) {
    const m = CUISINE_METROS[k];
    const d = distMi(lat, lng, m.lat, m.lng);
    if (d < bestD) { bestD = d; best = k; }
  }
  return bestD <= CUISINE_METRO_RADIUS_MI ? best : null;
}
