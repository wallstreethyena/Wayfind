...
// WayfindSyntheticMonitor is OUR OWN scheduled monitor
// (scripts/run-synthetic-monitor.mjs), self-identified via a fixed UA on every
// real Chromium context it opens (.github/workflows/synthetic-monitor.yml, every
// 30 minutes). It is not a crawler to refuse anywhere — it never fetches a
// partner link directly from here, only from its own "book-links" scenario via
// a plain fetch()  — but it is also not a person, and its page_visit /
// element_click / page_active_time events must never count as one. Matching it
// here (rather than lib/crawler.js's BOT_UA) suppresses PostHog entirely for
// its browser sessions via analyticsSuppressionReason() below, the same path
// isOwnerUser() already uses to keep the owner's own browsing out of the funnel.
const BOT_RX = /(?:bot\b|crawler|spider|slurp|headless|lighthouse|pagespeed|pingdom|uptimerobot|facebookexternalhit|whatsapp|google-inspectiontool|googleother|bingpreview|yandex|baidu|wayfindsyntheticmonitor)/i;
...
// ── Pre-ready capture queue ─────────────────────────────────────────────────
// PostHog boots at idle (4s ceiling), after supabase.auth.getSession(), so
// window.posthog does not exist for the first seconds of every page. Every
// app capture that fired in that window — share-link landings (share_open,
// then detail_open ~2s after load), above-the-fold commerce_impression /
// card_impression — used to test `window.posthog`, find nothing, and drop the
// event. The first-party events table recorded the same actions, which is how
// the gap was found (2026-09-23: share_open 100% missing from PostHog, and
// detail_open 35/21/24 in Supabase vs 0/0/0 in PostHog on 09-17..19).
//
// captureOrQueue is the one sanctioned client capture path:
//   - suppressed session (owner / internal-marked browser / bot / webdriver):
//     emits nothing and queues nothing;
//   - PostHog ready: captures immediately;
//   - not ready yet: holds the event in a BOUNDED in-memory queue with its
//     occurrence time. PostHogProvider drains it once init succeeds, and
//     clears it the moment auth or boot proves the session is suppressed.
// Nothing is persisted; a tab closed before init loses its queue, exactly as
// before. The cap means a page where PostHog never loads (no key, blocked
// script) holds at most PRE_READY_QUEUE_MAX small objects.
export const PRE_READY_QUEUE_MAX = 200;
export const PRE_READY_QUEUE_KEY = "__WF_PH_QUEUE";

function sessionSuppressed(win) {
  if (win.__WF_ANALYTICS_SUPPRESSED) return true;
  const nav = win.navigator || {};
  let storage = null;
  try { storage = win.localStorage; } catch (e) { storage = null; }
  return !!analyticsSuppressionReason({ storage, userAgent: nav.userAgent, webdriver: !!nav.webdriver });
}

/** @returns {"captured"|"queued"|false} */
export function captureOrQueue(win, event, properties) {
  if (!win || typeof event !== "string" || !event) return false;
  try {
    if (sessionSuppressed(win)) return false;
    const ph = win.posthog;
    if (ph && typeof ph.capture === "function") { ph.capture(event, properties); return "captured"; }
    const q = Array.isArray(win[PRE_READY_QUEUE_KEY]) ? win[PRE_READY_QUEUE_KEY] : (win[PRE_READY_QUEUE_KEY] = []);
    if (q.length >= PRE_READY_QUEUE_MAX) return false; // bounded: drop, never grow
    q.push([event, properties, new Date()]);
    return "queued";
  } catch (e) { return false; }
}

/** Deliver every queued event through a ready client, preserving occurrence time. */
export function drainPreReadyQueue(win, ph) {
  if (!win || !ph || typeof ph.capture !== "function") return 0;
  const q = Array.isArray(win[PRE_READY_QUEUE_KEY]) ? win[PRE_READY_QUEUE_KEY] : [];
  let n = 0;
  if (sessionSuppressed(win)) { q.length = 0; return 0; }
  while (q.length) {
    const [event, properties, timestamp] = q.shift();
    try { ph.capture(event, properties, { timestamp }); n++; } catch (e) {}
  }
  return n;
}

export function clearPreReadyQueue(win) {
  try { if (win && Array.isArray(win[PRE_READY_QUEUE_KEY])) win[PRE_READY_QUEUE_KEY].length = 0; } catch (e) {}
}
