...
export function placeModel(p) {
  ...
  else if (r && rev) { headline = name + " holds " + r + " stars across " + rev + " Google reviews"; accent = r; }
  ...
  // Audit (2026-09-23): a star rating with no named source is an
  // unattributed third-party rating — every rating this card shows is a
  // Google Places rating, so "Google" names it wherever the count appears
  // (the photo-layout equivalent, lib/heroCard.js#heroRatingLine, already did).
  const proof = sc ? footFrom(["Wayfind score", r && rev ? r + " from " + rev + " Google reviews" : cat || city])
                   : footFrom([cat, city, r && rev ? r + " from " + rev + " Google reviews" : ""]);
  // v8.82 — THE SEASON TRAVELS WITH THE LINK. `tone` is decided in
  // app/p/[id]/page.js from the place id and today's date (the same two
  // conditions that put the fall skin on the card in the feed), never from
  // anything the sharer types, so a link shared in December arrives in the
  // ordinary ink no matter how old the copy of the URL is. buildCard drops any
  // tone that is not in TONES, so an invented ?tone= is inert.
  const fall = q.tone === "fall";
  return buildCard({
    eyebrow: eyebrowFrom([fall ? "Fall" : "", city, mi ? mi + " mi" : ""]),
