...
// The selected pin is the same teardrop, lifted: the standard white ring plus a
// larger footprint, so the pin that matches the card in view reads at a glance.
// Only the rendered size changes (same viewBox), and MapKit keeps anchoring the
// image at its bottom centre, so the tip never leaves the place.
export const SELECTED_PIN_SCALE = 1.3;
export function mapPinSvg(family, { selected = false, scale = 1 } = {}) {
  const {color, path} = pinCategory(family);
  const width = +(34 * scale).toFixed(2), height = +(46 * scale).toFixed(2);
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 34 46"><defs><linearGradient id="rim" x2=".8" y2="1"><stop stop-color="#fff"/><stop offset=".4" stop-color="${color}"/><stop offset="1" stop-color="#fff"/></linearGradient><radialGradient id="body" cx=".32" cy=".2" r=".85"><stop stop-color="${color}"/><stop offset=".65" stop-color="${color}"/><stop offset="1" stop-color="#07121d"/></radialGradient></defs><path d="${PIN_PATH}" fill="url(#rim)" ${selected ? 'stroke="white" stroke-width="2"' : ''}/><path d="${PIN_PATH}" transform="translate(2 2) scale(.88 .9)" fill="url(#body)"/><path d="${path}" transform="translate(8 8) scale(.75)" fill="white" fill-rule="evenodd"/></svg>`;
}
...
export function applePinOptions(mapkit, family, { selected = false } = {}) {
  // MapKit JS 6 reads intrinsic SVG dimensions and anchors images at bottom center.
  // Do not offset the tip or use removed mapkit.Size/mapkit.DOMPoint constructors.
  return { image: { 1: mapPinUrl(family, selected ? { selected: true, scale: SELECTED_PIN_SCALE } : undefined) } };
}
