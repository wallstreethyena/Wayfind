...
// The whole recovery window is bounded by timeoutMs. When retry is enabled the
// first attempt receives 70% of the window, leaving a real second chance for a
// timeout instead of discovering at the deadline that there is no time left to
// retry. A fast HTTP/network failure leaves nearly the whole window for attempt 2.
export async function fetchRailJson(url, {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    // Leave part of the shared window for the second attempt only on attempt 1.
    const attemptBudget = attempt === 0 ? Math.max(100, Math.floor(remaining * 0.70)) : Math.max(1, remaining);
    const controller = new AbortController();
    let timedOut = false;
    const onCallerAbort = () => controller.abort(signal?.reason || new Error("caller cancelled"));
