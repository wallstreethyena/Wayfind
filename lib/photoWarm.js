...
async function defaultCachedServed(refs) {
  ...
  try {
    const [{ cgetMany }, { photoCacheKey }] = await Promise.all([import("./serverCache.js"), import("./placePhotoServe.js")]);
    for (let i = 0; i < refs.length; i += 150) {
      const chunk = refs.slice(i, i + 150);
      const hits = await cgetMany(chunk.map((r) => photoCacheKey(r, WARM_CHECK_WIDTH)));
      for (const r of chunk) {
        const h = hits.get(photoCacheKey(r, WARM_CHECK_WIDTH));
        if (h && !h.stale) out.add(r);
      }
    }
  } catch { /* fall back to probing everything */ }
  return out;
}
