...
if (looksValid) {
  try {
    live = createClient(url, anon, {
      // A second GoTrueClient using the primary client's default storage key
      // warns about undefined cross-client session behaviour even when this
      // reader never persists a session. Give the read-only client its own
      // namespace so it cannot contend with the signed-in client.
      auth: {
        persistSession: false,
        autoRefreshToken: false,
        detectSessionInUrl: false,
        storageKey: "wf-supabase-live-reader",
      },
