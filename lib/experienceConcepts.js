...
// Shared server/client definition for the dedicated Night Out product rail.
// Pair nighttime evidence with an activity noun: bare `night(s)` admits Trivia
// Nights, and bare `midnight` admits Midnight Pass, neither of which proves a
// guided after-dark product. Trolley is admitted only with ghost/haunted
// evidence, covering the verified Sarasota trolley without widening every
// daytime sightseeing trolley into Night Tours.
export const NIGHT_TOUR_RX = /\b(?:ghosts?|haunted)\b.{0,55}\b(?:tours?|walk(?:ing)?|crawls?|hunts?|experiences?|trolleys?|trams?|carriages?)\b|\b(?:tours?|walk(?:ing)?|crawls?|hunts?|experiences?|trolleys?|trams?|carriages?)\b.{0,55}\b(?:ghosts?|haunted)\b|\b(?:night(?:time)?|after[- ]dark|moonlight|moonlit)\b.{0,45}\b(?:tours?|walk(?:ing)?|cruises?|safaris?|paddles?|kayaks?|adventures?|experiences?)\b|\b(?:tours?|walk(?:ing)?|cruises?|safaris?|paddles?|kayaks?|adventures?|experiences?)\b.{0,45}\b(?:night(?:time)?|after[- ]dark|moonlight|moonlit)\b|\bevening\b.{0,45}\b(?:guided|walk(?:ing)?|tours?|cruises?|safaris?|paddles?|kayaks?)\b|\b(?:guided|walk(?:ing)?|tours?|cruises?|safaris?|paddles?|kayaks?)\b.{0,45}\bevening\b/i;

export function isNightTourProductTitle(title) {
  return NIGHT_TOUR_RX.test(String(title || ""));
}
...
export const CONCEPTS = Object.freeze({
  ...
  "night-tours": Object.freeze({
    label: "Night tours",
    query: "guided night ghost and evening tours",
    rx: NIGHT_TOUR_RX,
  }),
