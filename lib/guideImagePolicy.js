...
export function guideImageProblems(art, brief) {
  ...
  if (source && license) {
    ...
    const ccRights = (cc && license.href === expectedCc) || (art.license === 'CC0 1.0 Universal' && license.href === 'https://creativecommons.org/publicdomain/zero/1.0/');
    const commons = source.hostname === 'commons.wikimedia.org' && decodeURIComponent(source.pathname).startsWith('/wiki/File:') && ccRights && art.modificationNotice?.trim();
    // A single Flickr photo page (never an album, a search or a profile) whose
    // live licence was checked to be one of the same CC licences Commons allows.
    const flickr = source.hostname === 'www.flickr.com' && /^\/photos\/[^/]+\/\d+\/?$/.test(source.pathname) && ccRights && art.modificationNotice?.trim();
    // Direct-permission grant: the subject's own business gave Wayfind written
    // permission to use its own photographs, rather than a third-party stock
    // license. There is no separate license page to point at, so the rights
    // URL must be the same page as the credited source (no invented document),
    // and the license text must name who granted it and when it was confirmed
    // (Pinto's Farm, 2026-09-22: "Used with permission of Pinto's Farm
    // (owner-confirmed 2026-09-22); credit required").
    const permission = /^Used with permission of .+\(owner-confirmed \d{4}-\d{2}-\d{2}\); credit required$/.test(art.license || '') && license.href === source.href;
    if (source.protocol !== 'https:' || !(unsplash || pexels || commons || flickr || permission)) issues.push('unsupported-rights');
  }
