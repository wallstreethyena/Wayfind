...
export function socialIdentityHints(leads, creators, { ceiling = 2000 } = {}) {
  const ids = new Set(), names = new Set();
  const creatorMap = new Map((Array.isArray(creators) ? creators : []).map((row) => [creatorKey(row.platform, row.handle), row]));
  for (const lead of Array.isArray(leads) ? leads : []) {
    for (const value of [lead?.source_place_id, lead?.candidate_place_id]) if (value) ids.add(String(value));
    const creator = creatorMap.get(creatorKey(lead?.platform, lead?.handle));
    if (creator?.canonical_place_id) ids.add(String(creator.canonical_place_id));
    if (lead?.location_name) names.add(String(lead.location_name).trim());
    if (ids.size + names.size > ceiling) throw new Error("social_identity_hint_ceiling");
  }
  return { ids: [...ids], names: [...names].filter(Boolean) };
}

export async function readSocialIdentityInventory(db, leads, creators, { signal } = {}) {
  const hints = socialIdentityHints(leads, creators);
  const rows = [], seen = new Set();
  const read = async (field, values) => {
    for (let i = 0; i < values.length; i += 100) {
      let query = db.from("wf_inventory").select("place_id,name,lat,lng,metro,status")
        .eq("status", "OPERATIONAL").in(field, values.slice(i, i + 100)).limit(500);
      if (signal) query = query.abortSignal(signal);
      const { data, error } = await query;
      if (error || !Array.isArray(data)) throw new Error("social_identity_inventory_failed");
      for (const row of data) if (row?.place_id && !seen.has(row.place_id)) { seen.add(row.place_id); rows.push(row); }
    }
  };
  await read("place_id", hints.ids);
  await read("name", hints.names);
  return rows;
}
