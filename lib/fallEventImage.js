...
// September 10 publication checks: coordinates/identity alone do not prove
// usable image bytes. Keep these off the fall rails until an exact owned or
// rights-cleared photo is available; their canonical event pages remain live.
export const FALL_EVENT_IMAGE_HOLDS = new Set([
  "ami-bayfest-2026",
  "candlelight-halloween-siesta-key-2026",
  "runaway-pumpkin-5k-family-fest-2026",
...
export const FALL_EVENT_VENUE_PLACE_IDS = Object.freeze({
  ...
  // 2026-09-26 complete Fall Guide: these identities already exist in
  // production wf_events / wf_inventory and have exact venue matches. Keeping
  // them at the shared serve boundary means every fall surface gets the same
  // honest venue-photo fallback instead of the guide inventing its own.
  "halloween-horror-nights-orlando-2026": "ChIJvRBCrN9-54gRGZuuaCLGrQE", // Universal Orlando Resort
  "epcot-food-wine-festival-2026": "ChIJGzFs3q9_3YgRvZd1y2NSJOo", // EPCOT
  "fantasy-fest-2026": "ChIJs_tsm0ix0YgRmYbIX_M5CT8", // Key West citywide event anchor
  "haunted-sarasota-trolley-2026": "ChIJaVDYNb5Bw4gRbV2ffQ-VTfQ", // Discover Sarasota Tours
  "oktoberfest-tampa-curtis-hixon-2026": "ChIJlRUlG4nEwogRJOgu0Hf2n54", // Curtis Hixon Waterfront Park
  "gatorland-ghosts-goblins-2026": "ChIJ9RHZGx6H3YgRnWVYIWsHNPM", // Gatorland
  "tampa-pig-jig-2026": "ChIJ-U84wHnEwogR9ry4KMSoZW8", // Julian B. Lane Riverfront Park
  "tampa-riverwalk-trick-or-treat-2026": "ChIJc8QsSADFwogRH9awG-1qaCs", // Cotanchobee Fort Brooke Park
  "mount-dora-craft-fair-2026": "ChIJGSMzu2Oi54gR-rlDDL6V3Qs", // Downtown Mount Dora anchor
...
export function fallEventCardImageSrc(event, w = 640, inventoryRow = null) {
  if (FALL_EVENT_IMAGE_HOLDS.has(event?.event_id || event?.id)) return null;
  const hero = String(event?.hero_image || "").trim();
  // Only the exact licensed venue asset resolves this former image hold.
  if ((event?.event_id || event?.id) === "hunsader-pumpkin-2026"
    && hero !== "/guides/verified/hunsader-farms-goats.webp") return null;
