...
// contains no creator-video term at all — so the creator lift that changed Ryan's score
...
// "display score" (governed: base +0.2 video −0.2 far +0.6 trending) and a
...
import { corroborationTrend } from "./trendSignal.js";
import { readableScoreReceipt, scoreReceipt } from "./scoreExplanation.js";

const num = (v) => (typeof v === "number" && isFinite(v) ? v : null);

/**
 * v8.42 — CORROBORATION, stamped at the ONE place the number is computed.
 *
 * Owner, 2026-08-23: "if a place has multiple influencers then make sure to
 * make it rank higher and add a trending badge on it."
 *
 * lib/trendSignal.js's async decorator already covers the ranked sheets, but a
 * great many surfaces never await it — every rail pool, every landing list,
 * every intent page mints rows with `trending: false` because the only trend
 * inputs used to be a Supabase read on the render path. Corroboration is not:
 * it is a Set over a committed file, synchronous and free. So it belongs here,
 * beside the number, rather than in forty call sites that would drift.
 *
 * THREE RULES, all load-bearing:
 *
 *  • ONLY BEFORE THE SCORE IS COMPUTED. This runs after governedScoreOf's
 *    already-stamped early return, never before it. A row that was scored
 *    upstream keeps that score untouched — stamping the badge onto it here
 *    would print 🔥 beside a number that does not contain the +0.6, which is
 *    the shown!=sorted defect this whole module exists to prevent, wearing a
 *    different hat.
 *  • NEVER OVER A LIVE SIGNAL. If the row already trends on foot traffic or
 *    event proximity, that verdict and its reason stand. Real-time demand is a
 *    better answer to "why is this hot" than a filming history, and the reader
 *    should get the truer of the two.
 *  • NEVER ON MONETIZED INVENTORY. Same carve-out attachTrendSignals makes for
 *    experience/Viator rows, for the same reason: the trending bump must not
 *    touch anything carrying a commission, or "no paid placement" stops being
 *    true. Mirrored here because this path never went through that filter.
 */
function applyCorroboration(p, locName) {
  if (!p || typeof p !== "object") return;
  if (p.kind === "experience") return;
  if (p.trending) return;
  try {
    const t = corroborationTrend(p, locName);
    if (!t.trending) return;
    p.trending = true;
    p.trend_reason = t.trend_reason;
  } catch (e) {}
}
...
export function governedScoreOf(p, locName) {
  ...
  // Corroboration is resolved HERE, between the early return and the three
  // derivations below, so the disclosure it stamps and the number every branch
  // is about to produce are computed from the same row state, in that order,
  // once. See applyCorroboration's first rule.
  applyCorroboration(p, locName);
  if (p.wfScore != null) {
    const steps = [];
    const score = displayedWfScore(p, locName, steps);
    try { p.score_explanation = scoreReceipt(p, p.wfScore, score, steps, "stored"); } catch {} // frozen input still scores
    return score;
  }
  ...
  const steps = [];
  const score = governedWayfindScore(base, { hasCreatorVideo: video, distanceMi: dist, trending: !!p.trending }, steps);
  try { p.score_explanation = scoreReceipt(p, base, score, steps, "reviews"); } catch {}
  return score;
}

/**
 * Attach an official calculation receipt to a DETAIL place without changing
 * ranking. Cards and /p/{id} often arrive with a stamped or copied score and
 * no receipt (slimPlace copies governed_score; fetchPlaceById sets wfScore).
 * governedScoreOf then early-returns and ScoreExplanation has nothing to show.
 *
 * Re-runs the official scorer on a scratch copy. If that number matches the
 * chip (existing governed_score, or the number the scorer itself would show),
 * the receipt is attached. A mismatch stays unexplained — never a different
 * number than the chip, never a reverse-engineered 9.2. Returns a shallow
 * copy so a ranked list row is not mutated.
 */
export function attachOfficialScoreReceipt(place, locName) {
  if (!place || typeof place !== "object") return place;
  const next = { ...place };
  if (readableScoreReceipt(next)) return next;
  // The chip prefers a stamped governed_score, else displayedWfScore, else
  // the review formula. Official math must match that number or stay hidden.
  const shown = Number.isFinite(next.governed_score)
    ? next.governed_score
    : (next.wfScore != null
      ? displayedWfScore(next, locName)
      : (() => {
        const base = wayfindScore(next.rating, next.reviews);
        return base == null ? null : displayedWfScore({ ...next, wfScore: base }, locName);
      })());
  const scratch = { ...next };
  delete scratch.governed_score;
  delete scratch.score_explanation;
  // When the stored base is exactly the live review formula, take the reviews
  // branch so the receipt can name that origin. A nudged/owner-bumped wfScore
  // is not equal and stays on the stored path.
  const fromReviews = wayfindScore(scratch.rating, scratch.reviews);
  if (fromReviews != null && scratch.wfScore === fromReviews) delete scratch.wfScore;
  const steps = [];
  let score = null;
  let origin = null;
  let start = null;
  if (scratch.wfScore != null) {
    start = scratch.wfScore;
    score = displayedWfScore(scratch, locName, steps);
    origin = "stored";
  } else {
    start = wayfindScore(scratch.rating, scratch.reviews);
    if (start != null) {
      const dist = num(scratch.distMi) != null ? num(scratch.distMi) : num(scratch.distance_mi);
      let video = false;
      try { video = scratch.creator_video === true || hasCreatorVideoAt(scratch, locName); } catch (e) { video = false; }
      score = governedWayfindScore(start, { hasCreatorVideo: video, distanceMi: dist, trending: !!scratch.trending }, steps);
      origin = "reviews";
    }
  }
  let receipt = null;
  try { receipt = (Number.isFinite(score) && origin) ? scoreReceipt(scratch, start, score, steps, origin) : null; } catch {}
  if (Number.isFinite(score) && receipt && (shown == null || score === shown)) {
    next.score_explanation = receipt;
    if (!Number.isFinite(next.governed_score)) next.governed_score = score;
    return next;
  }
  if (next.score_explanation) delete next.score_explanation;
  return next;
}
