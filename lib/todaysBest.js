...
export function byVisibleScore(rows) {
  ...
  // = base + 7 for a creator video − 2 past 17 miles, clamped to 0..100.
  ...
  //     flat +0.7, and it is visible in the number for the same reason.
  //   • the capCreatorHead rule on this surface — with a flat +7 no single creator's
  ...
  const key = (r) => {
    ...
    const g = governedWayfindScore(q, { hasCreatorVideo: hasVideo, distanceMi: isFinite(r.distance_mi) ? r.distance_mi : null });
...
// diversifyHead, constrained by the law: a row may only be promoted past
// another when their DISPLAYED scores are equal (same rounded governed score).
// Variety breaks ties; it never contradicts the number the reader compares.
export function diversifyHeadScoreStable(rows, head = 3) {
  const out = [];
  const rest = (rows || []).slice();
  const seen = new Set();
  while (out.length < head && rest.length) {
