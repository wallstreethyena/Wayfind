...
const MEAL_CAP = {
  ...
  dinner: [
    // v9.0 — was max 1. A breakfast_restaurant under a dinner banner is a
    // CONTRADICTION (the file's own tier-0 definition), the same class as a
    // steak_house under breakfast, which IS capped at 0. The owner's 2026-09-07
    // report — "we don't want to see a breakfast place in the dinner spot" —
    // is exactly the one seat this 1 was reserving.
    { rx: /^(breakfast_restaurant|brunch_restaurant|bagel_shop|donut_shop)$/i, max: 0 },
...
export function tbPhotoUrl(photoRef, w = 240) {
  ...
  return "/api/photo?ref=" + encodeURIComponent(photoRef) + "&g=2&w=" + width;
}
...
export async function fetchThingsToDo({ lat, lng, localHour, tempF, condition, radiusMi = NEAR_RADIUS_MI, limit = 10, events = null, outdoorOK = true }) {
  ...
  try {
    ...
    try {
      ...
      if (ids.length) {
        const { data: eds } = await supabase.from("wf_editorial_servable").select("place_id,hook").eq("verified", true).in("place_id", ids);
