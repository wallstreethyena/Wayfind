...
import { creatorBoostFor, capCreatorHead } from "./creatorBoost.js";
...
export function byVisibleScore(rows) {
  ...
  const key = (r) => {
    ...
    // v6.96 — the creator boost applies HERE now. It never did before, which
    // meant the ranked list that LEADS the home page (the answer-first list,
    // the whole point of Ship 1) ignored creator videos entirely while the
    // discovery grid below it applied the full boost. Two lists on one screen,
    // disagreeing about the same places. creatorBoostFor() works in
    // wayfindScore points (0-100); this key works in 0-10, hence /10.
    const cb = creatorBoostFor(r);
    if (cb) r.creator_boost = cb; // carried so the card can say why
    return q / 10 - d + cb / 10;
  };
  const sorted = (rows || []).slice().sort((a, b) => (key(b) - key(a)) || ((b.reviews || 0) - (a.reviews || 0)));
  // Two head rules, in order. diversifyHead() stops the top 3 all being one
  // cuisine; capCreatorHead() stops the top 5 all being one creator. The
  // creator cap runs SECOND so it has the final say on the head — without it a
  // metro with 17 curated places renders a "ranked list" that is really one
  // person's account (measured: Miami's top 17 were all one creator).
  return capCreatorHead(diversifyHead(sorted), (r) => creatorBoostFor(r) > 0);
}
