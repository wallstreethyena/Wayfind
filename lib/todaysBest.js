...
export function daypartCompose(rows, ctx, minRows = 3) {
  ...
  const out = list.filter((r) => {
    if (r && r.kind === "experience") return true; // bookable tours are daypart-agnostic supplier rows
...
async function googlePicks({ lat, lng, radiusMi, category, limit }) {
  if (typeof fetch !== "function" || typeof window === "undefined") return [];
  try {
    const q = FALLBACK_QUERY[category] || ("best " + String(category || "places"));
