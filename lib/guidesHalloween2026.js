...
function pick(name, image, blurb, tip, eyebrow, appQuery = null, placeId = null) {
  const p = { name, image: `${ART}/${image}.webp`, imageAlt: `${name}, photographed for Wayfind's Orlando Halloween food guide`, imageNote: NOTE, blurb, tip, eyebrow, appQuery, indoor: true };
  if (placeId) p.placeId = placeId;
  return p;
}
...
export const HALLOWEEN_2026_GUIDES = {
  "orlando-halloween-food-2026": {
    ...
    faq: [
      ...
      { q: "Why are some Halloween menu items missing?", a: "Wayfind includes only items and locations that could be verified for Orlando. For example, the Pumpkin Spice Cold Boo was documented for Universal Studios Hollywood, so it is not presented here as an Orlando item." },
