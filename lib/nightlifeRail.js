...
// never no floor.
import { isQuarantinedHost } from "./linkQuarantine.js";
...
// ...and the veto that keeps the first tier honest.
//
// The eight VENUE_TYPES above are described as "never attached as amenities."
// That held while the candidate pool was shallow. It does not hold in the tail.
// Measured 2026-07-29 against a 254-venue Orlando pool:
//
//   Eddie V's Prime Seafood   primary=seafood_restaurant  carries night_club
//                             AND live_music_venue AND cocktail_bar
//   Ocean Prime               primary=seafood_restaurant  carries night_club
//   KaLa Rooftop              primary=restaurant          carries night_club
//   Cafe 34 Istanbul          primary=turkish_restaurant  carries hookah_bar
//
// Google's types[] is a union of every facet a place has, not a statement of
// what it primarily is. A steakhouse with a DJ on Saturdays carries night_club.
// Left alone, those four outrank Twin Peaks and House of Blues on the rail —
// Cafe 34 Istanbul (11,452 reviews, 4.7) took the #1 slot in Orlando nightlife.
//
// What separates them from a real venue that Places happens to type as a
// restaurant is how much of the types[] array is FOOD. Ole Red is
// `restaurant, live_music_venue, event_venue, bar` — one dining type. Eddie V's
// carries five. So: count the dining types, and above a threshold, treat the
// venue types as facets of a restaurant rather than an identity.
//
// The threshold is 4, and it is calibrated, not guessed. At 3 it wrongly vetoes
// Señor Frog's (9,336 reviews), Jimmy Buffett's Margaritaville and Splitsville,
// which are genuine I-Drive / Disney Springs nightlife. At 4 every intended veto
// still fires and every intended survivor survives. Fixtures below lock both
// sides — a threshold that only proves what it excludes is half a test.
export const DINING_VETO_THRESHOLD = 4;

const NON_SUFFIX_DINING_TYPES = Object.freeze([
...
export const isDiningType = (t) =>
  typeof t === "string" && (/_restaurant$/.test(t) || NON_SUFFIX_DINING_TYPES.includes(t));

export const diningWeight = (place) =>
  (Array.isArray(place && place.types) ? place.types : []).filter(isDiningType).length;

export function isNightlifeVenue(place) {
  if (!place) return false;
  const types = Array.isArray(place.types) ? place.types : [];
  // Tier 2 first: a bar-family PRIMARY type is the venue's own identity, and no
  // amount of food on the menu changes that. Twin Peaks (sports_bar, 2 dining
  // types), Tin Roof and Tom's Watch Bar all enter here and are never vetoed.
  if (BAR_PRIMARY_TYPES.includes(place.primaryType)) return true;
  // Tier 1: a venue-identity type in types[], UNLESS the array is mostly food,
  // in which case that type is an amenity of a restaurant.
  if (types.some((t) => VENUE_TYPES.includes(t))) {
    return diningWeight(place) < DINING_VETO_THRESHOLD;
  }
  return false;
}
...
export function publishableWebsite(websiteUri) {
  ...
  // 2026-09-02: a host on the hijacked-domain quarantine ledger is never
  // published either (lib/linkQuarantine.js).
  if (isQuarantinedHost(host)) return null;
  return websiteUri;
}
