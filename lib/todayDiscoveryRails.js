...
/**
 * PURE. Does ANY Today rail want this place?
 *
 * v8.98 — retrieval needs to ask identity BEFORE the cost bound (lib/ownedPool.js),
 * and the honest way to ask is to run the composer itself over a one-row pool.
 * Restating the ten memberships here as a disjunction would create a second
 * opinion that drifts the first time a rail's predicate changes — and the rails
 * deliberately OVERLAP (a spring is also nature, a beach is also water), so
 * "which one rail" is not even a well-formed question. "Would the composer place
 * it anywhere" is.
 */
export function claimsTodayRail(place, opts = {}) {
  if (!place) return false;
  return composeTodayDiscoveryRails([place], opts).rails.some((rail) => rail.places.length > 0);
}
