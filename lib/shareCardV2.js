...
// EMPTIED 2026-08-12 (owner: every image used for text share is out). The
// registry itself stays — it is the mechanism that stops a future card guessing
// a crop and slicing a face off, and it will be repopulated when the new design
// picks its art. What it must not do meanwhile is keep one lone photo alive on
// the share surface, which is what the coaster entry was doing.
//
// focusFor() already returns null for an unregistered image and callers already
// fall back to a photo-less card, so an empty registry is a supported state, not
// a broken one.
