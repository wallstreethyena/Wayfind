...
//
// "If there is an influencer video, I want that to add a zero point two to
//  the score. If the place is a nine point two, it will go to a nine point four. If
...
//  Wayfind score."
//
// On this file's 0–100 scale that is +2 and −2. THE NUMBER SHOWN IS THE
...
// great place (99 + 2 must render 10.0, not vanish — the v7.00 lesson).
export const CREATOR_VIDEO_BONUS = 2;   // +0.2 on the shown /10 scale
export const FAR_MILES = 17;            // strictly greater than → deduction
export const FAR_PENALTY = 2;           // −0.2 on the shown /10 scale

// ── THE TRENDING COMPONENT (owner-approved, 2026-08-07) ─────────────────────
// A place that lib/trendSignal.js marks trending gets +6 internal (+0.6
// shown): a 9.0 that starts trending shows 9.6, and it RANKS 9.6 — the bump
// is in the displayed number and the sort key identically, exactly like the
// creator-video term. Three conditions make it legitimate (and
// scripts/check-score-law.mjs + check-trend-signal.mjs enforce all three):
//   1. the signal is real demand data (foot traffic, event attendance),
//      never an affiliate or paid input — see lib/trendSignal.js;
//   2. it is bounded: +6, and the trending term alone may not mint a
//      perfect score — it caps at TRENDING_CAP (99 internal / 9.9 shown).
//      It never SUBTRACTS: a place already at 100 without it stays 100;
//   3. it is disclosed: every surface that applies it renders a 🔥 reason.
//      A surface that can't disclose it must not pass trending: true.
export const TRENDING_BONUS = 6;        // +0.6 on the shown /10 scale
export const TRENDING_CAP = 99;         // trending alone may not mint a 10.0
 ...
 * @param {boolean}     [ctx.trending]         lib/trendSignal.js verdict (real demand
 *                                             data only, and ONLY on surfaces that
 *                                             disclose the 🔥 reason)
 * @returns {number|null} 0–100, or null when the base is null (unrated)
 */
export function governedWayfindScore(base, ctx, trace) {
  if (base == null || !isFinite(base)) return null;
  const c = ctx || {};
  let s = base;
  // Optional calculation receipt. Recorded during the real calculation, never
  // reverse-engineered from a rounded final score. No provider content here.
  const record = (key, before) => { if (trace) trace.push({ key, delta: s - before, value: s }); };
  let before = s;
  if (c.hasCreatorVideo) s += CREATOR_VIDEO_BONUS;
  if (c.hasCreatorVideo) record("creator", before);
  before = s;
  const d = typeof c.distanceMi === "number" && isFinite(c.distanceMi) ? c.distanceMi : null;
  if (d != null && d > FAR_MILES) s -= FAR_PENALTY;
  if (d != null && d > FAR_MILES) record("distance", before);
  before = s;
  // Trending: +6, capped so the trend term alone cannot mint a 10.0 — and
  // NEVER a subtraction (a score already past the cap keeps its own value).
  if (c.trending) s = Math.max(s, Math.min(TRENDING_CAP, s + TRENDING_BONUS));
  if (c.trending) record("trending", before);
  before = s;
  s = Math.max(0, Math.min(100, Math.round(s)));
  if (s !== before) record("bounds", before);
  return s;
}
