...
// Owner directive 2026-09-25: find every legitimate Wayfind destination that
// scores 9.0 or better and is still missing from owned inventory. On the
// lib/wayfindScore.js 0-100 scale that is 90. Imported, never re-derived.
export const SCOUT_FLOOR = 90;
...
// place cannot reach the requested floor no matter how many people agree).
export function minReviewsFor(rating, floor = SCOUT_FLOOR) {
  ...
  // Floating-point equality at the rounding boundary can make the algebraic
  // ceiling one review too small (measured at 4.6 / floor 90: raw=276 but
  // wayfindScore(4.6, 276) is 89). Tighten against THE score function so the
  // SQL-facing inverse and the displayed score cannot disagree by one review.
  let n = Math.max(0, Math.ceil(need));
  while (wayfindScore(r, n) < floor) n++;
  while (n > 0 && wayfindScore(r, n - 1) >= floor) n--;
  return n;
}
