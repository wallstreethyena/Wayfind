...
export const FALL_PLACE_IDS = Object.freeze({
  ...
  "ChIJVQB8l1PEwogRfNZtGI6suIc": "Its 2026 fall menu brings back Pumpkin Spiced Rum, Apple Spiced Rum and six limited-run seasonal cocktails.", // Gasparilla Distillery & Cocktail Bar, Tampa
  "ChIJ1U_vFp0Xw4gRGKl6mJGJ9UI": "The current online menu lists pumpkin lattes, chai, matcha, cold brew and pumpkin bread; confirm availability before visiting.", // Joy Coffee, Bradenton
  "ChIJA-QkamE7w4gRfdzcxEHyPls": "Lakewood Ranch's autumn table — pumpkin and maple lattes, apple-pie French toast and an autumn chicken salad.", // ATRIA Cafe, Bradenton
...
export const FALL_OFFERING_SOURCES = Object.freeze({
  "ChIJ7QVjUK_FwogRaTLY8uxOico": { offering: "year-round spooky lounge: themed cocktails, oddities decor and recurring ghost programming", source: "https://www.spookeasylounge.com/calendar", verified: "2026-09-20", evidenceType: "year-round" },
  "ChIJIZt3d7DFwogRQ5Lg2tPMXyk": { offering: "year-round Dracula/Vlad-themed wine bar with themed drinks and immersive decor", source: "https://draculaslegacywinebar.com/", verified: "2026-09-20", evidenceType: "year-round" },
  "ChIJVQB8l1PEwogRfNZtGI6suIc": { offering: "2026 seasonal fall cocktail menu: Pumpkin Spiced Rum, Apple Spiced Rum and six limited-run cocktails", source: "https://www.gasparilladistillery.net/events/seasonal-fall-menu-launch", verified: "2026-09-20", evidenceType: "seasonal", season: "2026" },
  "ChIJ1U_vFp0Xw4gRGKl6mJGJ9UI": { offering: "current online menu lists Spiced Pumpkin Flat White, Classic Spiced Pumpkin Latte, Pumpkin Bread, Pumpkin Cloud Cold Brew, Pumpkin Shaken Espresso, Spiced Pumpkin Chai and Spiced Pumpkin Matcha Latte; confirm availability before visiting", source: "https://joycoffeebradenton.square.site/?fulfillment=PICKUP&location_id=LMTGQ2Y8V9RM8", verified: "2026-09-20", evidenceType: "seasonal", season: "2026" },
  "ChIJA-QkamE7w4gRfdzcxEHyPls": { offering: "current menu includes Pumpkin Spice Latte and Vermont Maple Latte", source: "https://order.toasttab.com/online/atria-cafe-lwr", verified: "2026-09-20", evidenceType: "seasonal", season: "2026" },
  "ChIJY5LPSxJAw4gRMhj-bOy9MKU": { offering: "Fall Specials menu: Spiced Pumpkin Latte, Caramel Apple Latte, Salted Maple Iced Flat White, Pumpkin Pie Matcha and Apple Cider Sparkler", source: "https://order.toasttab.com/online/buddybrewsarasota", verified: "2026-09-20", evidenceType: "seasonal", season: "2026" },
  "ChIJ9ZpMS-7jwogRWChZqTQG66g": { offering: "current menu includes Savory Roasted Pumpkin Bisque and Yellow Pumpkin Curry Di Mare", source: "https://solstpete.com/st-petersburg-warehouse-arts-district-sol-a-st-pete-bistro-food-menu", verified: "2026-09-20", evidenceType: "seasonal", season: "2026" },
  "ChIJ5crCip3EwogRQnhkbw_Ir6U": { offering: "current 2026 menu includes butternut squash zeppole, butternut squash soup, delicata squash burrata and pear salad", source: "https://www.opentable.com/r/on-swann-tampa", verified: "2026-09-20", evidenceType: "seasonal", season: "2026" },
  "ChIJn6X9ZlDEwogRTbyZDHcMf_0": { offering: "year-round haunted-history walking tours of Tampa and Ybor", source: "https://ghost-party.com/", verified: "2026-09-20", evidenceType: "year-round" },
  "ChIJd6lmgVh_3YgREzqTBf28i6U": { offering: "year-round walk-through haunted house with live actors, special effects and animatronics", source: "https://www.mortemmanor.com/haunt", verified: "2026-09-20", evidenceType: "year-round" },
  "ChIJUS9EYpll54gR63QOjYM4vDw": { offering: "year-round Orlando ghost tours plus an adults-only haunted pub crawl", source: "https://usghostadventures.com/orlando-ghost-tour/", verified: "2026-09-20", evidenceType: "year-round" },
  "ChIJ2Z9gE5eNk4gRz-Ey8y0ahfM": { offering: "year-round walk-through haunted attraction at Pier Park", source: "https://www.simon.com/mall/pier-park/stores/fear-at-the-pier", verified: "2026-09-20", evidenceType: "year-round" },
  "ChIJwZ_GK-d-54gRm5Ahg7PZYeY": { offering: "2026 Dead Coconut Club Harvest seasonal takeover with spooky specialty beverages and light bites during Halloween Horror Nights season", source: "https://media.universalparksusa.com/press-releases/universal-orlando-resort-opens-the-gates-to-halloween-horror-nights-2026-082826/", verified: "2026-09-20", evidenceType: "seasonal", season: "2026" },
});

export function fallPlaceEvidenceCurrent(placeId, todayStr) {
  const evidence = FALL_OFFERING_SOURCES[placeId];
  if (!evidence || !/^\d{4}-\d{2}-\d{2}$/.test(String(evidence.verified || ""))) return false;
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(todayStr || ""))) return false;
  const year = String(todayStr).slice(0, 4);
  if (evidence.evidenceType === "seasonal") {
    return evidence.season === year && String(evidence.verified).slice(0, 4) === year;
  }
  if (evidence.evidenceType !== "year-round") return false;
  const [vy, vm, vd] = evidence.verified.split("-").map(Number);
  const [ty, tm, td] = String(todayStr).split("-").map(Number);
  const ageDays = (Date.UTC(ty, tm - 1, td) - Date.UTC(vy, vm - 1, vd)) / 86400000;
...
export const FALL_PLACE_RAIL = Object.freeze({
  ...
  "ChIJVQB8l1PEwogRfNZtGI6suIc": "date-night",       // Ghost of Gasparilla
  "ChIJ1U_vFp0Xw4gRGKl6mJGJ9UI": "food",             // Joy Coffee fall menu
  "ChIJA-QkamE7w4gRfdzcxEHyPls": "food",             // ATRIA Cafe fall menu
  ...
  "ChIJ5crCip3EwogRQnhkbw_Ir6U": "food",             // On Swann fall menu
...
export const FALL_REJECTED_IDS = Object.freeze([
  ...
  "ChIJTzoiienhwogRbPa3GpuvBQU", // Paradeco St Pete — HOLD 2026-09-20: current official menu does not prove the fall items previously claimed
  "ChIJ11hsiYXEwogRjDBv39F04J8", // Oxford Exchange Tampa — HOLD 2026-09-20: current official menu did not prove the pumpkin-pancake/butternut claim; old third-party menu is not enough
  "ChIJB2B8mYzHwogRkZIDCDARWww", // Ice Screamin Tampa — HOLD 2026-09-20: current official site proves ice cream, not the horror/slasher theme previously claimed
