...
import { logEnvAuditOnce } from "./envAudit.js";

const MEM = globalThis.__wfServerCacheMem || (globalThis.__wfServerCacheMem = new Map());
export const DAY = 86400000;

// v6.35 — REFRESH-AHEAD (keep every card hot, never let a user hit day 31).
// A fresh entry becomes "due for refresh" well BEFORE its 30-day expiry, at a
// per-key JITTERED age in [20d, 27d). Serving surfaces `due` so the read path
// can trigger a background re-fetch that resets the clock — so entries in
// practice never reach 30 days, and because the trigger age is spread per key
// there is no synchronized day-30 expiry stampede (no quota spike). Pure +
// deterministic (hash of the key), so it is unit-tested and needs no clock.
export const REFRESH_MIN_MS = 20 * DAY;
export const REFRESH_MAX_MS = 27 * DAY; // safely under the 30-day ToS/fresh cap
export function refreshAgeFor(k) {
  // FNV-1a over the key → a stable bucket in [MIN, MAX). Same key → same age.
  let h = 2166136261;
  const s = String(k);
  for (let i = 0; i < s.length; i++) { h ^= s.charCodeAt(i); h = Math.imul(h, 16777619); }
  const span = REFRESH_MAX_MS - REFRESH_MIN_MS;
  return REFRESH_MIN_MS + (Math.abs(h | 0) % span);
}
// Due for a background refresh: only a FRESH entry (exp still in the future)
// that has aged past its jittered threshold. Expired/stale entries are NOT
// "due" — they take the normal live-fetch/inventory path, not a hot-swap.
export function refreshDue(k, wrote, exp, now = Date.now()) {
  if (wrote == null || !(exp > now)) return false;
  return (now - wrote) >= refreshAgeFor(k);
}
...
export async function cget(k, opts = {}) {
  logEnvAuditOnce(); // request-time env audit (once/process); broad server coverage alongside aiKey()
  const now = Date.now();
  ...
  const m = MEM.get(k);
  if (m) { const c = check(m.exp, m.wrote); if (c.ok) return { v: m.v, stale: c.stale, ageMs: m.wrote != null ? now - m.wrote : null, due: refreshDue(k, m.wrote, m.exp, now) }; }
  const s = sbEnv();
  if (!s) return null;
  try {
    ...
    return { v: row.v, stale: c.stale, ageMs: wrote != null ? now - wrote : null, due: refreshDue(k, wrote, exp, now) };
  } catch { return null; }
}
...
// Evict one row (2026-09-15). The photo lane proved a cached Google photo URI
// dead (403 from the host itself); a row that is known-wrong must not be
// served to the next reader for the rest of its 30-day clock. Removes the
// in-memory copy first so this lambda stops serving it immediately, then the
// shared row. Best-effort like cset: a failed delete is reported through
// lastWrite(), never thrown into a request path.
export async function cdel(k) {
  const now = Date.now();
  MEM.delete(k);
  const s = sbEnv();
  if (!s) { _lastWrite = { at: now, ok: false, why: "no supabase env" }; return false; }
  try {
    const r = await fetch(`${s.url}/rest/v1/wf_places_cache?k=eq.${encodeURIComponent(k)}`, {
      method: "DELETE",
      headers: { apikey: s.key, Authorization: `Bearer ${s.key}` },
    });
    _lastWrite = { at: now, ok: r.ok, status: r.status, op: "delete" };
    return r.ok;
  } catch (e) {
    _lastWrite = { at: now, ok: false, why: String((e && e.message) || e).slice(0, 300), op: "delete" };
    return false;
  }
}
...
// ── BATCH READ (v7.21) ────────────────────────────────────────────────────
// WHY: /api/image-score used to be one HTTP request per photo, each doing one
// cget → one PostgREST round-trip. A category tap asked for 85 verdicts, so the
// browser made 85 requests (client-capped 3 at a time ≈ 29 waves) and the server
// made 85 single-row queries — 13.3 seconds to look up values that were ALREADY
// cached. The verdicts were never the slow part; the per-item round-trip was.
//
// cgetMany answers N keys with at most ONE PostgREST query: the warm-lambda MEM
// map serves what it can, and everything still missing goes out in a single
// `k=in.(...)` filter. Same freshness/stale semantics as cget, key for key —
// this is cget's checks applied to a set, not a laxer second policy.
//
// Returns a Map of key -> { v, stale } for HITS ONLY. A missing key means "not
// cached", exactly like cget returning null.
export async function cgetMany(keys, opts = {}) {
  const out = new Map();
  const list = [...new Set((Array.isArray(keys) ? keys : []).filter((k) => typeof k === "string" && k))];
  ...
  for (const k of list) {
    const m = MEM.get(k);
    if (m) { const c = check(m.exp, m.wrote); if (c.ok) { out.set(k, { v: m.v, stale: c.stale }); continue; } }
    missing.push(k);
  }
  if (!missing.length) return out;
  const s = sbEnv();
  if (!s) return out;
  // PostgREST `in.(...)` needs each value quoted — our keys contain "|" and "/".
  const inList = missing.map((k) => '"' + String(k).replace(/"/g, '""') + '"').join(",");
  try {
    const r = await fetch(`${s.url}/rest/v1/wf_places_cache?k=in.(${encodeURIComponent(inList)})&select=*`, {
      headers: { apikey: s.key, Authorization: `Bearer ${s.key}` }, cache: "no-store",
    });
    if (!r.ok) return out;
    for (const row of await r.json()) {
      ...
      const c = check(exp, wrote);
      if (!c.ok) continue;
      MEM.set(row.k, { v: row.v, exp, wrote });
      out.set(row.k, { v: row.v, stale: c.stale });
    }
  } catch { /* fall through — misses just look uncached */ }
  return out;
}
