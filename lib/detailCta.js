...
//   1. Closed right now → "Add to plan" (never a false booking).
//   2. Place type selects the verb:
...
//        restaurant without deal, not reservable      → "Order delivery" / "See menu"
//        cafe / bakery                                → "Order pickup" / "See menu" / "Directions"
...
export function detailCtaLabel(type) {
  switch (type) {
    ...
    case DETAIL_CTA_TYPES.delivery: return "Order delivery";
    case DETAIL_CTA_TYPES.menu: return "See menu";
    case DETAIL_CTA_TYPES.pickup: return "Order pickup";
...
export function resolveDetailCta({ detail, kind, viaTours, locName, offers, openState }) {
  ...
  // 1. Closed → never a false booking.
  if (openState === false) {
  ...
  // 1.5 EXACT VERIFIED PRODUCT. Same registry the place card reads, so the two
  ...
  if (isRestaurant(detail)) {
    ...
    //
    // v8.44 — route through /api/eats/go, the server resolver, instead of the
    // client-side template link. The old uberEatsUrl returned null whenever
    // NEXT_PUBLIC_UBEREATS_TEMPLATE was unset AT BUILD TIME, so the rung fell
    // to an unmonetized "See menu" — (restaurant, menu) and (restaurant,
    // delivery) were the two largest primary_cta_null buckets in August
    // (37 + 33 events). The go route resolves the exact store server-side,
    // applies the template at RUNTIME, and emits provider_redirect_* — so this
    // rung now always renders for a named restaurant and is measurable.
    // `monetized` stays honest to the build-time template (it drives the FTC
    // disclosure): a template added later starts attributing on the server
    // immediately, and the disclosure catches up on the next build.
    const deliveryUrl = Aff.uberEatsGoUrl(detail.name, cityPart(locName), { lat: detail.lat, lng: detail.lng, surface: "detail_primary" });
    if (deliveryUrl) {
      return { type: DETAIL_CTA_TYPES.delivery, label: detailCtaLabel(DETAIL_CTA_TYPES.delivery), href: deliveryUrl, monetized: !!process.env.NEXT_PUBLIC_UBEREATS_TEMPLATE, provider: "ubereats", offerId: null, mapsUrl };
    }
  ...
  if (isCafeBakery(detail)) {
    ...
    // v8.44 — same server-resolver routing as the restaurant rung above.
    const deliveryUrl = Aff.uberEatsGoUrl(detail.name, cityPart(locName), { lat: detail.lat, lng: detail.lng, surface: "detail_primary" });
    if (deliveryUrl) {
      return { type: DETAIL_CTA_TYPES.pickup, label: detailCtaLabel(DETAIL_CTA_TYPES.pickup), href: deliveryUrl, monetized: !!process.env.NEXT_PUBLIC_UBEREATS_TEMPLATE, provider: "ubereats", offerId: null, mapsUrl };
    }
    return { type: DETAIL_CTA_TYPES.menu, label: detailCtaLabel(DETAIL_CTA_TYPES.menu), href: mapsUrl, monetized: false, provider: null, offerId: null, mapsUrl };
  }
