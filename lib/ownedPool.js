...
//
// 2026-09-23 (was: "WHY NOT WIDEN serveFromInventory() INSTEAD… it stays as it
// is") — THAT WAS WRONG, AND IT COST A CAFÉ. serveFromInventory's read was
// exactly the same single-capped-thousand-in-heap-order bug this module was
// built to fix, and it hit the same failure class from a different angle:
// Ryan's Coffee House, Parrish (ChIJo_IdHf0lw4gRHDbQNKBRE84), Food > Cafés —
// a real, 4.9★/205-review café — sat past row 1,000 of 1,598 food rows in the
// box and was never read, so no filter or rank downstream could find it.
// serveFromInventory now calls readOwnedCategory (this file) for its own
// exhaustive read, lazily (`await import("./ownedPool.js")`, matching this
// module's own lazy-import convention) so the two readers cannot drift back
// apart. Its 1.15 distance gate and top-N *ranking* are still load-bearing —
// unchanged, still owned by rankInventory — only the READ underneath them
// stopped being capped. Surfaces whose question is narrow (a real predicate,
// not a chip string) still come here directly, for the exact-radius admission
// and identity-injection this module provides that serveFromInventory does not.
...
export async function readOwnedCategory(env, category, box, opts = {}) {
  ...
  for (let from = 0; from < maxRows; from += page) {
    ...
    // Pre-migration fallback, same as the shared reader (lib/inventoryBoxBatch.js's
    // isMissingSecondaryColumn): a database without `secondary_categories` 400s
    // the OR form — and only on the VERY FIRST page, since that is the only
    // page whose failure could mean "the column doesn't exist" rather than "a
    // transient error mid-walk".
    //
    // 2026-09-23 (fix round, item 4) — SCOPED TO from===0 && status===400.
    // This used to fire on ANY `!res.ok` at ANY page, so two ordinary
    // transient 5xx responses on a LATER page silently swapped the URL SHAPE
    // for the rest of the walk — dropping the secondary_categories OR-clause,
    // not retrying the SAME query — which is how the independent audit
    // reproduced 100 missing rows reported `truncated:false`: a confidently
    // wrong, undercounted answer, never a visible failure. A 5xx belongs to
    // fetchPageWithRetry's own retry (lib/inventoryServe.js), never to a
    // change in what the query MEANS. See scripts/check-owned-pool-fallback-
    // scope.mjs for the fake-PostgREST reproduction and the fix.
    if (!res.ok && from === 0 && url === withSecondary && res.status === 400) {
      url = plain;
      // The primary attempt may have consumed most of the total budget.
      res = await doFetch(url, { headers: { ...headers, ...range }, cache: "no-store" }, pageBudget());
    }
