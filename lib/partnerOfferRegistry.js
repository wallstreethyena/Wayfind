...
export const PARTNER_OFFER_REGISTRY = Object.freeze({
  ...
  // ══ 2026-08-19 — one founder-verified in-season Florida tour ════════════
  // Browser-loaded the same day: H1 stayed "Clear Kayak Tour of Shell Key
  // Preserve and Tampa Bay Area"; short slug canonicalized to this long path.
  // SERVER-ONLY. The place card never sees this URL — it links
  // /api/commerce/go?provider=viator&offer=173028P1 and this row is what
  // that hop resolves. Do not copy the destination into a guide pick
  // (that is the #843 raw-href bug). Not 236862P2.
  "173028P1": offer("viator", "https://www.viator.com/tours/St-Petersburg/Clear-Kayak-Tours-of-Shell-Key/d5403-173028P1", "2026-08-19"),
});

// Most Viator offer ids still resolve as a live product_code lookup against
// wf_experiences (link_ok / fail_count). The one exception above — 173028P1 —
// is a founder-verified exact product URL so Book cannot fall through to a
// searchResults page when the catalogue row is missing. Intent-sheet Viator
// rows stay table-only and must not grow a shadowing registry row.
