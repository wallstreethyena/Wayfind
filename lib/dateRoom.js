...
/** Owner: Date Night is hard-capped at 27.0 miles. Not DRIVE_REACH_MI, not 120. */
...
const STREET_NAME = /\b(pop-?ups?|street fair|street festival|night market|block party)\b/i;
const STREET_TYPES = new Set(["festival", "community_festival"]);
...
/** Rooftop bars: typed rooftop_bar, or a rooftop-named bar/restaurant. ROOM_WORDS already lists "rooftop". */
export function isRooftopDatePlace(p) {
  ...
  return types.some((t) => /(?:^|_)bar$|restaurant|lounge/.test(t)) || isDateRoom(p);
}

/** Street events / pop-ups that already exist as inventory PLACES. Dated frontEvents stay events. */
export function isDateNightStreetEvent(p) {
  if (!p) return false;
  if (STREET_NAME.test(String(p.name || ""))) return true;
  const primary = primaryOf(p);
  if (STREET_TYPES.has(primary)) return true;
  if (primary) return false;
  return STREET_TYPES.has(typesOf(p)[0] || "");
}
