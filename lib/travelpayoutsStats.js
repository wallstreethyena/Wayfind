...
const FIELDS_LIST_ENDPOINT = "https://api.travelpayouts.com/statistics/v1/get_fields_list";
const PAGE_SIZE = 1000;
const MAX_PAGES_PER_CAMPAIGN = 5;
const MAX_ROWS = PAGE_SIZE * MAX_PAGES_PER_CAMPAIGN * TP_CAMPAIGNS.length;
// "action_type" is NOT a statistics field: production rejected it on
// 2026-09-23 with HTTP 400 "wrong field: action_type" (the first run that ever
// reached this request). Booking actions are selected by the documented
// `type = action` filter instead, so every row returned here is an action.
const REQUIRED_FIELDS = Object.freeze([
  "sub_id", "action_id", "campaign_id", "date", "created_at", "updated_at", "state",
]);
...
export function normalizeTravelpayoutsBooking(row) {
  ...
  // Booking actions are selected by the request's `type = action` filter (the
  // API rejects an action_type field). A row that labels itself as anything
  // else is refused, so it can never be reconciled as a booking.
  if (Object.hasOwn(row, "type") && row.type !== "action") throw new Error("booking row type is not action");
  if (Object.hasOwn(row, "action_type") && row.action_type !== "booking") throw new Error("booking action_type is not booking");
...
// A provider rejection must name its reason in the job pulse; a bare status
// hides the cause. Keep only a short single-line excerpt of the provider's
// own response body, with the access token redacted if it was ever echoed.
export function travelpayoutsErrorDetail(detail, token = "") {
...
// Diagnostics only, on a "wrong field" rejection: record the provider's own
// list of statistics field names in the runtime log so the next fix is read
// from the provider, not guessed. Never changes the outcome and never throws.
export async function logAvailableStatisticsFields({ token, fetchImpl = fetch, signal, log = console.error } = {}) {
  try {
    const response = await withSignal(() => fetchImpl(FIELDS_LIST_ENDPOINT, {
      method: "GET",
    ...
    const names = list
      ...
      .slice(0, 300);
    log(JSON.stringify({ tag: "travelpayouts_statistics_fields", status: response.status, count: names.length, fields: names }));
    return names;
  } catch {
    return null;
  }
    ...
    while (expectedTotal == null || offset < expectedTotal) {
      ...
      while (page.error) {
        ...
        if (offset !== 0 || !removable || fieldRepairs >= OPTIONAL_MONEY_FIELDS.length) {
          const reason = travelpayoutsErrorDetail(page.detail, token);
          if (page.status === 400 && /\bwrong\s+field\b/i.test(reason)) {
            await logAvailableStatisticsFields({ token, fetchImpl, signal });
          }
          throw new Error(`Travelpayouts statistics HTTP ${page.status}${reason ? `: ${reason}` : ""}`);
        }
        fieldRepairs += 1;
