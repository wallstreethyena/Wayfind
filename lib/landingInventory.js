...
import { isNightlifeVenue, railFloorFor } from "./nightlifeRail.js";
...
export function invPlaceToLanding(p) {
  ...
  const rawUrl = p.photo_url || p.photoUrl || signals.photo_url || signals.photoUrl || null;
...
/**
 * Nightlife inventory uses the rail's venue identity and attention floor.
 * Keep this as eligibility only: landing.js still owns score selection,
 * governed-score stamping, and final ordering.
 */
export function nightlifeLandingEligible(rows) {
  const pool = Array.isArray(rows) ? rows.filter(Boolean) : [];
  const floor = railFloorFor(pool);
  return pool.filter((p) => {
    const reviews = p.reviews != null ? p.reviews : (p.userRatingCount || 0);
    const status = p.status || p.businessStatus || null;
    return !!p.rating
      && reviews >= floor
...
export async function fetchLandingInventory(catSlug, city, opts) {
  ...
  // WO8 (2026-09-02) — threaded straight through to serveFromInventory, which
  // is the only place either option actually does anything. A caller that
  // does not set them (every landing page: rankedFor(catSlug, citySlug) with
  // no opts) gets byte-for-byte the prior behavior.
  const serveOpts = {
    skipEditorial: !!(opts && opts.skipEditorial),
    readCache: opts && opts.readCache,
  };
  const tryAt = async (radiusM) => {
    try {
      const rows = await serve(spec.cat, city.lat, city.lng, radiusM, n, spec.sub, serveOpts);
