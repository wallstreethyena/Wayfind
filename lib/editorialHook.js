...
//
// HOUSE BAR (owner lock, 2026-08-20) — GLOBAL, matches The Cracked Pepper Cafe.
// Gold (live take): "Winner of the 2023 Cuban Sandwich Festival's World's Best
// award, with a patio that overlooks a pond."
// Two-beat: one sourced distinction (award / named plate / craft) PLUS one
// physical why-sit (patio, dock, room, sand, pond). Short. Complete. Not a
// menu laundry list, address, hours, deal, name restatement, blog theme, or
// "you come for." Empty slot if that two-beat line cannot be sourced. Do not
// invent replacement copy. Ranking is unchanged.
// A plate-list-only line WITHOUT a why-sit is NOT the house bar — that is
// editorial law (docs/wayfind/PRODUCT_TRUTH.md). This gate must NOT auto-blank
// every comma-separated plate list: that would empty #861–#874 overnight with
// no sourced replacement. Junk blanks stay (#860 address/hours/host-theme).
// Plate-list tightening is Editorial's job.
...
// Miles-from-center is filler (owner, 2026-08-20 / live /nightlife/parrish 2026-08-29).
const HOOK_DISTANCE_FILLER = /\b\d+(?:\.\d+)?\s*(?:mi|miles?)\s+from\s+(?:the\s+)?(?:town|city)\s+center\b|\b\d+(?:\.\d+)?\s*(?:mi|miles?)\s+from\s+(?:downtown|the\s+center)\b/i;
// Generic vibe without a sourced distinction — the live Pangea / Mable / Cortez
// landing fillers. Not a plate-list blank: those stay in isUsableCardHook.
const HOOK_GENERIC_VIBE = /\b(?:moody lighting|hidden[- ]night[- ]out feel|relaxed local atmosphere|laid-back (?:\w+\s+){0,2}institution|where locals come for|cozy neighborhood bar with|creative cocktail lounge with)\b/i;
...
export function isDistanceFillerWhy(text) {
  return HOOK_DISTANCE_FILLER.test(String(text || ""));
}

export function isGenericVibeFiller(text) {
  return HOOK_GENERIC_VIBE.test(String(text || ""));
}

/** Landing why-it-fits: distance filler OR generic vibe. Empty instead. */
export function isFillerWhyFits(text) {
  const s = String(text || "");
  return isDistanceFillerWhy(s) || isGenericVibeFiller(s);
}
...
export function isUsableCardHook(text, name) {
  ...
  if (!s) return false;
  if (isDistanceFillerWhy(s)) return false;
