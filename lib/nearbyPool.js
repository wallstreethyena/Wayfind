 ...
 * not a merchandising cap: if it is ever reached, the status-aware rail caller
 * is told the ladder is incomplete instead of caching a partial truth.
 *
 * Existing callers receive the row array. With `opts.includeStatus`, the rail
 * cache owner receives `{ rows, degraded }` so a failed ladder cannot make a
 * retained thin-ring fallback look like a complete answer.
 */
export async function buildNearbyPool(origin, catSlug, opts) {
  const finish = (rows, degraded) => opts && opts.includeStatus ? { rows, degraded } : rows;
  const cfg = NEARBY_CATS[catSlug];
  if (!cfg) return finish([], false);
  if (!origin || !Number.isFinite(origin.lat) || !Number.isFinite(origin.lng)) return finish([], false);
  ...
  for (let ringIndex = 0; ringIndex < rings.length; ringIndex++) {
    const radiusMi = rings[ringIndex];
    ...
    try {
      ...
      if (!result || result.truncated) {
        console.warn(`nearbyPool: wf_inventory read failed for ${catSlug} at ${radiusMi}mi — incomplete paged result. Status-aware callers reject an incomplete ladder.`);
        continue;
      }
    } catch (e) {
    ...
    if (out.length >= NEARBY_TARGET_ROWS || ringIndex === rings.length - 1) {
      complete = true;
      break;
    }
  }
  best.sort(byTopRated);
  return finish(best, !complete);
}
