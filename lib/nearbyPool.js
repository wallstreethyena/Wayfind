...
export const NEARBY_CATS = {
  ...
  // v8.31.3 — an OUTING, not merely an attractions-gated row. The attractions
  // gate admits /\bspa\b|wellness/ on purpose (it feeds /things-to-do's Spa
  // subcategory), which is right for a browsable tab and wrong for a homepage
  // discovery rail: near Lakewood Ranch nine of this pool's top fifteen were
  // spas, med spas, gyms and a chiropractor, and near Parrish "Places You'd
  // Never Find" led with a medical spa and a wedding venue. lib/outing.js is
  // the rail-side question — would a reader GO there — and it is a veto, never
  // an allowlist, so the odd genuine local attraction still gets through.
 ...
 *  filter — not a target for how many CARDS ship, which is now uncapped and is
 *  decided far downstream. A ring that reaches this stops the ladder. */
export const NEARBY_TARGET_ROWS = 45;

// NO POOL CEILING (owner, 2026-08-22: "no more max on anything"). This was 60
// — "well above MAX_CARDS on purpose" — and with the card cap gone the reason
// for it went too. Every row here has already passed the category gate, the
// identity and the discovery filter; capping the survivors at sixty is the
// fifteen-row cap this module was written to delete, wearing a bigger number.
// The DB read is still bounded (limit=400 per ring, for cost and latency, not
// for merchandising) and NEARBY_TARGET_ROWS still stops the LADDER early so a
// dense downtown never widens past what is actually near the reader.
 ...
 *  a single rail's needs on purpose — every rail then filters this pool by its own
...
export function shapeNearbyRow(row, origin, gate, opts) {
  ...
  const shaped = {
    ...
    trending: false,
    trend_reason: null,
    // v8.83 — THE LINE THAT WAS SELECTED AND THEN DROPPED (owner, 2026-08-28:
    // "why are there silent cards"). `editorial` has been in the SELECT above
    // since this pool was written and it stopped here, so 991 already-written
    // lines within 25 miles of the flagship reader reached no card. Measured
    // against the live rails the same evening: 2 of 38 Tonight's Move cards
    // carried a line, 2 of 40 on Gems, 6 of 40 on Eat. Seven of those 38 have
    // an editorial sitting in the row this function was already holding.
...
export async function buildNearbyPool(origin, catSlug, opts) {
  ...
  // WO8 (2026-09-02) — same two levers as lib/inventoryServe.js
  // serveFromInventory, opt-in so lib/eventPairings.js (this pool's other
  // caller) is byte-for-byte unchanged. skipEditorial drops the long free-text
  // column from a bulk box read; the caller re-hydrates it for whatever
  // actually ships (railsData.js does this once, after fillRails, via
  // hydrateEditorialFor). readCache collapses an identical (category, box,
  // radius) read within one compute to a single network call.
  const skipEditorial = !!(opts && opts.skipEditorial);
  const readCache = opts && opts.readCache;
  const fields = skipEditorial
    ? "place_id,name,lat,lng,google_types,primary_type,signals,photo_ref,status"
    : "place_id,name,lat,lng,google_types,primary_type,signals,photo_ref,status,editorial";
  ...
  for (const radiusMi of rings) {
    ...
    try {
      ...
      const reqUrl = `${url}/rest/v1/wf_inventory?select=${fields}`
        + `&status=eq.OPERATIONAL&${q}&order=signals->reviews.desc.nullslast&limit=400`;
      const doRead = async () => {
        const resp = await doFetch(reqUrl, { headers: { apikey: anon, Authorization: "Bearer " + anon }, next: { revalidate: 3600 } });
        if (!resp || !resp.ok) return { ok: false, status: resp ? resp.status : null };
        return { ok: true, rows: await resp.json() };
      };
      const r = readCache && typeof readCache.get === "function"
        ? await readCache.get(`nb:${cfg.column}:${b.minLat.toFixed(4)}:${b.minLng.toFixed(4)}:${b.maxLat.toFixed(4)}:${b.maxLng.toFixed(4)}:${skipEditorial ? 1 : 0}`, doRead)
        : await doRead();
      if (!r || !r.ok) {
