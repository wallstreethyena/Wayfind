...
export function cardComplete(p) {
  ...
  // substance: at least ONE of a real rating, real review volume, or a photo —
  // a bare name with nothing behind it reads as a broken card, not a place.
  const hasRating = typeof p.rating === "number" && p.rating > 0;
  const hasReviews = typeof p.reviews === "number" && p.reviews > 0;
  const hasPhoto = !!(p.photo || (Array.isArray(p.photos) && p.photos.length));
  return hasRating || hasReviews || hasPhoto;
}
