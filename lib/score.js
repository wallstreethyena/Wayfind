// lib/score.js — Wayfind Score bands (v6.27). Pure logic, no JSX, so the
...
// Data-boundary validation: reject missing, nonnumeric, NaN, negative, or
// >10 values — corrupted scores must never render as real ones. Callers show
...
export function toDisplayScore(wf) {
  if (typeof wf !== "number" || !isFinite(wf)) return null;
  const s = Math.round((wf / 10) * 10) / 10;
