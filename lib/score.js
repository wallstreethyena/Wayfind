...
/**
 * A display score as TEXT, for rendering. "10" not "10.0", "9.4" stays "9.4".
 *
 * Owner, 2026-08-27: "we don't have to use the decimal for the Wayfind score,
 * the ten point zero. We can just say ten. That way reduces it a little bit
 * instead of having extra numbers."
 *
 * The rule is a trailing ".0", not a special case for ten — a 9.0 reads the
 * same way and there is no reason for the badge to spell out a zero it does
 * not need. toDisplayScore already rounds to one decimal, so this only ever
 * removes a digit that carries no information.
 *
 * It also makes the WIDEST badge narrower. scripts/test-place-card-layout.mjs
 * exists because the perfect score is the one badge that gains both a fourth
 * digit and the flame, and at 390px it sat on top of the card title (owner's
 * screenshot, 2026-08-27). Dropping the ".0" gives that corner slot back ~11px
 * on the only badge that ever needed it.
 */
