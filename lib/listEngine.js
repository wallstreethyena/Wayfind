...
// ── SHARE-CARD HELPERS — REMOVED 2026-08-12 ────────────────────────
// headlineSize, truncName, fitTickerItems and splitAccent lived here to serve
// this card's own layout. There is one share card now (app/api/og/card.jsx)
// and it does not use them:
//
//   • headlineSize stepped the type down in buckets by CHARACTER COUNT. In
//     Archivo 900 "W" is 1.000em and "i" is 0.319em, so it sized "Illinois"
//     and "WOMBAT" identically. lib/shareCard.js fits from the real advance
//     widths read out of the shipped TTFs.
//   • fitTickerItems and truncName packed the runners-up ticker, which the
//     one card does not draw — at the size a link preview is actually seen it
//     was one grey smear.
//   • splitAccent marked an accent phrase INSIDE a line. Satori reflows the
//     words around a coloured inline span, so accents are applied per LINE
//     now (accentLines in lib/shareCard.js).
//
// They are deleted rather than left exported: dead code with passing tests is
// how a repo grows guards that prove nothing about what ships.
