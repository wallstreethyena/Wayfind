...
// Everything the model was shown, as one string — the corpus verifyAtlasEditorial
// checks each number and proper noun against. Google's own fields count as
// sources, and so do our OWN owned-inventory fields (rating/reviews) — moved
// here (2026-09-04, WO-D-atlas-cache-batch) from app/api/cron/atlas-build/route.js
// so scripts/atlas-batch.mjs's owned-only path builds the identical corpus
// shape rather than a second, possibly-drifted copy. `ctx` need not carry
// every field (an owned-only ctx has no address/hours/google_summary) —
// absent fields simply contribute nothing, exactly like the live path when
// Google Details did not return them.
export function corpusOf(ctx, sources) {
  return [
