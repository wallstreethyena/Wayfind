...
// Our own commerce redirect (lib/commerce.js commerceHref). A curated event
// whose ticket is sold by an affiliate carries THIS as its ticket URL
// (lib/curatedEvents.js) — app-relative by design, so the partner URL never
// reaches the DOM. It is the one non-http shape a ticket URL may take; any
// other relative path is still rejected below.
export const COMMERCE_GO_RX = /^\/api\/commerce\/go\?(?:[\w.-]+=[\w.%:-]*&?)+$/;
export function isCommerceGoUrl(u) {
  return typeof u === "string" && COMMERCE_GO_RX.test(u);
}
...
export function resolveDestination(e) {
  ...
  // A curated row already HAS a page — /florida-events/<slug>, server-rendered
  // with the why-go, the parking, the insider tip and the JSON-LD. Sending the
  // reader straight out to the partner would throw away the only layer a
  // calendar cannot have. The ticket URL is not lost: the card keeps it as the
  // CTA (app/home.js EventRailCard: internal dest + e.url -> a real ticket
  // button), so the reader gets our page AND one tap to buy.
  if (e.curatedSlug) {
    return { dest: "/florida-events/" + e.curatedSlug, destKind: "internal", slug: e.curatedSlug, citySlug };
  }
...
// "Today" is anchored to the venue-local (US Eastern) calendar day, never the
// server's UTC day (see lib/siteTime.js). siteTodayStr is the app-wide single
// source of "today"; re-exported here so existing importers (app/api/events/route.js)
...
export function validateEvent(e, now = new Date()) {
  ...
  if (e.url && !isSafeHttpUrl(e.url) && !isCommerceGoUrl(e.url)) return { ok: false, reason: "invalid_url" };
...
// "Wayfind curated" outranks every aggregator because it is the only source
// here a person actually checked: a Tier-1 citation, a verification date, and
// an editorial hook. When the same event arrives from Ticketmaster too, the
// borrow rules below still take TM's buy URL if ours is missing — we keep the
// record, not the gaps.
const PROVIDER_RANK = { "Wayfind curated": 7, Ticketmaster: 6, SeatGeek: 5, Eventbrite: 4, Bandsintown: 3, "Manatee County Library": 3, "Local staples": 3, PredictHQ: 2, Google: 1 };
function dedupKey(e) {
...
export function dedupeAcrossProviders(list) {
  ...
  for (const e of list || []) {
    ...
    // 2026-08-07 — "enrich, don't compete" (EVENTS_PIPELINE_DIAGNOSIS §5 /
    // trend-signal spec): when the same event arrives from Ticketmaster (has
    // the buy URL) and PredictHQ (has the demand data), the TM record wins
    // the dedup and BORROWS the demand fields — rank / local_rank /
    // phq_attendance — so a bookable event also knows how big it is. These
    // feed lib/trendSignal.js (a place near a major event trends, disclosed)
    // and never touch the destination URL.
    if (keep.rank == null && other.rank != null) keep.rank = other.rank;
    if (keep.local_rank == null && other.local_rank != null) keep.local_rank = other.local_rank;
    if (keep.phq_attendance == null && other.phq_attendance != null) keep.phq_attendance = other.phq_attendance;
    map.set(k, keep);
  }
...
export function processEvents(results, { lat, lng, radius, city, now = new Date(), cap = 250 } = {}) {
  ...
  for (const r of results || []) {
    ...
    // A FAILING PROVIDER MUST SAY WHY (2026-09-03). Until this line, every
    // provider collapsed its failure to `ok: false` and the reason was thrown
    // away — so OpenWebNinja sat broken in production reporting `ok=false,
    // received=0` and nothing anywhere recorded whether that was a bad key, a
    // quota, or a moved endpoint. Diagnosing it meant guessing. `status` and
    // `reason` ride along when the provider supplies them, and are simply
    // absent otherwise, so a healthy entry keeps its exact old shape.
    const entry = {
      provider: r.provider,
    ...
    if (r && r.status != null) entry.status = r.status;
    if (r && r.reason) entry.reason = String(r.reason).slice(0, 200);
    else if (r && r.error) entry.reason = String(r.error).slice(0, 200);
    health.push(entry);
    raw = raw.concat(evs);
  }
  ...
  // v8.29.16 — PER-EVENT REACH. The feed's radius is tuned for "what is on
  // near me tonight", where 25 miles is generous. A DATED one-off is a
  // different question: Gasparilla happens once a year and a Parrish reader
  // wants to know about it even though Tampa is thirty-odd miles away. The
  // product already prices that difference elsewhere (DRIVE_REACH_MI,
  // EVENTS_NEAR_MI = 40), so a provider may declare its own honest reach on
  // the row (lib/curatedEvents.js CURATED_REACH_MI) and this guard honours the
  // larger of the two. It can only ever WIDEN, never narrow, and the card
  // still prints the city and the distance — so nothing can read as
  // round-the-corner that is not.
  const reachFor = (e) => {
    const own = Number(e && e.reachMi);
    return Number.isFinite(own) && own > maxMi ? Math.min(own, 100) : maxMi;
  };
  merged = merged.filter((e) => {
    if (e.civic) return true;
    if (e.lat != null && e.lng != null && lat != null && lng != null) return haversineMi(lat, lng, e.lat, e.lng) <= reachFor(e);
  ...
  // v8.29.18 — THE CAP MUST NOT DELETE A HAND-VERIFIED ROW. The list is sorted
  // by date and cut at `cap`, which is right for aggregator listings: a metro
  // can return thousands and the near-term ones are the useful ones. But the
  // curated schedule (lib/curatedEvents.js) is deliberately MONTHS out — the
  // whole point of it is "put this in your calendar" — so a date sort put six of
  // seven curated events past position 250 and the cap silently ate them.
  // Measured on production the day it shipped: the events screen shelf rendered
  // ONE card out of seven, and nothing said why.
  //
  // There are at most a couple of dozen curated rows in existence, each one
  // checked by a person, so they are lifted out before the cut and merged back.
  // The cap still governs everything an aggregator sends.
  const curatedRows = out.filter((e) => e && e.curated);
  const rest = curatedRows.length ? out.filter((e) => !(e && e.curated)) : out;
  const events = (curatedRows.length ? rest.slice(0, cap).concat(curatedRows) : out.slice(0, cap))
