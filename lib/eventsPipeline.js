...
// `now` is injectable for tests; date/time strings are provider-LOCAL and
// treated as opaque -- this pipeline never converts timezones, so a 7 PM
// local event can never drift to another hour or day here.
export function validateEvent(e, now = new Date()) {
  ...
  if (dt < todayLocal) return { ok: false, reason: "past" };
...
// Desirability rank for the "Happening near you" surface: high-demand ticketed
// events (concerts, comedy, sports, theatre) lead; free civic/library programs
// sink. Pure over the event's own fields so it is deterministic and testable;
// recency is the sort's secondary key, not baked in here.
export function eventValueRank(e) {
  if (!e) return -999;
  const seg = String(e.segment || "").toLowerCase();
  const src = String(e.source || "").toLowerCase();
  let s = 0;
  if (/music/.test(seg)) s += 42;
  else if (/sport/.test(seg)) s += 36;
  else if (/art|theat|comed/.test(seg)) s += 36;
  else if (/film/.test(seg)) s += 14;
  else if (/family/.test(seg)) s += 10;
  if (e.ticketed) s += 22;
  if (/ticketmaster|seatgeek/.test(src)) s += 16;
  else if (/bandsintown/.test(src)) s += 8;
  if (e.image) s += 6;
  if (e.price) s += 4;
  if (e.civic) s -= 60;
  if (/community/.test(seg)) s -= 45;
  if (/library|local staples/.test(src)) s -= 35;
  return s;
}
...
export function processEvents(results, { lat, lng, radius, city, now = new Date(), cap = 250 } = {}) {
  ...
  out.sort((a, b) => (eventValueRank(b) - eventValueRank(a)) || (a.date + (a.time || "")).localeCompare(b.date + (b.time || ""))); // v6.37: high-demand ticketed (concerts/comedy/sports/theatre) lead; civic/library sink; recency breaks ties
