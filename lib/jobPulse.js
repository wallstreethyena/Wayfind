...
/**
 * Read per-job health without collapsing "the query returned no rows" into
 * "the query could not be performed". Callers that report operational state
 * need this provenance; the compatibility wrapper below keeps the historical
 * fail-soft [] contract for product callers.
 */
export async function jobHealthDetailed(lookbackHours = 48) {
  const s = sbEnv();
  if (!s || !s.url || !s.key) {
    return { ok: false, rows: [], status: null, indeterminate: false, error: "no Supabase env (SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY)" };
  }
  try {
    ...
    if (!r.ok) {
      let detail = "";
      try { detail = (await r.text()).slice(0, 180); } catch { /* optional diagnostics */ }
      return { ok: false, rows: [], status: r.status, indeterminate: false, error: detail || `HTTP ${r.status}` };
    }
    let j;
    try {
      j = await r.json();
    } catch (e) {
      return { ok: false, rows: [], status: r.status, indeterminate: false, error: `invalid health JSON: ${String((e && e.message) || e).slice(0, 130)}` };
    }
    if (!Array.isArray(j)) {
      return { ok: false, rows: [], status: r.status, indeterminate: false, error: "invalid health payload (expected array)" };
    }
    return { ok: true, rows: j, status: r.status, indeterminate: false, error: null };
  } catch (e) {
    return { ok: false, rows: [], status: null, indeterminate: true, error: String((e && e.message) || e).slice(0, 160) };
  }
}

/** Read per-job health. Returns [] on any failure — never throws at a caller. */
export async function jobHealth(lookbackHours = 48) {
  const result = await jobHealthDetailed(lookbackHours);
