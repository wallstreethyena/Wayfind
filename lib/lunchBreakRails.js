...
export const LUNCH_BREAK_RAILS = Object.freeze([
  { id: "deli-sandwiches", title: "American Deli & Sandwiches", deck: "Stacked sandwiches built for lunch breaks." },
  { id: "chicken", title: "Chicken Favorites", deck: "The best crispy, grilled, and saucy chicken." },
  { id: "cuban-caribbean", title: "Cuban & Caribbean", deck: "Bold island flavors served fast." },
  { id: "mexican", title: "Mexican Bowls, Burritos & Tacos", deck: "Fast Mexican favorites made for midday." },
  { id: "pizza-italian", title: "Pizza by the Slice & Quick Italian", deck: "Quick slices and casual Italian favorites." },
  { id: "burgers", title: "Smash Burgers & Burgers", deck: "Great burgers served hot and fast." },
  { id: "healthy-fast", title: "Healthy Fast Options", deck: "Fresh, lighter lunches without the wait." },
]);
...
/**
 * PURE, and EXPORTED (v8.98) so retrieval can ask the question BEFORE the cost
 * bound rather than after it. Lunch Break's seven rails are narrow cuisine and
 * format identities; reading the top 400 of all `food` by Wayfind Score and
 * only then asking "is this Cuban" is the candidate-starvation shape
 * (lib/ownedPool.js). The route now hands THIS function to the reader, so there
 * is still exactly one opinion about which rail a place belongs to.
 * Returns a rail id or null.
 */
export function lunchRailMembership(place) {
  ...
  if (primary === "pizza_restaurant" || /\b(pizza|pizzeria|slices?)\b/.test(name)) return "pizza-italian";
...
export function composeLunchBreakRails(places) {
  ...
  for (const place of Array.isArray(places) ? places : []) {
    ...
    const railId = lunchRailMembership(place);
    if (!railId) continue;
    seen.add(place.id);
