...
const overlaps = (left, right) => { for (const value of left) if (right.has(value)) return true; return false; };
...
function membership(place) {
  ...
  if (CHICKEN_PRIMARY.has(primary) || overlaps(types, CHICKEN_PRIMARY) || /\b(chicken|tenders?|nuggets?|wings?)\b/.test(name)) return "chicken";
  if (overlaps(cuisines, CUBAN_CARIBBEAN)) return "cuban-caribbean";
  if (cuisines.has("mexican") || primary === "tex_mex_restaurant" || /\b(taqueria|tacos?|burritos?|birria)\b/.test(name)) return "mexican";
  if (cuisines.has("pizza") || cuisines.has("italian") || /\b(pizza|pizzeria|slices?)\b/.test(name)) return "pizza-italian";
  if (cuisines.has("burgers") || primary === "hamburger_restaurant" || /\b(smash\s*burgers?|hamburgers?|burgers?)\b/.test(name)) return "burgers";
  if (HEALTHY_PRIMARY.has(primary) || overlaps(types, HEALTHY_PRIMARY) || cuisines.has("vegan") || cuisines.has("vegetarian") || /\b(salads?|grain bowls?|protein bowls?|poke|a[cç]a[ií]|smoothies?|juice bar|healthy bowls?)\b/.test(name)) return "healthy-fast";
