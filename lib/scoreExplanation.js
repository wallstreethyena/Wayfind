...
// Zero-step receipts are real, but they are not an explanation. A 9.2 that
// stayed 9.2 must not render "starts at 9.2 / no additional adjustments".
export function visibleScoreMath(p) {
  const r = readableScoreReceipt(p);
  return r && r.steps.length > 0 ? r : null;
}
