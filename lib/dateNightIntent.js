...
/** Dinner is a meal in a date room — not a wine bar wearing the dinner slot. */
export function isDateDinner(p) {
  return !!(p && isDateRoom(p) && isMealPlace(p));
}
...
function byDinnerRank(a, b) {
  const sa = isSpecialDateDinner(a) ? 1 : 0;
  const sb = isSpecialDateDinner(b) ? 1 : 0;
