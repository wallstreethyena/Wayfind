  ...
  return (
    <div key={p.key} style={S.card}>
      ...
      {vids.map((v, i) => {
        ...
        // Only EMBEDDABLE platforms (TikTok/YouTube/Instagram) get the tap-to-load
        // player facade. Non-embeddable ones (Facebook /share/r/ reels) render as a
        // normal external social link below — never framed or marked up as a video.
        // NOTE: VideoObject rich-result schema is intentionally NOT emitted here —
        // it is deferred behind lib/videoObjectGate.js (owner decision; no re-hosting
        // of a creator's frame). Do not add VideoObject/og:video until that gate passes.
        return (
          <div key={i}>
            {isEmbeddable(v.platform, v.url) && <VideoFacade platform={v.platform} url={v.url} label={label} />}
            <div>
              {/* FOLLOWED link to the creator — deliberately no noreferrer/nofollow.
                  Doubles as the always-visible fallback if the player fails/post is removed. */}
