...
function placeCard(p) {
  ...
  // Only tokenless official players (TikTok/YouTube/Instagram) get a facade.
  // Facebook /share/r/ posts remain ordinary external links. VideoObject is
  // still deferred behind lib/videoObjectGate.js: no creator frame is rehosted.
  const hasPlayer = vids.some((v) => isEmbeddable(v.platform, v.url));
  const details = <>
    ...
    {vids.map((v, i) => {
      ...
      return <div key={i}>
        {/* FOLLOWED creator link and native fallback stay available in Details. */}
  ...
  return (
    <div key={p.key} style={S.card}>
      {hasPlayer ? <CreatorPlaybackDetails details={details} contentStyle={{ paddingTop: 12 }}>
        {vids.filter((v) => isEmbeddable(v.platform, v.url)).map((v, i) => {
          const pl = PLATFORM[v.platform] || {};
          return <VideoFacade key={i} platform={v.platform} url={v.url} label={`${p.title} on ${pl.label || v.platform}`} />;
        })}
  ...
  return (
    <div style={S.wrap}>
      ...
      <div style={{ margin: "0 0 18px" }}>
        <ShareButton url={pageShareUrl("/trending/" + slug)} title={`Trending spots in ${c.name}, ${c.state}`}
          text={`The places in ${c.name} trending on TikTok right now, with an honest take on each. On Wayfind.`}
          label="Share" tone="dark" event="page_share" meta={{ surface: "trending_city", city: slug, placement: "header" }} />
      </div>
...
export function TrendingIndexPage() {
  ...
  return (
    <div style={S.wrap}>
      ...
      <div style={{ margin: "0 0 18px" }}>
        <ShareButton url={pageShareUrl("/trending")} title="Trending and TikTok-famous places"
          text="Places trending on TikTok across Florida, with an honest take on each. On Wayfind."
          label="Share" tone="dark" event="page_share" meta={{ surface: "trending_index", placement: "header" }} />
      </div>
      {slugs.map((s) => (
