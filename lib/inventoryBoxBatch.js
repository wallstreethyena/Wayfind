...
function contentRangeTotal(headers) {
  const value = headers && typeof headers.get === "function" ? headers.get("content-range") : null;
...
// Stable order removes heap-order roulette. The exact Content-Range total is
// the exhaustion proof: PostgREST may cap the response below our URL limit.
// Any missing/mismatched proof discards the prime so authoritative per-city
// reads own the answer. The URL ceiling remains a second fail-closed guard.
async function fetchUnionBox(s, physical, box, limit, deadlineMs) {
  const h = { apikey: s.key, Authorization: `Bearer ${s.key}`, Prefer: "count=exact" };
  ...
  try {
    const r = await fetchDeadline(withSecondary, { headers: h, cache: "no-store" }, deadlineMs);
    if (!r.ok) return [];
    const rows = await r.json();
    if (!Array.isArray(rows) || rows.length >= limit) return [];
    const total = contentRangeTotal(r.headers);
    if (total === null || total !== rows.length) return [];
    return rows;
  } catch {
    return [];
  }
