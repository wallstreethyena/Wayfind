...
//
// 2026-09-11 — CANDIDATE INTEGRITY. The accelerator is never allowed to decide
// what exists. Its union read now reuses the same deterministic, paged reader
// as the identity-first surfaces. If the complete union cannot be proven inside
// its small time/row budget, it primes NOTHING and the authoritative per-city
// reader runs. A fast shortcut may save time; it may never hide a candidate.
import { DB_DEADLINE_MS } from "./fetchDeadline.js";
...
// 2026-09-10 — the consolidated read is an OPTIMISATION, never an authority.
// Production Lakewood Ranch exhausted the whole 9s rails budget inside
// anchor-pools. One enabling condition was that this optional prime could sit
// on the ordinary 4s DB deadline before the authoritative ranked reads even
// began. Healthy union reads historically land in hundreds of milliseconds;
// if the accelerator has not answered in 1.5s, abandon it and let the normal
// per-city path do its job. A failed/slow prime never changes which rows are
// eligible because this function already falls back by leaving the cache
// unprimed. Keep this strictly below DB_DEADLINE_MS so a future global DB
// timeout increase cannot silently give the accelerator the reader's budget.
...
async function fetchUnionBox(s, physical, box, limit, deadlineMs) {
  try {
    ...
    // The union is merely a cache prime. If it touches its runaway ceiling we
    // do not know whether another eligible row exists on the next page, so a
    // partial union is discarded rather than cached as an answer.
...
export async function primeConsolidatedInventoryReads(jobs, readCache, deps = {}) {
  ...
  for (const [physical, { sub, cities: cityMap }] of byPhysical) {
    ...
    for (const cluster of clusters) {
      ...
      // This is a RUNAWAY ceiling, not a merchandising cut. The union reader
      // pages deterministically up to it. Reaching the ceiling means the prime
      // is incomplete, so fetchUnionBox returns [] and the normal per-city read
      // owns the answer instead of enlarging a hidden lottery.
  ...
  // Independent categories used to wait in a serial waterfall before any
  // ranking could begin. Keep the same queries and keys, with at most four
  // reads in flight; a single failed group still leaves its normal fallback.
  let cursor = 0;
  await Promise.all(Array.from({ length: Math.min(4, batches.length) }, async () => {
    while (cursor < batches.length) {
      const batch = batches[cursor++];
      try { await batch(); } catch { /* this group is left unprimed */ }
    }
  }));
}
