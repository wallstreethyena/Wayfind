      ...
      batches.push(async () => {
        ...
        for (const city of cluster.cities) {
          for (const radiusM of [TIGHT_RADIUS_M, WIDE_RADIUS_M]) {
            ...
            // Trailing `:0:0` — offset:withMeta, added to serveFromInventory's
            // own key 2026-09-23. Every reader this prime feeds (railsData.js's
            // pool builders) passes neither, so this must match byte-for-byte
            // or the prime silently stops landing cache hits.
            const key = `srv:${physical}:${Number(city.lat).toFixed(4)}:${Number(city.lng).toFixed(4)}:${radiusM}:${N}:${sub}:0:1:0:0`;
