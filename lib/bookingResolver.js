...
function entityMatch(placeName, region, productTitle) {
  ...
  const hits = dist.filter((t) => hay.includes(t)).length;
...
export function scoreCandidate(place, product, opts = {}) {
  ...
  return {
    ...
    evidence: { entityMatch: em, categoryMatch: cm, specificity: sp, geoMatch: gm, geoMismatch, fanoutCount: Math.max(1, Number(fanoutFor(product, opts)) || 1) },
  };
}
