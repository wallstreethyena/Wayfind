 ...
 * curated-content allowlist. See guidePlaceHasSubstantiveDetail() there for
 * the >=80-char bar a guide blurb must clear before it counts as "detail";
 * a guide record failing that bar (or with no blurb at all) still supplies
 * name/lat/lng/category fallbacks but never flips hasDetails on its own.
 */
export function mergePlacePage(id, { skel, details: d, atlas, guide } = {}) {
  ...
  const description = (d && d.description) || (atlas && atlas.description) || (guideDetailed ? guide.description : null) || null;
  const hours = (d && Array.isArray(d.hours) && d.hours.length) ? d.hours : ((atlas && atlas.hours) || []);
  return {
    id,
    name,
    address,
    lat: d && d.lat != null ? d.lat : (skel && skel.lat != null ? skel.lat : (guide && guide.lat != null ? guide.lat : null)),
    lng: d && d.lng != null ? d.lng : (skel && skel.lng != null ? skel.lng : (guide && guide.lng != null ? guide.lng : null)),
