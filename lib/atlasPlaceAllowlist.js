...
import { placeDurableEligibility, EDITORIAL_WHY_MIN_LEN } from "./placeEligibility.js";
...
export function inventoryToSkeleton(row) {
  ...
  return {
    ...
    primary_type: un(row.primary_type),
    google_types: Array.isArray(row.google_types) ? row.google_types.map(un).filter(Boolean) : [],
    signals: sig,
...
export function preferInventorySkeleton(inventory, indexed) {
  ...
  return {
    ...
    primary_type: inventory.primary_type || indexed.primary_type || null,
    google_types: inventory.google_types?.length ? inventory.google_types : (indexed.google_types || []),
...
/**
 * skel: wf_place_ids/wf_inventory identity. details (d): live/cached Google
 * Place Details. atlas: publish-ready Atlas editorial card. guide: a GUIDES
 * pick record from lib/guidePlaceIndex.js (guidePlaceFor(id)) — the third,
 * curated-content allowlist. editorial: a verified wf_editorial_servable row
 * for this id (lib/placeIndex.js getVerifiedEditorial(id)), or null — the
 * fourth, Google-free content source. See guidePlaceHasSubstantiveDetail()
 * there for the >=80-char bar a guide blurb must clear before it counts as
 * "detail"; a guide record failing that bar (or with no blurb at all) still
 * supplies name/lat/lng/category fallbacks but never flips hasDetails or
 * durableEligible on its own.
 */
export function mergePlacePage(id, { skel, details: d, atlas, guide, editorial } = {}) {
  if (!skel && !atlas && !guide) return null;
  const sig = skel && skel.signals && typeof skel.signals === "object" ? skel.signals : {};
  const guideDetailed = !!(guide && typeof guide.description === "string" && guide.description.length >= 80);
  const editorialWhy = un(editorial && editorial.why_here);
  const name = (d && d.name) || (skel && skel.name) || (atlas && atlas.name) || (guide && guide.name) || null;
  if (!name) return null;
  const address = (d && d.address) || (atlas && atlas.address) || null;
  // PRIORITY ORDER for the merged description (also what the SEO meta
  // description reads first, lib/placeData.js placePageMetadata): a
  // verified wf_editorial_servable why_here, then a substantive GUIDES
  // blurb, then a publish-ready Atlas card's sourced two-beat take, and
  // only then Google's editorialSummary — Wayfind's own researched voice
  // always outranks Google's, and Google is the last resort, never the
  // gate (see lib/placeEligibility.js).
  const description = (editorialWhy && editorialWhy.length >= EDITORIAL_WHY_MIN_LEN ? editorialWhy : null)
    || (guideDetailed ? guide.description : null)
    || (atlas && atlas.description) || (d && d.description) || null;
  const hours = (d && Array.isArray(d.hours) && d.hours.length) ? d.hours : ((atlas && atlas.hours) || []);
  const lat = d && d.lat != null ? d.lat : (skel && skel.lat != null ? skel.lat : (guide && guide.lat != null ? guide.lat : null));
  const lng = d && d.lng != null ? d.lng : (skel && skel.lng != null ? skel.lng : (guide && guide.lng != null ? guide.lng : null));
  return {
    id,
    name,
    address,
    ...
    category: (d && d.category) || (skel && skel.category) || (atlas && atlas.category) || (guide && guide.category) || null,
    hours,
    description,
    mapsUri: (d && d.mapsUri) || (atlas && atlas.mapsUri) || null,
    businessStatus: (d && d.businessStatus) || (skel && skel.status) || null,
    hasDetails: !!(d || (atlas && (atlas.description || atlas.address)) || guideDetailed),
    // THE Google-free indexability signal (lib/placeData.js isIndexable) —
    // identity (name + (lat/lng or address)) AND at least one substantive
    // Wayfind-held text source. See lib/placeEligibility.js for why this is
    // deliberately NOT a function of `d` (Google Place Details/cache).
    durableEligible: placeDurableEligibility({ name, lat, lng, address, atlas, guide, editorial }),
    // Not folded into `city`/`address` above: a guide names a CITY, never a
    // street address, and a real Google/Atlas address must always win. This
    // is a fallback for cityOf()/the LocalBusiness PostalAddress ONLY when no
    // address exists at all — see lib/placeData.js cityOf and lib/placePage.js.
    guideCity: (guide && guide.city) || null,
    // Verified wf_editorial_servable fields (Wayfind's own researched copy,
    // no Google). Rendered by lib/placePage.js only when present.
    localTip: un(editorial && editorial.local_tip),
    bestTime: un(editorial && editorial.best_time),
    // Rendered as a "From the Wayfind guide" attribution + internal link back
    // on the place page (lib/placePage.js). Present whenever a GUIDES pick
    // names this id, independent of whether its blurb was needed for
    // hasDetails — an already-indexable Atlas/Google page still gets credited
    // internal-link traffic from its guide.
    guide: guide ? { slug: guide.guideSlug, title: guide.guideTitle, blurb: guide.description || null, alsoIn: Array.isArray(guide.alsoIn) ? guide.alsoIn : [] } : null,
  };
}
