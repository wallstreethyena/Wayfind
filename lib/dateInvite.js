...
const MAX = { place: 60, city: 32, when: 28, from: 24, note: 90 };
...
export function decodeInvite(raw) {
  ...
  try {
    ...
    return {
      ...
      from: cut(o.f, MAX.from), note: cut(o.n, MAX.note), id: cut(o.i, 40),
    };
  } catch (e) { return null; }
}
...
// ══ THE PAGE ════════════════════════════════════════════════════════════════
export function askHeadline() { return "Will you go out with me?"; }
...
export function yesText(inv, activity, when) {
  const a = activityFor(activity);
  const what = a ? a.label.toLowerCase() : "it";
  const w = cut(when, MAX.when);
  return "Yes!" + (a ? " " + what.charAt(0).toUpperCase() + what.slice(1) : "") +
         (w ? " on " + w : "") + " — it's a date.";
}
export function noText() { return "I want to, but not this time."; }
