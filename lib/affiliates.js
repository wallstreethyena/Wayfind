...
import { isHotelBookingVerified } from "./hotelBookingVerification.js";
...
// Shared earning hotel path. The server owns Stay22 attribution and destination.
export function hotelGoUrl(place, location) {
  if (!place?.name?.trim() || !isTrueLodging(place)) return null;
  // 2026-09-23 fail-closed gate (lib/hotelBookingVerification.js). An audit of
  // all 394 served hotels found only 40 whose Stay22 handoff actually lands on
  // the property named on the card; 79 landed on a different business and 250
  // on a generic area list. isTrueLodging proves the card IS a hotel, never
  // that the outbound link reaches THAT hotel, so it cannot be the gate on its
  // own. No verification record means no link — absence of evidence fails.
  if (!isHotelBookingVerified(place)) return null;
  const address = String(place.address || place.city || location || "").trim();
  if (!address) return null;
  const q = new URLSearchParams({ name: place.name, address, surface: "hotel_booking" });
  if (place.id) q.set("content", place.id);
  // 2026-09-22: forward coordinates only when BOTH are finite numbers, so
  // /api/hotels/go's normalizedHotelLocation (lib/hotelRedirect.js) can anchor
  // the outbound Booking.com search on this exact property instead of a
  // generic results page. One missing/bad value here still means no lat/lng
  // reaches the route — normalizedHotelLocation fails the pair closed either way.
  ...
  return "/api/hotels/go?" + q.toString();
}
