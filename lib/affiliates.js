...
export function viatorDirectUrl(url) {
  ...
  return VIATOR_PID ? `${url}${sep}pid=${encodeURIComponent(VIATOR_PID)}&mcid=42383&medium=link` : url;
}

export function hotelSearchUrl(query) {
  if (!CJPID || !query) return null;
  const dest = "https://www.booking.com/searchresults.html?ss=" + encodeURIComponent(String(query));
  return "https://www.anrdoezrs.net/links/" + encodeURIComponent(CJPID) + "/type/dlg/sid/wayfind-guides/" + dest;
}
