...
// v8.44 — the SERVER-RESOLVED delivery link. uberEatsUrl above builds a
// client-side template-wrapped SEARCH link and returns null when the template
// env is unset — which is why the Detail sheet's "Order delivery" rung fell to
// an unmonetized "See menu" (restaurant/menu was the single largest
// primary_cta_null bucket in August: 37 + 33 events). /api/eats/go is strictly
// better on every axis: it resolves the EXACT store page server-side (falling
// back to the attributed search), reads the template at runtime instead of
// bake time, and emits the provider_redirect_* events that make the click
// measurable. Every delivery/pickup CTA should route through it.
export function uberEatsGoUrl(name, city, opts = {}) {
  if (!name) return null;
  let u = "/api/eats/go?name=" + encodeURIComponent(String(name).slice(0, 120));
  if (city) u += "&city=" + encodeURIComponent(String(city).slice(0, 60));
  if (opts && isFinite(opts.lat) && isFinite(opts.lng)) u += "&lat=" + (+opts.lat).toFixed(5) + "&lng=" + (+opts.lng).toFixed(5);
  if (opts && opts.surface) u += "&surface=" + encodeURIComponent(String(opts.surface).slice(0, 60));
  if (opts && opts.clickId) u += "&click_id=" + encodeURIComponent(String(opts.clickId).slice(0, 64));
  return u;
}
