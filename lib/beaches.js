...
// v8.31 — THE BEACH IDENTITY, which did not exist until now.
//
// saysBeach() above is a TRIGGER FOR VETTING and says so in its own comment —
// "NOT a claim of truth". Nothing in the app ever asked whether a row IS a
// beach, so the Beach Day rail took whatever `category = 'beach'` and the
// discovery gate handed it. lib/placeFilter.js's beach gate admits on the
// NAME, and inventory categorises by name too, so within 23 miles of Parrish
// the rail's candidate set contained, all measured 2026-08-22:
//
//     Holmes Beach Tennis Courts          Siesta Key Beach Tennis Courts
//     Siesta Key Beach Public Pickleball  Siesta Key Beach Playground
//     Public Beach Access 5 / 10 / 11     St. Pete Beach Access
//     Coquina Beach Lifeguard / Tower     Lido Beach Pool & Pavilion
//     Coquina Beach Parking               Beach Therapy - (Asian massage)
//     Siesta key Beach,Florida            Cortez Beach, South Pier-
//         (a wedding venue)                   (a fishing pier)
//
// Same discipline as lib/breakfast.js after v8.30.1, and for the same reason:
// the primary type is what a place IS, and a token or a word in the name is
// only evidence. Three rules, in order.
//
// WHAT IT DELIBERATELY KEEPS: Manatee Beach Park and Turtle Beach Park (real
// beach parks whose primary type is `beach`), the dog beaches, Bayfront Park.
// WHAT IT DELIBERATELY DROPS: Apollo Beach Preserve — a nature_preserve on
// Tampa Bay with no swimmable Gulf beach, and 4,733 reviews that would have put
// it near the top of the rail.
const BEACH_PRIMARY_VETO = new Set([
  "tennis_court", "playground", "massage_spa", "fishing_pier", "wedding_venue",
  "sports_activity_location", "dog_park", "campground", "parking_lot",
  "hotel", "motel", "resort_hotel", "lodging", "travel_agency", "store",
  "nature_preserve", "marina", "restaurant", "bar", "cafe",
]);

// A facility AT a beach is not the beach. Whole words, the same boundary rule
// breakfast and quickService use — "Access" must be a word, so "Beach Access 5"
// is refused and "Accessible Beach" would not be.
export const BEACH_FACILITY_RX = /\b(access|parking|tower|lifeguard|pavilion|court|courts|pickleball|tennis|playground|rental|rentals|shop|store|resort|hotel|condo|villa|club|therapy|massage|marina|pier|cafe|caf\u00e9|bar|grill|restaurant)\b/i;

/**
 * PURE. Is this row a beach a reader can actually go and sit on?
 *
 * @param {{primaryType?:string, primary_type?:string, types?:string[], name?:string}} p
 */
export function isBeachPlace(p) {
  if (!p) return false;
  ...
  // 1. WHAT IT IS. A beach primary, or a beach token when the row carries no
  //    primary at all (the wf_best_picks shape).
  const claimsBeach = primary === "beach" || (!primary && types.includes("beach"));
  if (!claimsBeach) return false;
  // 2. …unless the primary names a different facility. Belt and braces: a row
  //    whose primary is `beach` cannot also be a tennis court, but a row with
  //    no primary and a `beach` token can be anything.
  if (BEACH_PRIMARY_VETO.has(primary)) return false;
  if (types.some((t) => BEACH_PRIMARY_VETO.has(t) && t !== "beach")) return false;
  // 3. A FACILITY AT a beach is not the beach. This is the rule that does the
  //    work: every row in the list above carries `beach` as its primary type
  //    and is still an access path, a car park or a pickleball court.
  if (BEACH_FACILITY_RX.test(String(p.name || p.title || ""))) return false;
  return true;
}
