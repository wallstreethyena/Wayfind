...
async function collectGoogle(publicRequest, nowMs, adapters) {
  try {
    const result = await adapters.fetchGoogleTrendingRss({ fetchImpl: async (url) => {
    ...
    for (const item of result.items) {
      ...
      for (const signal of adapters.conceptSignalsFromItems([item], { observedAt: new Date(observed).toISOString() })) {
