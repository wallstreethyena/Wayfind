...
//
// ⚠ THE SCHEMA BELOW IS DECLARED, NOT YET VERIFIED AGAINST A REAL EXPORT.
// Every column name here is therefore a HYPOTHESIS about Semrush's export format,
// sourced from their public UI vocabulary. The first real import must validate
// the header and fail loudly on drift.
//
...
//
// NEVER: log a whole source row, commit a CSV, or hand
 ...
 * private source data, and a transitive dependency on the path that reads it is a
...
export function readSnapshot(text) {
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    if (emptyRequired.length) {
      ...
      // are private source data and must not reach a log.
