  ...
  try {
    ...
    if (user || action !== "open") {
      try {
        ...
        localStorage.setItem("wf_taste_local", JSON.stringify(applyLocalTaste(cur, sig, now)));
        if (action !== "open" && localStorage.getItem("wf_personalize") == null) localStorage.setItem("wf_personalize", "on");
      } catch (e) {}
    }
