import { gateShut, spendAllow, spendAllowSkuTransition } from "./spendGate.js";
...
// COST GUARD (2026-09-04): every call through NEARBY is metered. This module
// receives its fetch by injection, so the gate is asserted here rather than at
// each call site — one place to read, one place to break.
async function nearbyAllowed(tier, authorize) {
  if (typeof authorize === "function") return !!(await authorize(tier));
  if (gateShut()) return false;
  return tier === "pro"
    ? spendAllow("nearby_pro")
    : spendAllowSkuTransition("nearby_pro", "nearby_enterprise");
}
const MASK = [
 ...
 */
export async function preflightTypes(types, key, fetchImpl, authorize) {
  const f = fetchImpl || fetch;
  const usable = [], rejected = [];
  for (const t of types) {
    try {
      if (!(await nearbyAllowed("pro", authorize))) return { usable, rejected, gated: true };
...
async function nearbyOnce(circle, type, key, f, authorize) {
  try {
    if (!(await nearbyAllowed("enterprise", authorize))) return { status: 0, places: [], gated: true };
    const r = await f(NEARBY, {
      method: "POST",
 ...
 */
export async function sweepDistricts(districts, types, key, fetchImpl, authorize) {
  const f = fetchImpl || fetch;
  const byId = new Map();
  const curve = [];
    ...
    for (const type of types) {
      ...
      for (let depth = 0; depth <= MAX_SUBDIVISION_DEPTH && frontier.length; depth++) {
        ...
        for (const circle of frontier) {
          ...
          const { status, places, gated } = await nearbyOnce(circle, type, key, f, authorize);
          if (gated) { budgetExhausted = true; break; }
          calls++;
