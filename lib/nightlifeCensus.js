...
async function nearbyAllowed(injectedTransport = false) {
  if (injectedTransport) return true;
  if (gateShut()) return false;
  return spendAllow("nearby_pro");
}
const MASK = [
...
export async function preflightTypes(types, key, fetchImpl) {
  ...
  for (const t of types) {
    try {
      if (!(await nearbyAllowed(!!fetchImpl))) return { usable, rejected, gated: true };
...
async function nearbyOnce(circle, type, key, f, injectedTransport = false) {
  try {
    if (!(await nearbyAllowed(injectedTransport))) return { status: 0, places: [], gated: true };
    const r = await f(NEARBY, {
      method: "POST",
    ...
    for (const type of types) {
      ...
      for (let depth = 0; depth <= MAX_SUBDIVISION_DEPTH && frontier.length; depth++) {
        ...
        for (const circle of frontier) {
          ...
          calls++;
          const { status, places } = await nearbyOnce(circle, type, key, f, !!fetchImpl);
