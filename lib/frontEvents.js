...
export function eventStature(e, bucketOf) {
  ...
  // The bucket is a PROXY for "someone sells seats to this". When the row
  // actually tells us — `ticketed === true`, which only a source that checked
  // ever sets — use the fact instead of the proxy. Halloween Horror Nights is a
  // ticketed event that buckets as community; it should not be ranked as if
  // nobody sells a ticket to it.
  if (TICKETED_KEYS.includes(b) || e.ticketed === true) n += 2;
  if (e.image) n += 1;
  if (e.destKind && e.destKind !== "internal") n += 1;
  // v8.29.16 — CURATION IS A SIGNAL, and the only one here that required a
  // person. A curated row (lib/curatedEvents.js) was checked against a Tier-1
  // source, carries a verification date, and has an editorial hook written for
  // it. Every other row in this pipeline is a listing nobody read. Weighted so
  // it leads the local field without displacing a real concert — the category
  // this app converts on — which is the balance the owner set in v7.02.
  if (e.curated) n += 3;
  return n;
}
...
// v7.24 — the RECURRING SERIES a listing belongs to, or null.
//
// A fixture title names the home side before "vs"/"v", and a residency or run
// names itself before a colon. Both are the shape that floods a rail: one team
// or one act with a schedule. Everything before the separator, normalised.
// Returns null when there is no separator, so a one-off concert is never
// capped against another — the rule may only ever thin a repeat.
export const SERIES_CAP = 2;
export function seriesKey(e) {
  const raw = String((e && (e.name || e.title)) || "").trim();
  if (!raw) return null;
  const m = raw.split(/\s+(?:vs\.?|v\.?|versus|@)\s+/i);
  const head = m.length > 1 ? m[0] : (raw.includes(":") ? raw.split(":")[0] : null);
  if (!head) return null;
  const k = head.toLowerCase().replace(/[^a-z0-9]+/g, "");
...
export function bestFirst(usable, bucketOf, featured) {
  ...
  for (const e of ranked) {
    ...
    try { b = bucketOf ? bucketOf(e) : null; } catch (err) { b = null; }
    const k = b || "?";
    // v7.24 — AND NO ONE SERIES EITHER. Measured on the live rail: 5 of the 12
    // events were Tampa Bay Rays home games ("…vs Baltimore Orioles", "…vs
    // Toronto Blue Jays", "…vs San Diego Padres"). The bucket cap above could
    // not see it — every one of them is bucket `sports`, so they sat well
    // inside CAP while reading as one team's schedule pasted into a discovery
    // rail. Same shape as the bucket rule and the same mercy: overflow goes to
    // the TAIL, so a reader who wants the whole home stand still scrolls to it.
    const s = seriesKey(e);
    const overSeries = s && (seriesCount[s] || 0) >= SERIES_CAP;
    if (!overSeries && (count[k] || 0) < CAP) {
      count[k] = (count[k] || 0) + 1;
