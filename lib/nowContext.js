...
export function siteHourFloat(now = new Date()) {
  try {
    const p = new Intl.DateTimeFormat("en-US", { timeZone: SITE_TZ, hour: "2-digit", minute: "2-digit", hour12: false }).formatToParts(now);
...
export function normalizeWeather(w) {
  ...
  return { tempF, feelsF, rainPct, condition: null, code };
}
...
export function weatherFlags(nw) {
  ...
  const isWet = severe || (nw.code != null && WET_CODES.has(nw.code)) || (nw.rainPct != null && nw.rainPct >= WET_RAIN_PCT);
...
export function nowContext(opts = {}) {
  ...
  const hour = Number.isFinite(Number(opts.hour)) ? ((Number(opts.hour) % 24) + 24) % 24 : siteHourFloat(now);
