...
export function funnelCoverage() {
  const detailPathGaps = UNINSTRUMENTED_MONEY_SURFACES.filter((s) =>
    s.file.startsWith("app/components/")
  );
  return {
    steps: FUNNEL_STEPS.length,
    uninstrumentedMoneySurfaces: UNINSTRUMENTED_MONEY_SURFACES.length,
    // The detail path cannot be read end-to-end while a detail-sheet component
    // still emits only legacy *_out events. Rails in app/home.js are a separate
    // surface and do not block reading the detail path.
    detailPathReadable: detailPathGaps.length === 0,
  };
}
