...
export function rankByHour(picks, hour) {
  // v6.97: the fallback used to be 0 (midnight) while the doc above claimed
  // "the caller's local hour" — a caller omitting `hour` buried every daytime
  // category (outdoor closes at 19) at noon. The fallback is now the shared
  // venue-local hour (check-one-clock's one sanctioned source).
  const h = Number.isFinite(hour) ? hour : siteHourFloat();
