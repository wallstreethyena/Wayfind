...
export function pickCachedCredits(credits, liveNames, perPlace = 3) {
  const per = new Map();
  const out = [];
  for (const c of Array.isArray(credits) ? credits : []) {
    ...
    out.push({ photo_name: c.photo_name, place_id: c.place_id, author_name: c.author_name, author_uri: c.author_uri || null, maps_uri: c.maps_uri || null });
  }
  return out;
}
