...
//
// ISOLATION (Gate-2): no affiliate/commercial signal enters the score, and NO
// product URL is ever hand-built — product_url is always runtime data off the
// API, never a literal. rankExperiences only orders a rail from the public
// rating + review evidence already printed on its cards. Selling-out flags,
// price and commission never affect that order.
...
export function destsWithin(loc, miles) {
  ...
  // v6.73: only fall back to the NEAREST market when it's realistically nearby
  // (≤150mi — just past the widest 120mi rung). A user in another state (e.g.
  // South Carolina, ~350mi from Orlando) must NOT be served Florida tours — they
  // get an empty set here so the rail can fetch live for their real location.
  const MAX_MARKET_MI = 150;
  return within.length ? within : (ranked[0].mi <= MAX_MARKET_MI ? [ranked[0].id] : []);
}
...
// The ONE ordering for every score-bearing bookable-experience collection.
// This is the same Bayesian calculation as lib/google.js wayfindScore, kept
// here because this module is pure and client/server safe. The integer is the
// exact 0-100 value PlaceScoreChip turns into the visible 0-10 badge. Reviews
// and title only break equal DISPLAYED scores; missing ratings sort last.
export function experienceWayfindScore(item) {
  const explicit = Number(item && item.wfScore);
  ...
  const reviews = Math.max(0, Number(item && item.reviews) || 0);
  const m = 60;
  const baseline = 3.9;
  const bayes = (reviews / (reviews + m)) * rating + (m / (reviews + m)) * baseline;
  return Math.round((bayes / 5) * 100);
}

export function rankExperiences(items) {
  return (Array.isArray(items) ? items : []).slice().sort((a, b) =>
    (experienceWayfindScore(b) - experienceWayfindScore(a)) ||
    ((Number(b && b.reviews) || 0) - (Number(a && a.reviews) || 0)) ||
    String((a && (a.title || a.name)) || "").localeCompare(String((b && (b.title || b.name)) || ""))
  );
}
