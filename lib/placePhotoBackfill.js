...
export async function runBackfill({
  ...
  const wikimedia = installWikimediaFetchPolicy();
  ...
  if (source !== "at-risk") {
    try {
      ...
      // A lost backlog read must not take down the fresh drain. The
      // at-risk/general scans still run; replay simply waits for the
      // next hour. Named on the return so a silent skip is visible.
      backlog = [];
    }
  }
  ...
  const replayPool = selectReplaySlice([...replayById.values()], { limit: Math.max(limit, scanLimit), now });
  ...
  await pool(candidates, POOL_SIZE, async (place) => {
    ...
    if (!wikimedia.canRequest()) {
      skipped++;
      details.push({ placeId: place.place_id, outcome: "skipped", reason: "wikimedia-backoff" });
      return;
    }
    let reason = null;
    try {
