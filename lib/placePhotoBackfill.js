 ...
 */
export function describeAtRisk({ atRiskUnavailable, atRiskTaken, atRiskScanned, source, atRiskStatus, atRiskCode, atRiskRetried } = {}) {
  if (atRiskUnavailable) {
    // The STATUS and the CODE, not just "it failed": a 57014 is a cold-cache
    // statement timeout that an index would end, and a 404 is a missing
    // relation. Same sentence, entirely different next move.
    const why = [atRiskStatus != null ? `HTTP ${atRiskStatus}` : null, atRiskCode || null].filter(Boolean).join("/");
    const tried = atRiskRetried ? ", retried once" : "";
    return `at-risk UNAVAILABLE (${AT_RISK_VIEW} read failed${why ? ` — ${why}` : ""}${tried} — general scan only)`;
  }
  if (source === "all") return "at-risk skipped (source=all)";
  // The retry marker rides ALONGSIDE the counts and never replaces them: a run
  // that recovered still did its work, and the note must show both.
  return `at-risk ${atRiskTaken || 0}/${atRiskScanned || 0}${atRiskRetried ? " (retried)" : ""}`;
}
...
// PostgREST returns it as an HTTP 500 whose BODY carries `code:"57014"` — the
// status alone cannot tell it apart from a genuine server fault, which is why
// the body is read before deciding whether to try again.
const STATEMENT_TIMEOUT_CODE = "57014";

/**
 * ONE read of the at-risk view. Never throws for an HTTP answer; a non-2xx is
 * returned as data so the caller can decide, on the CODE, whether trying again
 * is honest or just noise.
 */
async function readAtRiskOnce(s, fetchLimit) {
  const r = await fetch(
  ...
  try {
    ...
    // A non-JSON error body is not a reason to fail differently; the status
    // still decides.
  }
...
/**
 * The at-risk worklist, with EXACTLY ONE retry on a cold-cache statement
 * timeout.
 *
 * WHY (measured in production, 2026-09-09). The 19:35Z hourly run filed
 * `place-photos: wf_photo_at_risk unavailable` and made ZERO decisions. The
 * view joins ~10k live `photo|places/%` cache rows against wf_inventory through
 * split_part() with no supporting index: EXPLAIN reports 4,705 ms warm and
 * 44,674 shared buffers, of which 41,829 come from re-probing heap pages the
 * `k ~~ 'photo|places/%'` filter then discards (35,702 rows removed). Cold, it
 * exceeds PostgREST's statement timeout — a direct read returned
 * `{"code":"57014"}` after 12.4s, then succeeded in 0.9s on the very next try,
 * and again in 0.9s after that. The failure is cache temperature, and the
 * canceled statement is itself what warms the pages the retry reads.
 *
 * ONE retry, not a loop: two timeouts mean the query is genuinely too expensive
 * right now, and a third attempt only spends more of the run's budget to learn
 * the same thing. A 4xx is never retried at all — a missing relation or a
 * malformed request answers identically however many times it is asked.
 *
 * THIS IS INSURANCE, NOT THE FIX. The fix is a partial functional index on
 * wf_places_cache whose predicate matches the view's WHERE verbatim (`~~` has
 * no btree opfamily, so implication between two different LIKE patterns is
 * unprovable and a near-miss predicate would silently never be used). Until
 * that migration lands, this keeps the drain running instead of idling an hour
 * at a time.
 */
async function fetchAtRiskWorklist(s, fetchLimit) {
  let attempt = await readAtRiskOnce(s, fetchLimit);
  let retried = false;
  const worthRetrying = !attempt.ok && (attempt.code === STATEMENT_TIMEOUT_CODE || (attempt.status >= 500 && attempt.status <= 599));
  if (worthRetrying) {
    retried = true;
    attempt = await readAtRiskOnce(s, fetchLimit);
  }
  if (!attempt.ok) {
    const err = new Error(`${AT_RISK_VIEW} read failed: HTTP ${attempt.status}${attempt.code ? ` (${attempt.code})` : ""}`);
    err.status = attempt.status;
    err.code = attempt.code;
    err.retried = retried;
    throw err;
  }
  return { rows: attempt.rows, retried };
}
...
export async function runBackfill({
  ...
  let atRiskCode = null;
  let atRiskRetried = false;
  if (wantAtRisk) {
    try {
      const worklist = await fetchAtRiskWorklist(s, scanLimit);
      atRiskRows = worklist.rows;
      atRiskRetried = worklist.retried;
    } catch (e) {
      ...
      atRiskCode = e && e.code ? e.code : null;
      atRiskRetried = !!(e && e.retried);
    }
  }
  if (source === "at-risk" && atRiskUnavailable) {
    return {
      ...
      atRiskCode,
      atRiskRetried,
      // Names the status AND the SQLSTATE. "unavailable" alone sent an operator
      // looking for an outage when the answer was a cold query plan.
      note: `${AT_RISK_VIEW} unavailable (${atRiskStatus != null ? `HTTP ${atRiskStatus}` : "no status"}${atRiskCode ? `/${atRiskCode}` : ""}${atRiskRetried ? ", retried once" : ""})`,
    };
  }
  ...
  await pool(candidates, POOL_SIZE, async (place) => {
    ...
    try {
      ...
      if (photo) {
        ...
        // THE HEADLINE INVARIANT OF THIS CHANGE: no row, ever, for an answer
        // we did not get to see. The place keeps no wf_place_photo row, so it
        // stays in the worklist and the next run tries again.
        deferred++;
        details.push({ placeId: place.place_id, outcome: "deferred", reason: reason || "unknown" });
      } else {
        rejected++;
