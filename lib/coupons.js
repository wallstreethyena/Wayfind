...
export const COUPONS = [
  ...
  //
  // Two-way attribution by design: the ?aid=127667 click AND the code itself
  // (Klook pays a revenue share on orders placed with an authorized code).
  //
  // ⚠ OPEN DECISION FOR THE OWNER (2026-08-02, audit F5). This is the one
  // monetized coupon link still rendered as a raw partner URL, and it is
  // EXEMPTED BY NAME in check-coupon-link-attribution rather than fixed,
  // because fixing it means choosing which Klook program gets paid and that is
  // not a code question:
  //
  //   - Routing it through /api/commerce/go would wrap it with Travelpayouts
  //     (PROVIDERS.klook -> tpDeepLink, promoId 4110 / campaignId 137, marker
  //     750791), REPLACING this `aid` attribution with a different program's.
  //   - PROVIDERS.klook also resolves only EXACT curated products — "no
  //     provider-wide search or homepage escape hatch" — and this is a
  //     homepage, so it has no registry entry to resolve through.
  //   - Klook 403s every fetcher, so a real landing page for this code cannot
  //     be verified from here. A 403 is not evidence a page exists.
  //
  // Exposure is bounded: it expires 2026-08-02, so the card auto-hides
  // immediately and the raw link stops rendering. To close it properly, add a
  // browser-verified Klook landing URL to lib/partnerOfferRegistry.js, point
  // this at its offer id, and drop the exemption.
  ...
  { id: "cpn-viator-manatee-walk-bradenton", business: "Manatee Watching Walking Tour", area: "Bradenton",
    ...
    code: null, url: commerceHref({ provider: "viator", offerId: "5560271P1", surface: "coupons", contentId: "cpn-viator-manatee-walk-bradenton" }), expires: "2026-08-10",
