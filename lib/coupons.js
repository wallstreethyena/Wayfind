...
import { CLIPP_MARKETS, CLIPP_MERCHANT_OFFERS, CLIPP_INDEX_REVERIFIED } from "./clippOffers.js";
...
// 2026-08-11 audit: the tripwire fired (guard check-guide-deal-cards went red
// when this date lapsed — working as designed). Re-checked the same day:
// clipp.com/states/fl/cities/sarasota answers HTTP 200 with the live deals
// page shell; tampa 403s a non-browser fetcher, which is Clipp's documented
// bot-blocking, not an outage. Owner: give the four city pages a 60-second
// browser look before 08-24 — the card auto-hides again then if nobody does.
// RENEWED 2026-08-23, a day before it would have lapsed. All four city pages
// were opened in a real browser: tampa / sarasota / bradenton / orlando each
// answered HTTP 200 with live certificate inventory on the page. Renewing this
// one also keeps the Tampa summer guide's dealCards resolvable — that guide
// references cpn-clipp-fl-tampa, and check-guide-deal-cards would have failed
// every build from Aug 25 the way the Adventure Island card just did (#911).
// 2026-09-01 audit: all four first-party city pages (Sarasota, Bradenton,
// Tampa, Orlando) remain indexed with live local-deals inventory. Keep this a
// short fuse: the next release audit must re-check the market pages instead of
// turning a rotating catalogue into a permanent promise.
// 2026-09-15: the fuse fired on schedule (main went red on check-guide-deal-
// cards the morning after 09-14 — working as designed, and it also silently
// re-pointed the Sarasota half-price guide's primary CTA at an untracked
// geckosgrill.com URL, which is the second reason the guard exists). Re-audited
// the same morning in a real browser, all four city pages: each renders ITS
// OWN city-tagged certificate inventory with prices, not the geo-defaulted
// feed — Sarasota (Five-O Donut, Clean Eatz, Armageddon Laser Tag…), Tampa
// (Brown Bag Coffee, Barrio, Tico's Bakery…), Bradenton (Orange Blossom
// Coffee, Peach Cobbler Factory, El Warike…), Orlando (Vicky Bakery, Three
// Point Bowling, Blue Coast Burrito…). Next-but-one Monday, same convention.
const CLIPP_AUDIT_EXPIRY = "2026-09-28";
...
// safe direction.
// RENEWED 2026-08-23. All 48 certificate pages were opened in a REAL browser
// (Clipp is behind Akamai — bare fetchers get 403, which is why this renewal has
// always been browser-gated and why the constant sat lapsed from Aug 18):
// 40 rendered live with Buy Now / Add to Cart, 8 came back SOLD OUT and were
// deleted from lib/clippOffers.js rather than carried. Three of the eight
// (Guac Shop, Voodoo Valrico, QDOBA Riverview) were the registry's own
// prune-first candidates; the other five — Bavaro's, Irish Pub on 4th,
// Bubbakoo's, El Jalisco Englewood, Klaw Komotion — are new this run.
// RENEWED 2026-09-15 (lapsed since 08-31; every merchant card had been auto-
// hidden for two weeks — safe, and revenue on the floor). All 53 certificate
// pages opened in a real browser: 47 live with Add to Cart + Buy Now, 6 SOLD
// OUT and deleted from lib/clippOffers.js (see the ledger note there). Next-
// but-one Monday, same convention as CLIPP_AUDIT_EXPIRY.
const CLIPP_MERCHANT_AUDIT_EXPIRY = "2026-09-28";
const CLIPP_INDEX_REVERIFIED_IDS = new Set(CLIPP_INDEX_REVERIFIED.offerIds);
// A location-verified Clipp row stores the venue's own Google photo as
// `places/<PLACE_ID>/photos/<PHOTO_ID>`. That leading segment IS the venue's
// Google Place ID — the same identifier a place card carries — so the row
// already contains an exact, unambiguous hook to the venue. It was just never
// read. Returns null for anything that is not that shape, so a hand-written or
// malformed ref can never produce a bogus key.
export function placeIdFromPhotoRef(ref) {
  const m = /^places\/([^/]+)\/photos\//.exec(String(ref == null ? "" : ref));
...
const CLIPP_MERCHANT_COUPONS = CLIPP_MERCHANT_OFFERS.map((o) => ({
  ...
  area: o.area,
  title: o.title,
  details: `Buy on Clipp, show it at ${o.merchant}. Rotates weekly — can sell out.`,
  code: null,
  url: commerceHref({ provider: "clipp", offerId: o.offerId, surface: "coupons", contentId: o.offerId }),
  expires: CLIPP_INDEX_REVERIFIED_IDS.has(o.offerId) ? CLIPP_INDEX_REVERIFIED.expires : CLIPP_MERCHANT_AUDIT_EXPIRY,
  cta: "Get certificate",
  badge: o.badge,
  ...
  icon: o.icon || null,
  // v8.41 — a row may ship the PLACE ID on its own instead of a photo ref, and
  // for new rows that is the better trade twice over. The ref is ~600 characters
  // that every visitor downloads (13 of them pushed the route past its 500KB gz
  // budget, which is a ratchet and not something to raise); and CouponCard's own
  // note says a stored ref EXPIRES while the place id does not — its thumbnail
  // already falls back to /api/photo?place=<id>, which fetches the venue's
  // CURRENT first photo. So a placeId-only row is smaller and stays fresh.
  venuePhotoRef: o.photoRef || null,
  // v7.41 — THE PLACE ID, LIFTED OUT OF THE PHOTO REFERENCE. See the long note
  // above couponForPlace() for why this is the only honest hook for these rows.
  // v8.41: an explicit `placeId` on the row wins; otherwise it is lifted out of
  // the photo ref exactly as before. Both are the same identifier — the ref's
  // leading segment IS the place id — so the two paths cannot disagree.
  placeId: o.placeId || placeIdFromPhotoRef(o.photoRef),
...
export const COUPONS = [
  ...
  // ── BARRACUDA "BOTTLES & BEATS" (2026-08-25) — the Coconut Grove partner
  // sheet's first merchant deal, harvested VERBATIM from @barracudagrove's own
  // Instagram announcement the day it posted (instagram.com/p/DceojNAoZN7/).
  // Every claim is the merchant's wording: 20% off all craft beer bottles &
  // cans, 99-cent wings and live music, Wednesdays from 7pm, minimum 6 wings,
  // in-house only. Expiry follows the weekly-audit Monday convention — the
  // card auto-hides unless the deals audit re-verifies the promo is still on.
  {
    id: "cpn-barracuda-bottles-beats",
    business: "Barracuda",
    area: "Miami",
    title: "Bottles & Beats — Wednesdays from 7pm",
    details: "Every Wednesday night at the Cuda: 20% off all craft beer bottles & cans, 99\u00a2 wings and live music from 7pm (minimum 6 wings, dine-in only). Happy hour runs 11:30am\u20137pm weekdays.",
    code: null,
    url: "https://www.instagram.com/p/DceojNAoZN7/",
    expires: "2026-09-07",
    verifiedOn: "2026-08-25",
    cta: "See the announcement",
    badge: "Wednesday deal",
    intents: ["nightout", "friends"],
    match: ["Barracuda Bar", "Barracuda Bar & Grill", "Barracuda Taphouse & Grill"],
  },
...
// v8.19 — venuePlaceId (owner: "the coupons were fetching images — not sure
// what happened"). Intent-tag artwork was removed for honesty (a baseball
// promo wore a restaurant photo); the VENUE'S OWN photo is the honest image,
// and the placeId is the stable key for it (photo refs expire; placeIds
// don't). dealArtwork() renders /api/photo?place=<venuePlaceId> when no
// committed image exists. Resolved 2026-08-19 against wf_inventory by name,
// each verified to carry a live photo. Gecko's chain rows use the location
// nearest the deal's own area.
  { id: "cpn-geckos-19th-hole", venuePlaceId: "ChIJH9Womvo8w4gR3b99nAsCHXY", business: "Gecko's Grill & Pub — all locations (incl. SR70 Lakewood Ranch, SR64 Braden River)", area: "Sarasota-Manatee",
  ...
  { id: "cpn-geckos-happy-hour", venuePlaceId: "ChIJH9Womvo8w4gR3b99nAsCHXY", business: "Gecko's Grill & Pub — all locations", area: "Sarasota-Manatee",
  ...
  { id: "cpn-geckos-bar-bingo-hillview", venuePlaceId: "ChIJRcH7zGBHw4gRek5qQU0_8wg", business: "Gecko's Grill & Pub, Hillview (Sarasota)", area: "Sarasota",
  ...
  { id: "cpn-pie-on-main-lunch", business: "Pie On Main, 1507 Main St, Sarasota", area: "Sarasota",
    ...
    // v8.41 — the exact venue, read off Google on 2026-08-23: "Pie on Main",
    // 1507 Main St, Sarasota — the same street address this row's own business
    // string already carried, so there is nothing to guess. One Pie on Main
    // exists; the name variant stays as the belt.
    venuePlaceId: "ChIJlYrpCEpBw4gR9Gqna7VKtgY",
    intents: ["eatnow"], match: ["Pie On Main", "Pie on Main"] },

  { id: "cpn-ringling-museums-for-all", venuePlaceId: "ChIJ6y2dx9g_w4gRWoZIRhLk-JI", business: "The Ringling", area: "Sarasota",
  ...
  { id: "cpn-zootampa-heroes-summer", venuePlaceId: "ChIJBQ5SjLHGwogRL4X19g4J5tI", business: "ZooTampa at Lowry Park", area: "Tampa",
  ...
  { id: "cpn-ringling-free-mondays", venuePlaceId: "ChIJ6y2dx9g_w4gRWoZIRhLk-JI", business: "The Ringling", area: "Sarasota",
  ...
  { id: "cpn-ringling-bluestar-military", venuePlaceId: "ChIJ6y2dx9g_w4gRWoZIRhLk-JI", business: "The Ringling", area: "Sarasota",
  ...
  { id: "cpn-mote-military-free", venuePlaceId: "ChIJRyOEfAo5w4gR664aD_YYBLU", business: "Mote Marine Laboratory & Aquarium", area: "Sarasota",
    ...
    // v8.41: "Mote Science Education Aquarium" is what GOOGLE calls this venue
    // today — read off the Places record for the very id on this row, 2026-08-23.
    // Every variant here was a guess at the name; none of them was the live one,
    // which is how this pill went dark. The venuePlaceId above is now the real
    // hook (see _CPN_BY_PLACE_ID); this line is the belt for a card that arrives
    // with a name and no id.
    intents: ["familyfun", "cozyindoor"], match: ["Mote Marine Laboratory & Aquarium", "Mote Marine Laboratory", "Mote SEA", "Mote Marine Aquarium", "Mote Science Education Aquarium"] },

  { id: "cpn-marauders-thirsty-thursday", venuePlaceId: "ChIJW183nLIXw4gRcuHFpjCkCGc", business: "Bradenton Marauders", area: "Bradenton",
  ...
  { id: "cpn-agave-bandido-taco-tuesday", venuePlaceId: "ChIJ7QsLgxQ5w4gRSLtMON-sOaI", business: "Agave Bandido", area: "Lakewood Ranch",
  ...
  { id: "cpn-sarasota-art-museum-second-sundays", venuePlaceId: "ChIJx1DBNl1Aw4gR6Zi9XjSh7r4", business: "Sarasota Art Museum", area: "Sarasota",
...
// ── ATTACHING A COUPON TO A PLACE CARD ──────────────────────────────────────
//
// THE BUG THIS FIXES (owner, 2026-08-12: "make sure these coupons are placed on
// the place cards that match the coupon"). The pill was wired into the cards
// months ago and rendered for almost nothing, because the ONLY hook was the
// business name and Clipp's merchant names are Clipp's LISTING names:
//
//     "Marco's Pizza - Fletcher Ave"      Google returns  "Marco's Pizza"
//     "QDOBA - Gandy"                     Google returns  "QDOBA Mexican Eats"
//     "Toastique - E Cumberland"          Google returns  "Toastique"
//
// Measured on the live list: 15 of 67 live coupons carry a " - <location>"
// suffix, none of them had a `match` array, and _cpnNorm strips punctuation but
// not the suffix — so those keys could never be hit. The pill was dead for every
// one of them.
//
// AND THE OBVIOUS FIX IS THE DANGEROUS ONE. Stripping the suffix collapses
// FOUR Marco's Pizza rows (Fletcher Ave, Riverview, Lithia, Bradenton) and TWO
// QDOBA rows onto one key, and the index keeps whichever it saw first — so a
// reader in Bradenton would be shown, and could buy, the Tampa certificate.
// That is wrong-city monetized inventory, which lib/dealSheet.js already treats
// as worse than showing nothing at all.
//
// So the hook is the GOOGLE PLACE ID, which the row already stores inside its
// photoRef and which is exact by construction: 47 of the 48 merchant rows carry
// one, all 47 distinct. A place card whose id is not in that set gets no pill —
// fail closed, same direction as the geo-gate.
// v8.41 — `venuePlaceId` COUNTS AS IDENTITY, and until now it did not.
//
// The static cards learned their exact Google Place ID in v8.19, but for a
// different job: dealArtwork renders /api/photo?place=<venuePlaceId> so a card
// has a picture. This index only ever read `placeId`, the field the Clipp
// merchant rows use. So eleven live coupons — three Gecko's, three Ringling,
// ZooTampa, Mote, the Marauders, Agave Bandido, Sarasota Art Museum — knew
// precisely which venue they belonged to and were still resolved by NAME.
//
// Two things that costs, both measured against the real Google names on
// 2026-08-23:
//   · Mote's pill was DEAD. Google renamed the venue "Mote Science Education
//     Aquarium"; the coupon says "Mote Marine Laboratory & Aquarium", so the
//     name index missed and the card showed no offer at all.
//   · The three Gecko's coupons all key on one Google name, "Gecko's Grill &
//     Pub", and a name map keeps whichever it saw first — so the Hillview
//     bar-bingo certificate could surface on the all-locations card and vice
//     versa. That is the wrong-branch failure check-coupon-place-match exists
//     to stop, arriving through the door it did not cover.
//
// `placeId` still wins where a row has both: it is the Clipp-harvest field and
// is lifted from the row's own photoRef, so it cannot disagree with the venue.
const _CPN_BY_PLACE_ID = (() => {
  const m = new Map();
  for (const c of COUPONS) {
    const k = c && (c.placeId || c.venuePlaceId) ? String(c.placeId || c.venuePlaceId) : null;
    if (k && !m.has(k)) m.set(k, c);
  }
  return m;
})();

// Place cards carry the id as `place_id` (Google rows) or `id` (everything that
// has already been normalised); both hold the same value. A resource-style
// "places/<ID>" is accepted and reduced, so a caller passing the raw Places API
// name still resolves.
const _cpnPlaceKey = (v) => {
  const s = String(v == null ? "" : v).trim();
  if (!s) return "";
  const m = /^places\/([^/]+)$/.exec(s);
  return m ? m[1] : s;
};

/**
 * The one resolver every card surface should call. Identity first, name second.
 * Returns null when it cannot prove the coupon belongs to THIS venue.
 */
export function couponForPlace(place, todayIso) {
  if (!place) return null;
  for (const raw of [place.place_id, place.id]) {
    const k = _cpnPlaceKey(raw);
    if (!k) continue;
    const c = _CPN_BY_PLACE_ID.get(k);
    if (c) return couponIsLive(c, todayIso) ? c : null;
  }
  return couponForPlaceName(place.name, todayIso);
}

// Kept for callers that genuinely have nothing but a name. It matches only the
// hand-written `business`/`match` strings, which are written to be what Google
// actually returns — it never sees the suffixed Clipp listing names, so it
// cannot produce the wrong-location match described above.
