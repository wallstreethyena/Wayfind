...
// ── GEO GATE (v6.72) ────────────────────────────────────────────────────────
// couponsForIntent filters by INTENT only. That was survivable while the strip
// lived on one in-app sheet in a Sarasota-Manatee-only product; it is not
// survivable now that the strip renders on nine standalone intent pages that
// accept any lat/lng. Live-verified 2026-07-31: /tonight and /date-night for
// ORLANDO rendered "Bradenton Marauders", "Gecko's Grill — Lakewood Ranch" and
// "Clipp — dining certificates in Sarasota".
//
// That is the owner's own example of an unacceptable recommendation, verbatim:
// "a Sarasota deal shown in Orlando". A deal 130 miles away is not a weaker
// offer, it is a wrong one, and it is worse than no strip at all because the
// strip is the surface that carries the money.
//
// NATIONWIDE deals are the exception and are genuinely valid anywhere — Clipp
// and Klook are national programs, and their `area` says so.
const NATIONWIDE_AREAS = new Set(["United States", "Nationwide", "US", "USA"]);

// A coupon's `area` is editorial free text, so it is mapped explicitly rather
// than string-matched against market keys. An UNMAPPED area is treated as
// LOCAL-BUT-UNKNOWN and shown only when we cannot resolve the user's market
// either — never promoted to nationwide, because guessing wrong here is the bug.
const AREA_MARKET = {
  "Bradenton": "sarasota-manatee",
  "Lakewood Ranch": "sarasota-manatee",
  "Sarasota": "sarasota-manatee",
  "Sarasota-Manatee": "sarasota-manatee",
  "Tampa": "tampa-bay",
};
// Markets that share one commercial coupon pool. Sarasota and Bradenton are one
// trade area; Tampa/St Pete/Clearwater are another.
const MARKET_GROUP = {
  sarasota: "sarasota-manatee", bradenton: "sarasota-manatee",
  tampa: "tampa-bay", stpete: "tampa-bay", clearwater: "tampa-bay",
};

export function couponAreaGroup(c) {
  const a = (c && c.area) || "";
  if (NATIONWIDE_AREAS.has(a)) return "*";
  return AREA_MARKET[a] || null;
}

/**
 * Intent-filtered AND geo-filtered live coupons.
 *
 * `market` is a MARKETS key (lib/destinations.marketForLocation). When it is
 * null — no location, or a location outside every known market — only
 * nationwide deals are returned. Absent beats wrong.
 */
export function couponsForIntentIn(intentId, market, todayIso) {
  const group = market ? (MARKET_GROUP[market] || null) : null;
  return couponsForIntent(intentId, todayIso).filter((c) => {
    const g = couponAreaGroup(c);
    if (g === "*") return true;      // nationwide: valid everywhere
    if (!g) return false;            // unmapped local area: never shown out of caution
    return g === group;              // local: same trade area only
  });
}
