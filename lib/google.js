...
import { setLocal, capMapToBudget, QCACHE_BUDGET_CHARS } from "./localStore";
import { hasPlacePhotoRef } from "./placePhoto.js";
import { knownCityGeocode } from "./knownCityGeocode.js";
import { placeDisplayName } from "./placeDisplayName.js";
...
// v8.48 — NAMESPACE BUMPED, DELIBERATELY. Free mode spent hours serving lean
// Pro-mask rows (no rating/userRatingCount). normalize() keeps such a row — it
// only needs a location — so `cached()` saw a NON-EMPTY list and wrote it to
// localStorage under an 8-DAY TTL. Every reader who browsed during the outage
// now holds a cache full of rows the card gate refuses, and `cached()` serves
// them without touching the network: the server fix is live and correct, and
// the browser never asks. Measured on the owner's own session — Food > Cafés
// rendered "Nothing here right now" while /api/places/search returned 18 good
// places to the same page. A new key orphans every poisoned entry on the next
// load, at zero cost and zero Google spend.
const QCACHE_KEY = "wfq_v2";
const QCACHE_KEY_LEGACY = "wfq_v1";
...
function qwrite(key, v) {
  try {
    ...
    // BYTES, NOT ENTRIES (v7.08). QCACHE_MAX is a bound on the wrong unit. At
    // the size these entries actually reach — measured on the owner's phone,
    // 52 entries weighing 4,189,930 characters, ~80KB each — a cap of 80
    // entries is a cap of 6.4MB, larger than the whole 5MB localStorage quota.
    // The entry cap can therefore never be what stops this cache; the quota is,
    // and when the quota stops it, it stops EVERY OTHER WRITE IN THE APP too:
    // favorites, likes, the chosen location, the collapsed rails. This key was
    // holding 80% of the reader's storage budget. See lib/localStore.js.
    setLocal(QCACHE_KEY, capMapToBudget(all, QCACHE_BUDGET_CHARS));
  } catch (e) {}
}
// The cache is now SELF-DEFENDING. A row with no rating signal can never render
// (lib/score.js cardComplete), so caching one only teaches the browser to skip
// the network and show nothing. Applied on READ as well as WRITE, because the
// entries that caused this incident are already on readers' devices: a hit that
// sanitises to empty is treated as a MISS and refetched, so a poisoned entry
// heals itself the first time it is touched. Non-arrays pass through untouched
// (findPlace returns a single place).
function serveable(v) {
  return Array.isArray(v) ? v.filter(hasScoreSignal) : v;
}
function cached(key, fn) {
  const hit = serveable(qread(key));
  // Array.isArray check first: a sanitised-to-empty list must fall through to a
  // real fetch, never be served as "we looked and there is nothing here".
  if (hit && (!Array.isArray(hit) || hit.length)) return Promise.resolve(hit);
  if (_inflight.has(key)) return _inflight.get(key);
  const pr = fn().then((v) => { _inflight.delete(key); const out = serveable(v); const keep = Array.isArray(out) ? out.length > 0 : !!out; if (keep) qwrite(key, out); return out; }).catch((e) => { _inflight.delete(key); throw e; });
  _inflight.set(key, pr);
  return pr;
}

// One-shot eviction of the pre-v8.48 namespace. It is dead weight the moment
// the key above changes, and the v7.08 note on qwrite records that this cache
// had grown to ~80% of the reader's 5MB localStorage budget — leaving it there
// would keep starving favourites, likes and the chosen location of quota.
try {
  if (typeof window !== "undefined" && localStorage.getItem(QCACHE_KEY_LEGACY)) localStorage.removeItem(QCACHE_KEY_LEGACY);
} catch (e) {}
...
export const SUBFILTERS = {
  ...
  attractions: [
    ...
    { id: "themeparks", label: "Theme Parks", query: "theme parks and amusement parks" },
...
export async function geocodeCity(query) {
  // City submission must not turn a server-side denial into a paid browser
  // request. Owned markets resolve locally; unknown input can still resolve
  // through the guarded autocomplete/details flow before this fallback runs.
  return knownCityGeocode(query);
}
...
async function _reverseGeocodeUncached(lat, lng) {
  // Reverse geocoding is paid. The server route owns its shared cache and
  // finite ledger; the browser must never bypass a 503 budget/gate denial by
  // loading the Maps SDK and making the same paid request directly. A missing
  // label is fail-soft for the UI, while spend remains fail-closed.
  try {
    const r = await fetch("/api/geocode?lat=" + Number(lat).toFixed(5) + "&lng=" + Number(lng).toFixed(5));
    if (!r.ok) return "";
    const d = await r.json();
    return d && typeof d.name === "string" && d.name ? d.name : "";
  } catch (e) {
    return "";
  }
}
...
import { hasScoreSignal } from "./score.js";
import { lawfulSort } from "./lawfulOrder.js";
export { wayfindScore };
...
// A proxy miss is final. Retrying with the browser key bypasses the shared
// ledger and was the client-reachable cost path this module must prevent.
async function proxySearch(q, center, radius, n, cat, sub) {
  if (typeof window === "undefined" || !center || !q) return [];
  try {
...
function restToPlace(p) {
  return {
    ...
    photos: (p.photos || []).map((ph) => ({
      ...
      getURI: (opt) => ph._directUri || photoProxyURL(ph.name, (opt && opt.maxWidth) || 640),
    })),
  };
}

// The free owned-library endpoint uses the same REST place shape as the
// existing search proxy. Share its display normalization without a lookup.
export function normalizeSearchPlace(place) {
  if (!place || !place.id || !Number.isFinite(place.location?.latitude) || !Number.isFinite(place.location?.longitude)) return null;
  const normalized = normalize(restToPlace(place));
  return { ...normalized, _searchDistanceMi: Number.isFinite(place.distanceMeters) ? place.distanceMeters / 1609.344 : null };
}

// Build the same-origin cached-proxy URL for a Google photo resource name.
export function photoProxyURL(name, w) {
  if (!name) return null;
  return "/api/photo?ref=" + encodeURIComponent(name) + "&g=2&w=" + (w || 640);
}
...
function normalize(p) {
  ...
  return {
    id: p.id,
    name: placeDisplayName(p, "Unnamed"),
...
export async function fetchPlaceById(id) {
  // Shared /p/{id} links used to begin with the browser Maps SDK. If that
  // script was slow, blocked, or returned no row, home.js had no detail to
  // open and the reader saw the freshly-mounted homepage instead. Use the
  // guarded server route first; it can also fall back to Wayfind inventory.
  try {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 6000);
    let response;
    try {
      response = await fetch("/api/places/details", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ placeId: id, kind: "place" }),
    ...
    if (response.ok) {
      ...
      const normalized = payload && payload.place ? normalize(restToPlace(payload.place)) : null;
...
// `cat` (optional, 2026-09-15): the wf_inventory category this search stands in
// for ("attractions", "food", "nightlife", "shopping", "hotels"). It is NOT part
// of the Google request — the server only reads it when it cannot buy a fresh
// answer (gate shut / free budget spent / 429) and then serves the OWNED library
// for that category instead of an empty list. Without it the curated tiles
// ("Best things to do today", "Top 10 Food", …) went blank the morning the
// September text_pro ledger hit 4800/4800, while the same library had 40
// attractions within 25 miles of Parrish. Callers that pass nothing are unchanged.
export function searchNearbyPlaces(query, center, radiusMiles = DEFAULT_RADIUS_MI, cat) {
  if (!query || !center) return Promise.resolve([]);
  return cached(qkey("near", query, center, radiusMiles + (cat ? "|" + cat : "")), () => _searchNearbyPlaces(query, center, radiusMiles, cat));
}
async function _searchNearbyPlaces(query, center, radiusMiles = DEFAULT_RADIUS_MI, cat) {
  ...
  try {
    ...
    const places = await proxySearch(query, center, radiusMeters, 10, cat || undefined);
...
async function _findPlace(query, center) {
  ...
  try {
    const places = await proxySearch(query, center || null, 16000, 1);
...
async function _searchPlaces(categoryId, subId, center, radiusMeters = DEFAULT_RADIUS_M, vibeId = "all", keyword = "") {
  ...
  const places = await proxySearch(textQuery, center, radiusMeters, 20, categoryId, subId);
  let list = (places || []).map(normalize).filter(Boolean);
  ...
  // v6.63 — THE POOL IS ORDERED BY THE NUMBER ON THE CARD.
  //
  // This block used to read, in its own words: "wfScore (displayed on cards)
  // reflects pure quality and doesn't change. _sortScore adds a proximity
  // bonus…" — i.e. it stated outright that the order and the printed number
  // were two different things. On a 0–100 scale it charged 1.3 points per mile
  // past 4, capped at 30, which is more than four times the entire creator-video
  // bonus and fifteen times the law's distance term. A 9.2 at twelve miles lost
  // 10.4 points of rank to a 9.0 at three and rendered below it. Together with
  // the v4.24 "near-first" reshuffle underneath (which pushed anything past 20
  // miles below everything closer, regardless of score) this was the single
  // largest source of shown!=sorted in the app, because this list is the DEFAULT
  // order of the browse feed and of every sheet that does not re-sort.
  //
  // lib/wayfindScore.js already prices distance, visibly, at a flat −0.2 past 17
  // miles, and that term is inside the chip. Distance is therefore already
  // counted once; counting it again here — invisibly, at 15× the weight — is the
  // defect, not a tuning choice. Proximity survives as the tie-breaker between
  // rows showing the same number, which is where it belongs: given two 9.2s, the
  // near one first.
  list = lawfulSort(list, (p) => -(Number(p.distMi) || 0));
  ...
  try {
    ...
    try {
      response = await fetch("/api/places/details", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ placeId, kind: "detail" }),
        signal: controller.signal,
      });
    } finally {
      clearTimeout(timer);
    }
    if (!response.ok) throw new Error("details proxy " + response.status);
    const payload = await response.json();
    if (!payload || !payload.place) throw new Error("details unavailable");
    ...
    // Heal a photoless list row. Only accept a real Google photo resource
    // name — never invent a photo_ref. Empty Places photos stay empty.
    let photos = [];
    let photo = null;
    let photoRef = null;
    try {
      if (place.photos && place.photos.length) {
        const uri = (ph, w) => (ph && ph.name ? photoProxyURL(ph.name, w) : null);
        photos = place.photos.slice(0, 6).map((ph) => uri(ph, 640)).filter(Boolean);
        photo = photos[0] || null;
        const firstName = place.photos[0] && place.photos[0].name;
        if (hasPlacePhotoRef(firstName)) photoRef = String(firstName);
      }
    } catch {}
    return { ok: true, editorial, reviews, hours, phone, website, photos, photo, photoRef };
  } catch (e) {
    ...
    return { ok: false, error: { name: (e && e.name) || "Error", message: (e && e.message) || "" }, editorial: null, reviews: [], hours: null, phone: null, website: null, photos: [], photo: null, photoRef: null, _resolved: true };
  }
}
