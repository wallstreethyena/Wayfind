...
import { geocodeKnownCityFirst } from "./knownCityGeocode.js";
...
export async function geocodeCity(query) {
  return geocodeKnownCityFirst(query, async () => {
...
async function _reverseGeocodeUncached(lat, lng) {
  // v6.99 (P1 speed): try the server proxy FIRST — /api/geocode answers from a
  // SHARED 30-day cell cache with the server key, so a first-time visitor's
  // homepage never downloads the Maps JS bootstrap just to name their city.
  // Any failure (route absent, key missing, 4xx/5xx) falls through to the SDK
  // path below unchanged — fail-soft in the only safe direction.
  try {
...
export async function fetchPlaceById(id) {
  ...
  // The SDK remains a local-development fallback when no server key exists.
  try {
    ...
    // A configured production proxy failure is final: retrying the same paid
    // provider from the browser doubles work and recreates the blank-route
    // race. Local development still reaches the SDK path via the 501 above.
    return null;
  }
  try {
...
async function _searchNearbyPlaces(query, center, radiusMiles = DEFAULT_RADIUS_MI) {
  ...
  try {
    // v6.99 (P1 speed): SDK import moved into the fallback branch below.
    ...
    if (!places) {
      const { Place } = await getLoader().importLibrary("places"); // SDK fallback only
      try {
...
async function _searchPlaces(categoryId, subId, center, radiusMeters = DEFAULT_RADIUS_M, vibeId = "all", keyword = "") {
  ...
  // v6.99 (P1 speed): the Maps JS SDK loads ONLY if the server proxy fails.
  // This line used to run before proxySearch, so every homepage boot paid the
  // full maps.googleapis.com bootstrap even when the proxy answered everything.
  const baseFields = [
    "id", "displayName", "location", "rating", "userRatingCount",
  ...
  let places = await proxySearch(textQuery, center, radiusMeters, 20, categoryId, subId);
  if (!places) {
    const { Place } = await getLoader().importLibrary("places"); // SDK fallback only
    try {
...
export async function fetchPlaceDetail(placeId) {
  try {
    ...
    await place.fetchFields({
      ...
      fields: ["editorialSummary", "reviews", "regularOpeningHours", "nationalPhoneNumber", "websiteURI", "photos"],
    });
