  ...
  for (const generation of chain) {
    ...
    try {
      ...
      if (!data || !Array.isArray(data.results)) {
        attempts.push({ generation, status: r.status, reason: "malformed" });
        continue;
      }
      attempts.push({ generation, status: r.status, reason: "ok", results: data.results.length });
      return { ok: true, status: r.status, generation, data, results: data.results, attempts, reason: "ok", empty: data.results.length === 0 };
    } catch (e) {
