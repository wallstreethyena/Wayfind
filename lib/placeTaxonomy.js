...
// `adjudicatedSection` is OPTIONAL and is the last net only — see the
// adjudication branch in lib/placeCategory.js classify(). Omitting it (every
// existing caller) is byte-for-byte the previous behaviour.
export function classifyPlace(types, primaryType, name, adjudicatedSection) {
  const { category, tags, via } = classify({ types, primaryType, name, adjudicatedSection });
