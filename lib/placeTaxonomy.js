...
// type-poor places. Returns { category, tags }. category is null when nothing
// classifies the place (the seeder then relies on the anchor list, or skips it).
export function classifyPlace(types, primaryType, name) {
  const set = asSet(types);
  let category = categoryOfType(primaryType);          // strongest single signal
  if (!category) category = categoryFromTypes(set);    // full-type precedence
  if (!category) category = categoryFromName(name);    // type-poor safety net
  if (!category) return { category: null, tags: [] };
  return { category, tags: tagsFor(category, set, name) };
}
