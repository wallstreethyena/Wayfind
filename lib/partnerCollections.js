...
const photoUrl = (ref) => (ref ? "/api/photo?ref=" + encodeURIComponent(ref) + "&g=2&w=640" : null);
...
export const PARTNER_COLLECTIONS = [
  {
    ...
    // The one-line promise the sheet opens with — answers the owner's ask
    // ("tell the user where to go, where to stay") before the first card.
    guide: "A local's guide to the Grove — where to eat, drink, train and stay, each pick ranked by Wayfind.",
    // GROVE 33 (2026-08-25) — the partner's own program, researched from
    // coconutgrovena.com same day (facts: 10%+ card, individual from $99/yr,
    // business $199/yr, events on Luma — members free +1, $40 door otherwise,
    // "est. 1873" from the old post office). NO named merchants are claimed —
    // CGNA publishes no public list yet, and we never assert a specific
    // business participates. The joinUrl carries Wayfind utm so every referred
    // member is attributable — that measured referral stream IS the revenue
    // conversation with the CGNA (same evidence pattern as the Rio report).
    program: {
      name: "Grove 33",
      tag: "Partner program",
      line: "The neighborhood's own card: at least 10% off at participating Grove spots — eat & drink, shop, wellness, services — every visit, all year.",
      detail: "Membership from $99/yr: the digital card arrives the same day, and CGNA gatherings are free for you +1 (non-members: $40 at the door).",
      joinUrl: "https://coconutgrovena.com/membership?utm_source=wayfind&utm_medium=partner_sheet&utm_campaign=grove33",
...
export function hydratePartnerCollection(collection, places, center) {
  ...
  const list = (places || []).map((p) => ({
    ...
    // The CGNA's curated editorial (lib/partnerCollectionsData.js `ed`): role
    // (where-to-go/where-to-stay), why it was picked, what not to miss, a tip,
    // a good-for tag. HookDetail's partnerSplash path renders this instead of
    // the generic auto-blurb. `price` feeds the existing card price slot.
    editorial: p.ed || null,
    price: p.ed && p.ed.price ? p.ed.price : undefined,
  }));
  return {
    ...
    guide: collection.guide,          // one-line "where to go / where to stay" intro
    program: collection.program || null, // Grove 33 module (tracked partner CTA)
    places: list,
  };
}
