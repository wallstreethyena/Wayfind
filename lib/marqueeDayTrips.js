...
import { DISNEY_WORLD_PLACE_ID } from "./placeDisplayName.js";
...
// were Disney Springs, Epcot, Animal Kingdom, Disney World, Hollywood Studios,
...
export const MARQUEE_DAY_TRIPS = Object.freeze([
  { key: "disney_world", operator: "disney", ownerNamed: true, tier: 1, name: "Disney World", town: "Lake Buena Vista", lat: 28.3771857, lng: -81.57074, placeId: DISNEY_WORLD_PLACE_ID, proof: /^(?:walt )?disney world(?:® resort| resort)?$/i, types: ["amusement_park", "theme_park", "tourist_attraction"] },
...
export function verifiedMarqueeRow(anchor, places) {
  for (const p of Array.isArray(places) ? places : []) {
    ...
    if (anchor.placeId && String(row.id) !== String(anchor.placeId)) continue;
