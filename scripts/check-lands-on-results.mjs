...
ok(/import\s*\{\s*landOnResults\s*\}\s*from\s*"\.\.\/lib\/lazyLandOnResults"/.test(homeCode),
   "app/home.js must import the shared lazy landing entry point");
...
ok(/import\s*\{\s*landOnResults\s*\}\s*from\s*"\.\.\/\.\.\/lib\/lazyLandOnResults\.js"/.test(railCode),
   "app/components/DaypartRail.js must use the shared lazy landing entry point");
