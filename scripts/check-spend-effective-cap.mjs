...
ok(await withGate("open", () => effectiveCap("details_pro", 7500)) === 7500, "explicit open mode: month_cap is the ceiling, exactly");
ok(await withGate(null, () => effectiveCap("details_pro", 7500)) === null, "unset gate fails closed");
ok(await withGate("shut", () => effectiveCap("details_pro", 7500)) === null, "shut mode: no ceiling — the call cannot happen");
ok(await withGate("open", () => effectiveCap("details_pro", 0)) === null && await withGate("open", () => effectiveCap("details_pro", "x")) === null, "a missing/invalid cap is fail-closed (null), never unlimited");
