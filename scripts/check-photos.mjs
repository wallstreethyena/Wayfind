...
if (!route.includes("GOOGLE_MAPS_SERVER_KEY") || !/authorizeSpend:\s*\(\)\s*=>\s*getRecovery\(\)\.then/.test(route) || !/getRecovery\(\)\.then\([^\n]*spendAllow\("photos"\)/.test(route)) fail("same-place recovery must run at the lazy photo spend boundary before Google authorization");
