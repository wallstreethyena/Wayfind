#!/usr/bin/env node
/**
 * test-owner-places-2026-08-29c — lock Gabe's 2026-08-29c Florida pin batch.
 *
 ...
 * nothing (Chicken Guy location unverified, Coming Soon, mobile, Bern).
...
const ingest = readFileSync(join(ROOT, "scripts/ingest-verified-2026-08-29c.mjs"), "utf8");
const notes = readFileSync(join(ROOT, "data/atlas/HOLD-2026-08-29c.md"), "utf8");
...
ok(HOLD_GOOGLE.every((p) => /SoFresh Downtown Sarasota|Mobius Sarasota|Icy-N-Spicy|Belladukes Gourmet Market|La Birra Bar Coral Gables|Canelita Cheesecake Coral Gables|Canelita Cheesecake Miami|Divilma Clermont|The Luce, St\. Pete Beach/.test(p.name)),
  "the nine hold_google_id names are the intended ADD list");
ok(!HOLD_GOOGLE.some((p) => /Chicken Guy/i.test(p.name)),
  "Chicken Guy is a location HOLD, not an intended ADD pin");
ok(!HOLD_GOOGLE.some((p) => /Lakewood Ranch/i.test(p.name)),
  "this batch pins Downtown Sarasota SoFresh, not Lakewood Ranch");
ok(HOLDS.some((h) => /Chicken Guy/i.test(h.name)) &&
   ...
   HOLDS.some((h) => /SoFresh Bradenton/i.test(h.name)) &&
...
const sofresh = HOLD_GOOGLE.find((p) => /SoFresh Downtown Sarasota/i.test(p.name));
ok(sofresh && sofresh.lat === 27.338273 && sofresh.lng === -82.541473 && sofresh.coordSource === "official-maps-embed",
  "SoFresh Downtown uses the official Maps-embed ll= — not a Places geocode");
ok(sofresh && sofresh.officialMapsCid === "4880743690588062748",
  "SoFresh Downtown records the official embed CID and does not promote it to a ChIJ");
ok(sofresh && /1455 Second/i.test(sofresh.address) && /Sarasota/i.test(sofresh.address),
  "SoFresh Downtown is 1455 Second St Sarasota — not Lakewood Ranch, not Bradenton");
ok(sofresh && /vegan/i.test(sofresh.knownFor) && /Whole Foods/i.test(sofresh.knownFor),
  "SoFresh Downtown two-beat is official diet flags + Whole Foods on 2nd Street");
ok(sofresh && sofresh.placeId == null,
  "SoFresh Downtown does not reuse the Tampa SoFresh ChIJ already in the repo");
...
ok(icy && /Sichuan/i.test(icy.knownFor) && /Building E/i.test(icy.knownFor),
  "Icy-N-Spicy two-beat is official Sichuan / Europe-Asia flavors + Miramar Pkwy Bldg E");
ok(icy && /Miramar/i.test(icy.address) && !/Ocean Drive|1250/i.test(icy.address),
  "Icy-N-Spicy pin is Miramar, not the official-site Carlyle / Ocean Drive room");
...
const luce = HOLD_GOOGLE.find((p) => /The Luce/i.test(p.name));
ok(luce && luce.lat === 27.7338367 && luce.lng === -82.7470566 && luce.coordSource === "osm",
  "The Luce uses the OSM named way — not a Places geocode");
ok(luce && luce.osm && luce.osm.way === 523258468,
  "The Luce records OSM way 523258468");
ok(luce && /largest pool/i.test(luce.knownFor) && /Sunset Social/i.test(luce.knownFor),
  "The Luce two-beat is official largest pool + Sunset Social");
ok(luce && /sand/i.test(luce.knownFor),
  "The Luce why-sit stays on the sand — official 'set directly on the sand'");
ok(luce && chipIdentity("attractions", "beaches", {
  name: luce.name, types: luce.types, primaryType: luce.primaryType, primary_type: luce.primaryType,
}) === false, "The Luce is a hotel on the sand, not a public beach card");
ok(luce && chipIdentity("hotels", "beach", {
  name: luce.name, types: luce.types, primaryType: luce.primaryType, primary_type: luce.primaryType,
}) === true, "The Luce name + hotel types must pass hotels → beach");
...
const guy = HOLDS.find((h) => /Chicken Guy/i.test(h.name));
ok(guy && /unverified|confirm/i.test(guy.note) && /25769 Sierra Center/i.test(guy.address + guy.note),
  "Chicken Guy HOLD records the Cypress Creek candidate and marks the location unverified");
ok(guy && /Guy Fieri/i.test(guy.note) && /Tampa Premium Outlets/i.test(guy.note),
  "Chicken Guy HOLD still records the two-beat + address without minting a card");

const eis = HOLDS.find((h) => /Eiswerkstatt/i.test(h.name));
ok(eis && /Switzerland|Bern|out of Florida/i.test(eis.note + eis.address + eis.status),
  "Eiswerkstatt is held as out of Florida — no international pin");
...
ok(/SoFresh Downtown Sarasota/.test(ingest) && /Mobius Sarasota/.test(ingest) && /The Luce/.test(ingest),
  "ingest names the hold_google_id shops");
ok(/Chicken Guy/.test(ingest) && /Dirty Sara-Soda/.test(ingest) && /Happy Hands World/.test(ingest) && /Sloan/.test(ingest) && /Eiswerkstatt/.test(ingest) && /Divilma Orlando/.test(ingest),
  "ingest HOLDs the unverified / coming-soon / mobile / Bern names");
...
ok(/SoFresh Downtown Sarasota/.test(notes) && /1455 Second/.test(notes),
  "HOLD notes name Downtown Sarasota SoFresh, not only Lakewood Ranch");
ok(/Chicken Guy/.test(notes) && /unverified/i.test(notes),
  "HOLD notes mark Chicken Guy location unverified");
ok(/Dirty Sara-Soda/.test(notes) && /Happy Hands World/.test(notes) && /Sloan/.test(notes) && /Eiswerkstatt/.test(notes) && /Divilma Orlando/.test(notes),
  "HOLD notes name the requested HOLDs");
...
ok(/Coming Soon/.test(notes) && /Happy Hands/.test(notes),
  "HOLD notes keep Happy Hands play-center Coming Soon");
...
// Red-prove: a beach primary would pass Activities → Beaches — The Luce must not.
ok(chipIdentity("attractions", "beaches", {
  name: "The Luce, St. Pete Beach", types: ["beach"], primaryType: "beach", primary_type: "beach",
}) === true,
  "control: a beach primary WOULD pass Beaches — that is why The Luce uses hotel types");
ok(chipIdentity("attractions", "beaches", {
  name: "The Luce, St. Pete Beach", types: ["hotel", "lodging"], primaryType: "hotel", primary_type: "hotel",
}) === false, "the shipped Luce types must fail Activities → Beaches");

if (fail.length) {
  console.error("test-owner-places-2026-08-29c: FAIL");
  for (const m of fail) console.error("  - " + m);
  process.exit(1);
}
console.log(`test-owner-places-2026-08-29c: OK — ${n} assertions; ${ADD.length} Atlas cards; ${HOLD_GOOGLE.length} google-id holds; ${HOLDS.length} HOLDs`);
