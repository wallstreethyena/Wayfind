...
ok(/revealController = new AbortController\(\)/.test(home) && /signal: revealController\.signal/.test(home) && /clearTimeout\(revealTimer\)/.test(home), "a stalled browser reveal settles and releases its animation state");
...
ok(/fetchDeadline\(`\$\{s\.url\}\/auth\/v1\/user/.test(route) && /fetchDeadline\(`\$\{s\.url\}\/rest\/v1\/rpc\/wf_consume_lunch_reveal/.test(route), "auth verification and allowance consumption have server deadlines");
