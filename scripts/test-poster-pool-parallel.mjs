...
const CATS = ["things-to-do", "restaurants", "beaches", "nightlife", "drive"];
...
function nearbyFixture(cat) {
  if (cat === "beaches") throw new Error("nearby rejection control");
...
assert.equal(actual.pools.beaches.length, 8, "a rejected nearby source preserves the ranked fallback");
...
await loadPools("tampa", {
  ...
  deps: {
    ...
    buildNearbyPool: async () => { readsAfterRejectedPrime++; return []; },
  },
});
...
console.log(`test-poster-pool-parallel: OK — output parity, thin/trending fallbacks, rejection handling, one prime, honest stage, ranked/nearby overlap, and peak ${parallel.peak}/${POOL_READ_CONCURRENCY}; old serial overlap=${serial.overlap}${process.argv.includes("--red-child") ? "" : "; cap=1 real-source mutation went red and restored"}`);
