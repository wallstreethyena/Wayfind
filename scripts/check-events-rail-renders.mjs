...
// RENDERED there. Every hop is where the last one broke.
{
  ...
  ok(/selRail\.id === "datenight" \|\| selRail\.id === "birthday" \|\| selRail\.id === "breakfast" \|\| selRail\.id === "break" \|\| selRail\.id === "eat" \|\| selRail\.id === "events"/.test(rail),
    "Date Night, Birthday, Breakfast, Lunch Break, Actually Worth Eating, and Events own their answers and cannot fall through into generic venue place cards");
}
