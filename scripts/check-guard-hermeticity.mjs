...
const EXEMPT = {
  ...
  "check-doc-ownership.mjs":
    "2026-09-09 incident: GitHub Actions run 34390086158 (head b3cc9a48) printed \"SKIP — confirmed shallow checkout does not contain origin/main\", the suite reported 592/592 green, and PR #1197 merged an AGENTS.md edit by a lane two minutes later. actions/checkout used the default depth 1, so the guard could not read the base it needs. GITHUB_ACTIONS is not the verdict by itself — it marks the one run (the merge gate) that may read GitHub-authenticated owner approval (GITHUB_TOKEN, GITHUB_EVENT_NAME, GITHUB_EVENT_PATH, GITHUB_REPOSITORY, GITHUB_API_URL: an /owner-approve <head sha> PR comment by an owner login; commit author names are never evidence since 2026-09-23). Off the merge gate an owner-only change always fails. It also selects STRICTNESS when the evidence is unreachable: on the run that decides a merge, an unreadable base is a failure; on a Vercel build, which runs this suite through prebuild on its own shallow clone and is not a merge gate, it stays a skip so a deploy cannot be blocked by clone depth. Answering differently in those two shells is the point, and both answers are proved hermetically in test-doc-ownership-shallow.mjs, which sets GITHUB_ACTIONS explicitly for child processes the way check-monetized-degrade.mjs does.",
  ...
  "check-rpc-schema-contract.mjs":
    "same shape as check-inventory-integrity.mjs: it talks to live Supabase (wf_rpc_signatures()) and the credentials are the CONNECTION, not the verdict. Without them it prints SKIPPED and exits 0; with them the verdict comes entirely from production's own reported RPC argument names compared against a static scan of app/**,lib/**. Not wired into prebuild at all (see EXCLUDED in check-guard-manifest.mjs) — it runs in the scheduled canary workflow, because production's RPC contract must never be able to block a code deploy",
  "check-migration-reconciliation.mjs":
    "same shape as check-inventory-integrity.mjs: it talks to live Supabase (wf_migration_ledger(), wf_schema_objects()) and the credentials are the CONNECTION, not the verdict. Without them it prints SKIPPED and exits 0; with them the verdict comes entirely from which migrations are actually applied. Not wired into prebuild at all (see EXCLUDED in check-guard-manifest.mjs) — it runs in the scheduled canary workflow, because production's applied-migration state must never be able to block a code deploy",
  "check-fall-registry-integrity.mjs":
    "same shape as check-inventory-integrity.mjs: its static checks (set equality across FALL_CARD_IDS/FALL_PLACE_IDS/FALL_PLACE_RAIL/FALL_OFFERING_SOURCES, rejected-id exclusion, verified-in-window-or-evergreen, expired-offering exclusion) run unconditionally and decide their verdict entirely from lib/fallPool.js + lib/fallEvidence.js source, never from env. The credentialed section ADDS two live checks (OPERATIONAL/excluded, name-only-vs-real-name) on top of that — without credentials it logs SKIPPED for that section only and its own verdict is still all-static; with credentials the added verdict comes entirely from wf_inventory row data, never from which key happened to be in the shell. It IS wired into prebuild (scripts/guards.txt) unlike its live-Supabase siblings above, on purpose: the static half must gate every build (a registry entry that lapses must stay visible as a build-time fact, not wait for a scheduled canary). As of the 2026-09-23 independent PR #1495 audit fix round it is GREEN — the second-pass re-verification resolved every entry the first pass could not current-season-verify (five removed fail-closed, two kept via the evergreenThemeLive/offeringWindow rule fixes; see the guard's own header comment) and the guard's own verifiedInSeasonWindow gate was fixed to require the CURRENT season rather than reading the year off the verified date itself",
  "test-free-photo-serving.mjs":
    "same shape as check-cache-refresh.mjs: WAYFIND_GATE, SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, WAYFIND_PHOTOS_PAID, GOOGLE_PHOTOS_MONTH_CAP and GOOGLE_MAPS_SERVER_KEY are SAVED (the one remaining read, purely for later restore), then set in-process to fixture values it chooses to drive the real app/api/photo route's GET handler — sourced and re-executed against a stub fetch and injected resolver doubles — through every gate/recovery/free-photo branch, then restored in a finally. It cannot delegate to a child process because the assertion is a call-count on a SINGLE shared ledger-fetch stub across every scenario in one run (proving a free photo takes zero ledger grants, by count, not by reading a reason string). The one other process.env.SUPABASE_URL reference (the fetch stub's URL match) reads a local SUPABASE_URL_FIXTURE constant this file defines, not the shell, so it does not appear above",
  "test-photo-vault-wiring.mjs":
    "same shape as test-free-photo-serving.mjs: CRON_SECRET, NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY are SAVED (the reads flagged above, purely for later restore), then set in-process to fixture values this file chooses (a fixed CRON_SECRET, a fixture Supabase URL) to drive the real app/api/cron/place-photos route's GET handler — sourced and re-executed with runBackfill/recordPulse/jobCannotRun/jobFailed swapped for doubles — through its auth-gate, empty-run-pulse and ?source= branches, then restored to the saved values afterward (including inside the one sub-scenario that deletes CRON_SECRET to prove the 401, which restores it immediately). It cannot delegate to a child process because the assertion needs the SAME loaded route module instance to answer differently as process.env.CRON_SECRET changes within one test run, exactly like check-cache-refresh.mjs's gate-branch assertions. Every OTHER process.env-shaped reference in this file lives inside `deps.env`/`sbEnv` FIXTURE OBJECTS this file constructs itself (e.g. `{ SUPABASE_URL: SB.url, ... }` passed into findFreePhoto/runBackfill/storePhoto as an injected dependency) — never the ambient shell — so they do not appear above",
  "test-photo-warm.mjs":
    "same shape as test-photo-vault-wiring.mjs: CRON_SECRET, VERCEL_ENV and PHOTO_WARM_MAX are SAVED (the reads flagged above, purely for later restore) then set in-process to fixture values this file chooses (a fixed CRON_SECRET, VERCEL_ENV='preview'/deleted, an unset PHOTO_WARM_MAX) to drive the real app/api/cron/photo-warm route's GET handler — with the global fetch trapped to throw on any call — through its non-production-skip and auth-gate branches, then restored to the saved values in a finally. It cannot delegate to a child process because the assertion needs the SAME loaded route module instance to answer differently as process.env.VERCEL_ENV/CRON_SECRET change within one test run, exactly like check-cache-refresh.mjs's gate-branch assertions. Every other assertion in this file drives lib/photoWarm.js's runPhotoWarm() directly with an injected fetchImpl/clock and reads no ambient env at all.",
};
