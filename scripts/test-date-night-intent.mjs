...
}
{
  ...
  ok(/const railOwnsItsOwnAnswer = !!\(selRail && \(selRail\.id === "datenight" \|\| selRail\.id === "birthday" \|\| selRail\.id === "breakfast" \|\| selRail\.id === "eat" \|\| selRail\.id === "events"\)\)/.test(src),
    "the drop knows Date Night, Birthday, Breakfast, Actually Worth Eating, and Events answer for themselves");
