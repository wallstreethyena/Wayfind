...
assert.match(rail, /className="wf-place-card wf-rail-card is-no-take"/, "every affiliate result uses the iconic Wayfind rail-card contract");
...
assert.match(rail, /WayfindScoreBadge score=\{score\} staticRoot/, "the ranked score is shown without nesting a button inside the card link");
...
assert.equal((html.match(/wf-place-card wf-rail-card is-no-take/g) || []).length, 30, "all 30 rendered products use the iconic Wayfind card");
