...
// ------------------------------------------------------------- (1) the gate
ok(/const isWayfindPick = !!curatedFor\(p\) && \(dispScore == null \|\| pickEligibleByScore\(dispScore\)\);/.test(card),
  "PlaceCard still derives isWayfindPick from curatedFor + pickEligibleByScore — the seal is editorial AND score-gated, so a curated-but-weak place never wears it (see test-score-band: pick blocked on yellow/red)");
