...
const EXCLUDED = {
  ...
  "check-rpc-schema-contract.mjs": "runs in the scheduled canary workflow, not prebuild — it reads PRODUCTION Supabase (wf_rpc_signatures()) to compare every db/supabase.rpc() call site's argument names against the live function signature, and a stale production schema must never be able to block a code deploy. Wrong coupling in that direction; see .github/workflows/canary.yml. Added 2026-09-07 after #1153 shipped a 5-arg RPC caller against a still-3-arg production function (the 08:23 UTC popularity cron did zero work across all four providers) — see supabase/migrations/20260907_wf_deploy_contract_audit.sql.",
  "check-migration-reconciliation.mjs": "same reasoning as check-rpc-schema-contract.mjs, same incident — runs in the scheduled canary workflow, not prebuild, because it reads PRODUCTION Supabase (wf_migration_ledger(), wf_schema_objects()) to confirm every supabase/migrations/*.sql file was actually applied, not merely merged. Added 2026-09-07.",
  "check-ui-owner-approved.mjs": "GitHub PR gate only (.github/workflows/ui-owner-approved.yml). It reads pull-request labels and the PR file list. Wiring it into prebuild would turn every Vercel deploy of a card-CSS fix red until a human added the label — the opposite of the Vercel source-only contract. Founder visual approval is a merge gate, not a compile gate.",
};
