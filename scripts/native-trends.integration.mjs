// Production bindings, real shared taxonomy/scoring and RSS adapter, fake IO.
...
const window = wikiWindow(now);
const history = {items: Array.from({length:14}, (_,i)=>({
  timestamp:new Date(window.start+i*DAY_MS).toISOString().slice(0,10).replace(/-/g,'')+'00', views:i<7?100:200,
}))};
let topics=[],snapshot=null,external=0;
const calls=[];
const fetchImpl=async (url,opts={})=>{
  ...
  if(u.pathname.endsWith('/events'))return Response.json([]);
...
const out=await runNativeTrends({url:'https://fixture.supabase.co',key:'not-a-real-key'}, {nowMs:now,fetchImpl,sleepImpl:async()=>{}});
assert.equal(external,7);
assert.equal(out.snapshot.topics,6);
assert.equal(out.snapshot.status,'complete');
...
assert.equal(pickleball.component_scores._sourceCount,2);
assert.equal(pickleball.component_scores._growthSourceCount,1);
assert(pickleball.trend_score<75);
...
console.log('native-trends production bindings: real model, taxonomy, RSS and all 7 bounded external requests passed');
