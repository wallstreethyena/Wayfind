...
| 20260907084309 | wf_wikipedia_popularity_eligibility | Production also dropped the (integer) and (text, text[], integer) overloads; the repo never did (20260907_wf_popularity_attempt_ledger.sql lists the drop as a follow-up) | 3 | Only the 5-argument function is live | Capture migration prepared; applying it needs owner approval |
...
`docs/proposals/20260923_wf_capture_live_function_state.sql` records both. On
production it would be a no-op: the function body is byte-identical to the live
one (md5 656ac41b8ff3f886ed1e10cdbe06a23b), and the dropped overloads do not
exist there.

**Status: prepared, not applied.** It was merged into `supabase/migrations/` in
#1486, but applying it to production was blocked pending the owner's approval,
and a committed file that production has not applied turns the canary's
migration check red. So it waits here. To adopt it: move the file, unchanged,
back to `supabase/migrations/` and apply that exact committed file to
production in the same step; `check-migration-reconciliation` then matches it.
Until then the repository is behind production on these two functions, exactly
as it has been since August; production itself is correct.
