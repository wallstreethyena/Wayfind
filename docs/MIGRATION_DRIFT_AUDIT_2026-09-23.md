...
| 20260907084309 | wf_wikipedia_popularity_eligibility | Production also dropped the (integer) and (text, text[], integer) overloads; the repo never did (20260907_wf_popularity_attempt_ledger.sql lists the drop as a follow-up) | 3 | Only the 5-argument function is live | Captured by `20260923_wf_capture_live_function_state.sql` (owner approved; applied) |
...
`supabase/migrations/20260923_wf_capture_live_function_state.sql` records both. On
production it is a no-op: the function body is byte-identical to the live one
(md5 656ac41b8ff3f886ed1e10cdbe06a23b), the header matches the live definition
(language sql, stable, search_path=public, security invoker), CREATE OR REPLACE
keeps the live grants, and the dropped overloads do not exist there.

**Status: applied.** The owner approved applying it on 2026-09-23. It was held in
`docs/proposals/` (#1490) until then so main never carried a migration that
production had not run. It moved back byte for byte (md5 f842b86f) and was applied
from that exact committed file right after this change merged; the ledger row is
recorded in `scripts/migration-apply-log.jsonl`.
